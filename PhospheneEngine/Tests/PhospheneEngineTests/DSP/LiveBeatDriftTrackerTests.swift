...
struct LiveBeatDriftTrackerTests {
    ...
    // MARK: 19. BUG-007.3 regression — Schmitt hysteresis preserves lock through ±50 ms tempo wander

    /// Mechanism C from BUG-007.3: natural-music tempo variation walks instantDrift
    /// to ±40–50 ms within minor expressive timing, even when grid BPM is correct.
    /// Pre-Schmitt code dropped lock 5+ times in 50 s on Everlong (planned, prepared);
    /// the asymmetric ±60 ms stale-OK gate must hold lock through the same drift envelope.
    ///
    /// Synthetic input: 158 BPM grid + onsets that walk drift sinusoidally between
    /// −50 ms and +50 ms over 60 s. With pre-fix code, ≥ 4 lock drops occur as the
    /// instantDrift sweep crosses outside the ±30 ms tight gate. With Schmitt hysteresis,
    /// once locked the ±60 ms stale gate keeps the lock alive.
    @Test("schmittHysteresis_preservesLockThroughExpressiveTempoVariation")
    func test_schmittHysteresisPreservesLockThroughExpressiveVariation() {
        let bpm = 158.0
        let grid = makeUniformGrid(bpm: bpm, beats: 256, beatsPerBar: 4)
        let tracker = LiveBeatDriftTracker()
        tracker.setGrid(grid)
        ...
        for frame in 0..<Int(totalSeconds * fps) {
            let t = Double(frame) / fps
            onsetAccum += 1.0 / fps
            let isOnsetTime = onsetAccum >= beatPeriod
            // Sinusoidal drift wander: ±50 ms at 0.05 Hz (one cycle per 20 s).
            let wanderMs = 50.0 * sin(2.0 * .pi * t / 20.0)
            let onsetTime = isOnsetTime ? (t + wanderMs / 1000.0) : t
            if isOnsetTime { onsetAccum -= beatPeriod }

            let result = tracker.update(
                subBassOnset: isOnsetTime, playbackTime: onsetTime, deltaTime: dt
            )
            if case .locked = prevLock, case .locking = result.lockState {
                lockedToLockingTransitions += 1
            }
        ...
        // swiftlint:disable:next line_length
        #expect(lockedToLockingTransitions <= 1, "BUG-007.3 Schmitt regression: expected ≤1 lock drop in 60 s of ±50 ms expressive drift wander; got \(lockedToLockingTransitions)")
    }

    // MARK: 20. BUG-007.3 — currentDriftSlope returns nil before sufficient samples

    @Test("driftSlope_insufficientSamples_returnsNil")
    func test_driftSlopeInsufficientSamplesReturnsNil() {
        let grid = makeUniformGrid(bpm: 120, beats: 64)
        let tracker = LiveBeatDriftTracker()
        tracker.setGrid(grid)

        // Advance < 5 s of playback with no onsets → no samples → nil slope.
        let dt: Float = 1.0 / 60.0
        for frame in 0..<60 {
            _ = tracker.update(subBassOnset: false, playbackTime: Double(frame) / 60.0, deltaTime: dt)
        }
        #expect(tracker.currentDriftSlope() == nil, "Slope should be nil before 5 samples accumulate")
    }

    // MARK: 21. BUG-007.3 — flat drift returns near-zero slope

    @Test("driftSlope_flatDrift_returnsNearZero")
    func test_driftSlopeFlatDriftReturnsNearZero() {
        let grid = makeUniformGrid(bpm: 120, beats: 64)
        let tracker = LiveBeatDriftTracker()
        tracker.setGrid(grid)

        // Feed perfectly-aligned onsets for 12 s → drift stays near 0 → slope ≈ 0 ms/s.
        let beatPeriod = 0.5  // 120 BPM
        let dt: Float = 1.0 / 60.0
        var nextBeatTime = 0.0
        for frame in 0..<Int(12.0 * 60) {
            let t = Double(frame) / 60.0
            let isOnset = t >= nextBeatTime
            if isOnset { nextBeatTime += beatPeriod }
            _ = tracker.update(subBassOnset: isOnset, playbackTime: t, deltaTime: dt)
        }

        let slope = tracker.currentDriftSlope()
        #expect(slope != nil, "Slope should be available after 12 s of onsets")
        if let slope { #expect(abs(slope) < 1.0, "Flat-drift slope expected near 0 ms/s; got \(slope)") }
    }

    // MARK: 22. BUG-007.3 — linearly walking drift returns slope close to truth

    /// Onsets arrive at the right cadence but every onset's *time* is shifted by a
    /// linearly growing offset (e.g. +1 ms per onset). The tracker's drift EMA tracks
    /// this offset and `currentDriftSlope` recovers the underlying ms/s rate.
    @Test("driftSlope_linearWalkingDrift_recoversSlope")
    func test_driftSlopeLinearWalkingDriftRecoversSlope() {
        let grid = makeUniformGrid(bpm: 120, beats: 256)
        let tracker = LiveBeatDriftTracker()
        tracker.setGrid(grid)

        // True drift slope: each onset shifts forward by an additional 4 ms (= 8 ms/s at 120 BPM).
        let beatPeriod = 0.5
        let dt: Float = 1.0 / 60.0
        let driftPerOnsetMs = 4.0
        var nextBeatTime = 0.0
        var onsetCount = 0
        for frame in 0..<Int(15.0 * 60) {
            let t = Double(frame) / 60.0
            let isOnset = t >= nextBeatTime
            var onsetTime = t
            if isOnset {
                onsetTime = nextBeatTime + Double(onsetCount) * driftPerOnsetMs / 1000.0
                onsetCount += 1
                nextBeatTime += beatPeriod
            }
            _ = tracker.update(subBassOnset: isOnset, playbackTime: onsetTime, deltaTime: dt)
        }

        let slope = tracker.currentDriftSlope()
        #expect(slope != nil, "Slope should be available after 15 s of onsets")
        if let slope {
            // drift = nearest − pt; onsets pushed forward in time produce negative drift.
            // True magnitude: 4 ms/onset × 2 onsets/s = 8 ms/s. Tolerance ±2 ms/s.
            #expect(slope < 0, "Forward-walking onsets should produce negative drift slope; got \(slope)")
            #expect(abs(abs(slope) - 8.0) < 2.0, "Expected |slope| ≈ 8 ms/s; got \(slope)")
        }
    }
}
