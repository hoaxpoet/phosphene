...
struct LiveBeatDriftTrackerTests {
    ...
    // MARK: 42–48. BUG-017 / CS.1.y — cold-start phase acquisition
    //
    // The cached grid carries a reliable tempo but a per-track ±½-beat phase
    // error (preview-clip phase used as track phase). Cold-start acquisition
    // measures the error from the first live sub-bass onsets (circular mean)
    // and applies a one-shot gross correction the ±50 ms steady-state EMA
    // cannot make on its own.

    // MARK: 42. Large phase error — EMA-uncorrectable, fixed by cold-start

    /// A correctly-tempo'd grid whose phase is wrong by +150 ms — larger than
    /// the ±50 ms steady-state EMA search window, so the EMA alone cannot fix
    /// it (BUG-017's failure mode). Cold-start acquisition must snap the phase.
    @Test("coldStart_correctsLargePhaseError")
    func test_coldStartCorrectsLargePhaseError() {
        let tracker = LiveBeatDriftTracker()
        tracker.audioOutputLatencyMs = 0
        tracker.setGrid(makeUniformGrid(bpm: 120, beats: 64))

        let period = 0.5
        let phaseError = 0.150   // onsets land 150 ms before the cached beats
        _ = drive(tracker, durationSeconds: 8.0) { t in
            let nearestOnset = round((t + phaseError) / period) * period - phaseError
            return abs(t - nearestOnset) < 0.005
        }
        #expect(tracker.coldStartCorrectionApplied,
                "cold-start must apply a gross correction for a 150 ms phase error")
        #expect(abs(tracker.currentDriftMs - 150.0) < 30.0,
                "drift should converge to the +150 ms phase error; got \(tracker.currentDriftMs)")
        if case .locked = tracker.currentLockState {} else {
            #expect(Bool(false), "expected .locked after cold-start correction + 8 s tracking")
        }
    }

    // MARK: 43. HUMBLE-scale error at a slow tempo

    /// HUMBLE — CS.1's worst failure (76 BPM, +338 ms, 0 % pass). 338 ms ≈ 0.43
    /// beat at 76 BPM; the acquisition must still correct it within the budget.
    @Test("coldStart_humbleScalePhaseError_76bpm")
    func test_coldStartHumbleScalePhaseError() {
        let bpm = 76.0
        let period = 60.0 / bpm
        let beats = (0..<96).map { Double($0) * period }
        let downbeats = stride(from: 0, to: 96, by: 4).map { Double($0) * period }
        let grid = BeatGrid(beats: beats, downbeats: downbeats, bpm: bpm,
                            beatsPerBar: 4, barConfidence: 0.9,
                            frameRate: 50.0, frameCount: 1500)
        let tracker = LiveBeatDriftTracker()
        tracker.audioOutputLatencyMs = 0
        tracker.setGrid(grid)

        let phaseError = 0.338
        _ = drive(tracker, durationSeconds: 10.0) { t in
            let nearestOnset = round((t + phaseError) / period) * period - phaseError
            return abs(t - nearestOnset) < 0.005
        }
        #expect(tracker.coldStartCorrectionApplied,
                "cold-start must apply a correction for HUMBLE's +338 ms error")
        #expect(abs(tracker.currentDriftMs - 338.0) < 45.0,
                "drift should converge near +338 ms; got \(tracker.currentDriftMs)")
        if case .locked = tracker.currentLockState {} else {
            #expect(Bool(false), "expected .locked after HUMBLE-scale cold-start correction")
        }
    }

    // MARK: 44. Aligned grid — cold-start resolves as a no-op

    /// A correctly-phased grid: cold-start must NOT apply a gross correction —
    /// it resolves as a no-op and steady-state tracking is untouched. This is
    /// the regression-safety path every pre-existing aligned-onset test takes.
    @Test("coldStart_alignedGrid_resolvesAsNoOp")
    func test_coldStartAlignedGridNoOp() {
        let tracker = LiveBeatDriftTracker()
        tracker.audioOutputLatencyMs = 0
        tracker.setGrid(makeUniformGrid(bpm: 120, beats: 64))
        let period = 0.5
        ...
        #expect(!tracker.coldStartCorrectionApplied,
                "an aligned grid must not trigger a cold-start gross correction")
        #expect(abs(tracker.currentDriftMs) < 20.0,
                "drift should stay near 0 for an aligned grid; got \(tracker.currentDriftMs)")
        if case .locked = r.lockState {} else {
            #expect(Bool(false), "expected .locked — steady-state tracking unaffected")
        }
    }

    // MARK: 45. Scattered onsets — declines (½-beat-ambiguity gate)

    /// Onsets that do not form a phase cluster (alternating on-beat then
    /// half-beat) → low circular resultant → cold-start declines rather than
    /// risk a confident wrong-phase lock.
    @Test("coldStart_scatteredOnsets_declines")
    func test_coldStartScatteredOnsetsDeclines() {
        let tracker = LiveBeatDriftTracker()
        tracker.audioOutputLatencyMs = 0
        tracker.setGrid(makeUniformGrid(bpm: 120, beats: 64))
        let period = 0.5
        // Even onsets on a beat (residual 0), odd onsets on the half-beat
        // (residual ±P/2) — antipodal residuals, resultant ≈ 0.
        let onsetTimes: [Double] = (0..<8).map { i in
            i % 2 == 0 ? Double(i) * period : Double(i) * period + period / 2.0
        }
        let dt: Float = 1.0 / 100.0
        var t = 0.0
        var idx = 0
        while t < 6.0 {
            var onset = false
            if idx < onsetTimes.count, abs(t - onsetTimes[idx]) < 0.005 {
                onset = true
                idx += 1
            }
            _ = tracker.update(subBassOnset: onset, playbackTime: t, deltaTime: dt)
            t += Double(dt)
        }
        #expect(!tracker.coldStartCorrectionApplied,
                "scattered onsets must not trigger a cold-start correction")
    }

    // MARK: 46. Too few onsets — declines (rhythmless intro, honest limitation)

    /// A near-silent intro — a single onset within the budget window. Cold-start
    /// cannot form a cluster → it declines, leaving the seeded phase untouched
    /// (honest limitation; design doc §6.7).
    @Test("coldStart_tooFewOnsets_declines")
    func test_coldStartTooFewOnsetsDeclines() {
        let tracker = LiveBeatDriftTracker()
        tracker.audioOutputLatencyMs = 0
        tracker.setGrid(makeUniformGrid(bpm: 120, beats: 64))
        let dt: Float = 1.0 / 100.0
        var t = 0.0
        while t < 5.0 {
            let onset = abs(t - 1.0) < 0.005   // a single onset at t = 1 s
            _ = tracker.update(subBassOnset: onset, playbackTime: t, deltaTime: dt)
            t += Double(dt)
        }
        #expect(!tracker.coldStartCorrectionApplied,
                "a single onset cannot resolve cold-start — it must decline")
    }

    // MARK: 47. Per-track — re-arms on setGrid

    /// Cold-start acquisition is per-track: a correction applied on one track
    /// must not leak into the next. `setGrid` re-arms it.
    @Test("coldStart_reArmsOnSetGrid")
    func test_coldStartReArmsOnSetGrid() {
        let tracker = LiveBeatDriftTracker()
        tracker.audioOutputLatencyMs = 0
        tracker.setGrid(makeUniformGrid(bpm: 120, beats: 64))
        let period = 0.5
        let phaseError = 0.150
        _ = drive(tracker, durationSeconds: 5.0) { t in
            let nearestOnset = round((t + phaseError) / period) * period - phaseError
            return abs(t - nearestOnset) < 0.005
        }
        #expect(tracker.coldStartCorrectionApplied, "correction applied on track 1")

        tracker.setGrid(makeUniformGrid(bpm: 100, beats: 64))
        #expect(!tracker.coldStartCorrectionApplied,
                "setGrid must re-arm cold-start acquisition for the new track")
    }

    // MARK: 48. Steady-state lock holds after a cold-start correction

    /// After a cold-start gross correction, the BUG-007.x lock machinery must
    /// behave normally — acquire .locked promptly and hold it without
    /// oscillation. Steady-state preservation is the CS.1.y regression gate.
    @Test("coldStart_steadyStateLockHoldsAfterCorrection")
    func test_coldStartSteadyStateLockHoldsAfterCorrection() {
        let tracker = LiveBeatDriftTracker()
        tracker.audioOutputLatencyMs = 0
        tracker.setGrid(makeUniformGrid(bpm: 120, beats: 256))
        let period = 0.5
        let phaseError = 0.150
        let dt: Float = 1.0 / 60.0
        var prevLock: LiveBeatDriftTracker.LockState = .unlocked
        var lockedToLockingTransitions = 0
        var firstLockedTime: Double?
        var t = 0.0
        while t < 40.0 {
            let nearestOnset = round((t + phaseError) / period) * period - phaseError
            let onset = abs(t - nearestOnset) < (1.0 / 120.0)
            let r = tracker.update(subBassOnset: onset, playbackTime: t, deltaTime: dt)
            if case .locked = r.lockState, firstLockedTime == nil {
                firstLockedTime = t
            }
            if case .locked = prevLock, case .locking = r.lockState {
                lockedToLockingTransitions += 1
            }
            prevLock = r.lockState
            t += Double(dt)
        }
        #expect(tracker.coldStartCorrectionApplied)
        #expect((firstLockedTime ?? Double.infinity) < 6.0,
                "lock should be acquired within ~6 s of the corrected phase")
        // swiftlint:disable:next line_length
        #expect(lockedToLockingTransitions <= 2, "steady-state lock must hold after a cold-start correction; got \(lockedToLockingTransitions) drops")
    }
}
