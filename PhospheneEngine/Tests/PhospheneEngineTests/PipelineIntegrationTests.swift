...
@Test func featureVectorLayoutMatchesShaderExpectation() throws {
    // The shader preamble (in PresetLoader) defines FeatureVector as 24 floats.
    // This test verifies the Swift FeatureVector matches that layout exactly.
    #expect(MemoryLayout<FeatureVector>.size == 96,
            "FeatureVector must be 96 bytes (24 floats), got \(MemoryLayout<FeatureVector>.size)")

    // Verify specific field offsets by writing known values and reading as float array.
    guard let device = MTLCreateSystemDefaultDevice() else {
        throw IntegrationTestError.noMetalDevice
    }
    guard let buffer = device.makeBuffer(length: 96, options: .storageModeShared) else {
        throw IntegrationTestError.metalSetupFailed
    }
    ...
    memcpy(buffer.contents(), &features, 96)
    let floats = buffer.contents().bindMemory(to: Float.self, capacity: 24)
