    ...
    float hue, float d0, float d1, float normalZ)
{
    ...
    v.position = float4(p0 + ap0, 0, 1); v.uv = float2(0, 0); m.set_vertex(bv + 0u, v);
    v.position = float4(p0 - ap0, 0, 1); v.uv = float2(0, 1); m.set_vertex(bv + 1u, v);
    v.normal.y = d1;
    v.position = float4(p1 + ap1, 0, 1); v.uv = float2(1, 0); m.set_vertex(bv + 2u, v);
    v.position = float4(p1 - ap1, 0, 1); v.uv = float2(1, 1); m.set_vertex(bv + 3u, v);
    ...
    uint3 tgid [[threadgroup_position_in_grid]])
{
    ...
    // ── Threads 1–8: anchor dots ───────────────────────────────────────────────
    if (lid <= 8u) {
        uint ai = lid - 1u;
        uint bv = 4u + ai * 4u;
        uint bi = 6u + ai * 6u;

        if (ai >= ac) { write_offscreen(m, bv, bi); return; }

        float2 aPos = anchor_pos(web, ai);
        float  dotR = 0.011f * (1.0f + anticipation * 0.65f);
        // anchorPulse: dots pulse to beat
        if (stage == 0u) dotR *= 1.0f + anticipation * 0.5f;
        float2 p0 = float2(aPos.x, aPos.y - dotR);
        float2 p1 = float2(aPos.x, aPos.y + dotR);
        write_quad(m, bv, bi, p0, p1, dotR, dotR, aspect, hue, 0.9f, 0.9f, opacity + 10.0f);
        return;
    }
...
{
    // Unpack normal: normal.z >= 10 → dot geometry (circular mask); else → strand.
    bool  isDot    = in.normal.z >= 10.0f;
    float webOpacity = isDot ? (in.normal.z - 10.0f) : in.normal.z;
    webOpacity = saturate(webOpacity);

    // Edge mask: circular for dots, cross-strand soft fade for strands.
    float edgeAlpha;
    if (isDot) {
        float2 c = in.uv * 2.0f - 1.0f;           // [-1,1]×[-1,1] centred
        edgeAlpha = smoothstep(1.15f, 0.35f, length(c));
    } else {
        float across = in.uv.y;                     // 0=edge, 1=edge, 0.5=centre
        edgeAlpha = smoothstep(0.0f, 0.22f, across) * smoothstep(1.0f, 0.78f, across);
    }
    ...
    // Bass quiver: wave phase-locked to beat_phase01 so one cycle propagates per beat.
    // Fires from the hub on the downbeat and reaches the outer edge by the next beat.
    float dist = in.normal.y;                       // 0 = hub, 1 = outer edge
    float quiver = 0.0f;
    if (!isDot) {
    ...
    float anticipation = smoothstep(0.75f, 1.0f, features.beat_phase01) * 0.22f;

    // Bioluminescent glow: exponential falloff — vivid bright core, dark tapered edges.
    float glow = isDot ? 1.0f : exp2(-dist * 3.0f);

    // HSV → RGB: high saturation for vivid bioluminescent color; dark base + bright hub.
    float hue = in.normal.x;
    float sat  = 0.92f;
    float brt  = 0.20f + glow * 0.65f + quiver + anticipation;
    brt = saturate(brt);

    float3 color = hsv2rgb(float3(fract(hue), sat, brt));

    // Additive blend: output color × final intensity.
    float intensity = edgeAlpha * webOpacity;
