...
///
/// > Deprecated for tracks with a cached BPM prior (BSAudit.3, 2026-05-24).
/// > Tracks where `MIRPipeline.installBPMPrior(bpm:character:beatsPerBar:)`
/// > has been called with a non-zero BPM drive `FeatureVector.beatPhase01` /
/// > `beatsUntilNext` from `LiveBeatDriftTracker` instead. This class remains
/// > the load-bearing fallback for **reactive mode** — ad-hoc playback,
/// > preview-unavailable tracks, or any path where Beat This! offline
/// > analysis did not run. New shader code should not assume this is the
/// > active source of phase.
