...
struct PresetSessionReplay: AsyncParsableCommand {
    ...
    private func resolvePreset(_ name: String) throws -> [RouteSpec] {
        ...
        // BUG-088: `aurora_veil` used to resolve here. AV.7 reauthored the preset
        // as a nimitz "Auroras" port and deleted all three routes those specs
        // described (vocal-pitch hue, bass-transient brightness, drum kink), so
        // replaying against them produced verdicts about a shader that no longer
        // exists. Removed rather than rewritten — re-add specs only from the
        // routes AuroraVeil.json actually declares.
        case "murmuration":
        ...
        default:
            throw ValidationError(
                "Unknown preset '\(name)'. Registered: murmuration, skein. "
