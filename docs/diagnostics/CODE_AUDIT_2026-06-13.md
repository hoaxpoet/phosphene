...
> **Status rows reconciled 2026-06-18** against the authoritative `KNOWN_ISSUES.md` bug entries + `ENGINEERING_PLAN.md` §Phase CLEAN (per [[feedback_status_from_authoritative_surface]] — these hand-maintained summary tables lag the bug entries). Marked ✅ done (were unmarked-but-complete): **Phase 0** 0.1–0.5 (EP "CLEAN.0 complete"), **Phase 1** 1.1–1.4/1.6/1.7 (EP "Phase 1 fully complete"; BUG-031/032/033 Resolved), **Phase 2** 2.1–2.3 (EP "Phase 2 complete"). Resolution-stamped: **CLEAN.4.1** (sites 1+2 validated 2026-06-17, stale "pending" removed), Part-B **G2** (BUG-053 Resolved), **G7/G8** (CLEAN.1.6/1.7 closed), **G13** (CLEAN.5.4 build-repro), **G15** (reduce-motion now live; rest deferred D-169), **G12** (deferred D-169). Prior passes: G1 (`f24650a`), BUG-039 + BUG-053/3.7-fix (`4d20103`). **BUG-037 reconciled (Matt confirmed 2026-06-18):** KNOWN_ISSUES §BUG-037 had drifted internally — the Open-Index row said "✅ RESOLVED 2026-06-18" but the full-entry **Status:** line still read "Resolved pending Matt's M7 visual cert." Matt confirmed it is tested + resolved; the entry Status line was flipped to ✅ RESOLVED 2026-06-18 (matching the Open Index, the audit 3.4 row, and the EP CLEAN.3.4 bullet).
...
| G7 | **Dynamic concurrency validation** — no ThreadSanitizer scheme / stress harness | **CONFIRMED → CLOSED** (CLEAN.1.6 ✅, TSan scheme + stress harness landed 2026-06-13; EP Phase 1 complete) — static review can't prove absence of races; validated the P1 fixes | **P1** | P1 |
| G8 | **End-to-end session-lifecycle integration test** (connect→prepare→play→track-change→end→restart) | **CONFIRMED → CLOSED** (CLEAN.1.7 ✅, E2E session-lifecycle test landed 2026-06-13; EP Phase 1 complete) — was: only per-VM unit + beat-grid wiring tests; the orphaning class lived exactly here | **P1** | P1 |
...
| G12 | **Cold-install / resource-bootstrap** (LFS absent, no weights/fixtures/permissions, empty defaults) | **PLAUSIBLE (untested) → DEFERRED** (CLEAN.7.8 ⏸, D-169 — public-release-readiness; YAGNI until a public build) — no first-run-degraded path verified | P2 | P7 |
| G13 | **Build reproducibility & toolchain pinning** (Package.resolved, Xcode/Swift, LFS-present check) | **CONFIRMED → RESOLVED 2026-06-15** (CLEAN.5.4 ✅ — `.xcode-version` pin + committed `Package.resolved` (`-disableAutomaticPackageResolution`) + `check_lfs_smudged.sh` pre-build gate; EP §CLEAN.5.4) | P2 | P5 |
...
| G15 | **Reduce-Motion / reduce-transparency / contrast a11y** | **PARTIAL → reduce-motion now wired + live app-wide** (`AccessibilityState`); reduce-transparency / increase-contrast **DEFERRED** (CLEAN.7.7 ⏸, D-169 — YAGNI until a public build) | P3 | P7 |
| G16 | **Memory-footprint absolute baseline + soak-growth ceiling** | **RESOLVED CLEAN.4.7 (2026-06-18)** — `MemorySoakGateTests`: deterministic frame-count soaks of FFT→MIR (2.5K) + `StemAnalyzer` (1.2K) fail if `phys_footprint` grows past 25 MB after warmup (catches gross leaks; both green run isolated). `phys_footprint` is process-wide → `SOAK_TESTS=1 swift test --filter MemorySoakGate` (~7.7 s); the parallel suite/closeout skips it. Slow creep + the absolute 2 h curve stay the manual `SoakRunner` diagnostic. (Was: `MemoryReporter` existed but nothing asserted a ceiling.) | P2 | P4 ✅ |
...
|----|------|-----------|--------|
| CLEAN.0.1 ✅ | Merge stranded BUG-030 fix from `dreamy-bell` (`679363a9`,`4d95bf88`) to main | on main; KNOWN_ISSUES BUG-030 = Resolved; dup-playlist repro green | **DONE 2026-06-13** — BUG-030 Resolved (`ba4e1cae`); EP §CLEAN.0 ✅ |
| CLEAN.0.2 ✅ | Triage diverged branches (Glass Brutalist `791857e2`+5; naughty-goldstine AGC3.6/FBS `46e40ed8`) | each = land / park / discard, recorded | **DONE 2026-06-13** — salvage refs preserved (Glass Brutalist/AGC3.6); divergence later resolved 2026-06-15 (EP §CLEAN.2.3.8/DOC.6.2); EP §CLEAN.0 ✅ |
| CLEAN.0.3 ✅ | Green-baseline verification on main | `closeout_evidence.sh` block: engine+app build, tests, swiftlint --strict, DocIntegrity all green; tempo-fixture-missing fixed | **DONE 2026-06-13** — closeout ALL GREEN (engine 1457 / app 382 / lint 0 / doc 9); EP §CLEAN.0 ✅ |
| CLEAN.0.4 ✅ | File audit backlog as trackable issues + land this plan as ENGINEERING_PLAN Phase CLEAN + write critical-path sequencing doc | every P1/P2 pickable; plan rows exist | **DONE 2026-06-13** — landed as EP §Phase CLEAN (authoritative queue); EP §CLEAN.0 ✅ |
| CLEAN.0.5 ✅ | Prune ~18 merged worktrees + duplicate tips; document a minimal worktree-hygiene rule | merged worktrees gone; rule in RUNBOOK | **DONE 2026-06-13** — 22 worktrees removed, salvage refs kept; EP §CLEAN.0 ✅ |

### Phase 1 — P1 correctness: concurrency, leaks, runtime resilience (June, core)
| ID | Item | Done-when | Timing |
|----|------|-----------|--------|
| CLEAN.1.1 ✅ | Instrument + diagnose BUG-031/032 family | logging/scaffolding exposes unlocked stem I/O + orphan hijack; root cause in KNOWN_ISSUES (commit, stop) | **DONE 2026-06-13** — BUG-031/032 Resolved; EP §CLEAN.1.1 ✅ |
| CLEAN.1.2 ✅ | BUG-031 fix: serialize StemSeparator across live+prep (lock full input→predict→output, or per-path instance; return-by-value) | race regression test red-pre/green-post | **DONE 2026-06-13** — BUG-031 Resolved (strategy A, Matt-approved); EP §CLEAN.1.2 ✅ |
| CLEAN.1.3 ✅ | BUG-032 fix: cancel prep in `endSession`; gate prep by session generation; stop `resumeFailedNetworkTracks` second loop; fix source-mutation-before-guard | lifecycle + failure-path tests | **DONE 2026-06-13** — BUG-032 Resolved; EP §CLEAN.1.3 ✅ |
| CLEAN.1.4 ✅ | BUG-033 fix: decouple/throttle dashboard snapshot off per-frame + skip when hidden; `[weak self]` for assign | ViewModel deinit/retain tests; no 60 Hz tree invalidation | **DONE 2026-06-13** — BUG-033 Resolved; EP §CLEAN.1.4 ✅ |
...
| CLEAN.1.6 ✅ | `[GAP-7]` ThreadSanitizer scheme + concurrency stress harness (overlapping live+prep, rapid start/end churn) | TSan-clean under stress; validates 1.2–1.3 | **DONE 2026-06-13** — TSan scheme + stress harness landed (`tsan_stress.sh`); EP §CLEAN.1.6 ✅ |
| CLEAN.1.7 ✅ | `[GAP-8]` E2E session-lifecycle integration test | drives connect→prepare→play→track-change→end→restart; would catch the orphan class | **DONE 2026-06-13** — E2E lifecycle test landed; EP §CLEAN.1.7 ✅ |

### Phase 2 — Security + honest-UI (June)
| ID | Item | Done-when | Timing |
|----|------|-----------|--------|
| CLEAN.2.1 ✅ | Remove Spotify client secret from bundled `Info.plist` (PKCE) | secret absent from build; auth-code flow verified E2E | **DONE** — secret removed; PKCE flow; EP §CLEAN.2 (Phase 2 complete) ✅ |
| CLEAN.2.2 ✅ | OAuth correctness: fix re-entrant `login()` leak + stray timeout, refresh double-spend (in-flight dedup), consolidate token providers; + P3s (state param, encoding, host validation, Keychain checks) | tests for re-entrancy/refresh; providers unified | **DONE** — re-entrancy/refresh/providers fixed; EP §CLEAN.2.2 ✅ |
| CLEAN.2.3 ✅ | Wire-or-hide dead UI (LF capture-mode picker, two "Use Apple Music" no-ops, Swap-preset stub) + close localization-gate bypass | no shipped no-op control; checker scans ViewModels/ContentView | **DONE** — dead UI removed (per-app capture + Audio tab deleted, CLEAN.2.3.x); localization gate widened; EP §CLEAN.2.3 ✅ |
...
| CLEAN.3.6 ✅ | SessionRecorder running-vs-actually-writing invariant (BUG-039 follow-through) | invariant asserted; recovery confirmed | **DONE 2026-06-17** — `finish()` now asserts the invariant (append-success counter → video-outcome summary + loud `BUG-039 invariant VIOLATED` on the silent-stop signature); recovery test extended to confirm appends resume. Tests: `test_bug039Invariant_silentStopPredicate`, `test_videoWriterDeath…`. BUG-039 RESOLVED 2026-06-18 (Matt's live multi-session confirm — silent-stop signature no longer occurs). |
...
| CLEAN.3.7-fix ✅ | `[GAP-2]` Fix the live-MIR rate wiring + reconcile doc + add regression gate (was 3.7b/c) | live MIR consumes the actual tap rate; doc matches code; gate locks it | **✅ RESOLVED 2026-06-16** — fix `91a973e`; validated by Matt (session `20-22-12Z`, Limo Wreck 44.1 kHz local-file → live MIR adopted 44100 Hz). BUG-053 Resolved. [This row read "pending" until 2026-06-18; the BUG-053 entry had it Resolved since 2026-06-16 — stale audit-row drift, corrected] |
...
| CLEAN.4.7 ✅ | `[GAP-16]` Memory-footprint soak baseline + peak-RSS regression gate | steady-state + 2 h growth curve measured; gate added | **DONE 2026-06-18.** Gate = `MemorySoakGateTests`: two frame-count soaks (FFT→MIR 2.5K, `StemAnalyzer` 1.2K) assert `phys_footprint` growth < 25 MB after warmup, both green (no leak). `phys_footprint` is process-wide, so the gate is **`SOAK_TESTS`-gated + run isolated**: `SOAK_TESTS=1 swift test --filter MemorySoakGate` (~7.7 s, `.serialized`); the parallel `swift test`/closeout **skips** it (a full-suite run swamps it — +625 MB of unrelated GPU/stem churn — same reason the 5-min check is `SOAK_TESTS`-gated). Catches gross/whole-object leaks; the absolute steady-state + 2 h growth curve stays the **manual `SoakRunner` diagnostic** (`Scripts/run_soak_test.sh`) — a device-time measurement, not a CI gate. swiftlint 0. |

### Phase 5 — DevOps: CI/CD + gate enforcement (June, high leverage)
| ID | Item | Done-when | Timing |
|----|------|-----------|--------|
| CLEAN.5.1 ✅ | CI pipeline: build + engine/app tests + swiftlint --strict on push/PR | green on main; required check | ✅ 06-15 — **GREEN on `main` (run #4, `86c6532`)** after 3 CI-vs-dev env fixes (macos-26 for Sendable-Metal SDK; `#require` for swiftlint 0.63.3 drift; drop Apple-Music `PlaylistConnector`); the **required-check branch-protection toggle is now ENABLED** — `main` protection requires the `fast-gate` context (verified 2026-06-18 via the GitHub branch-protection API; gate runs on every push to `main` + every PR, all green). **CLEAN.5.1 fully complete.** |
...
| CLEAN.6.2 | BUG-042 re-express structural stream at ~2 Hz section scale; recalibrate vs real session streams; re-express BUG-035/040 tests | **✅ DONE 2026-06-19 (BUG-042 RESOLVED)** — 2 Hz decimation + `minNoveltyFloor` 0.02→0.01; live-validated SLTS (5 sections @ conf 0.91). Surfaced two downstream preset-variety follow-ups (reactive scoreGap; LF planning re-enable) → **LFPLAN** increment (§KNOWN_ISSUES BUG-042 / EP §LFPLAN) |
...
| ID | Item | Timing |
|----|------|--------|
| CLEAN.7.1 ✅ | BUG-049 SkeinCanvasHold isolation; BUG-048 app-scheme sandbox (1,439 engine tests) | **DONE 2026-06-18 (reconcile)** — both bugs already RESOLVED (BUG-048 `e110b1ca`, BUG-049 `a6899893`; KNOWN_ISSUES §Resolved). Their regression gates (`SchemeTestActionRegressionTests`; SkeinCanvasHold's fixture-generated armed path) both confirmed green in this session's closeouts. No residual test-infra — each shipped with its gate. |
| CLEAN.7.2a ✅ | Fast local test tier (`Scripts/test_fast.sh`) | **DONE 2026-06-18** — pure-logic core (~978/1524 engine tests) in ~13 s, green in worktrees; the local mirror of the CLEAN.5 CI fast gate. Full `swift test` stays the closeout gate; `--filter <Suite>` for targeted loops. RUNBOOK §Build and Test. (Split from CLEAN.7.2 per Matt's "faster suite, keep coverage" call.) |
| CLEAN.7.2b ✅ | Shared fixture builders (dedup the 174-site FeatureVector/StemFeatures duplication); frame-budget/per-frame-allocation regression; visual/audio-coupling regression (BUG-038/041); per-test tags | **Builders landed 2026-06-18** — `FeatureFixtures.makeFeatureVector` / `makeStemFeatures` (TestDoubles) + `FeatureFixturesTests` proving byte-equivalence to the struct inits (migration = pure readability change, provably identical bytes). Rollout across ~40 files (biggest wins in stem-routing/preset suites) + the new regression tests + per-test tags remain. **Migration wave 1 (2026-06-18):** migrated SpectralHistoryBufferTests' one-shot sites (green); **finding** — the construct-count overstated the dedup: most sites reuse a `var fv` across loops/appends where the builder doesn't help, and several init-only sites (e.g. MurmurationStemRouting) are already minimal. Genuine win is one-shot multi-field constructions + all NEW tests. So remaining migration is **opportunistic** (migrate clean one-shot sites when touching a file), not a mandated sweep. **Remainder reconcile (2026-06-18):** the audit's other 7.2b items were filled by post-audit work, NOT rebuilt (duplication is the very thing this row warns against) — **coupling regression** = `RayMarchPipelineTests.test_smoothLightIntensity_*` (BUG-038, resolved `5c349eb`) + `AuroraTrackStartWarmupTests` (BUG-041), both green; **frame-budget** = `RenderLoopPerformanceTests` (<8 ms) + the SSGI / RayIntersector budget gates; **per-frame allocation** = `MemorySoakGateTests` (CLEAN.4.7) on the two heaviest per-frame audio paths (FFT→MIR, StemAnalyzer — where BUG-036 lives). **Per-test tags descoped** — the fast/slow tiering is delivered by `test_fast.sh` (7.2a), which works for both swift-testing + XCTest, whereas native `.tags` cover only the swift-testing half and would duplicate it. A render-path allocation soak was considered and **declined** (no evidence of a render per-frame leak; CLEAN.4.4 fixed the known over-allocs, guarded by goldens + DrawableResize — building one would be speculative). **7.2b COMPLETE.** |
| CLEAN.7.3 ✅ | Doc-drift quick fixes: ARCHITECTURE Module Map; `LumenPatternState` 376→568 B; canvas-hold spec | **DONE 2026-06-18 (D-168)** — Module Map gap was **62 files** (not 18), incl. 4 certified presets (Skein/Murmuration/Dragon Bloom/Fata Morgana) + recent infra; all backfilled and completeness now **gated** (`DocIntegrityTests.moduleMapCompleteness`, the D-161 mechanize-ratchet). Stride fixed at 2 sites (376 B + 336 B → 568 B). Canvas-hold = the now-documented Skein entry (D-142). DocIntegrity 11/11, swiftlint 0. |
...
| CLEAN.7.5 ✅ | Resolve ARCHITECTURE↔CAPABILITY_REGISTRY + SHADER_CRAFT↔design-doc overlap; create a CLAUDE.md demote candidate to regain budget | **DONE 2026-06-18.** (1) **CLAUDE.md demote** — the §Defect Handling "Multi-increment process" was a *verbatim duplicate* of `DEFECT_TAXONOMY.md §Defect Process by Severity`, and the "Domain-specific artifact requirements" were lookup reference; both demoted (process → 1-line pointer + shape; artifacts → relocated to DEFECT_TAXONOMY) keeping the always-loaded rules (evidence-before-impl, fix-obligations, manual-validation). **Freed 154 words → headroom 67 → 221 (99% → 95.7%).** (2) **ARCH↔REGISTRY** — verified *complementary, not duplicative* (code/contract vs capability status) and previously uncross-linked; added a §Presets scope-delineation cross-link. (3) **SHADER_CRAFT↔design** — verified the audit overstated it: §18 (painterly craft) is explicitly "the authoring handbook for the mark vocabulary" and already cross-refs `SKEIN_DESIGN §1.4/§5.5`; §10 recipes already reference the per-preset design docs. Already delineated → no churn. |
...
| CLEAN.7.7 ⏸ DEFERRED | `[GAP-15]` Live/app-wide Reduce-Motion + reduce-transparency + increase-contrast | **DEFERRED 2026-06-18 (D-169).** Reduce-Motion is already wired + live (`AccessibilityState`). Reduce-transparency + increase-contrast deferred as YAGNI until a public build — no public users, and the a11y basics (reduce-motion, VoiceOver labels, photosensitivity notice) already cover daily single-user dev use. Revisit at release-readiness, or if the dev runs those macOS settings. |
| CLEAN.7.8 ⏸ DEFERRED | `[GAP-12]` Cold-install / resource-bootstrap resilience (degraded-but-honest first run) | **DEFERRED 2026-06-18 (D-169).** Public-release-readiness (degraded-but-honest first run for fresh installs); no public installs exist → YAGNI until a public build. Revisit at release-readiness. |
