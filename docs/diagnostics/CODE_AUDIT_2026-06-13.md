...
| CLEAN.4.7 ✅ | `[GAP-16]` Memory-footprint soak baseline + peak-RSS regression gate | steady-state + 2 h growth curve measured; gate added | **DONE 2026-06-18.** Gate = `MemorySoakGateTests`: two frame-count soaks (FFT→MIR 2.5K, `StemAnalyzer` 1.2K) assert `phys_footprint` growth < 25 MB after warmup — deterministic, runs in `swift test`/closeout (~5.6 s), both green (no leak). Catches gross/whole-object leaks; the absolute steady-state + 2 h growth curve stays the **manual `SoakRunner` diagnostic** (`Scripts/run_soak_test.sh`) — a device-time measurement, not a CI gate. swiftlint 0. |
