...
| CLEAN.3.4 ⏳ | BUG-037 Arachne chord-count single source of truth (CPU/shader/test) | one constant; goldens regen | **CODE-COMPLETE 2026-06-17 (M7 PENDING).** CPU `spiralChordsTotal` is the single source — shader reveals by the CPU-normalized `spiralPacked` (hardcoded 441 + test's 104 retired); 200 cap raised to `maxSpiralChords=600` (degenerate guard). Build-phase-temporal → `.stable` golden unchanged (no regen needed). Test `spiralChordCountHonoursProduct`. **Manual M7 (the build reveal) pending Matt.** |
