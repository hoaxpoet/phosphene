...
| G6 | **Disk-full / write-failure** in SessionRecorder + caches | **CONFIRMED** — no `volumeAvailableCapacity`/ENOSPC handling | P2 | P3 |
...
| G9 | **Photosensitivity flash-safety as an enforced invariant** (Harding/WCAG ≤3 flashes/s) | **PARTIALLY ENFORCED — 3/7** (CLEAN.7.6 / D-164 + CLEAN.7.6b Stage 1, 2026-06-16) — `FlashAnalyzer` + `PhotosensitivityCertificationTests` gate the certified set: FFO + Murmuration + **Nimbus** (the last via CLEAN.7.6b Stage 1 ticking its `.direct` follower state) validly measured SAFE (0 flashes/s). The remaining **4** render static in the single-pass harness → tracked, fail-loud-on-drift: Lumen/Dragon Bloom/Fata Morgana (rayMarch multi-pass) + Skein (mvWarp feedback). Those 4 + the output-side runtime clamp (`RayMarchPipeline:94` flag slot) → **A-next** multi-pass real-`RenderPipeline` harness | **P1 (safety)** | P7 `[DEC]` → A-next |
...
| G14 | **Loose diagnostic artifacts bloating repo** (`docs/diagnostics` 50 MB) | **CONFIRMED precisely** — LFS covers `VISUAL_REFERENCES/**/*.png` but **not** `diagnostics/**/*.png` → in git history | P2 | P7 `[DEC]` |
...
| CLEAN.4.5 | `[GAP-3]` NaN/Inf robustness sweep on audio→GPU path | degenerate-audio (silence/DC/0-len) produces no NaN in SceneUniforms | Stretch |
...
| CLEAN.7.4 | `[GAP-14]` Extend `rotate_docs.sh` to diagnostics/prompts; remediate the un-LFS'd 49 MB PNG blob | June/Stretch `[DEC]` |
...
| CLEAN.7.6 | `[GAP-9]` `[DEC D-164]` Photosensitivity: certifiable flash-safety invariant — **measurement gate ✅ partial; FFO + Murmuration (7.6) + Nimbus (7.6b Stage 1) = 3/7 SAFE, 2026-06-16**; the 4 multipass/feedback presets (Lumen/Dragon Bloom/Fata Morgana rayMarch + Skein feedback) + output-side luminance clamp → A-next multi-pass real-pipeline harness | June ✅ / A-next |
