...
| Poke "punch" (GLAZE.5 uplift A) | `stems.drumsEnergyDev` → `pokeStrength` boost (swirl jabs harder) | fast (transient) | per-stem A |
| Display glow (GLAZE.5 uplift A) | `stems.vocalsEnergyDev` → bounded ≤0.12 comp-lift add (display-only) | slow (swell) | per-stem A |
| Downbeat camera push (GLAZE.7 uplift C) | `features.barPhase01` (cached grid, energy-gated) → comp `uv` lunge | discrete (per-bar) | the "snap" |
...
| **GLAZE.4** ⏳ wash-out fixed, pending live M7 | Tune the faithful base to **base cert / live M7 confirmed**. **Wash-out fix** (`9371bad`): the comp's flat `+1.0` brightness floor → `kGlazeCompLift = 0.4` (the dominant wash lever; dark ground + bright structure) + a SELF-LIMITING seed (`seed·(1−ret)`, bounds the accumulator sub-white). | meanLuma 0.82/25% sat → 0.34/9.5% at PLAYBACK LENGTH (8000-frame render, both tracks); field stabilises dark, no creep; gates green. Matt live M7 sign-off on the base + the exact floor (gate for uplifts, FA #65). |
| **GLAZE.5 (A)** ⏳ code-complete, pending live M7 | Uplift A — per-stem instrument routing (`7804d77`): **drums → poke-punch** (`drumsEnergyDev`→`pokeStrength`), **vocals → bounded glow** (`vocalsEnergyDev`→ ≤0.12 comp-lift, display-only, wash-safe); bass/other lateral + fullness lift unchanged. | mechanism gate (drums boost poke / vocals raise glow ≤0.12) + replay/render drive drums+vocals from real stems; wash-safe at playback length (calm @8000 = 0.378, no creep); one-primitive-per-layer holds (FA #67); 10/10. M7 = Matt (punch/glow feel + other-role choice). |
| **GLAZE.6 (B)** ⏳ code-complete, pending live M7 | Uplift B — HDR glossy bloom (`861c9a4`): a bounded, DISPLAY-ONLY luminous halo around the bright structure (reuses `blur3`; `kGlazeBloom·max(0, blur3luma−thresh)`, palette-tinted). Kept the warp clamped (no re-unclamp) → the GLAZE.4 wash fix holds; bloom is comp-side, can't re-accumulate. | wash-safe at playback length (calm @8000 = 0.380, localised to the wave, no creep); **flash-safe** — Glaze added to MultiPassFlashHarness, MEASURED peak **0.00 flashes/s** SAFE; 10/10. `kGlazeBloom`/`Threshold` = M7 knobs. |
| **GLAZE.7 (C)** ✅ M7-accepted (Matt live, the connection lands) | Uplift C **RECONCEIVED** (`685927c`): "treble shiver" was dead (no treble in the material) → a discrete **downbeat camera push** instead — the connection "snap" Matt's higher bar wants (Nacre precedent: a smooth field needs one visible beat-locked motion). On `features.barPhase01`≈0 (cached grid, energy-gated) the comp `uv` lunges toward the viewer + settles. | MOTION not brightness → flash-safe (0.00/s, pulses in the harness); sampling zoom → no wash; cached BeatGrid → cold-start-safe; gated → silence holds; mechanism gate (peaks on downbeat, off at silence) + 11/11. `kGlazePush` = M7 knob. M7 = Matt: does it land on the beat + give the connection? |
| **GLAZE.8** ✅ (2026-06-29) | Certification — the flag flip makes the gates *enforce*. | ✅ Matt live M7 sign-off (GLAZE.7 connection lands); `Glaze.json certified:true` + shipped description (NACRE.5 honesty); `FidelityRubricTests.certifiedPresets += Glaze`; `PhotosensitivityCertificationTests.multiPassMeasured += Glaze` (flash covered by `MultiPassFlashHarnessTests`'s real `renderGlaze`, MEASURED 0.00/s); ARCHITECTURE Module Map + plan updated. Production rotation. |
