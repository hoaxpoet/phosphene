...
> **GLAZE.3 learning (wash-out → energy-adaptive decay; reusable for any feedback preset, `61f62d4`):** a
> FIXED feedback decay that looks right on calm music **washes out on dense/loud music** — the per-frame
> energy injection (seed coverage scales with how fast the audio sweeps the tail) outpaces a constant bleed,
> and the accumulator steady-state ~`1/(1−decay)` is hyper-sensitive near 1, so the field over-fills → the
> comp's `+1.0` lift reads bright everywhere with no dark valleys. Fix: **make the decay energy-adaptive** —
> `warpDecay = baseDecay − k·fullnessEMA` — so it drops on loud passages to hold global luminance steady
> (D-157), and is a **no-op at low energy** so the calm look is unchanged. Cherub Rock: meanLuma 0.69→0.57
> (≈ the calm 0.62 baseline). Trade: active music gets slightly softer/less-accreted structure (tunable `k`).
...
| **GLAZE.3** ⏳ code-complete, pending live M7 | Base audio coupling (§6). **Anchor route** (`479f145` → stem-swapped `a34f9d3`): anchor X ← EMA(bassStemDev)−EMA(otherStemDev), anchor Y ← EMA(four-stem fullness), off the stem deviation primitives (D-026); the spring integrates → smooth momentum. Started on the bass/treble bands; Matt's M7 ("loosely connected") + the session analysis (bands near-dead, stems 4× denser) moved it to stems. **Seed fill** (`3d0691e`): the seed band rides the spring tail Y (`gu.seedY`) so it sweeps the frame with the audio (anchor alone moves only the poke; the seed was a fixed band — the 2b.2 band-like gap). **Wash-out fix** (`61f62d4`): energy-adaptive decay (`warpDecay=0.96−0.16·fullnessEMA`) holds luminance steady on dense music (Cherub Rock meanLuma 0.69→0.57, ≈ calm 0.62), no-op on calm. | ✅ route firing in session-replay (real stems.csv via `GLAZE_SESSION_CSV`: pokeX span 1.085, tailY span 1.048); wash-out before/after measured; one-primitive-per-layer holds (FA #67); mechanism gate + replay green; **live M7 = Matt** (gate for GLAZE.4). |
