...
| **GLAZE.2b.2** | Port the FAITHFUL BASE: 3-mass spring warp center (time-driven silence idle; audio→GLAZE.3) + faithful warp (pixel-eq swirl-poke + blur emboss + channel flow) + faithful comp (multi-scale unsharp from blur1/2/3 + glazePalette + contrast). | Side-by-side reads as the glossy contour-jelly; spring idles alive at silence; → Matt live M7. |
