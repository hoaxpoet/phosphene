...
## Operating rules — Do-not list (moved from CLAUDE.md, DOC.4, 2026-06-11)

These eleven Arachne-specific operating rules lived in CLAUDE.md §What NOT To Do from the D-092 → D-095 era. They moved here at the DOC.4 pruning pass (Arachne untouched since ~2026-05-12; the rules bind any future Arachne session, so they live with the design doc that session must read). Text preserved verbatim.
...
The per-increment shader/behaviour narrative that lived inline in the Module Map entry (DOC.3 borderline-call B split). Verbatim:
