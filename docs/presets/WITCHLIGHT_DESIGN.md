...
3. **`tonal_phase_fifths` is alive on all four captures — the divergence axis is founded — but its angular rate varies ~10× between tracks.** Full ±π range everywhere. Raw, it is far too noisy to steer a pen (median per-frame step 0.31–1.04 rad), so it needs the circular EMA that CR.1.2 already established (smooth sin/cos, recombine via `atan2` — never the raw ±π sawtooth). Smoothed, here is what it actually does:
   ...
   ⚠ **These three constants were re-confirmed, NOT re-derived, on 2026-08-17 (BUG-095).** Between 2026-08-04 (`FTR.3g`) and the fix, `TonalAnalyzer` applied its own EMA to this phase, so the shipped preset smoothed it twice and travelled only 0.72 / 1.00 / 3.77 circles — up to **4× short of this row**, on the track where harmony moves most. The gate kept passing because the QG.1 fixtures predate FTR.3g and carry a raw phase column. The tempting move was to re-derive 2.1 / 1.7 / 15.4 down to today's numbers and call the doc stale; measuring first showed the doc was right and the pipeline was broken. Removing the redundant source EMA reproduces this row to within 2 %: **2.09 / 1.80 / 15.10**. Treat these figures as a fingerprint of the driver, not a tolerance to be widened.

   ⚠ **Certification note — and Matt's M7 answered it on 2026-08-18: the certified motion WINS, and this row is no longer the shipped target.** Witchlight was certified 2026-08-07, *during* the double-smoothed window, so the sign-off was on a build whose pen turned 3–4× less than this row specifies. Restoring the row's travel therefore moved the preset away from the approved build. On the restored single pole Matt read the result as *"slightly less coupled to the beat … drifts a bit more out of sync over time"*.

   Measured on his session `2026-08-18T14-09-35Z`, the beat machinery was **bit-identical** between the two builds — 50 downbeat bursts, 50 off-beat pulses, head flares landing within 10 % of a beat 86 % of the time (chance 20 %), pen speed swing 4.13× — and the only quantity that moved was heading turns, **50 → 74**. Nothing about beat *timing* changed; what fell was the accents' *legibility* against a stroke wandering ~50 % more.

   So `WitchlightTuning.phasePreTau` (0.8 s nominal, shipped at **0.4 s**) makes the second pole explicit and **local to this preset**. The engine fix stands for every other consumer — Nacre, Cymatic and Fractal Tree each smooth on their own and were certified WITHOUT a second pole, and Matt confirmed Nacre reads correctly after the change. `WitchlightPathTests` now asserts the shipped two-pole travel (0.51 / 0.74 / 2.26 on the fixtures) rather than this row.

   ✅✅ **SECOND M7, 2026-08-18 (`2026-08-18T18-04-06Z`, Matt: *"Witchlight looks good"*) — this is the one that closes the gap the first M7 left open.** That session ran at 16.7 ms median with **4 frames (0.0 %) over the frame-time cap**, so BUG-097 was dormant and the accents fired at their designed rate: **55 downbeats : 162 off-beat pulses (2.95:1)**, alignment 67 % within 10 % of a beat, head never off frame. So Matt has now approved the calm stroke and the working accents **together**, which the first M7 could not cover.

   ⚠ **Two honest limits on that.** (a) The session ran the **WL.13** binary — `WitchlightPath.o` compiled 11:10:12, WL.14 merged 13:03:44, the app linked 13:03:49 — so it does not exercise WL.14's code. It does not need to: with 0 % of frames over the cap, clamped and unclamped `dt` are identical by construction, which is also why the offline probe reproduces that build faithfully. (b) **WL.14 itself is therefore still un-validated live** — it only changes behaviour under load, and no session carrying the fix has yet been recorded on a loaded machine. The bad case is evidenced only offline (6 → 105 off-beat pulses on `2026-08-18T16-10-38Z`).

   ✅ **M7 CONFIRMED LIVE, 2026-08-18** (`2026-08-18T16-10-38Z`, Matt: *"Looks good overall"*). The shipped two-pole response measures **42 heading turns** on that session against 74 pre-fix and 50 on the certified build, with the head never leaving frame. ⚠ Scope of the sign-off: it covers the STROKE and the brightened ribbon. It does **not** cover the beat accents — that session was the worst BUG-097 case measured (48.8 % of frames over the frame-time cap, 6 off-beat pulses in 110 s against ~130 implied), so the accents were largely absent from what he was judging. BUG-097 was fixed immediately afterwards at WL.14 and restores that session to 105.

   ⚠ **This row is kept, not deleted, because it is still the correct reading of a single-pole response** — and because the gap between it and the shipped figures is the whole lesson: a design measurement describes a pipeline, not a look. Only Matt's eye decides which pipeline the preset should have.

   **A side effect worth recording.** A calmer stroke sweeps fewer pixels, so ribbon share fell 0.406 % → 0.368 % against a 0.40 % floor (WL.2-g) — a gate that had only 1.5 % headroom to begin with. Fixed at the shading layer as the gate's own message prescribes: the bead halo's falloff widened 2.8 → 2.1 *inside* the existing quad, giving 0.433 % **and 16 distinct beads, up from 13**. That is the opposite direction from WL.2-j's failure, which is the tell that the quad extent (`WL_HALO_EXTENT`) was the wrong lever and the profile was the right one.
