...
**WL.13 amendment (2026-08-07) — where the pen learns `home`, and what it draws until it does.**

The steer is `θ̇ = clamp(c · wrap(φ̄ − home), ±ω_max)`, so the SIGN of the excursion from tonal home is the sign of the curvature. `reset()` runs on every track change and used to leave `home` at 0 rad — an arbitrary point on the circle of fifths with no relationship to the song about to play — and a 12 s EMA then took ~30 s to walk to the real centre. For that whole window the excursion held one sign, the curvature held one sign, and the pen wound a coil. **30 s is also the trail window, so the entire visible figure for the first half-minute of every track was the convergence rather than the music.**

Measured (WL.13, three recorded sessions of 2–4 min): heading monotonicity **0.82 / 1.00 / 1.00** over the first 15 s, 0.88 / 0.49 / 0.44 over the first 30 s, and **0.10–0.23 over the whole run** — the coil was a cold-start artefact, not the mechanism. Cross-track heading correlation on the 21 s fixtures, which are entirely inside that window, read **r = +0.995** for the two harmonically-quiet tracks; on the real sessions, different music read r = −0.09…+0.12, with two captures of the *same* track at +0.974 as a control.

Two changes, and the second is the fallback for tracks the first cannot reach:

1. **The tonal home is measured before playback.** `SessionPreparer` already runs the full `MIRPipeline` over every preview clip, so it now reports the circular mean of `tonal_phase_fifths` as `TrackProfile.tonalHomeFifths` (`nil` below a resultant length of 0.10 — a harmony with no centre worth naming). `WitchlightPath.ingestTonalHome` installs it through the CPU-only runtime bridge that `sectionIndex` and `beatDriftSeconds` already use, so the steer is right from frame 1 and no `FeatureVector` field or MSL layout contract is touched.
2. **Without one, the pen runs straight.** `homeSettleSeconds` (15 s) suppresses steering until home is worth trusting, and `home` warms up as an unbiased running circular mean (`min(homeTau, age)`) so its first steer reads a real centre rather than a point 15 s into a ramp. Matt's call, 2026-08-07: an honest straight line while the key is still being placed beats a coil that claims to be the harmony and is not.

Result on the fixtures: worst cross-track pair **+0.995 → +0.808**, first-15 s monotonicity 0.30 / 0.04 / 0.03. On the sessions: whole-run monotonicity **0.05**, turns/trail 1.51 / 1.32 / 1.46. This is now a standing gate with a ceiling of 0.90, a same-track control, and a falsification test that drives two different fixtures through an identical harmony-blind steer and asserts the ceiling trips (measured r = +0.970) — see `WitchlightPathDivergenceProbe`.

**The cost, and the second WL.13 change.** A pen that no longer coils covers more ground per 30 s trail, so the auto-fit zooms out and the ribbon thins: distinct bright cores 15 → 4 against a floor of 8. The recovery is `baseSpeed` 0.10 → **0.060** (Matt: *"slow the pen down so the ribbon fills the frame again"*). It works for a geometric reason worth recording, because it is the opposite of the intuition: `baseRadius` is **screen**-space while bead spacing is `baseSpeed / emissionHz` in **world** space, so what decides whether beads read as separate cores is spacing × `viewScale` against a fixed diameter. A zoomed-out fit shrinks the numerator alone. Slowing the pen shortens the trail, the fit comes back in (`viewScale` 1.16 → 1.94), and screen spacing returns to what §3.3's emission rate was solved for — 0.0158 × 1.94 = 0.0307, against 0.0264 × 1.16 = 0.0306 before. Cores recover to 11. **`emissionHz` is deliberately NOT re-solved against the new speed**: that derivation assumed a world-space bead, and coupling the two only removes beads (ribbon share 0.247–0.355 % across the sweep, against 0.359–0.387 % with emission left alone).

Two consequences are open and are Matt's calls, not constants to move:

- **`headingTurnsPerTrail` on So What is 0.69.** ω_max is `baseSpeed / minTurnRadius`, so a slower pen turns less per trail — but the larger part is that So What's harmony genuinely does not move (§2.3; WL.13 measured φ̄ displacing 0.28 rad per 10 s, half a step on the circle of fifths). Its old 1.92 was the coil inflating the number. Matt lowered the QG.5 floor 1.20 → **0.65** on 2026-08-07 with that stated: on a track whose harmony is static, the honest figure is close to an arc. The band's remaining detection range is correspondingly narrow — WL.2's arc-drawing model measured 0.20–0.73, so 0.65 still catches most of it but not all.
- **Ribbon share is 0.375 % against a 0.40 % floor** (0.406 % before WL.13). Speed moves it only weakly — 0.359 → 0.404 across a 2.5× slowdown — and the gate's own text names this a *shading* fix (widen and brighten the halo, lift the line) and explicitly not `baseRadius` or `emissionHz`. Open at the close of WL.13.
