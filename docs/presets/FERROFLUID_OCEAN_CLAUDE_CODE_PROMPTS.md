...
**Phase 1 — Scaffolding (no audio yet).** Build the particle buffer + height-map bake compute kernel + sceneSDF read + render-pipeline wiring. Particles at FIXED positions for testing — no SPH physics yet. **Particle initial positions:** place each particle at the XZ coordinate a `voronoi_smooth` cell-center pass would emit (hex grid + per-cell `voronoi_cell_offset` hash). A strict regular grid would (a) fail the "no regression vs Phase A" acceptance below and (b) trip the README's "Perfectly regular hexagonal lattice" anti-reference. The cell-hash offsets are what give smooth Voronoi its organic non-uniformity; replicating them in the particle initial positions preserves the Phase A visual character with the new geometric source. Acceptance: render produces a hex-pack pyramid field equivalent to the current Phase A output — scale + density as anchored in `docs/VISUAL_REFERENCES/ferrofluid_ocean/README.md`'s `01_macro_*` annotation. **Phase 1 closeout evidence:** standard 4-fixture contact sheet at 1920×1080 (silence / steady-mid / beat-heavy / quiet), rendered on the Phase 1 branch and on `main`, side-by-side, with a written per-fixture verdict ("no regression" / "regression in X channel" / etc.). No live-music recapture at Phase 1 — defer to Phase 3 where particle-motion audio routing is in place to exercise. STOP and surface to Matt after Phase 1 — if scaffolding produces structurally different output than the smooth-Voronoi current, the integration has a problem before audio gets added.
