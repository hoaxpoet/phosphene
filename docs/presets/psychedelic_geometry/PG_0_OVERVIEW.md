...
| PG.4 | **Truchet Loom** | Multiscale Truchet weave — arc-tiles subdivide into smaller tiles of the same motif, self-similar across scales | `direct_time_modulation` (crisp frames; subdivision is the motion) | **Complexity / density mapping** — musical busyness drives how finely tiles shatter into nested weaves | lightweight* |
...
3. **Build order.** *Recommendation:* **PG.4 Truchet Loom first** (lowest risk — pure 2D, no feedback, well-trodden Shadertoy prior art; proves the density-mapping routing), then **PG.1 Mandala** (proves the `mv_warp` breathing + downbeat bloom), then **PG.2 Droste** (beat-lock, higher risk), then **PG.5 Poincaré** (Möbius math), then **PG.3 Mandelbox** (highest fidelity bar, own risk budget). *Default if silent:* that order.
