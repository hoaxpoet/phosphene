...
- **Mesh-preset lighting:** only via `passes: ["ray_march"]` + `setMeshGBufferEncoder` closure (`RenderPipeline+PresetSwitching.swift:167–177`); consumer = `FerrofluidMesh`. **⚠ Both were deleted at RECON.15 (2026-08-26)** — this route no longer exists in the engine; recover from git history if a future preset needs it. Direct `mesh_shader` path cannot do PBR materials/lighting.
