...
**Fullness + dance pass (2026-07-17, Matt M7 #1 — "not as full as the reference, dance is limited; keep the violet crown").** Fullness comes from ray COUNT + sector width, NOT a bright body floor (raising `kBandFloor` reads as fog): `kStriationFreq 11→15`, `kBandHalfWidth 0.30→0.40`, `kCrownFadeR 0.40→0.34` (rays extend higher), `kBandFloor` kept low (0.13). Dance: `kStriationAdv 0.15→0.38` (curl sway — rays wave/curl), a **sideways travelling brightness wave** across the rays (`kTravelSpeed`) + shimmer, BOTH scaled by `motionAmp` (quicken on swells); `kBandMeanderAmp 0.12→0.17`, `kBandDrift 0.05→0.09` (the whole arc sways more). darkFraction 0.32 (still > 0.30), no clip.

**Reactivity — the dance is driven by the `other` stem (2026-07-17, Matt M7 #3 "address music reactivity").** Instrumenting real MIR (love_rehab render window) showed the previous dance driver `mid_att_rel` is nearly DEAD on real music (min −0.006 / max +0.006) — `saturate(0.5 + 0.5·mid_att_rel)` pinned `motionAmp` ~constant, so the curtain barely reacted. `other_energy_dev` (the synth/pad stem — Aurora Veil's song-defining anchor per [[feedback_av_other_stem_load_bearing]], previously routed to NOTHING after the reauthor) swings 0.04→0.64. The dance vigour (sway/travel/shimmer rate) is now `mix(midProxy, saturate(other_energy_dev·kOtherDanceScale), stemMix)` — `other` after warmup, mid proxy for the first ~10 s. Validated on the `AURORA_REAL_GIF` harness (the real-audio feedback loop — the synthetic swell GIF cannot validate reactivity, FA #27). Manifest `motion_amp` primitive → `otherEnergyDev`. Routing table (one primitive per layer, FA #67): dance←`other_energy_dev`, brightness←`bass_dev`, hue←`vocals_pitch`, kink←`drums_energy_dev` (rare), stars←`bar_phase01`.

**Performance — curl_noise out of the march loop (2026-07-18, Matt M7 #4 "computer performance was awful, very choppy" on an M2 Pro).** The reauthor called `curl_noise` (= 6·`fbm8` = **48 noise octaves**) TWICE inside `aurora_footprint`, which runs once per march step × 48 steps → **~4 600 Perlin evals per pixel** (~9.5 B per 1080p frame). Fix: the two advection warps are now computed **once per ray** in `aurora_march` (each pixel still gets its own warp from `rd.xz`, constant along the ray → more coherent) using **`fbm4`** (a smooth [-1,1] scalar) instead of `curl_noise`. Noise cost per pixel: 4 608 → **~8 Perlin evals (~576× fewer)**. The old per-STEP curl only *looked* smooth because the running-average smeared it; sampled once it marbled the rays, and `curl_noise`'s ×50-amplified fbm8 detail was the wrong tool — `fbm4` is smooth at a single sample. Organic ray curve is recovered with cheap along-ray meander in `aurora_rays` (per-ray out-of-phase sines) + `kStriationAdv 0.38→0.80`. (`FBM.metal`: "avoid fbm8 inside ray-march loops" — now heeded.)

**Half-resolution render (2026-07-18, Matt M7 #5 — still choppy after the noise cut, across ALL presets).** The noise cut alone did NOT fix the choppiness → the fragment shader was not the (whole) bottleneck. The real gap: `bindAuroraVeilRuntime` never set a direct render scale, so the 48-step convergence march ran at **full native resolution** — 4–7× the pixels on a high-DPI Mac-mini panel — while the identical-profile Nimbus march has always used `setDirectRenderScale(0.5)` (½-res + upscale, ~4× cheaper). Aurora Veil now does the same (`PhospheneApp/VisualizerEngine+Presets.swift`). Confirmed resolution-bound: framerate went 322→2511 frames/session. BUT 0.5× **undersampled the thin rays + 1px stars** ("doesn't look like the aurora" — the stars nearly vanish under the ½-res→native upscale, and Aurora Veil's high-frequency rays are unlike Nimbus's forgiving soft gas). Resolved by (a) hoisting the per-step `atan2`/`length` out of the march — `atan2(rd.z,rd.x)` is t-invariant and `rad = t·|rd.xz|`, so both are computed once per ray, not 48× (the drum kink becomes a small per-ray angular offset, gates green) — which buys back enough cost to (b) raise the render scale to **0.66×** (1.7× more pixels than 0.5×) for crisp rays + stars while staying smooth. NOTE: if choppiness ever persists on *lightweight* presets too (e.g. Waveform), the bottleneck is systemic (real-time ML stem sep / SessionRecorder I/O / a global native-resolution target), not this preset — needs frame-time instrumentation, not more shader work.

**Final constants (2026-07-17):** `kStriationFreq 15.0`, `kRayWidth 0.15`, `kCrownFadeR 0.34`, `kStriationAdv 0.38`, `kTravelSpeed 0.55`, `kBandFloor 0.13`, `kBandCenterAngle 1.62`, `kBandHalfWidth 0.40`, `kBandMeanderAmp 0.17`, `kBandDrift 0.09`, `kElevTilt 0.55`, `kCrownCurve 1.6`, `kCrownH 0.286`, `kAuroraGain 1.35`, `kLookPitch 0.52` (already an overhead curtain; Q1 met without re-tilting). Palette crown kept **violet/magenta** (Matt M7 #1). The nimitz tri-noise chain (`aurora_tri`/`tri2`/`mm2`/`tri_noise_2d`, `kSubstrateSpd`) is **retired**.
