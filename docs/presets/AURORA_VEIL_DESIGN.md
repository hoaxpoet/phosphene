...
> **⚠ Amendment 2026-07-19: AV.7 reauthor — faithful nimitz port (D-185). THIS SUPERSEDES §5.2–§5.10 BELOW.** Aurora Veil is now a faithful MSL port of nimitz's "Auroras" (Shadertoy `XtGGRt`), not a derivation from it. Every AV.2–AV.6 addition — the Lawlor footprint `F(x)`, the three parallax columns, the band undulation, the drum kink, the traveling waves, the eight-then-three audio routes — was **deleted**. The shader had also drifted from the reference itself (3D march flattened to a 2D column, a global time rotation added to `triNoise2d`, gain 1.8 → 2.4); the port restores nimitz's algorithm and constants exactly.
>
> **What the preset is now:** a static **upward** view of the sky — no horizon, no ground, no reflection — with green ray-curtains overhead warming toward magenta near the horizon, over nimitz's starfield. Reactivity is three non-competing axes: stars keep the **downbeat** (`bar_phase01`, gated by `pulse_amp01`), brightness **breathes with the mood envelope** (`arousal`, clamped 0.85–1.15, plus a subordinate `bass_att_rel` lift), and colour **warms with the mood** (`valence` → whole-palette phase). **CERTIFIED 2026-07-19** on Matt's M7 sign-off.
>
> **§5.10 (the AV.5 footprint reauthor) is retired, not implemented.** It reserved D-185 but was never written up or shipped; D-185 now records this reauthor and the supersession. Sections §5.2–§5.9 describe the pre-AV.7 multi-column/mv_warp architecture and are retained as history only — read them for the failure record, not as a description of the shipped preset. The still-current design rationale is D-185 plus the shader header.
...
### 5.10 Footprint reauthor (amended 2026-07-14 — AV.5) — ⚠ RETIRED, NEVER SHIPPED (superseded by the AV.7 port, D-185)
