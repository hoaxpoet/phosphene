...
**Signal confirmed (no plumb):** `StemFeatures.{vocals,bass,other,drums}AttackRatio` exist and are populated on the same struct the geometry already consumes; dynamic in the real session (0.31→3.0, baseline ~1.0). **Validation:** `test_articulation_shortensStaccatoLines` — staccato art=1.00 / legato art=0.00, mean particle age **0.37 s (staccato) vs 7.64 s (legato)** (~20× shorter lines); `test_articulation_contactSheet` (RICERCAR_VISUAL=1) renders the side-by-side still (LEFT choppy-dense short strokes / RIGHT long flowing arcs — clearly distinct). Beethoven video harness: no regression, INTENSITY r=**+0.69** (unchanged from FL.13), and the legato-default look (harness has no band-stems → AttackRatio 0 → all-legato) is the intact FL.13 flowing field. **The gate is Matt's live eye on the band-stem (rock/pop) path — the headless harness can't produce Open-Unmix stems, so articulation is live-only.**
