...
**Aesthetic direction — Matt's call (2026-07-08):** shown the full "flowing curtain of light" vs sparse "distinct weaving ribbons" on the real-audio video, Matt chose **sparse weaving ribbons over open dark space** (ref 01 spirit). Tuned there (~1200 bright long-lived particles, dark ground, decay 0.95). **STATUS: code-complete, pending live M7** — green (engine+app build, lint 0, flow gate + substrate/registry/count tests) but the "is this Fantasia enough" felt/craft call is Matt's live review, not self-certifiable. Follow-ups if it clears: longer/smoother arcs (lower turbulence) if streaks read too curly; loosen/tighten the family vertical zones (teal high / violet mid / gold low) per Matt's eye.
