...
**NEXT — FL.14 (fresh session; prompt `prompts/RICERCAR-FL.14-prompt.md`):** staccato vs legato LINE CHARACTER (Matt: staccato → shorter choppier lines; strings → more flowing). Signal exists: per-stem **`AttackRatio`** (baseline ~1.0, peaks 3.0). Design challenge: per-colour trail length on a single shared-decay trail (candidate: per-family particle life). Constraint: "choppy LINES" must NOT become "choppy MOTION" (the FL.12 herky-jerky rejection).
