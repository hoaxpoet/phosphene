...
**★ REVERTED TO FL.10 (2026-07-09, Matt's call) — FL.11→FL.14 ALL undone; head is the confirmed-brilliant FL.10 code.** After FL.14's articulation was reverted to the FL.13 baseline, Matt live-read the FL.13 baseline ITSELF: *"No connection to the music. Pure chaos. Dislike immensely."* Diagnosis (measured, git-diff FL.10↔FL.13): **FL.10's felt connection was a whole-field ENERGY SURGE** — `flowSpeed = 0.0015 + 0.0040·energy`, a slow drift at rest surging when loud, the entire field moving together with the music. FL.12/FL.13 **gutted it** — cut the surge to `0.0012 + 0.0016·energy` (barely responsive) and moved the energy response into slow, *independent* per-colour drift currents (each turning its own way at 0.001–0.005) + removed the beat accents → no unified surge, four colours wandering aimlessly = "pure chaos, no connection." (Confirmed by the render gate: FL.10 driven `lit=0.638`/rest `0.000` — a dramatic surge; FL.13 driven `lit=0.215`/rest `0.006` — barely moves.)

**★★ THE RECONCILIATION (Matt: "FL.10 showed potential but was not perfect; I got excited by the direction" + "I'm going to want music reactivity some way or another").** FL.10 is the **foundation, not the destination** — its whole-field energy surge is the rock-solid *connection* that must never be lost, but Matt wants Ricercar to grow richer. **Confirmed direction (AskUserQuestion 2026-07-09): "instruments as distinct voices"** — each instrument/section its own colour with its own character (the "orchestra painting itself"), all riding the same shared surge so it stays connected. This was the FL.13/FL.14 *aim*; the executions were wrong, not the goal.

**★★★ THE PRINCIPLE for doing it right — MOTION stays shared, IDENTITY rides on PRESENCE.** Every FL.11→FL.14 failure put per-instrument character into MOTION (independent drift, beat-lock, particle life) — and motion is exactly what carries the connection, so differentiating it fractured the field into "chaos, no connection." The fix: **all voices ride the one whole-field energy surge (motion = shared → always moves with the music), and each voice's identity is expressed through PRESENCE instead** — its colour blooms brighter/denser/fuller (brightness + local particle density, NOT its own motion direction) when its instrument plays, recedes when it doesn't. Presence MUST be driven by a reliable per-instrument LOUDNESS/energy signal (the FL.13 band-stem∣family hybrid was the right signal — wrongly wired to motion; FL.14's AttackRatio was a ratio that fires when quiet). Presence gated by actual loudness cannot "appear disconnected" — by construction a voice only surfaces when its instrument is loud. **Validate any such increment in SIMULATION against the failed session streams (`19-35`, `20-01`, `20-20`) before a live look; add one change at a time with a live look between each** (the FL.11→14 collapse came from stacking four unconfirmed increments between Matt's looks). FL.11→FL.14 code recoverable from git (FL.11 `4edb4e9`, FL.12 `cf6241f`, FL.13 `3a11bfa`, FL.14 `3242d7a`→`9eaa72a`).

--- superseded FL.11→FL.14 records below (kept for the recoverable history) ---

**FL.14 — staccato vs legato LINE CHARACTER: ATTEMPTED, then REVERTED (2026-07-09, Matt's Choice). Head is back at the FL.13 code.** Matt asked for staccato/percussive material to paint shorter/choppier lines and sustained material to flow. Mechanism tried: per-family particle **LIFE** driven by each colour's per-stem **`AttackRatio`** (short life → fast respawn → choppy; long life → flowing ribbons), recomputed at respawn — motion untouched (only respawn cadence changes, so no FL.12 herky-jerky). Two live misses, then reverted:

- **FL.14 live (session `19-35-13`): "it all looks legato."** Instrumented root cause: real AttackRatio is baseline ~1.0 with brief sparse spikes (~4 % of frames >1.5), and an instantaneous map → 0.3 s symmetric EMA → linear life collapsed it to mean env 0.05 → life 15.2 s (legato everywhere).
- **FL.14.1 recalibration (peak-hold env + geometric life) live (session `20-01-24`): "teal and orange just appear, unconnected to the music."** ★★ **DECISIVE root cause (instrumented):** `AttackRatio` is a *ratio* (attack ÷ sustained), so it **spikes when a stem is QUIET** — measured **40 % of drums and 51 % of "other" attack-spikes fired while that stem's energy was flat** (e.g. 64–88 s: drums energy ~0.03–0.09 but articulation 0.39–0.49 → teal went choppy with no drums playing). The env was gated only by *total* energy, never by the *specific* instrument's presence → line-character changed disconnected from what's heard.
- **Matt's call (AskUserQuestion): revert to the connected FL.13 baseline** (colours move + brighten with the music, no line-character) rather than attempt a third live fix.

★★ **LESSONS (durable):** (1) **A ratio is not a presence signal** — `AttackRatio`/onset-sharpness spikes on quiet material; any visual it drives MUST be gated by the source's actual loudness/presence (energy), or it fires disconnected (an FA #31 / FA #67 cousin). (2) Calibrate against the REAL signal distribution, not an idealized "baseline 1.0, peaks 3.0" story (that hid the baseline-with-sparse-spikes reality; a symmetric smoother then erased the sparse transients). (3) On orchestral tutti the four band-stems move together (energies ~0.27–0.30 each) → any per-colour behaviour reads at the SECTION level, not as one identifiable instrument line (the FL.13 stem→family limit). **If line-character is retried, the design must energy-gate articulation per stem (choppy only when that instrument is both present AND sharp) and be validated against the `19-35`/`20-01` session streams in simulation before any live look.** The FL.14/FL.14.1 code + tests are recoverable from git (commits `3242d7a`→`9eaa72a`).
