...
**Evidence:** `NACRE_FIFTHS`-injected diag frames show the palette shift **gold-green → teal-green** across a key change (shifted-but-related, not a jump); D-181 verdict table PASS on all mandatory traits + anti-refs (esp. no hue-strobing); flash re-measured **0.50 flashes/s SAFE**; `RouteCoverageTests` green (`hue_from_harmony`→`tonalPhaseFifths`, `saturation_from_consonance`→`tonalConsonance` both fire on all 3 fixtures); sidecar `description` + `audio_routes` updated (NACRE.5 honesty). Saturation's isolated visual effect is confounded by feedback chaos in a still — an M7 watch-item.

**Round 1 M7 (session `2026-07-10T15-00-03Z`): "not sure if it worked."** The data proved the pipeline was healthy — tap alive (bass mean 0.19, ~197 s), `tonal_phase_fifths` swept the full ±π range (10 621 distinct), `tonal_consonance` mean 0.107 (well above the 0.05 gate). So the coupling was **active with good signal** — but **invisible**. Root cause (quantitative): round 1 was `palette = time + harmonyOffset`; over 197 s the CLOCK advanced **~13.7 full palette cycles** while the harmony offset spans only **±0.5 cycle** — time dominated harmony ~27:1, so the palette rotated through every colour on the clock regardless of the key, and never held on a vamp. An additive nudge on a full time rotation cannot read.

**Round 2 (2026-07-10): harmony SETS the position, the clock is demoted.** `nu.hueShift` now carries the FULL palette phase, computed CPU-side: `harmonyAnchor·gate + driftAccumulator`, where the drift rate is the faithful ~14.4 s cycle at silence/atonal (rate 1.0) and nearly stops when tonal (0.06 ≈ 240 s) — a continuous accumulator so the rate change never snaps. The key you're in IS the colour; on a vamp the hue **holds** (the palette's motion then comes from the harmony moving through the song's chords), on a modulation it **moves**, at silence it restores the faithful full rotation. Shader: the palette seed reads `nacrePalette(nu.hueShift)` (not `nu.time + hueShift`); `nu.time` still drives grain + radial pulse. **Evidence:** dev-preview frames now show a *legible* separation — key A gold-green vs key B green-teal at the same time (harmony sets position), and the same key **holds** its colour across 4× the elapsed time (clock demoted); flash re-measured 0.50 flashes/s SAFE; routes unchanged (still green). **Matt re-M7 (2026-07-10): "looks good" — SIGNED OFF.** Nacre keeps its certification with the real harmony-hue coupling; the round-1 "additive nudge on a full rotation is invisible" lesson is the durable finding. Parked round-3 levers (only if wanted): the tension→rim-dispersion secondary (fixture-breadth blocks the gate today) and widening the hue mapping if the ~30°/fifth travel ever reads too subtle.
