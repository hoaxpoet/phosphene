...
| Body colour (cool↔warm) | **warmth = valence + energy** (energy = arousal + bloom swell) — smoothed in state | Mood + energy — slow global (NB.6, **NB.10 D-144**) |
...
- **M7**: ✅ **PASSED** (Matt, live, 8 tracks, session `2026-06-05T20-33-47Z`) — mood travels cool↔warm per track (Pyramid Song cool, Love Shack/In Undertow/Love Rehab/Atlas warm, Sad Song / A Girl In Port travel) and bodies read properly sized after the r1.6 bloom recalibration. Beat-grid live phase ("too active / not synced" on some tracks) accepted as a known limitation, deferred to **D-145**.
...
- **NB.9 — certification. ✅ CERTIFIED 2026-06-05** (Matt M7, session `20-33-47Z`, 8 tracks). Acceptance invariants (§5.7) gated, golden dHash registered, anti-reference manual check clean, M7 passed — `Nimbus.json` `certified: true`, `"Nimbus"` in `certifiedPresets`. **First certified `volumetric` preset.** M7 r1/r1.5 did not certify but ran the stranded old `main` build (the NB.10 changes were on an un-integrated worktree branch — [[feedback_worktree_changes_reach_build]]); cert passed on the first build that actually had them. Beat-grid (D-145) deferred; palette-extension a noted future enhancement (§8).
...
### Post-cert future enhancements (noted, not scheduled)

- **Extend the mood palette beyond cool-purple ↔ warm-gold (Matt, M7 cert 2026-06-05).** The certified mood axis is a two-pole `mix(kNimbusMoodCool, kNimbusMoodWarm, warm01)` — vivid indigo-violet to rich amber/gold. Matt: "the three-state mood approach is fine, but we should consider a future enhancement to extend the colour family beyond cool purples and warm golds. This could be a fun update." A richer family (e.g. a multi-stop palette ramp keyed on `warm01`, or a valence×arousal 2D colour field, or per-stem tint per §8.2) would broaden the expressive range without changing the energy-warmth model. Pure shader change (the poles + the `mix` are the only colour machinery). Not scheduled.
- **Beat-grid live phase (D-145).** The "too active / not synced" feel on beat-driven tracks is bounded by the shared cached-grid phase, not Nimbus — its own infrastructure project (after Skein). See `docs/diagnostics/BEAT_GRID_LIVE_PHASE_PROJECT_2026-06-05.md`.
