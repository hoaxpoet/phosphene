...
| LM.3.2 | **Substantive look ships here.** Band-routed beat-driven dance (Decision D.5): each cell hashed into one of four teams (30 % bass / 35 % mid / 25 % treble / 10 % static); cells advance their palette index discretely on rising-edges of `f.beatBass / beatMid / beatTreble` — debounced 80 ms, scaled by energy. Per-cell `period ∈ {1, 2, 4, 8}` controls how many team-beats between advances (Pareto distribution ≈ 50–60 % of cells changing per second). Brightness uniform with hash jitter + bar pulse from `f.bar_phase01 ^ 8`. Per-track palette seed perturbations bumped (kSeedMagnitudeA/B/C/D was 0.05/0.05/0.10/0.20 → 0.20/0.20/0.30/0.50). **Calibration follow-up 2026-05-09**: cell density `kCellDensity` 30 → 15 (cells across 60 → 30 per Matt's M7-prep review); palette endpoints widened so HV-HA and LV-LA produce genuinely different palettes (cool → blue-base, warm → red-base) rather than permutations of the same colour wheel. | ⏳ awaiting M7 |
