...
**Status:** Active. Revised 2026-05-09 (LM.3.2) after both LM.3 and LM.3.1 were rejected in production. Changes in this revision (see §11 Revision History): the continuous-palette-cycling form of Decision D.4 (cells advance via `accumulated_audio_time × kCellHueRate`) is retired because Spotify volume normalisation pulls mid + treble bands toward zero (BUG-012) and `accumulated_audio_time` advanced ~10× too slowly to read as cell motion in production. Decision D.5 — **band-routed beat-driven dance** — is adopted in its place: cells advance their palette index discretely on each FFT-band beat, with a per-cell team assignment (bass / mid / treble / static) sampled from a hash + per-track seed. The LM.3.1 agent-driven static-light-field variant is also retired ("the bright pools dominated the visual story", Matt 2026-05-09); cell brightness in LM.3.2 is uniform with hash jitter, the dance is in the *colour change*, not the brightness. Per-track seed magnitudes bumped (kSeedMagnitudeA/B/C/D was 0.05/0.05/0.10/0.20 — now 0.20/0.20/0.30/0.50) so different tracks visibly differ at the same mood. The original LM.1 / LM.2 / LM.3 / LM.3.1 implementations stand as scaffolding that proved the slot-8 binding + procedural palette + per-cell hash + rising-edge tracking; the **substantive energetic-dance look ships at LM.3.2**.
...
### Decision D — Cell color sourcing — RESOLVED 2026-05-09: D.5 (LM.3.2)

> **Iteration history.** D.1 (cell-quantized agent sample) was implemented in LM.2 and produced a smooth gradient blob — adjacent cells got nearly identical colours from a smooth analytical light field. D.4 (per-cell `palette()` keyed on `cell_hash + accumulated_audio_time × kCellHueRate + mood`) replaced it at LM.3 — cells got distinct identities, but in production the cycling didn't show: Spotify volume normalisation pulls mid + treble bands toward zero (BUG-012), so `accumulated_audio_time` advanced ~0.045 / sec instead of ~0.5 / sec, and the time-driven cycle was effectively static for entire songs. LM.3.1 tried to recover the audio-driven feel by adding an agent-position-driven static-light field as backlight character — Matt 2026-05-09 rejected this: "fixed-color cells with brightness modulation; the bright pools dominated the visual story." Both D.4 and D.4 + LM.3.1 are retired.

**Adopted: D.5 — Band-routed beat-driven dance.** Each cell is hashed (`cell_id ^ trackSeedHash`) and assigned to one of four teams. The cell's palette index ratchets discretely on each beat the team registers — *not* continuously on a time accumulator. Per-cell brightness is uniform with hash jitter; the visual story is colour change synced to the song's beat, not brightness modulation.

```
cellHash      = lm_hash_u32(cell.id ^ lm_track_seed_hash(lumen))
team          = cellHash % 100        // 0–29 bass, 30–64 mid, 65–89 treble, 90–99 static
period        = {1, 2, 4, 8} from cellHash >> 8 & 0x7  // Pareto Pareto-shaped
team_counter  = lumen.{bass,mid,treble}Counter         // CPU-side rising-edge of f.beat{Bass,Mid,Treble}
step          = floor(team_counter / period)
cell_t        = float(cellHash & 0xFFFF) / 65535
phase         = cell_t + step × kPaletteStepSize + smoothedValence × kPaletteMoodPhaseShift
hue_rgb       = palette(phase, a, b, c, d)             // V.3 IQ cosine palette; a, b, c, d shifted by mood + per-track seed
intensity     = (0.85 + 0.15 × hash_jitter) × (1 + 0.30 × bar_phase01^8)
albedo        = clamp(hue_rgb × intensity, 0, 1)
```

**Why this works:**

- **Cells dance in time with the song's beat.** Rising-edge of `f.beatBass` (or mid / treble) increments the team's counter exactly once per beat (debounced 80 ms), scaled by an energy term so loud beats advance further than quiet ones. The cell's palette index moves only on its team's beats — *the dance is locked to the music*, not to a wall-clock cycle.
- **Different cells dance to different bands.** Bass-team cells (~30 % of the panel) advance on kicks; mid-team cells (~35 %) on the melody carrier (typical pop / electronic); treble-team cells (~25 %) on hats / cymbals; static-team cells (~10 %) hold their colour for the whole track. The team is per-cell, sampled from the hash — neighbouring cells often belong to different teams, so the panel reads as a *coordinated ensemble* rather than three monolithic regions.
- **Different songs dance differently.** A track with a strong kick and quiet vocals produces visible bass-team movement and mostly-frozen mid-team cells. A track with a busy melody and minimal drums produces the inverse. The dance reflects the song's instrumentation, not just its tempo.
- **Cell density target ~50–60 % changing per second.** With Pareto-shaped periods (~37.5 % period 1 / 25 % period 2 / 25 % period 4 / 12.5 % period 8), three teams firing at independent rising-edges, and a typical 4-on-the-floor beat at 120 BPM (2 bass beats / sec), the aggregate density lands close to Matt's "bubbling cauldron" register. The `kPaletteStepSize = 0.137 ≈ 1/φ²` ensures consecutive steps land far apart on the palette wheel — a single advance moves the cell ~50° around the hue circle, so the change is visible without strobing.
- **Static cells, rotated per track.** The team hash is XOR'd with `lm_track_seed_hash(lumen)`, so the 10 % of cells in the static team change identity from track to track. A cell that holds its colour for all of "Love Rehab" might be a bass-team cell for "Pyramid Song".
- **Per-track palette identity.** The seed perturbs `(a, b, c, d)` by ±0.20 / 0.20 / 0.30 / 0.50 (LM.3.2 magnitudes — bumped from LM.3's ±0.05 / 0.05 / 0.10 / 0.20). Two tracks at the same mood now produce visibly different palette character.
- **Silence rests.** Counters stay at 0 → step stays at 0 → all cells display their base palette colour. `f.bar_phase01` = 0 → no bar pulse. The panel is uniformly bright (0.85–1.0 from hash jitter) and vivid; no fade to grey, no cream, no spotlit gradient.

**Brightness is uniform.** LM.3.1's agent-position-driven static-light field is retired. The four `LumenLightAgent` slots are still ticked CPU-side for ABI continuity but their `intensity` and `colorR/G/B` fields are unused by the LM.3.2 shader. If LM.4's pattern bursts need agent positions later, the data is still on the GPU buffer.

**Bar pulse adds structural emphasis without overriding cells.** `1 + 0.30 × bar_phase01^8` produces a brief +30 % flash in the last ~8 % of each bar — visible at downbeats on tracks with installed BeatGrid, no-op on reactive tracks (the term collapses to 1.0). The team-counter dance still fires reactively because the engine increments `barCounter` every 4 bass beats as a fallback (CPU-side; not yet routed into the shader at LM.3.2).

**Reference-image fidelity:** `05_lighting_pattern_glass_dual_color.jpg` shows distinct vivid colour zones across the panel — LM.3.2 produces the same character via per-cell palette sampling, with the added energetic register that adjacent cells can step independently on different beats.
...
| LM.3 | Per-cell colour identity from `palette()` keyed on cell hash + `accumulated_audio_time × kCellHueRate` + mood (D.4). Procedural palette via IQ cosine, no cream baseline (E.3). **Result rejected in production: cells did not visibly cycle because Spotify volume normalisation (BUG-012) under-reads mid + treble bands → `accumulated_audio_time` advanced ~10× too slowly to read as motion.** Per-cell hash + procedural palette + mood-coupled palette parameters proven correct; the *time-driven cycling* mechanism is what failed. | ⚠ rejected |
| LM.3.1 | Agent-position-driven static-light field as backlight character (kAgentStaticIntensity = 0.50, sharper falloff) on top of LM.3's cell field. **Result rejected by Matt 2026-05-09: "fixed-color cells with brightness modulation; the bright pools dominated the visual story."** Brightness modulation is not the visual register the preset is meant to occupy. | ⚠ rejected |
| LM.3.2 | **Substantive look ships here.** Band-routed beat-driven dance (Decision D.5): each cell hashed into one of four teams (30 % bass / 35 % mid / 25 % treble / 10 % static); cells advance their palette index discretely on rising-edges of `f.beatBass / beatMid / beatTreble` — debounced 80 ms, scaled by energy. Per-cell `period ∈ {1, 2, 4, 8}` controls how many team-beats between advances. Cells hold their previous palette state until the next team beat (no dark gap between beats). HSV palette gives saturated jewel tones; per-cell hash + per-track seed produce inter-track variety. Frosted-glass diffusion baked into albedo via Voronoi `f2 - f1` distance — clean white halo at cell boundaries, no per-pixel artifacts. **Eight calibration rounds**: round 1 baseline; round 2 widened mood palette + cell density 30→15; round 3 per-channel seed perturbation; round 4 HSV palette + emission gain 4→1; round 5 frosted-glass surface character; round 6 beat envelope (added then dropped at round 8); round 7 frost in albedo (no normal artifacts); round 8 dropped beat envelope per real-music review (no dark "pulse off"). | ✅ M7 pass 2026-05-10 (session `2026-05-10T15-44-27Z`) — Matt: "Awesome. Finally. The movement of the color in the cells is looking good. I'd consider this a 'pass.'" Carry-forward: track-to-track colour variation could be wider (deferred to LM.6 polish or future tuning). |
...
- **2026-05-10 (LM.3.2 ✅ pass)** — eight calibration rounds (2026-05-09 / 2026-05-10) culminating in real-music session sign-off. Final shape: HSV-driven palette (saturated jewel tones) + Voronoi-distance-driven frost diffusion baked into albedo (clean white cell-edge halo) + cells hold their previous palette state until next team beat (no envelope-driven dark gap) + bar pulse on each downbeat. Round-by-round narrative in `contact_sheets/LM.3.2/README.md`. Carry-forward: per-track colour variation could be wider (Matt 2026-05-10) — defer to LM.6 polish.
- **2026-05-09 (LM.3.2)** — second pivot in 24 hours. LM.3's continuous-palette-cycling form of D.4 was rejected after live-session capture (BUG-012: Spotify volume normalisation under-reads mid + treble bands; `accumulated_audio_time` advanced ~10× too slowly to register as cell motion). LM.3.1's agent-position-driven backlight character was rejected by Matt 2026-05-09 ("fixed-color cells with brightness modulation; the bright pools dominated the visual story"). Decision D.5 — band-routed beat-driven dance — adopted: cells hashed into four teams (bass / mid / treble / static), advance palette index discretely on rising-edges of `f.beatBass / beatMid / beatTreble` with hash-derived periods. Brightness uniform with hash jitter; bar pulse from `f.bar_phase01 ^ 8`. Per-track palette seed magnitudes bumped (was 0.05 / 0.05 / 0.10 / 0.20; now 0.20 / 0.20 / 0.30 / 0.50). The LumenPatternState GPU contract grew from 360 → 376 B (added four band counters: bassCounter / midCounter / trebleCounter / barCounter). The LumenLightAgent slots stay on the contract for ABI continuity; their `intensity` and `colorR/G/B` fields are unused by the LM.3.2 shader. LM.4 (pattern bursts) still scoped on top of the LM.3.2 cell field.
- **2026-05-09 (LM.3 / first pivot)** — design pivot after LM.2 production review. "Meditative co-performer" framing replaced with "energetic dance partner". Decision D.1 retired (cell-quantized agent sample produced gradient blob, no visible cells); D.4 adopted (per-cell colour identity from `palette()` keyed on cell hash). Decision E.1 (cream-baseline mood tint) and E.2 (4 authored palette banks) retired; E.3 adopted (procedural palette via V.3 IQ cosine, mood shifts `(a, b, c, d)` continuously). Cream baseline retired across the board. LM.8 retired; the substantive look ships at LM.3. Acceptance criteria §7.5 rewritten around per-cell colour identity + vivid palette. Reference `05_lighting_pattern_glass_dual_color.jpg` promoted to dominant fidelity reference.
