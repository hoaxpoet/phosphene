...
8. **Calibration round 4 (2026-05-10): HSV palette.** The V.3 IQ cosine `palette()` was retired from `lm_cell_palette`; cells now compute hue / saturation / value directly and run `hsv2rgb()`. Hue = `moodHueCentre + (cell_t - 0.5) × 0.40 + step × kPaletteStepSize + (seedA × 0.30 + seedD × 0.50)` where `moodHueCentre = mix(0.65, 0.02, warm)` biases the panel toward blues at cool mood and reds/oranges at warm mood. Saturation = `clamp(mix(0.85, 0.98, arousal) + seedB × 0.05, 0.78, 1.00)`. Value = `clamp(mix(0.85, 1.00, arousal) + seedC × 0.03, 0.80, 1.00)`. The IQ palette form was structurally pastel-prone (channels desynchronise → mid-saturated mid-tones); the HSV form gives saturated jewel hues by construction. The legacy `kPaletteACool/AWarm/BSubdued/BVivid/CUnison/COffset/DComplementary/DAnalogous` constants and `kSeedMagnitudeA/B/C/D` constants are retained on the file for ABI continuity but unused by the round-4 path. **Compounding fix: `kLumenEmissionGain` (RayMarch.metal) reduced 4.0 → 1.0** — the HSV palette is vivid without HDR boost, and the prior 4.0 was clipping saturated channels above 1.0 in the harness's float→Unorm conversion (production with PostProcessChain ACES tonemap handled it; harness without tonemap did not). Bloom no longer engages on individual cells — that's correct for the uniformly-vivid stained-glass aesthetic.
...
| LM.3.2 | **Substantive look ships here.** Decision D.5 — band-routed beat-driven dance: cells hashed (`cell_id ^ trackSeedHash`) into bass / mid / treble / static teams (30 / 35 / 25 / 10 percent), advance palette index discretely on rising-edge of `f.beatBass / beatMid / beatTreble` (debounced 80 ms, scaled by `beatStrength`). Per-cell `period ∈ {1, 2, 4, 8}` Pareto-distributed from hash. HSV-driven palette gives saturated jewel tones; per-cell hash + per-track seed produce inter-track variety. Frosted-glass diffusion baked into albedo via Voronoi `f2 - f1` distance — clean white halo at cell boundaries, no per-pixel artifacts. Cells hold previous palette state until next team beat (no envelope-driven dark gap between beats). Brightness uniform with hash jitter `[0.85, 1.0]`; bar pulse `+30 % × bar_phase01^8` on each downbeat. `LumenPatternState` extended +16 B with four band counters (struct grows 360 → 376 B). LM.3.1 agent-position static field retired. **Eight calibration rounds 2026-05-09 / 2026-05-10** (round 1 baseline → round 2 widened mood palette + cell density 30→15 → round 3 per-channel seed perturbation → round 4 HSV palette + emission gain 4→1 → round 5 frosted-glass surface character → round 6 beat envelope → round 7 frost in albedo → round 8 dropped beat envelope) captured in `contact_sheets/LM.3.2/README.md`. M7 tunables: `kPaletteStepSize` (0.137), `kBarPulseMagnitude / Shape` (0.30 / 8.0), `kBassTeamCutoff` / `kMidTeamCutoff` / `kTrebleTeamCutoff` (30 / 65 / 90), `beatTriggerHigh` / `beatDebounceSeconds` (0.5 / 0.08 s), `kSeedMagnitude{A,B,C,D}` (0.20 / 0.05 / 0.20 / 0.50), `kCellIntensityBase / Jitter` (0.85 / 0.15), `kFrostBlendWidth / Strength` (0.04 / 0.60), `kCellDensity` (15 → ~30 cells across visible frame). Vividness gate met. p95 ≤ 2.8 ms. | ✅ **M7 pass 2026-05-10** (session `2026-05-10T15-44-27Z`) — Matt: "Awesome. Finally. The movement of the color in the cells is looking good. I'd consider this a 'pass.'" Carry-forward: track-to-track colour variation could be wider (defer to LM.6 polish). |
...
- **2026-05-10 (LM.3.2 ✅ pass)** — eight calibration rounds (2026-05-09 / 2026-05-10) culminating in real-music session sign-off (session `2026-05-10T15-44-27Z`). Final shape: HSV-driven palette + Voronoi-distance-driven frost diffusion baked into albedo + cells hold their previous palette state until next team beat (no envelope-driven dark gap) + bar pulse on each downbeat. Round-by-round narrative in the contact-sheet README. Carry-forward: per-track colour variation could be wider (Matt 2026-05-10) — defer to LM.6 polish.
