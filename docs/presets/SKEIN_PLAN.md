...
| **Skein.ENGINE.3** | Structural-section signal → preset tick | engine | — (land before Skein.5's structure) | ⏳ **scoped** (Matt chose option (a), a deliberate engine increment) — gated `RenderPipeline.setStructuralPrediction` (mirrors `setMood`) delivers live `StructuralPrediction` to `SkeinState.tick`; byte-identical, no GPU-contract change; prompt `~/Downloads/SKEIN.ENGINE.3_structure_plumbing_session_prompt.md` |
| **Skein.5** | Mood + structure + anticipation + locus flag | preset | Skein.3, **ENGINE.3** (for structure) | ⏳ **scoped** — mood (valence/arousal, FA #25) + anticipation (`beatPhase01`, FA #33) + locus (flagged); structure CONSUMES the ENGINE.3 signal (gated on `confidence`, subtle); prompt `~/Downloads/SKEIN.5_mood_structure_session_prompt.md` |
...
### Skein.ENGINE.3 — Structural-section signal → preset tick ⏳ (scoped 2026-06-09; Matt chose option (a))
**engine · depends on: — (must land before Skein.5's structure) · gate: signal reaches `SkeinState.tick` end-to-end; every other preset byte-identical; no GPU-contract change**

Plumbs the live `StructuralPrediction` (`MIRPipeline.latestStructuralPrediction` — `sectionIndex`/`sectionStartTime`/`predictedNextBoundary`/`confidence`) to the preset tick via a **gated `RenderPipeline.setStructuralPrediction(_:)`** (mirroring `setMood` — lock-guarded `latestStructuralPrediction`, default `.none`), called from `VisualizerEngine+Audio.swift` alongside `setMood`; the Skein tick closure reads it and passes it to an extended `SkeinState.tick(…structure:)`. **Delivers + proves the signal only — the structural VISUAL is Skein.5** (infra-before-preset, FA #59/#60). CPU-only (no `FeatureVector`/`Common.metal` change). Byte-identical for all other presets (the setter is inert at `.none`). Matt chose this **deliberate engine increment (option (a))** over an in-state proxy for real section-awareness. Prompt: `~/Downloads/SKEIN.ENGINE.3_structure_plumbing_session_prompt.md`.

---

### Skein.5 — Mood + structure + anticipation + painter-locus flag ⏳ (scoped 2026-06-09)
