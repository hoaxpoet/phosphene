...
| **Skein.5.4** | Two painting techniques: pour drips vs independent flicks | preset | Skein.5.3b | 🔶 **built 2026-06-10 on `claude/skein54-splatter` — NOT merged; live look APPROVED (Matt: "I like it"), round-2 speed tune APPROVED ("the speed adjustments look good"); BUG-044 (LF track-change canvas wipe) found in the live read + fixed on the branch; pending the wipe-verification listen → merge** — the pour sheds drips beside the line ∝ pour volume (`lineFlow`, τ-clocked); flicks land anywhere ≥ 0.20 UV from the painter with their own throw angle (lobed blot + 1–3 flung threads w/ terminal droplets + power-law teardrop satellites; magnitude = soft-saturated dev excess in `burst.size`); emission timing UNCHANGED, no GPU-struct change (`sharpness < 0` = drip marker); round-2: spatter rate −41 %, pour starts +13 % |
