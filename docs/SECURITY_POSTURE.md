...
- **No telemetry / no cloud.** On-device ML; the only outbound traffic is user-initiated metadata fetches (Spotify / Apple Music / iTunes lookup). Tapped audio and session recordings (`~/Documents/uzume_sessions/`) are **never uploaded** — `SessionRecorder` has no `URLSession`/network path (verified).
