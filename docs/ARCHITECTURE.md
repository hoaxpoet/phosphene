...
PhospheneEngine/
  ...
  Presets/
    ...
    Shaders/Filigree.metal → Filigree preset backdrop (`filigree_ground_fragment` — pure-black Kintsugi ground, drawn in particle mode then fully covered by the trail). Passes `["feedback", "particles"]`; the actual visual + the agent kernels are `PhysarumGeometry` + engine `Renderer/Shaders/Physarum.metal`. PHYS.2.
