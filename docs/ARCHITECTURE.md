...
PhospheneApp/               → SwiftUI shell, views, view models
  ...
  MusicKitFetcher.swift     → Contains `ITunesSearchFetcher: MetadataFetching` (filename drift — file name does not match contained class; rename pending per CA.5-FU-3). Free, unauthenticated iTunes Search API for genre + duration. No MusicKit dependency.
