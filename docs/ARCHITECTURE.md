...
5. **Analysis pipeline** (composed in `SessionPreparer+Analysis.analyzePreview(...)`, run inside `Task.detached`):
   ...
   - **Batch section detection** (SECDET.3b, D-170): `SectionDetector` resamples the full-track decode to 22050 mono and runs McFee/Ellis spectral clustering on the cached BeatGrid → `TrackProfile.sectionStartTimes` (interior boundary times). Replaces the live-novelty boundary source (`strongBoundaryTimes`, retired); the grid is now resolved before the profile so detection can beat-sync. Streaming previews can't span the track → the planner's coverage gate falls back to equal slices (LFPLAN.5). Validated end-to-end on raw PCM at F@3 = 1.000 vs the lab.
...
PhospheneEngine/
  ...
  DSP/
    ...
    StructuralAnalyzer      → Section boundary prediction (70% duration consistency + 30% repetition similarity), feeds latestStructuralPrediction on MIRPipeline. CA.1 audit note: runtime per-frame work currently consumed only at preparation time by SessionPreparer. NOTE (SECDET.3b, D-170): its **boundary role is retired** — section boundaries now come from the batch McFee/Ellis detector (`SectionDetector`). The *count* role lives on (`latestStructuralPrediction.sectionIndex` → `estimatedSectionCount`, the equal-slice fallback); `boundaryTimestamps`/`boundaryNoveltyScores` remain for diagnostics but are unread by production.
    SectionFeatureExtractor → SECDET Stage A (D-170). Beat-synced 252-bin log-CQT (median) + 13 MFCC (mean) at 22050 Hz, from already-decoded mono PCM + beat frame indices, matching the lab spec (precompute.py / tune_corpus.py) exactly. Validated vs librosa goldens: Cq recurrence-SSM corr 0.9995, MFCC corr 1.0. Feeds the McFee/Ellis clustering (Stages B/C). Offline/batch, one run per track (like the cached BeatGrid).
    ConstantQTransform      → SECDET Stage A. 252-bin log-CQT in dB (amplitude_to_db ref=max) via a Brown-Puckette sparse spectral kernel (norm=1 filters, librosa sparsity=0.01), center=True, hop=512. Per-bin gain differs from librosa's recursive CQT but cancels in the recurrence (column-pair distances), so F-scores transfer. Perf: one 65536-pt FFT/frame; ≈ 2 min/track in *debug* but **2.7 s/track in release** (`-O`) — the debug figure is a scalar-inner-loop artifact, so no per-octave-downsampling port is needed (SECDET.3a).
    MFCCProcessor           → SECDET Stage A. 13 MFCC matching librosa.feature.mfcc: STFT(nFFT=2048, hop=512, periodic Hann, center=True zero-pad) → Slaney mel (128, fmin=0, fmax=sr/2, norm=slaney) → power_to_db(ref=1, top_db=80) → DCT-II (ortho). Validated corr 1.0 vs librosa.
    VDSPRealFFT             → SECDET Stage A. Reusable forward real FFT (vDSP) returning the standard-DFT half-spectrum (vDSP_fft_zrip's 2× undone; DC/Nyquist unpacked). Shared by ConstantQTransform (65536) and MFCCProcessor (2048).
    SpectralSectionDetector → SECDET Stage B (D-170). McFee/Ellis Laplacian spectral clustering: turns SectionFeatures (cqSync/msSync) into section-boundary times via embedding median(9,1) + cumulative-norm row-normalize → k-means(k=5) → label-change boundaries → enforce_min(8 s). Reproduces the lab boundaries exactly (F@3 vs tune_corpus.py = 1.000 on the goldens). The offline/batch detector that replaces the novelty boundary source at Stage C.
    SectionDetector         → SECDET Stage C façade (D-170, SECDET.3b). Public entry wiring the section pipeline into prep: resample full-track PCM → 22050 mono (AVAudioConverter, mirrors BeatThisPreprocessor.resample) → beat-sync on the cached BeatGrid → SectionFeatureExtractor + SpectralSectionDetector → interior boundary times (strips the framing 0/duration to match the planner contract). Called from `SessionPreparer+Analysis.makeProfile` (Step 8). Validated end-to-end vs the lab on raw PCM: F@3 = 1.000 (SectionDetectorTests). Empty (too short / no grid / streaming preview) → caller maps to nil → equal slices. **SECDET.5:** Beat This! caps its grid at ~30 s (`BeatThisModel.tMax`=1500 frames), so on a longer track McFee would segment only the first 30 s (live-test failure: 282 s track → boundaries bunched < 30 s → coverage gate → equal slices). `fullTrackBeats` extends the grid past its last beat at the median inter-beat period so the beat-sync covers the whole track — synthetic beats beyond the tracked region suffice for the recurrence pooling (offline: 30 s-capped grid + extension F@3 0.48 ≥ the full-beat baseline).
    RecurrenceGraph         → SECDET Stage B. The deterministic affinity stages, each replicating its librosa/scipy counterpart (validated: fused-affinity corr 0.999 vs golden): recurrence_matrix (mutual k-NN width=3, med_k_scalar bandwidth, sym) → timelag median(1,7) → MFCC path diagonal → μ-balanced fuse → symmetric normalized Laplacian. Dense N×N (N = beat count ≈ 300–600). Symmetrizes A before the Laplacian (the timelag asymmetry is a reflect-padding artifact; keeps the spectrum well-conditioned in [0,2] vs the lab's float64-only ill-conditioned modes — boundaries match either way).
    SymmetricEigen          → SECDET Stage B. Dense symmetric eigendecomposition via LAPACK `ssyev` (Accelerate) for the bottom-k Laplacian eigenvectors. First LAPACK use in the codebase (classic CLAPACK — deprecation-warning-only, non-fatal in SPM; upgrade path noted in-file).
    KMeans                  → SECDET Stage B. Hand-rolled Lloyd's k-means + k-means++ seeding (deterministic LCG, n_init restarts, lowest inertia) standing in for sklearn KMeans on the spectral embedding. Clustering is invariant to orthogonal transforms of the embedding, so exact sklearn-seed parity is unnecessary.
  ...
  Orchestrator/             → AI VJ: preset selection, transitions, session planning, live adaptation, reactive mode (Phase 4 complete — see ENGINEERING_PLAN.md; BUG-015 tracks the missing App-layer runtime invocation of `applyLiveUpdate(...)`)
    ...
    SessionPlanner+Segments → V.7.6.2 multi-segment walk extracted from SessionPlanner.swift for file-length compliance. `makeSections(...)` partitions the track on the detected `sectionStartTimes` (LFPLAN.5) or uniformly by `profile.estimatedSectionCount`; `planSegments(...)` loops over sections emitting one or more `PlannedPresetSegment` bounded by `min(remainingInSection, preset.maxDuration(forSection:))`. **SECDET.6:** `sectionEligibleCatalog(...)` gates completion-gated presets (`wait_for_completion_event`) out of a *non-last real* section when their `natural_cycle_seconds` exceeds the section length — otherwise the preset reserves its full cycle and swallows the later sections via `coveredUntil`, collapsing per-section transitions to one (live-test 2: Arachne's 150 s cycle ate a 203 s track's 4 later boundaries). `recentHistory` 50-entry trim per D-080 rule 6.
  ...
  Session/
    ...
    SessionPreparer+Analysis → `nonisolated static func analyzePreview(...)` composing the analysis pipeline inside `Task.detached`: stem separation → analyzer warmup → MIR → full-mix BeatGrid → metadata-driven beatsPerBar override → drums-stem BeatGrid (DSP.4) → GridOnsetCalibrator (BUG-007.8) → batch section detection (SECDET.3b, `SectionDetector` → `makeProfile`) → CachedTrackData. SECDET.3b moved the BeatGrid ahead of the profile so section detection can beat-sync on it. The load-bearing Session-side composition seam for DSP + ML + Audio.
