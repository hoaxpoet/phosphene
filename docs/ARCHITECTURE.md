...
PhospheneEngine/
  ...
  Renderer/
    ...
    RenderPipeline+Glaze → Glaze warp+composite pass driver (`drawWithGlaze`/`renderGlaze`: warp → display comp → swap; the glossy contour-gel branch, mirroring Nacre/FataMorgana). `GlazeUniforms` (32 B at fragment buffer(1) of both passes); per-frame uniform population (time + drawable aspect/texel + total-energy seed gate `glazeSeedEMA`). Dispatched by the `isGlaze` flag on the bundle/state. GLAZE.2a wired stub (faithful spring-physics warp + blur-pyramid sheen at 2b).
  ...
  Presets/
    ...
    Shaders/Glaze.metal → **Glaze — `direct + mv_warp` glossy contour-gel preset** (`certified: false`; port of butterchurn `Flexi + stahlregen - jelly showoff parade`). GLAZE.2a STUB: `glaze_warp_fragment` (gentle advection + decay + palette-tinted, silence-floored, energy-gated seed + `[0,1]` clamp) + `glaze_comp_fragment` (sample + mild contrast + sRGB-decode); driven by `GlazeUniforms` from `RenderPipeline+Glaze`. Faithful 3-mass spring-physics warp + 3-level blur-pyramid emboss/sheen at GLAZE.2b.
