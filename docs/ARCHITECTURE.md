...
PhospheneEngine/
  ...
  Presets/
    ...
    Shaders/Nacre.metal → **Nacre — `direct + mv_warp` iridescent jello-mirror preset** (`certified: false`, pending NACRE.4; faithful port of butterchurn `$$$ Royal - Mashup (431)`). `nacre_warp_fragment` (unsharp + 0.9 in-warp decay + value-noise grain + palette-tinted volume-gated core seed + `[0,1]` clamp) + `nacre_comp_fragment` (radial-pulse emboss → chromatic-dispersion `inversesqrt` filaments + sine cells + slow roam + sRGB-decode); driven by `NacreUniforms` from `RenderPipeline+Nacre`. D-171.
