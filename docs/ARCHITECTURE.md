...
PhospheneEngine/
  ...
  Presets/
    ...
    Shaders/Floret.metal → **Floret — `direct + mv_warp` radial-bloom preset** (`certified: false` — FLORET.2a WIP wiring stub; port of butterchurn `suksma - Rovastar - Sunflower Passion`). 2a stub: `floret_fragment` (near-black loader placeholder), `mvWarpPerFrame`/`mvWarpPerVertex` (zoom + slow rot), `floret_warp_fragment` (decay + faint palette wash + volume-gated core seed + `[0,1]` clamp), `floret_comp_fragment` (display feedback + sRGB-decode); driven by `FloretUniforms` from `RenderPipeline+Floret`. The faithful mechanic (1/r² vortex + z² conformal fold warp; 3-fold radial-pulse high-pass kaleidoscope comp; 4-disc seed) + audio + uplifts land at FLORET.2b/.3 (`TODO(FLORET.2b)` markers in-file).
