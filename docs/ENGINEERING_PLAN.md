...
**PHYS.4 live M7 ❌ (2026-06-27):** "lava lamp at the speed of years" — merge read but too slow + connection still obscure (2nd negative). **PHYS.5 — merge/divide re-scope ⏳ code-complete, pending live M7 (2026-06-28).** Matt's redirect: the spectacle is cells visibly MERGING and DIVIDING synced to music. True continuous bidirectional division is reaction-diffusion's domain → a SEPARATE future preset (Matt's call); Filigree does **option A** — a fast dramatic merge + a re-seed that **respawns agents uniformly** to regrow fine (physarum ratchets to coarse irreversibly; 5 attempts confirmed; the only "divide" is a trail-wipe re-seed). **Polarity (Matt's pick):** LOUD → fine/busy/bright (divide), QUIET → few calm cells (merge). **Sync:** the re-seed BURSTS on an energy SURGE (drop/build > baseline+0.22, 2.5 s cooldown) so the divide lands on the musical moment; per-beat `hit` pulse retained; calm baseline (low explore, persistence 0.90–0.93) so motion is event-driven, not frantic churn. **Done-when:** ✅ render arc cycles coarse↔fine on the surge (cell counts 74↔150), re-seed bounded+gradual (maxΔ 0.018/frame, no black/white-out), per-beat/stability/framerate green, lint 0, app builds. **→ pending Matt's live M7** — 3rd connection attempt; if burst-on-surge still doesn't read synced live, physarum can't carry music-sync and we lean on the separate reaction-diffusion preset.
