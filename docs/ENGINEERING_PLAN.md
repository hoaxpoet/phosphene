...
- **SECDET.3b — Wire-in (pending).** Replace the boundary source in `SessionPreparer.analyzeMIR` (`strongBoundaryTimes` → the batch detector on full-track 22050 features). Restructure so the BeatGrid is resolved *before* section detection (McFee needs the track's beats; today the grid is Step 5, after the profile). Decide the live `StructuralAnalyzer.process()` novelty path's fate explicitly (sections come from the cached batch result, like BeatGrid; keep `estimatedSectionCount` if still wanted). Output contract preserved: `TrackProfile.sectionStartTimes` → planner untouched.
