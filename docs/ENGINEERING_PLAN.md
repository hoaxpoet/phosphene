...
### Increment Skein.4.1 — Colour-per-stroke (deferred) — pending a new session
The pour line's colour = the dominant stem (`SkeinState` argmax) applied uniformly along the redrawn 40-frame tail each frame, so a dominant-stem switch recolours the recent stroke ("the colour changes in the middle of a stroke," Matt M7 2026-06-09). Fix: freeze the line colour PER-SEGMENT at lay-time (a small `SkeinState` colour-breakpoint ring packed into `SkeinUniforms`, looked up per tail-sample-tau in `skein_geometry_fragment`) so already-laid paint keeps its colour and a colour change reads as a new pour. Deferred by Matt to a fresh session at the Skein.4 M7 pass (context budget); diagnosis + approach + the open product decision (continuous per-segment colour vs a genuine break/new-pour) are in `~/Downloads/SKEIN.4.1_color_per_stroke_session_prompt.md` + memory `project_skein_preset.md`. Not started.
