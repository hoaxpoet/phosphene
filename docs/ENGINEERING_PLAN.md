...
### Phase IFC — Instrument-Family Capture (D-177): IFC.1 model+license ✅, IFC.2 MPSGraph port + parity ✅, IFC.3 per-family activity + normalization ✅, IFC.4 pipeline integration ✅, IFC.5 validation 🟡 (diagnostic + real-audio done; orchestral corpus pending) (2026-06-30)
...
- **IFC.5 — validation (🟡 partial).** New `InstrumentFamilyDumper` (`executableTarget`, retained-diagnostic): decodes a clip → runs the **production** `InstrumentFamilyAnalyzer` (incl. the 44.1→32 kHz resample IFC.2's parity skipped) → per-window strings/brass/woodwinds/percussion table + leader-per-window (`--out` JSON). **Production-path validated on real audio:** on `so_what.m4a` (Miles Davis) **brass leads** (peak smoothed 0.42/dev 0.50), percussion at the ride/drum moments, strings ≈ 0 — correct acoustic discrimination on a real recording; `money`/`love_rehab` (electric/electronic) correctly read near-zero orchestral families (absence-reporting); `pyramid_song` subtle strings stay approximate (documented ceiling). **Still pending: full-orchestra corpus** (strings-dominant + wind-ensemble trade — the Ricercar cases) needs the Beethoven-class clips, which were dev-only in `/tmp` and are gone → **blocked on an audio source** (Matt supplies, or authorizes re-fetching the public-domain clips), not a code gap. Detail: scoping §13.
- **Scope guard:** recognition + per-family interpretation + plumbing only; IFC.4 validated by pipeline correctness, **not** musical feel (that arrives in IFC.6). **Next: close IFC.5's orchestral-corpus gap** (once audio is available) → **IFC.6** (Ricercar drive-layer swap + optional low/high-strings 5-section split; tune `devGain` against real per-family p99 + add a `dev` floor for the leader-flap; disk-persist the series).
