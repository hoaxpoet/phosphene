...
### Phase IFC — Instrument-Family Capture (D-177): IFC.1 model+license ✅, IFC.2 MPSGraph port + parity ✅, IFC.3 per-family activity + normalization ✅, IFC.4 pipeline integration ✅ (2026-06-30)
...
- **Scope guard:** recognition + per-family interpretation + plumbing only. IFC.4 is validated by pipeline correctness, **not** musical feel (that arrives in IFC.6). **Next = IFC.5** (orchestral-fixture validation: per-family firing evidence on string/brass/wind/tutti clips) → IFC.6 (Ricercar drive-layer swap + the optional low/high-strings split for the 5 sections; tune `devGain` against the real per-family p99; disk-persist the series).
