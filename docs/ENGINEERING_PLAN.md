...
### Increment Skein.ENGINE.3 — Structural-section signal → preset tick ⏳ (scoped 2026-06-09; Matt chose option (a))
**Discovered as a prerequisite of Skein.5's structure sub-feature** (the increment was split: `StructuralPrediction` is DSP/orchestrator-only and does not reach the preset tick). Matt chose **option (a) — a deliberate engine increment** (over an in-state proxy / deferral) for real section-awareness, honouring infra-before-preset (FA #59/#60). Scope: a **gated `RenderPipeline.setStructuralPrediction(_:)`** (mirroring `setMood` — lock-guarded `latestStructuralPrediction`, default `.none`) called from `VisualizerEngine+Audio.swift` alongside `setMood`, reading `MIRPipeline.latestStructuralPrediction`; the Skein tick closure reads it and passes it to an extended `SkeinState.tick(…structure:)`. **CPU-only** (no `FeatureVector`/`Common.metal` change), **byte-identical** for every other preset (the setter is inert at `.none`), golden-locked (`PresetRegressionTests` + DB/FM). **Delivers + proves the signal only; the structural VISUAL is Skein.5.** After ENGINE.3 the app is **visually identical to today**. Prompt: `~/Downloads/SKEIN.ENGINE.3_structure_plumbing_session_prompt.md`. Not started.

### Increment Skein.5 — Mood + structure + anticipation + painter-locus ⏳ (scoped 2026-06-09; Matt-chosen next)
Mood (`valence → palette warmth/saturation`, `arousal → vigour/density`; smoothed in `SkeinState`, never via `setFeatures` — FA #25), anticipation (painter wind-up/release on `beatPhase01` — beat-phase, not raw onset, FA #33; honour the cold-start caveat), the structural bias (CONSUMES the **Skein.ENGINE.3** signal — boundary → palette emphasis + density + soft region lean, gated on `confidence` so the allover read survives ambient material), and a flagged painter-locus (off by default). Depends on Skein.3 + Skein.4.1 + **ENGINE.3** (structure). Prompt: `~/Downloads/SKEIN.5_mood_structure_session_prompt.md`. Not started.
