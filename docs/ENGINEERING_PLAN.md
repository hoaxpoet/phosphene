...
### Increment PUB.2 — weights LFS → Release-asset cutover ✅ (2026-07-21)
Executed the PUB.2 §1 cutover (Matt-confirmed decision 2, 2026-07-12): the ~167 MB ML weights no longer ship via LFS — they're the `ml-weights-v1` GitHub Release asset, fetched + checksum-verified by `Scripts/fetch_weights.sh`. Matt published the Release (classifier-gated); the archive was built + byte-verified against `SHA256SUMS` (479 `.bin` + 3 `manifest.json` = 482 entries) and the fetch path was proven end-to-end from a simulated fresh checkout against the live Release (download → checksum → unpack → post-unpack verify, all green) **before** untracking anything. Then: `git rm --cached` the weights (kept on disk), `.gitignore` `Weights/*` except `SHA256SUMS`, dropped the two `*.bin` LFS rules from `.gitattributes`, and swapped CI's LFS weights cache+pull for a `fetch_weights.sh` step (cache keyed on `SHA256SUMS`). The obsolete `Verify LFS smudged` CI step was removed — `fetch_weights.sh`'s own post-unpack verify is the new integrity gate (`check_lfs_smudged.sh` is now dead: `git lfs ls-files` sees no weights; retire in a follow-up). **The big clone-cost lever — non-destructive, no history rewrite.** Done-when: ✅ Release live; fetch verified; weights untracked + ignored; CI cutover; `SHA256SUMS` still tracked. **NOT pushed** — awaiting Matt's "yes, push"; CI exercises the new path only after push. `docs/PUBLISHING.md` §1 flipped to done.
