...
1. **Beat-sync program (D-202)** — the largest active program. Phase TRK is ⏸ **PARKED** (D-206) and must not reopen without a changed premise about the **grid**, not the tracker. **DBN.3** is the live thread but its stated gate ("blocked on FT.1") is **satisfied-but-unresolved**: FT.1 landed 2026-07-31 with a **negative** payoff, so the block condition is met while the decision it was waiting for was never made — see the DBN.3 row for the successor state. **FT.3 is COMPLETE** (tasks 1–3 2026-07-31, tasks 4–6 2026-08-19 — `BarLineEstimator` built, parity 1.6e-7, **not wired**); **FT.3.1** is specced and awaiting Matt (`docs/prompts/FT31_GRID_METRICAL_LEVEL.md`), and the port's 7/9 decline rate is its case. Owner section: §Phase FT / §Beat-Sync Program.
