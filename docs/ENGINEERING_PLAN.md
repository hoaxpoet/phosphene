...
- **FL.4 ⏳** audio wiring (family → which colours bloom/which ribbons brighten; zero-lag `*_dev` → flow vigour + ribbon undulation + splat force; beats → splat impulses). Gated on FL.3's eyeball pass. **FL.5 ⏳** preset integration (factory mirroring `makeMitosisGeometry`, `passes: ["particles"]` sidecar, app-side `setParticleGeometry`) then live M7.
