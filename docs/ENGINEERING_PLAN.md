...
**Measured state 2026-08-07 (MD.0):** 28 sidecars − 2 diagnostics = **26 production presets, 18 certified**, of which **7 are inspired-by (all certified)** and 11 Phosphene-native. Gap to the 20-certified threshold: **2 presets.** Composition is still **open** — D-115 has been "Matt's call pending" since 2026-05-12, so the *mix* at 20 is undetermined even though the *count* is nearly there. MD.0's working assumption is **C' (10 + 10)**, treating D-119's ≥ 50 % inspired-by as a steady-state target rather than a first-release gate; this **diverges from the recorded A' (7+13) default**, on the grounds that A' was priced on §12.1's "~2–3 days per preset" and the record falsifies it (Witchlight ten increments, Meniscus eleven). A' needs six more uplifts before first release; C' needs three. See MD.0 DECISION-NEEDED #2 and the D-115 amendment block.
