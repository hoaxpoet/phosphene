...
- **CLEAN.1.2 BUG-031 fix** ✅ (2026-06-13, strategy **A** — Matt-approved via AskUserQuestion). `StemSeparator.separate()` holds `lock` across the full input→predict→output critical section + returns stems by value (`StemSeparationResult.stemWaveforms`); both call sites read the result, not the shared `stemBuffers`. Regression: `StemSeparatorConcurrencyTests` (per-caller-correct stems under concurrency, threshold-free). 3 inline test doubles updated to populate `stemWaveforms`. **Fix-implemented, NOT Resolved — needs Matt's manual concurrency validation + integration to main** (KNOWN_ISSUES BUG-031).
- **CLEAN.1.3 BUG-032 fix** ✅ (2026-06-13). Per-instance `streamingSessionGen` guard (twin of `localFileSessionGen`) on the prep-completion closure; `endSession()` cancels the prep task (mirrors `cancel()`); `resumeFailedNetworkTracks` single-flight (awaits the in-flight loop); `startSession` source-mutation moved after the state guard (both variants). Regression: `SessionLifecycleGenerationTests` (end-then-restart guard, rejected-startSession source order) + `SessionRecoverySingleFlightTests` (single-flight). **Fix-implemented, NOT Resolved — needs Matt's manual lifecycle validation + integration to main** (KNOWN_ISSUES BUG-032).
