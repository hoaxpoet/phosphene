...
- **MOOD-FLUX.2 — fix: align the offline FFT magnitude formula to live ✅ code-complete 2026-07-08, pending Matt's live M7 (commit below).** One-line-of-intent change in `SessionPreparer+Analysis.swift` (+ the `CorpusCensusRunner` census mirror): `vDSP_zvabs` + `×2/fftSize` instead of `sqrt(power/fftSize)`. **No retrain / no labels needed** — the model is correct; only the offline feature extraction was wrong. **Validated:** flux z **+38 → +1.43**; the correction is a uniform 16× (σ=0) so it generalizes corpus-wide without a re-run; 103 mood/MIR/session-prep/spectral tests green incl. `MoodClassifierGolden` (classifier untouched — feature-extraction fix); blast radius measured benign (`mir_bpm` 0/40 changed, `key` 6/40 empty→resolved only, centroid ratio-invariant). **Outstanding (required, blocking §Resolved):** Matt's live M7 — before/after **preset picks** on known tracks. Not resolved until live-confirmed.
