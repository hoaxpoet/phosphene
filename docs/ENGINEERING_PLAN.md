...
### Increment CA.1-FU-1 — Per-frame structural chain (doc-state closeout, folded by BUG-015) ✅ (2026-05-21)

Doc-state closeout for the final outstanding CA.4 audit follow-up. CA.1-FU-1 was re-scoped by the CA.4 audit (2026-05-20) to "gate the per-frame `StructuralAnalyzer` + `NoveltyDetector` + `SelfSimilarityMatrix` chain in `MIRPipeline.process` to prep-time only" — a CPU saver that was correct *if* BUG-015's fix sourced runtime predictions from somewhere other than `MIRPipeline.latestStructuralPrediction`. The audit's §Resolution narrative laid out a decision matrix at `ORCHESTRATOR.md:462-463`: option (a) keeps the gate if BUG-015 uses `.none` / a sentinel; option (b) drops the gate and the per-frame chain stays on if BUG-015 reads `latestStructuralPrediction`. BUG-015 (commit `b3f1efd9`, 2026-05-21) took option (b): the wire at `PhospheneApp/VisualizerEngine+Orchestrator.swift:287-311` passes `mir.latestStructuralPrediction` directly to `applyLiveUpdate(...)`, and the inline comment at `:272-277` explicitly cites the fold. CA.1-FU-1 closes here without any code change — gating the per-frame chain to prep-time only would now break the BUG-015 wire. Doc-state cleanup only: backlog row Status flipped to ✅ Resolved 2026-05-21 (folded by BUG-015); the audit's §Resolution narrative gained a closeout note at `ORCHESTRATOR.md` recording which branch BUG-015 took; engineering plan entry added (this); release notes entry added.

### Increment CA.4-FU-2 — Source-comment doc-drift cleanups (doc-state closeout) ✅ (2026-05-21)

Doc-state closeout for the second CA.4 audit follow-up. CA.4-FU-2 specified two source-comment corrections: (a) `PresetScorer.swift:86` D-030 → D-032; (b) `PresetSignaling.swift:9-10` "Arachne does NOT emit yet — wiring is V.7.8" → current-state description citing V.7.7C.2 / D-095 + BUG-011 round 8 wiring. Verification 2026-05-21: **both corrections were already landed inline with the CA.4 audit increment itself** (commit `faee28a7`, 2026-05-20). The CA.4 audit's own release notes documented the change (*"in-source comments at PresetScorer.swift:86 (D-030 → D-032) and PresetSignaling.swift:9-10 (current-state rewrite per D-095 + BUG-011 round 8)"*), but the CA.4-FU-2 backlog row in `ORCHESTRATOR.md` was left in "Ready now" state — an internal inconsistency in the audit deliverable. Post-fact grep confirms: `PresetScorer.swift:86` reads `## Weight rationale (D-032)`; `PresetSignaling.swift:9-16` describes the wired V.7.7C.2 emission path + the BUG-011 round 8 orchestrator-side subscription. No code change in this increment. Doc-state cleanup only: backlog row Status flipped to ✅ Resolved 2026-05-20 (bundled by CA.4 audit), engineering plan entry added, release notes entry added.

### Increment CA.4-FU-1 — Demote `DefaultLiveAdapter.transitionPolicy` dead field ✅ (2026-05-21)

Follow-up to the 2026-05-20 CA.4 Orchestrator audit. The `transitionPolicy: any TransitionDeciding` field on `DefaultLiveAdapter` was constructor-injected and stored at `PhospheneEngine/Sources/Orchestrator/LiveAdapter.swift:176, 188, 191` but never read — both evaluation paths (`evaluateBoundaryReschedule(...)` and the mood-override path) construct `PlannedTransition` / `LiveAdaptation.PresetOverride` values directly without going through `TransitionDeciding.evaluate(...)`. Independent of BUG-015 (the field stayed dead even after the wire landed in commit `b3f1efd9`). Option (a) shipped: parameter + stored field removed; init is now `init(scorer: any PresetScoring = DefaultPresetScorer())`. Post-edit grep returns zero `transitionPolicy` hits across `LiveAdapter*.swift`. Validation: `swift test --filter LiveAdapter` 11/11 pass; full Orchestrator suite 114 tests / 17 suites pass; `xcodebuild -scheme PhospheneApp build` clean; SwiftLint introduced zero new violations. Capability registry updated: backlog row Status flipped to Resolved 2026-05-21; file-level entity row for the field marked resolved; §production-orphan section gained a resolution stamp citing the post-fix grep. No test changes required — existing tests use `DefaultLiveAdapter()` or `DefaultLiveAdapter(scorer:)`; the parameter removal is a non-breaking change.
