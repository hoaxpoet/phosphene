...
**Remaining:** **PHYS.2** graduate to a registered preset (`Filigree.json` sidecar + `filigree_ground_fragment` backdrop + `ParticleGeometryRegistry` + `VisualizerEngine` factory/resolve + production-path test; app-layer, needs `xcodebuild` verify). **PHYS.3** live wiring + the audio-lock listen (gating real-feel test; needs the branch on Matt's checkout). **PHYS.4** tuning (cap the bloom over-fill, return speed, web long-hold drift). **PHYS.5** certification (M7, rubric, flash-safety cert, `certified: true`). Full plan: `docs/presets/FILIGREE_DESIGN.md §9`.
