...
**AV.4–AV.6 (feature branch `claude/aurora-veil-streak-core-4bad40`, superseding the AV.3 pause — the active-curtain reauthor landed INTO Aurora Veil itself, not a separate Phase AC preset).** AV.4 ✅ footprint feasibility spike + motion-GIF harness. AV.5 ✅ streak-field/footprint reauthor (design §5.10/5.11). **AV.6 (band-footprint rebuild) — ⏳ code-complete, pending live M7.** Rebuilt `F(x,z)` as a localized auroral band and solved the whole-arc defect: the marched curtain now reads as distinct converging RAYS, not fog, via a convergence-frame ray generator (ridges in angle, coherent in radius — design §5.12). Overhead-curtain framing, angular-sector negative space, exposure recalibrated for the sRGB target, green-base→violet-crown palette with an elevation tilt (§5.11 gap fix). **Done-when:** all AV gates + PresetAcceptance + lint + app build green (✅ 2026-07-17); marched rays read as filaments over dominant dark sky (✅ F-map + march spikes); **Matt live-M7 sign-off on the dance + colour (⏳ PENDING — goldens NOT regenerated until sign-off).** Open colour call for M7: `ref_240` measures green→cyan (no violet crown); the shipped palette follows the design-intent violet crown (refs 01/07) — Matt's pick whether to keep or revert to strict footage fidelity.
