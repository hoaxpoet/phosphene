...
- **SECDET.3c — Cache (pending).** Bump PersistentStemCache schema (v4 → v5) so stale v4 entries don't replay the old novelty boundaries as McFee sections (the LFPLAN.6/.8 lesson: changing what the cache *means* needs a version bump).
