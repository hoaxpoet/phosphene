...
- **SECDET.2 — Graph + clustering (pending).** Recurrence affinity → timelag median filter → MFCC path diagonal → μ-balanced fuse → normalized Laplacian → LAPACK `ssyev` bottom-k eigvecs → median(9,1) + cumulative-norm → hand-rolled k-means(k=5) → boundaries → 8 s merge. Validate boundaries vs `tune_corpus.py` on the golden tracks. LAPACK `ssyev` is new to the codebase — spike the symmetric eigensolver early (second risk after CQT).
