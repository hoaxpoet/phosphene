...
### NB.9 — planned
Per `NIMBUS_PLAN.md`: ~~NB.7 Page (CUT — §1.3)~~ → NB.9 certification (acceptance invariants §5.7, golden registration, anti-reference manual check, then Matt M7). NB.5-as-Pulse cut; NB.8 done early; mood (NB.6) done. A certified Nimbus = the band playing one packet-matching cool-gas body: beat (per stem) + energy swell + mood, fitting Tier-2 budget via the half-res path.
