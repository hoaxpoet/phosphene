...
### Increment FD.1 — Fractal Descent Mandelbox maquette + perf gate 🔨 (2026-07-23; supersedes PG.3 Mandelbox Cathedral per Matt — camera falls instead of holding)

Ported the Rrrola/Fragmentarium Mandelbox distance estimator verbatim (FA #73) into a new `ray_march` preset, with a self-similar scale-descent that wraps seamlessly (a Mandelbox is bounded, so descent is `zoom = |Scale|^fract(phase)`, not a translation). Deliberately audio-inert so the task-3 gate measured geometry alone. **Gate initially FAILED at 8.06 ms** (§A8 stop-and-report); the DE port was verified correct against the curated `mandelbox_cathedral` references (fan vault, muqarnas) BEFORE any further work — the discipline the three prior PG-slate M7 deaths (KS / Truchet / Kleinian) each skipped. RMPERF.1 (above) then brought cap 8 to 5.58 ms. Preset stays a not-certified maquette; the FD build (materials, god-rays, fog, hero audio, cap-8 lock) is the next increment. Count 26 → 27. **Open items for the FD build:** the preset-agnostic `applyAudioModulation` already drives fog from `f.arousal` + camera/light from `f.bass` — collides with FD's two hero routes (FA #67); and the open "vast chamber" frame (fold-open driven) needs re-measurement once that routing exists.
