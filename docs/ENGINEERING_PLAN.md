...
**Status:** ⏳ awaiting M7 review on real session. LM.3.2 commit pending. Tests + harness contact sheets green. Increment is **not** done until Matt reviews the preset on a real-music session against the LM.3.2 contact-sheet README's six-point checklist and gives a green light. *Recurring process error noted: LM.2 / LM.3 / LM.3.1 were all prematurely marked ✅ on commit landing; all three were subsequently rejected on visual review and required follow-up work. Phase LM increment status stays ⏳ until visually approved on real music.*

**Carry-forward.** LM.4 ships pattern bursts (radial ripples on drum onsets, sweeps on bar boundaries) that inject extra per-cell brightness — pattern colour comes from the same per-cell palette so a ripple takes the colour of the cells it crosses. The design doc covers per-stem hue affinity as an optional LM.5 sub-decision (deferred until LM.4 review tells us whether the LM.3.2 unified-palette feel needs stem differentiation). **M7 review surface for LM.3.2: a real session capture against a varied playlist.** The contact-sheet stills show team assignment + one-step advance correctly; the load-bearing checks are time-evolution (cells advancing on each successive beat at ~50–60 % cells / sec target) and per-track palette identity (different palette character across tracks at the same mood). If the dance feels too dense, raise the team cutoffs (more cells static); if too sparse, cap periods at 4 instead of 8. If track-to-track variation is still subtle, push `kSeedMagnitudeD` higher (0.50 → 0.65).
