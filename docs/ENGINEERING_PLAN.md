...
- **MOOD-FLUX.3 — dependent cleanups ⏳ (after .2 M7).** Correct the DECISIONS sample-rate-delta note (CENSUS.3 measured cross-path centroid skew ~9 %, not the recorded ~22 %); the live mood path was confirmed unaffected (it already used the correct `FFTProcessor` scale). Consider routing offline `analyzeMIR` through `FFTProcessor` directly (vs the parallel reimplementation) so the two paths can never drift again — the deeper mechanization of this bug's class.

## Phase TONAL — Continuous harmonic state (Tonal Interval Vector) 📋 scoped, awaiting go/no-go (2026-07-08; D-178 Proposed; scoping doc `docs/TONAL_ANALYSIS_SCOPING.md`)
