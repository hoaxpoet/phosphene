...
- **TONAL.2b — corpus calibration run ⛔ pending Matt's machine (needs the SSD corpus).** Run `swift run TonalDumper --manifest tools/data/corpus_pilot_1000.csv --root "/Volumes/Extreme SSD" --out tonal_pilot.json` → set `TonalAnalyzer`'s placeholder constants (consonance floor, tension p99, flux threshold/p99) from the measured percentiles → `docs/diagnostics/TONAL_PILOT_REPORT.md` with the distributions + validation (known-modulation phase migration, one-chord-vamp near-zero flux, jazz/rock/electronic distinguishable, percussion/ambient below the gate). Constants land with measured provenance (IFC.6 discipline).
...
### Increment QG.3.2 — Coupling warning-tier review flag ✅ (2026-07-10, D-183)
...
### Increment QG.3.1 — Coupling measurable for all 13 (shared multi-pass render) ✅ (2026-07-10, D-183)
