...
- **DBN.3 — wire as `BeatGridResolver` alternative, benchmark A/B.** Next, but **§9.6's observation-model fix should land first** — wiring in a decoder with a known meter-selection bias would A/B the bias, not the decoder. Gate on Take Five 5/4, Money 7/4, Solsbury Hill 7/4 — all currently decoded as 4 — plus no-regression on suite 1 and the three passing suite-2 AMLt scores.
