...
> **KSRETIRE.1 / D-187 (2026-07-20):** Kinetic Sculpture — RMENV's intended first consumer (KSRB.2) — was retired before opting in. **RMENV is retained** as a completed opt-in, byte-identical capability with **no production consumer yet**; the deferred production per-draw env/manager wiring (was KSRB.2) lands when a future ray-march preset consumes it. Not dead code — do not delete.
...
**Kinetic Sculpture half — CANCELLED (KSRETIRE.1 / D-187, 2026-07-20).** The KSRB.1 geometry rebuild + Phase RMENV material lift were built to deliver this half, but the chrome-in-a-gallery result read as a "tinker toy" and a psychedelic pivot drifted off-concept; Matt stopped Kinetic Sculpture and retired it. Its `docs/VISUAL_REFERENCES/kinetic_sculpture/` set was deleted with the preset. (Phase RMENV engine work is retained for a future ray-march consumer — see KSRETIRE.1 / D-186.)
