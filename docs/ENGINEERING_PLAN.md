...
- **⌫ REVERTED TO FL.10 (2026-07-09, Matt's call). FL.11→FL.14 all undone; head = confirmed-brilliant FL.10.** Matt live-read the FL.13 baseline "No connection to the music. Pure chaos. Dislike immensely." Diagnosis (git-diff FL.10↔FL.13): FL.10's felt connection is a **whole-field ENERGY SURGE** (`flowSpeed = 0.0015 + 0.0040·energy`, the field surges together when loud); FL.12/13 gutted it (cut to `0.0012 + 0.0016·energy`) and moved energy into slow *independent* per-colour drifts + removed beat accents → aimless chaos (render gate: FL.10 driven lit=0.638/rest 0.000 vs FL.13 0.215/0.006). **RECONCILIATION** (Matt: FL.10 = foundation not destination; "I got excited by the direction"; confirmed direction = **"instruments as distinct voices"**): FL.10 was never un-reactive — its brilliance WAS its reactivity; FL.11→14 replaced working reactivity with broken kinds (beat-lock/per-colour-drift/attack-ratio). **PRINCIPLE for the retry — motion stays SHARED (all voices on the one surge = connection), identity rides on PRESENCE (a voice blooms brighter/denser when its instrument is loud, never its own motion), driven by the reliable band-stem∣family hybrid gated by actual loudness.** Validate in SIM against the `19-35`/`20-01`/`20-20` streams before any live look; one change at a time with a live look between. FL.11→14 recoverable (`4edb4e9`/`cf6241f`/`3a11bfa`/`3242d7a`→`9eaa72a`). Full record: `RICERCAR_DESIGN §FANTASIA REBUILD`.
