...
### Phase IFC — Instrument-Family Capture (D-177): IFC.1 model+license ✅, IFC.2 MPSGraph port + parity ✅, IFC.3 per-family activity + normalization ✅, IFC.4 pipeline integration ✅, IFC.5 validation ✅ (2026-06-30) — next IFC.6 Ricercar drive-layer swap
...
- **Scope guard:** recognition + per-family interpretation + plumbing + validation; IFC.4/5 validated by pipeline correctness + real-audio firing, **not** musical feel (that arrives in IFC.6). **Next = IFC.6** (Ricercar drive-layer swap + optional low/high-strings 5-section split; tune `devGain` against real per-family p99 + add a small `dev` floor for the near-zero-material leader-flap; disk-persist the series). This resolves Matt's Ricercar hold ("unless there is instrument separation, I will hold").
