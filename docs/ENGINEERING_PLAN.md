...
Mood (`valence → palette warmth/saturation`, `arousal → vigour/density`; smoothed in `SkeinState`, never via `setFeatures` — FA #25), anticipation (painter wind-up/release on `beatPhase01` — beat-phase, not raw onset, FA #33; honour the cold-start caveat), the structural bias (CONSUMES the **Skein.ENGINE.3** signal — boundary → palette emphasis + density + soft region lean, gated on `confidence` so the allover read survives ambient material), and a flagged painter-locus (off by default). Depends on Skein.3 + Skein.4.1 + **ENGINE.3** (structure). **Prerequisite discovered by AUDIT.1 (2026-06-09): BUG-035 — `NoveltyDetector` re-registers every section boundary ~4-5× once the similarity ring wraps, corrupting `predictedNextBoundary`/`sectionIndex`/confidence (the exact ENGINE.3 signal Skein.5 consumes). Fix BUG-035 before or as step 1 of Skein.5.** Prompt: `~/Downloads/SKEIN.5_mood_structure_session_prompt.md`. Not started.
