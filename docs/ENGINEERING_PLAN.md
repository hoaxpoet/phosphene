...
**Matt's M7 on `2026-08-20T21-57-26Z`: *"Looks great. Ready to certify I think."*** Session verified by binary, not assumed — the running app was built 16:56:54 local from the exact `d92974d6` (WIRE.2/.3) source, and Ricercar was selected three times and held continuously through session end. **21st certified preset**, first in the `painterly` family's live catalog, first `mesh`-free particle preset to certify off the onset-driven-mark mechanism (a distinct paradigm from Skein's marks-on-top or Filigree's reaction field).

**The automated L2 gate reads FAIL, and that is a known false negative, not waived blind.** `FidelityRubricReportTests` reports `L2_deviation_primitives: no deviation primitives found` because the static analyzer scans `.metal` source text for literal tokens (`bass_dev`, `mid_rel`, …) and Ricercar's onset detection is entirely CPU-side, in `RicercarEchoGeometry.advance()`. Read directly rather than trusted: `advance()` computes `onset = (levFast − levMed) / max(0.08, levMed)` — a RELATIVE local transient, the deviation-primitive pattern by construction, just not spelled the string the analyzer greps for — and gates mark sharpness on `feat.bassDev`/`midDev`/`trebDev` directly. **`Skein` — genuinely certified — sits at the identical 2/4 for the identical reason**, confirming this is a precedented, accepted exception class (`FidelityRubricTests`' own comment: *"the heuristic gate is a sanity-check, not a strict cert prerequisite … the load-bearing gate is Matt's reference-frame review"*) rather than something invoked freshly here. L1 (silence fallback) and L3 (performance) both pass automatically; L4 is Matt's review, now given.

**Performance had never actually been measured.** `PresetFrameBudgetTests` carried Ricercar in its `UNVERIFIED` list — no case existed in `MultiPassRenderHarness.render(preset:)`, so the mandatory *"performance within tier budget"* item was unverifiable by the standing gate, not merely unverified. Added `renderRicercar` (the same `ParticleGeometry` dispatch pattern as Meniscus/Witchlight, `ensureAllocated` called first per RICERCAR-WIRE.2's contract) and moved Ricercar into `multiPassPresets`. Measured: **5.93 ms, 1.0× the roster median** at 1080p — squarely mid-pack, no new baseline concern.

**Same gap, same fix, for D-157 flash-safety.** No flash test existed for Ricercar either. Added `ricercar_isFlashSafe`, reasoning about the actual mechanism rather than reusing another preset's worst-case-train assumption: the onset detector has a refractory period, so it cannot fire faster than its own ceiling regardless of input — the worst-case beat train drives it at that ceiling, not at the train's rate. Measured **0.00 flashes/s peak, Δluma 0.019** (6.3× the 0.003 responsiveness floor, so a real measured response, not a vacuous static pass) — safe with wide margin, consistent with a preset whose whole visual character is small individual marks against a slow-decaying trail, never a full-field pulse.

**The flow geometry's expiry, honored.** WIRE.1's commit recorded a deliberate exception: `RicercarFlowGeometry`/`RicercarFlow.metal` kept with no consumer as the fallback if ECHO failed its M7, "delete the flow geometry in that increment" if it certified instead. It certified. Deleted: `RicercarFlowGeometry.swift`, `RicercarFlow.metal`, `RicercarFlowRenderTests.swift`, `RicercarFluidVideoHarness.swift` (its one test, `test_realAudioResponseVideo`, existed only to drive the deleted geometry). `RicercarSubstrateTest.swift`'s wiring assertions were unaffected — they check the sidecar's particles pass, backdrop fragment, and registry membership, none of which name a specific geometry type — and its comments/description were updated in place rather than treated as dead weight. `PresetLoaderCompileFailureTest.swift`'s roster-history comment was annotated, not rewritten (FTR.5 precedent): it was true when FL.10 landed, and stays a true record of that.

**⚠ Known gap, not addressed here.** No persistent real-audio-with-music diagnostic harness exists on `main` for Ricercar today. The WIRE.2/.3 look sessions used a throwaway scratch test (`EchoCrispScratch.swift`, written and deleted each time) rather than a committed tool. `origin/claude/ricercar-echo-look-prompt-bd7993` still carries `test_echoPrototypeVideo`/`test_echoSyncDiagnostic`, the equivalent tool from the pre-merge ECHO work — porting it is a small, separate increment if a future look session wants it, not folded into this certification.

Roster: **21 / 28 sidecars certified** (2 diagnostic, not roster presets — unchanged count). `Ricercar.json certified: true`. `FidelityRubricTests.certifiedPresets` updated.

Engine suite green, lint 0, app builds, `RouteCoverageTests` green (confirmed unaffected by the WIRE.3 mechanism swap).
