...
### Phase IFC — Instrument-Family Capture (D-177): IFC.1 model+license ✅, IFC.2 MPSGraph port + parity ✅, IFC.3 per-family activity + normalization ✅ (2026-06-30)
...
- **Scope guard:** recognition + per-family interpretation only. **No** FeatureVector/StemFeatures wiring (that is IFC.4); the visual engine is source-agnostic so Ricercar's R.3/R.4 visual work is not wasted. **Next = IFC.4** (surface the per-family activity into `FeatureVector`/`StemFeatures` as a GPU-contract update; run the tracker over the preview clip's window sequence during pre-analysis; ship the CC-BY attribution notice) → IFC.5 (orchestral-fixture validation) → IFC.6 (Ricercar drive-layer swap + the optional low/high-strings split for the 5 sections; tune `devGain` against the real per-family p99).
