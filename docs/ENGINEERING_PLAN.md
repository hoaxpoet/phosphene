...
**Follow-up — RN.4:** RN.2's deferred surfaces (persisted `UserDefaults` keys, on-disk `phosphene_*` paths, shader/sidecar comments) plus the public-copy wording decisions listed in the RN.3 closeout — all of which need Matt's pick.
...
**Follow-up — RN.3:** persisted `UserDefaults` key scheme and the on-disk `phosphene_*` output paths (both user-visible → Matt's call), and the `.metal` / preset-`.json` brand-prose batch (needs the `preset-session` skill). Start from RN.2's tip.
