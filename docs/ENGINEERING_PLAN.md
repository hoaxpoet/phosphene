...
The foundation is implemented and tested: native Metal render loop with a data-driven render graph; Core Audio tap capture with provider abstraction; full MIR + stem-separation pre-analysis (MPSGraph Open-Unmix, Beat This!); session lifecycle from playlist connection through planned playback (streaming + local-file paths); the Orchestrator scoring/planning/adaptation stack; and the ray-march / feedback / mesh / particle preset substrates with the certification pipeline. Test infrastructure: swift-testing + XCTest across unit, integration, regression, and performance categories; SwiftLint strict; protocol-first DI with test doubles. Quality-gate mechanization (Phase QG: per-preset audio route-coverage, mandatory render-comparison sheets, per-paradigm multi-frame harness templates) and continuous input-chain health classification (Phase ASH: `SignalHealthMonitor`) are in place.
...
- **CENSUS.4 — full-corpus run (27,639) + census report ✅ (2026-07-10; commit below).** Ran the full corpus single-rate (30 s window) through the production pipeline — **27,638 analysed cleanly, 1 AVFoundation-unreadable**; ~4h49m wall-clock, driven by a resume-loop supervisor (the runner was repeatedly SIGKILL'd `rc=137` under external memory pressure — no jetsam log — so ~75 fresh-process batches each resume-skipped done rows). Results SSD-only; only `docs/diagnostics/CENSUS_FULL_REPORT.md` (+ the `tools/census_report.py` extension: `--title`, dual-rate section skipped when absent, new §Swing) is in-repo. Confirms four pilot findings at 27× n and **overturns the fifth**: (1) **mood-flux saturation is GONE** — post-BUG-066 the corpus reads flux **+1.53σ** (was +38σ), corroborating the fix; the remaining mood signal is std-compression (features cluster tighter than DEAM) → the §5 Tier-1 `mood_scaler.json` re-estimate candidate stands, re-scoped; (2) **D-154 10 % gate is a smooth-continuum line** — 32.9 % above 0.10, 34.1 % irregular → threshold-review candidate; (3) **K-S F#-minor bias is population-level** — 36.2 % (10,016 tracks) → §8.5 ground-truth precondition; (4) **classical 48.7 % beat-irregular is the true rubato outlier** (jazz 37.8 %, regressed toward the 33.7 % baseline at scale) → the Deep swing-ratio increment is *conditional* on wanting beat-lock to reach swung material; (5) **cross-path skew NOT re-measured** (single-rate by design; pilot's ~9 % stands). **Swing exploration: Light path** (report-only, per the CENSUS.4 DECISION-NEEDED default) — §Swing quantifies the gate locking out 49 % of classical / 38 % of jazz. No threshold/scaler/constant changed (scope guard held).
...
- **TONAL.1b — `SpectralCartograph` tonal trace row ✅ code-complete (2026-07-08).** The Cartograph BR timeseries panel goes 3→5 rows: added **5TH φ** (circle-of-fifths phase, teal, wrapped-sawtooth with a ±π-wrap guard) + **CONS** (consonance, pink, scaled to its pilot p99 0.32). Two new 480-float trace rings land in the **reserved region** of `SpectralHistoryBuffer` (offsets 2430/2910 — **no buffer resize**, buffer was already 4096 floats; other presets read only their own offsets, zero blast radius); `append()` stores plot-ready (fifths normalized (θ+π)/2π, consonance /p99) so the shader's uniform row-drawing works. Shader (`SpectralCartograph.metal`: kRowH 1/3→1/5 + kOffFifths/Consonance + wrap guard) + Core-Text labels (`SpectralCartographText`, repositioned 5 rows). **Done-when:** ✅ `SpectralHistoryBufferTests` (+ new tonal-ring normalize/scale/clamp round-trip) + `SpectralCartographTests` render + `CommonLayoutTest` green; engine + **app build** (Metal compiles) green; lint 0. **Pending: Matt's live view** (a dashboard — verified on-screen at the TONAL.3 listen, the objective cross-check against his ear).
- **TONAL.2a — `TonalDumper` executable ✅ code-complete (2026-07-08).** Retained-diagnostic CLI (IFC.5 pattern; `Sources/TonalDumper`, deps Audio/DSP). Runs the **production** MIRPipeline (ChromaExtractor + TonalAnalyzer) over decoded audio at 1024-pt FFT / **512-hop** (the live ~94 Hz cadence, so flux/tension calibration transfers). Two modes: `--audio` → per-window fifths/thirds-phase + consonance/tension/flux table + summary percentiles (`--out` JSON series); `--manifest`/`--root` → batch the CENSUS pilot → corpus-wide + per-genre percentiles = the calibration deliverable (`--clip-seconds` default 30 = preview parity). Pure math (`TonalStats`: percentile / circular-mean / window-bucket / CSV) split out + **6 unit tests**; registered in `Package.swift` + AUDIT_KEEPLIST + Module Map (D-168). Lint 0. **Wiring smoke-test** (a generated C→G chord WAV, NOT corpus data — FA #27): CLI decodes → runs → the fifths phase migrates ~−19°→−79° at the chord change, 93.8 Hz, consonance above the floor. **No production constant changed.**
- **TONAL.2b — corpus calibration run ✅ done (2026-07-08).** Ran the dumper over the full 1000-track stratified pilot from the mounted SSD (**2.66M frames, 0 unreadable**). **★ Validation:** per-genre consonance ranks **classical 0.180 > jazz 0.140 > … > rock 0.108 > hiphop 0.101** — the ordering tracks how much sustained clear harmony each genre carries (TIV measures harmony, not noise); tension highest for classical (0.045) then jazz. **Constant correction:** the placeholder `consonanceFloor = 0.12` sat at the corpus **median** (0.117) → would have gated off half the library; recalibrated to the atonal floor **0.05** (≈p3–4) + width **0.03** (full signal by p22, all genre medians pass). Saturation p99s documented for TONAL.3's Nacre coupling (soft-saturate, not clamp-to-1): consonance 0.32 / tension 0.163 / flux 0.110. **Also fixed a real TONAL.2a bug** (`90da54a`): the manifest reader split on `\n` but Swift fuses `\r\n` into one grapheme → every CRLF manifest parsed as 0 tracks (the CENSUS gotcha); `TonalStats.splitLines` + a CRLF unit test. Report: [`docs/diagnostics/TONAL_PILOT_REPORT.md`](diagnostics/TONAL_PILOT_REPORT.md). Constants carry measured provenance (IFC.6). **Done-when: ✅** pilot run clean, constants set + provenance-documented, report committed, tonal tests green, lint 0.
- **TONAL.3 — Nacre consumption ✅ M7 SIGNED OFF (2026-07-10, round 2).** Matt's re-M7 on round 2: "looks good." Nacre keeps its certification with the real harmony-hue coupling (`tonal_phase_fifths`) replacing the centroid proxy. **The whole TONAL phase (.0–.3) is complete.** **Round 1 M7 read as "not sure if it worked"** — the session data proved the coupling was ACTIVE (fifths swept the full range, consonance mean 0.107 above the gate) but MASKED: `palette = time + offset` let the clock rotate ~13.7 palette cycles/song vs harmony's ±0.5, so it read as the usual time rotation. **Round 2** makes harmony SET the hue position — `nu.hueShift` is now the full palette phase (`harmonyAnchor·gate + demoted-clock-drift`; the clock nearly stops when tonal so a vamp HOLDS its colour, restores the faithful full rotation at silence). Dev-preview frames now show a legible separation (key-A gold-green vs key-B green-teal, and the same key holds across 4× the time); flash re-measured 0.50/s SAFE; routes green. See NACRE_PLAN §13. Nacre's hue channel upgraded from the NACRE.3 centroid-deviation **proxy** to real harmony: **hue ← `tonal_phase_fifths`** on a circle-of-fifths mapping (full ~14.4 s `nacrePalette` cycle, so the 12 keys span the wheel and fifth-adjacent keys land on adjacent colours), circular-EMA'd as a unit vector so modulations glide (wrap-safe), consonance-gated to the neutral rest state so silence/percussion falls back to the faithful time rotation. **Saturation ← `tonal_consonance`** (atonal MUSIC desaturates; silence stays faithful — energy-aware so the D-019 warmup palette isn't washed). **Tension → rim-dispersion DEFERRED to round 2** — it under-develops on 30 s preview fixtures so the QG.1 route-coverage gate can't exercise it (fixture-breadth, not a dead route); FA #67 stays clean with 2 routes. Hue never strobes (the Nacre anti-reference) — it's a slow harmonic signal, not per-beat energy. **Evidence:** rendered `NACRE_FIFTHS`-injected frames show the palette shift **gold-green → teal-green** across a key change (shifted-but-related, not a jump); D-181 verdict table PASS on all mandatory traits + anti-refs; flash re-measured **0.50 flashes/s SAFE**; `RouteCoverageTests` green (both tonal routes fire on all 3 fixtures); sidecar `description` + `audio_routes` updated to the shipped coupling (NACRE.5 honesty; `hue_from_harmony`→`tonalPhaseFifths`, +`saturation_from_consonance`). Untouched presets' goldens unaffected (Nacre-scoped change); lint 0; engine + app build green. **Pending: Matt's live M7** — the "does the real signal read better than the proxy" verdict is his ear (2 rounds max, PHYS escalation; revert-keep-infra if round 2 fails). Saturation/tension are M7 watch-items (their isolated effect is confounded by feedback chaos in a still).

## Phase QG — Quality-gate mechanization ✅ (2026-07-09→10; D-180 / D-181 / D-182)
...
## Phase ASH — Audio signal health 🔨 (2026-07-10; D-183)
...
`VolumetricLithographRayMarchHarnessTest` copy-adapts `RayMarchPathHarnessTemplate` (D-182) with the Lumen slot-8 follower dropped (VL has no CPU follower — the template's documented degenerate case). 60 silence frames through the live `RayMarchPipeline.render` seam (G-buffer → lighting → composite → post). **Non-degenerate + luma floor 0.35 mean / 0.83 max** (D-037 alive, not BUG-016 black); golden composite dHash `0x13CFC77D5EB3A349`. **A/B validated** (not a tautology): perturbing the terrain phase axis drifts the composite 23 bits — golden trips, drift metric registers.

**Harness gap found and closed:** the template sets `sceneUniforms` once, but the live path rewrites `sceneParamsA.x = features.accumulatedAudioTime` every frame (`RenderPipeline+RayMarch.swift:114`) — and VL's entire world (terrain phase + camera) rides that axis, so the un-mirrored harness rendered 60 identical frames. The live write is now mirrored in the loop.
...
**§7 reference re-curation ✅ done this increment.** The naturalistic set moved to `_superseded_naturalistic/` (now the primary anti-reference); four committed trait images replace it — `01` radial-symmetry structure, `02` domain-warp character, `03` continuous saturated hue flow (palette hero), `04` angular colour organisation (Delaunay, PD). Every image carries a **disregard-this** annotation, because on both of the first two the stock-site description was contradicted by looking at the file (`01` is pale, not saturated; `02` is two-tone, not iridescent). The hero is **link-only** (Matt's call): "flight through a folding world" is a Shadertoy/demoscene genre and those frames are copyrighted, so Shane's *KIFS Flythrough*, Kali's *Fractal Land*, and *Playing with Kaleidoscopic IFS* are cited by URL and flagged as look anchors, **not port targets**. **Fractal Fly-By (then named Fractal Descent) added as an anti-reference** — the nearest in-catalog trap, since VL collapsing into "Mandelbox Cathedral with a moving camera" is the design doc's own §1 warning. **Remaining hole, named not papered over:** the lighting/specular slot has no committed anchor.

**Next:** Session 2 from task 2 (macro geometry-space warp) — the reference gate is now clear.
...
**Done-when:** the exit-65 "Could not launch “PhospheneAppTests”" failure has a proven root cause, and a stray-app failure is distinguishable from a genuine app-test regression in the closeout evidence block. Both met.

**Root cause: a running instance of the app under test blocks the XCTest host launch.** The test host *is* `PhospheneApp.app`; `Info.plist` sets `LSMultipleInstancesProhibited` (U.11, for `phosphene://` OAuth callback routing), so LaunchServices refuses the second instance no matter which DerivedData path it comes from. `build` and `build-for-testing` were unaffected because neither launches anything. It looked machine-wide precisely because the blocker is a *running process*, not a build product — so it survived checkouts, worktrees, HEAD~1, `lsregister -f -R -trusted`, and bundle delete+rebuild, every one of which targets build output. Proven by A/B/A (3 green runs, 403 tests → launch the app → identical command fails exit 65 verbatim → quit → green), corroborated by the unified log (PID 35320 alive 15:47–18:19; every `test` in that window failed at runner launch; the 16:47 xcresult reads `failedTests: 1, passedTests: 0`).

**No app-side change.** `LSMultipleInstancesProhibited` is load-bearing — dropping it breaks OAuth callback routing and lets a test host contend with a live session for the system-audio tap. Remediation is operational (quit the app first; documented in `RUNBOOK.md §Build and Test`). The repo-side fix is diagnostic only: `Scripts/closeout_evidence.sh` Step 2 recognizes the signature and labels it, **including the inverse case** — a runner launch failure with no `PhospheneApp` running is flagged as a distinct, unexplained defect rather than silently absorbed into BUG-072. ★ The general lesson: when a failure survives every remedy aimed at one layer, the cause is in a different layer — here, `ps` would have answered in seconds what a day of build-artifact remedies could not.
...
Cut the fixed per-hit-pixel `sceneSDF` overhead in `PresetLoader+Preamble.swift`'s `raymarch_gbuffer_fragment` from **11 evals to 7**: tetrahedral normal (4 taps, IQ's trick) replacing the 6-tap central difference, and a 3-sample AO cone (rescaled step/weight for the same reach + occlusion) replacing 5. ~36 % off the overhead every ray-march preset pays regardless of march length. **Motivation:** FD.1's perf gate showed Fractal Fly-By's Mandelbox DE (~10 fold iterations/eval) is expensive enough that this fixed overhead — not the 128 march steps (measured: halving them changed FD's cost ~0.2 ms) — is the budget driver. **Result (M2 Pro, 1080p, `FractalFlyByBudgetProbeTests`):** FD cap 8 enclosed 6.38 → **5.58 ms**, cap 10 8.01 → 7.57 ms; Lumen Mosaic control ~1.0 → ~0.92 ms. **Verified non-breaking:** all 27 presets held within the `PresetRegressionTests` 8-bit dHash tolerance (a broken normal/AO shifts far more), plus an A/B render of Volumetric Lithograph — indistinguishable. This unblocks the Fractal Fly-By build at iteration cap 8 (visually near-identical to 10) with real FD.2–FD.3 headroom. Engine capability, no production consumer flipped.
...
Matt stopped Kinetic Sculpture (D-188). After several redesigns the preset never found the right direction: the chrome-in-a-gallery look (KSRB.1 + the Phase RMENV material lift built to serve it) read as a "tinker toy," and a psychedelic-iridescent pivot drifted into a different concept rather than fixing KS. A fresh **psychedelic-geometry preset** will be authored separately, from a new spec. Removed in its entirety: `KineticSculpture.metal` / `KineticSculpture.json`, `KineticSculptureTests`, the KS-only `KineticSculptureMotionGifHarness`, `docs/presets/KINETIC_SCULPTURE_DESIGN.md`, and `docs/VISUAL_REFERENCES/kinetic_sculpture/`. Wiring: `expectedProductionPresetCount` 26 → 25; `FidelityRubricTests.expectedAutomatedGate` KS entry removed (KS was never in `certifiedPresets`); `PresetRegressionTests` KS golden + KSRB.1 comment removed; `PresetVisualReviewTests` / `MaxDurationFrameworkTests` KS rows removed; `SessionRecorderTests` log literal repointed to Lumen Mosaic. **GoldenSessionTests regenerated deterministically** — with KS gone, Session C track 2 (mid-energy Rock) falls KS → Membrane (the runner-up geometric/reaction fit); every other slot is byte-identical (a single runner-up substitution, not a planning regression). Shared-path comments that named only KS (`RayMarch.metal`, `IBL.metal` `ibl_proc_env`, `PresetDescriptor+SceneUniforms.swift`, app `VisualizerEngine+Audio.swift`) generalized. **Phase RMENV (D-187) engine work is RETAINED** — multi-light `SceneUniforms`, `ibl_env`/gallery, per-preset background, `MultiLightSceneUniformsTests` + `IBLEnvironmentTests` all kept; RMENV is a completed opt-in capability with **no production consumer yet** (KS was its intended first consumer), awaiting a future ray-march preset — intentional per Matt, not dead code. The shared ray-march path (Volumetric Lithograph, Test Sphere, `RayMarchPipeline`, `RayMarch.metal` / `IBL.metal`) is untouched. Docs synced: DECISIONS D-188 + D-187 retention note, ARCHITECTURE Module Map, CAPABILITY_REGISTRY/PRESETS, ENGINE/RENDER_CAPABILITY_REGISTRY (RMENV rows note KS retired / awaits consumer), Milestone D roster 26 → 25, RELEASE_NOTES_DEV. Roster now 25 production presets, 14 certified (Aurora Veil certified AV.7 on main). Recover from git history if the concept is revived.

### Increment KSRB.1 — Kinetic Sculpture geometry rebuild ✅ (2026-07-20) — SUPERSEDED

> **Retired at KSRETIRE.1 / D-188 (2026-07-20).** This geometry rebuild and the Phase RMENV material lift built to serve it are moot — Matt stopped Kinetic Sculpture (chrome-in-a-gallery read as a "tinker toy"; a psychedelic pivot drifted off-concept). Kept for history; RMENV itself is retained (see KSRETIRE.1 above).

Replaced the retired periodic 3-axis rod grid (`ks_rep`, the preset's own documented anti-reference — "reads as engineering drawing, not sculpture") with the hero reference: Lippold *Flight* radiating strands. **Three interpenetrating hub-pair fan systems** (offset centres, tilted axes, distinct scales — refs 01/02, Matt's call after a single-bicone first pass read as a closed lantern) + a demoted waist tension ring; per-member hash jitter (no two members identical); per-system + whole-sculpture bounding spheres keep the 100+ `sd_capsule` unions affordable. Thin-strand rendering is clean — the feasibility spike's shimmer caveat confirmed in the production pipeline via a new **`KineticSculptureMotionGifHarness`** (`KS_GIF=1`, the deferred-ray-march analogue of the Aurora motion GIF). Camera reframed to fill the frame; `family: abstract`→`geometric` corrected in the reference README (D-123 deleted `abstract`; setting it would silent-drop the preset, FA #44). **Materials are intentionally under-served at this baseline** — chrome reflects the single-light uniform-grey IBL as flat "putty"; that is the boundary Matt chose to fix via **Phase RMENV** (option (b), 2026-07-20; not the luminous-glowing-wire alternative). **Done-when: ✅** shader compiles, catalog count holds 26; `PresetAcceptanceTests` 4/4; KS `PresetRegressionTests` golden regenerated (deliberate geometry change — regens again at KSRB.2); DocIntegrity 12/12; engine + app build. Design: `docs/presets/KINETIC_SCULPTURE_DESIGN.md`. **Next: Phase RMENV** (multi-light + gallery IBL + dark background), then KSRB.2 consumes it.

### Phase RMENV — Ray-march render environment ✅ capability-complete (RMENV.1–.4 ✅ 2026-07-20; awaits a future consumer)

> **KSRETIRE.1 / D-188 (2026-07-20):** Kinetic Sculpture — RMENV's intended first consumer (KSRB.2) — was retired before opting in. **RMENV is retained** as a completed opt-in, byte-identical capability with **no production consumer yet**; the deferred production per-draw env/manager wiring (was KSRB.2) lands when a future ray-march preset consumes it. Not dead code — do not delete.

**RMENV.1 — Multi-light deferred lighting ✅ (2026-07-20).** `SceneUniforms` 128 → 240 B: `light1/2/3` + `lightingParams.x` (count 1…4) appended after `sceneParamsB` (existing offsets unchanged), updated in lockstep across all four definitions (Common.metal + Swift struct + both preamble mirrors). `makeSceneUniforms` decodes up to 4 sidecar `scene_lights` (5th+ ignored, documented); the `RayMarch.metal` lighting loop sums Cook-Torrance over `lightingParams.x` lights, shadowing only light 0 (key). **Byte-identical at count 1** — every existing ray-march preset golden unchanged (only Aurora Veil's 3 pre-existing AV.6 drifts remain). Multi-light proven: `MultiLightSceneUniformsTests` 4/4 (decode) + a throwaway 3-light KS render showed distinct key/rim/fill directional highlights. Engine + app build, swiftlint 0, DocIntegrity 12/12. GPU-contract + registry docs updated.

**RMENV.2 — Selectable high-contrast gallery IBL ✅ (2026-07-20).** The default IBL (`ibl_proc_env`) is a smooth warm-grey gradient — a mirror reflecting it reads as flat "putty" (KS README §4.1). Added `ibl_gallery_env` (bright HDR skylight strips over dark ceiling gaps, faint wall pilasters, dark polished floor) + an `ibl_env(dir, envType)` dispatcher; the two IBL bake kernels take an `envType` (default 0 → **byte-identical** cubemap, so no existing golden moves); `IBLManager(envType:)` bakes it; `PresetDescriptor.environment` ("gallery" → 1) selects it. Verified: all ray-march goldens byte-identical (only Aurora Veil's pre-existing drift); `IBLEnvironmentTests` proves envType 1 bakes a different cubemap than 0; a throwaway `environment:"gallery"` + 3-light KS render showed the strands finally reading as **dark polished chrome with bright specular streaks** (skylight reflections) instead of putty. Wired through the render harnesses; **production per-draw env selection deferred to KSRB.2** (needs the RenderPipeline manager cache + descriptor thread-through, exercised when KS opts in live). Registry + SHADER_CRAFT §17 + `IBL.metal` docs updated.

**RMENV.3 — Per-preset background ✅ (2026-07-20).** The `RayMarch.metal` miss path renders `ibl_env(rd, bgEnv)` when `SceneUniforms.lightingParams.y != 0` (the environment type, set by `makeSceneUniforms`), else the original `rm_skyColor` sky. So a preset with an environment shows that environment as its backdrop — the sculpture sits IN the gallery it reflects, against a dark ground (fixes the pale-tone floor; refs 01–03 want negative space). **Opt-in & byte-identical**: env 0 keeps the exact sky path, all ray-march goldens unchanged (only Aurora Veil's pre-existing drift). A throwaway gallery KS render showed the sculpture grounded in a coherent grey gallery interior with a wall/floor horizon instead of floating in pale sky. Registry row + D-187. **RMENV.4** = the phase's DECISIONS record (D-187) + this registry pass (done inline). **Phase RMENV capability-complete.** (Was: next KSRB.2, KS opts into gallery + lights + dark background + production per-draw wiring — but KS was retired KSRETIRE.1 / D-188 before opting in; that production wiring now awaits a future ray-march consumer.)



Shared-renderer capability so ray-march metals read as metal and scenes can have a dark gallery ground — the "V.12 material lift" that parked Kinetic Sculpture, done generally (benefits Lumen Mosaic / Ferrofluid / future architectural presets), KS as the intended first consumer (since retired KSRETIRE.1 / D-188; capability retained for a future consumer). Three limits, each confirmed by rendering during KSRB: single-light deferred lighting, uniform-grey IBL (chrome reflects flat grey = putty), shared pale sky background. **Design constraint: every capability opt-in and byte-identical** — a preset that doesn't opt in renders exactly as today, so each increment's gate is "all existing ray-march goldens byte-identical" (no catalog re-baseline). Matt's calls (2026-07-20): **4 lights** (key/rim/fill/accent), **engine-first** (whole RMENV, then resume KSRB.2), **procedural** gallery environment. Increments: **RMENV.1** multi-light deferred lighting (SceneUniforms N-light layout — the 4-definition GPU-contract lockstep risk) → **.2** selectable high-contrast gallery IBL → **.3** per-preset dark background → **.4** registry + contract docs → KSRB.2 consumer. Scope: `docs/RENDER_ENVIRONMENT_SCOPING.md`.

### Increment GBRETIRE.1 — Glass Brutalist retired ✅ (2026-07-19)

Executes the 2026-07-19 concept-viability-gate decision (Milestone D roster survey; D-186). Glass Brutalist — the original ray-march scene preset and the subject of D-020 — is removed in its entirety: `GlassBrutalist.metal` / `GlassBrutalist.json`, its dedicated tests (`GlassBrutalistTests`, the GB-only `RayMarchSDFDiagnosticTests`, and the `GlassBrutalistValidationTests` JSON suite inside `RayMarchDiagnosticTests`), and `docs/VISUAL_REFERENCES/glass_brutalist/`. **Why retired, not tuned:** D-020 makes the concrete deliberately audio-static, so the hero subject can never be an instrument — the musical role is structurally hollow (Gate 1 fail); plus 2006-tier fidelity (Gate 2 fail; V.12 rebuild scoped + abandoned). Wiring: `expectedProductionPresetCount` 27 → 26; `FidelityRubricTests.expectedAutomatedGate` GB entry removed; `PresetRegressionTests` / `PresetVisualReviewTests` / `MaxDurationFrameworkTests` GB rows removed; app `cameraDollySpeed` GB case dropped (default 0). **GoldenSessionTests fixtures regenerated deterministically** — with GB gone, Waveform (the sole `waveform`-family preset) inherits GB's mellow-jazz slots (Session B now Waveform×5, Session C reslots), the same sole-family clustering already documented for Membrane in Session A; not a planning regression. Shared ray-march path (Kinetic Sculpture, Volumetric Lithograph, `RayMarch.metal` / `IBL.metal`, generic `SceneUniformsConstructionTests`) untouched; the `.ssgi` pass + `cameraForward.w` free lane stay on the GPU contract though no production preset now uses them. D-020 stays Accepted (rule governs future architectural ray-march presets) with a retirement pointer. Docs synced: DECISIONS D-186 + D-020 annotation, ARCHITECTURE Module Map + camera notes, CAPABILITY_REGISTRY/PRESETS, ENGINE/RENDER_CAPABILITY_REGISTRY, RELEASE_NOTES_DEV. Recover from git history if a future architectural ray-march concept revives it.
### Phase AV.7 — Aurora Veil reauthored as a faithful nimitz port ✅ CERTIFIED (2026-07-19; Matt M7 sign-off; D-185)

Aurora Veil had churned AV.2 → AV.6 without certifying, because each round added machinery on top of the nimitz recipe it derived from (footprint `F(x)`, 3-column parallax, band undulation, drum kink, traveling waves) while the shader itself quietly drifted from the reference — the 3D ray march flattened to a fake 2D column, `triNoise2d` grown a global time rotation nimitz never had, gain raised 1.8 → 2.4. Textbook FA #65. **Done-when:** ✅ resolved by porting the reference instead of deriving from it.

**The port.** nimitz "Auroras" (Shadertoy `XtGGRt`, 2017) retyped to MSL with algorithm + constants intact: real 3D ray march (`ro`/`rd`, sampling `bpos.zx`), 5-octave domain-warped triangular noise, the running-average smear, per-march-step H(z) palette (Lawlor & Genetti WSCG 2011), his `bg()`/`stars()`/horizon fade/1.8 gain. 644 lines of AV.2–AV.6 accretion deleted. Adapted only for the harness (`iTime`→`f.time`, `gl_FragCoord`→`in.position.xy`, `iResolution`→baked 1080p, y-flip).

**Framing + reactivity** (five live M7 rounds with Matt). Static **upward sky** — reflection/horizon/ground removed, camera pitched up; the camera pan was **deleted** rather than slowed because `stars()` is view-direction-indexed, so any camera motion scintillates the whole starfield (fixed "stars twinkle too much" + "don't like the camera movement" in one change). Three non-competing axes: stars→**downbeat** (`bar_phase01`, gated by `pulse_amp01`), brightness→**mood envelope** (`arousal`, clamped 0.85–1.15) with a subordinate smoothed-bass lift (`bass_att_rel`), colour→**mood** (`valence` → whole-palette phase).

**Certification.** Automated gate `[✓] 3/4` (L4 manual by definition); flash-safety **MEASURED 0.00 flashes/s** (the beat-stars are flash-safe by sparse footprint, D-157); regression goldens regenerated (old hashes 32–38 Hamming bits away — a different image by design). Obsolete gates re-pointed rather than dropped: the vocal-pitch→hue test became the mood-colour test, the bass-dominance test became "bass lift stays subordinate to the mood breathe", and the silence stratification test now asserts green-over-red + altitude colour spread instead of the deleted horizon composition. `certifiedPresets` += "Aurora Veil"; sidecar `certified=true`, `audio_routes` + description rewritten. Licensing: CC-BY-NC-SA port shipped **credited** on Matt's explicit call.

**★ Durable findings** (full detail in D-185): mood envelopes — not deviation primitives — are the right driver for a *gentle* response (`bass_dev` is spiky, `mid`/`treb` deviations near-flat on real music); `bass_att_rel` is the gentle deviation primitive that satisfies L2 without CPU-side smoothing; a **crown-only colour shift is perceptually invisible** under nimitz's `exp2(-i*0.065-2.5)` weighting; beat-sync legibility is a property of the **grid**, not only the mapping (the same mapping read unsynced on reverb-washed dream-pop and locked on Cherub Rock).
...
Turned ASH.1's classifier into user-facing surfacing + a machine-written per-session chain verdict, so degraded audio never reaches an M7/reel/fidelity review unflagged. **Task 1 — one health toast:** a once-per-session `band=low` nudge (DECISION-NEEDED opt 1), wired in `PlaybackErrorBridge` off `engine.$signalHealth`, reusing the existing `audioLevelsLow` case (Spotify "Normalize Volume" copy when `engine.isSpotifySource`, generic otherwise — zero new strings). **`deadTap` is NOT toasted** — the `AudioStallOverlayView` fix-ladder card already covers it earlier (~10 s) and more prominently (Matt's call); `sampleRateMismatch` stays overlay/log-only. **Task 2 — `ChainAnalyzer`** (`PhospheneEngine/Sources/Shared/`) grades a finished session dir → `chain_health.json` + a `CHAIN_HEALTH: verdict=<clean|degraded|broken> reasons=[…]` line, run in-process at `SessionRecorder.finish()` (every session self-grades) and out-of-process via `Scripts/analyze_session_chain.sh` (`ChainHealthAnalyzer` executable — retroactive grading of pre-ASH dirs). Verdict from raw_tap.wav peak (broken < −15 / degraded −15…−12 dBFS) + session.log scans (deadTap→broken; band=low/DRM/reinstalls→degraded); missing artifacts noted, never fatal. **Does not gatekeep** playback/prep/session-start. **Task 3 — empirically falsified & rescoped:** the specced "attenuate Love Rehab → onset count drops → flag degraded" mechanism does not work — the `beatBass` onset count is **AGC-invariant** (attenuation, dynamic compression, hard limiting, −50 dB all hold ~11-12/5 s; proven on a real capture through the production pipeline). Onsets are level-independent rhythm (D-026); normalization is caught by the PEAK check (RUNBOOK step 1), not onsets. The onset median is now **REPORTED in chain_health.json, never gated** (Matt's call). **Done-when:** ✅ engine + app build; `ChainAnalyzerTests` 7/7 (clean real Love Rehab → clean+11 onsets; band=low+DRM→degraded; deadTap→broken; pre-ASH dir no-crash; low-onset→NOT-gated; peak dBFS units; JSON+log-line write); `PlaybackHealthToastTests` 4/4 (fires once, no re-toast, healthy ignored, Spotify-vs-generic copy) — app 392 tests green; swiftlint strict 0; DocIntegrity 12/12; `analyze_session_chain.sh` grades a clean fixturegen dir `clean` + a real historical dir `clean` (−5.1 dBFS). **Task 4 live M7 ✅ SIGNED OFF (2026-07-10):** Matt walked both flows live — band=low induced one toast that named the remediation and did **not** re-fire after the setting was corrected (once-per-session-per-cause holds); end-session `chain_health.json` present with a sensible verdict. Matt: "yes for both." **Follow-up:** none (the greenlit deliverables were the toast + analyzer; onset-gating was the dropped path).

### Increment QG.3.2 — Coupling warning-tier review flag ✅ (2026-07-10, D-193)

Matt's call on the QG.3.1 distribution: ship the coupling gate as a **warning tier, not a hard cert blocker**. `CouplingReportTests.isReview` flags a preset `REVIEW` ("coupling not measured as present") when its best-fixture peak composite r is **< 0.15 AND** fails to clear its own noise control by **≥ 0.10** — both must fail, so a feedback preset that clears its (high autocorrelation) control passes below the absolute floor, and any preset above the floor passes. Judged per-preset on the best fixture (never a global all-fixtures threshold). The report prints a `VERDICT` block + summary; **it is informational, never a `#expect` / certification failure** (M7 seat is the authority). Standing boundary coverage: `reviewPredicateBoundaries` (runs without GPU/env). **Current verdict: 10 ok / 3 REVIEW** — Glaze (+0.13, a deliberately calm preset), Ferrofluid Ocean (+0.08, single-fragment-approximated), Nacre (+0.05, subtle camera-push coupling); all 3 certified + M7-approved → the flag is a proxy artifact, not a defect. `closeout_evidence.sh` Step 6 points at the report + verdict. **Done-when: ✅** warning-tier predicate + verdict block + boundary test; baseline doc marks QG.3.2 shipped; no cert assertion; no KNOWN_ISSUES. **Optional next:** validate the proxy against Matt's felt ordering before any *blocking* gate (Nacre-low / Skein-high anchors).

### Increment QG.3.1 — Coupling measurable for all 13 (shared multi-pass render) ✅ (2026-07-10, D-193)

Matt's QG.3 follow-up ("make measurable first; the gate flip is QG.3.2"). QG.3 found the metric measurable for only 2/13 presets (the offline single-fragment/zeroed-state render made 11 static). Fix: extract the photosensitivity flash gate's headless multi-pass render into a shared **`MultiPassRenderHarness`** (one faithful render, parameterized by drive train + per-frame reducer — FA #66), and drive it with the REAL reconstructed-fixture train (FA #27). The 10 multi-pass / feedback / follower presets now render through their real feedback loops; the 3 single-pass presets (Ferrofluid Ocean, Murmuration, Nimbus) keep the single-fragment path + a ticked Nimbus follower. **Flash gate byte-identical after the extraction** (Dragon Bloom Δ0.904, Nacre 0.063–0.153 unchanged — the render is shared, not reimplemented). **Result: all 13 measurable.** 11/13 clear their own noise floor (Skein +0.69, Dragon Bloom +0.47, Filigree +0.39, Lumen Mosaic +0.37 …); the floor is **per-preset** (feedback presets autocorrelate → higher floor, e.g. Dragon Bloom control +0.13), not global. Nacre (+0.05, subtle camera-push coupling) + Ferrofluid Ocean (+0.08, single-fragment-approximated) read weak for **proxy/render-fidelity reasons, not defects** (both certified + M7-approved). **Recommendation → QG.3.2 = a WARNING tier, not a hard cert gate** (a hard floor would false-red the two M7-approved weak presets); validate the proxy against Matt's felt-coupling ordering first. **Done-when: ✅** shared harness + all-13 measurable; flash gate green + identical; re-baseline (`QG3_COUPLING_BASELINE.md`) + D-183 + registry; no KNOWN_ISSUES (no non-proxy below-floor preset). **STOP before QG.3.2** — the gate flip + its tier are Matt's call on the distribution.

### Increment QG.3 — Audio-visual coupling metric (report-first) ✅ (2026-07-10, D-192)

### Increment ASH.1 — Signal health monitor: engine core + debug overlay ✅ (2026-07-10, D-183)

Turned the RUNBOOK's manual signal-chain triage catalog into running code. New engine type `SignalHealthMonitor` (`PhospheneEngine/Sources/Audio/`) consumes the raw pre-AGC tap and classifies input-chain health over a rolling 5 s window into one `SignalHealth`: `peakBand` (healthy ≥ −12 / low −15…−12 / critical dBFS, the RUNBOOK bands), `deadTap` (tap `.silent` continuously past a 45 s confirm — beyond the [3,10,30]s reinstall backoff; gated to process-tap modes; catches the default-output permission-invalidation trap *and* the cold broken tap by duration alone), `sampleRateMismatch` (default-output nominal rate outside 44.1/48 kHz). Observes only — never touches `SilenceDetector`, the reinstall backoff, or AGC (D-165 acts; D-183 classifies). Realtime-safe `ingest` (vDSP peak, no per-buffer alloc); classification + CoreAudio output-rate query + `onHealthChanged` emit on a non-realtime queue. Wired into `VisualizerEngine+Audio` (feed + reset + `SIGNAL_HEALTH:` session.log line on state change) and `+Capture` (signal-state feed with tap-mode gate) + the debug overlay `health:` line. **Done-when:** ✅ engine + app build; `SignalHealthMonitorTests` 12/12 (peak-band classification at attenuated real-waveform gain, dead-tap timing + non-tap gate + short-silence exclusion, rate-mismatch, dedup, reset); swiftlint strict 0; Module Map + RUNBOOK §"Signal health monitor" + AUDIO.md registry + D-183. No user-facing strings (ASH.2). **Task 3 validation (2026-07-10, against a live session's real capture — no synthetic audio, no re-grant):** (a) peak band ✅ — `raw_tap.wav` from session `2026-07-10T15-39-46Z` run through the verbatim `classify()` thresholds: global peak −5.1 dBFS `healthy`, 25 consecutive 5 s music windows all `healthy` (−5…−10 dBFS), and a real `healthy → critical` transition on the silent tail (125–129 s, −30 dBFS); (c) sample-rate query ✅ — `queryDefaultOutputSampleRate()` on real hardware returned 44100 Hz → `sampleRateMismatch=false` (in-family; note the default *output* reads 44.1 kHz while the tap captured at 48 kHz); (b) dead tap — armed, not observed (fires only on an actual broken tap; the chain was healthy). Classification validated on real audio/hardware for 2 of 3 detectors (the third is an anomaly detector); the live in-app `SIGNAL_HEALTH` wiring is covered by build + 12 unit tests and self-confirms on the first session after a rebuild that includes ASH.1 (Matt's running build predated it). **Next:** ASH.2 — user-facing surfacing + the degraded-audio certify/record policy (its DECISION-NEEDED block).

### Increment DOC.11 — scheduled documentation pruning pass ✅ (2026-07-09, no changes)

Ran the RB.3 five-pass pruning + D-161 ratchet check after the QG.1/TESTFLAKE.1/QG.1.2/QG.1.3/QG.2/DOC.9/Ricercar-FL.10 stack. **Finding: the doc system is already lean and fully compliant — no manual pruning required.** CLAUDE.md is **3,164 est. tokens** (45 % of the 7,000 cap; DOC.9's skills migration did the heavy trim); `rotate_docs.sh` reports ENGINEERING_PLAN / KNOWN_ISSUES / RELEASE_NOTES all already rotated; `DocIntegrityTests` 12/12 green (token cap, EP-rotation, citation resolution, skill integrity). Per-pass: (1) Failed Approaches — CLAUDE.md holds only a 13-row pointer gap-table (RB.2-curated); (2) Decisions — D-171→D-181 all recent/relevant, older shipped decisions already in `DECISIONS_HISTORY.md` (2,989 lines); (3) CLAUDE.md sections — already pointer-ized; (4) Current Status — pure pointers; (5) EP — >14-day bodies rotated, headers stay by design; (6) Skills — 5 files/195 lines, no handbook prose-duplication, pointers resolve. Ratchet: token budget under cap (no one-in-one-out needed); no new always-loaded rule added (the TESTFLAKE.1 "build after a refactor" lesson correctly went to memory — a deterministic gate expresses it, so it fails the always-loaded admission test); no prose rule hit the violated-twice→mechanize threshold. The mechanization (rotation gate + DocIntegrity + DOC.9) is doing its job.

### Increment TESTFLAKE.1 — parallel-load test flakiness: deterministic waits + env-gated perf ✅ (2026-07-09)

Killed the ~15 full-parallel-battery flakes the Ricercar FL.10 merge exposed (its GPU/CPU-heavy render tests tipped timing-sensitive tests over their wall-clock thresholds) — all passed in isolation, so environmental, not regressions. **Root fix, not budget-widening** (the failing helpers had already been widened 3→15→30 s and still flaked): **(1) session-prep waits** (`SessionManagerTests.waitForReady`, `ProgressiveReadinessTests.waitUntilNotPreparing`, `SessionLifecycleE2ETests`) await the real background task via the shared `awaitSessionReady` helper — `sessionPreparationTask.value`, race-free under MainActor serialization — instead of racing `Date() < deadline`. Because a *bare* await has no upper bound (it hung 27 min under two concurrent test batteries), the helper **races the task against a 120 s hang-cap** and `Issue.record`s a loud failure on the cap — deterministic on the slip case, fail-fast (not hang) on a genuine deadlock/pathological starvation. `SessionManager.sessionPreparationTask` widened `private`→`private(set)` for the `@testable` read. **(2) Frame-budget tests** (4× Mitosis/Physarum `Holds the 60 fps frame budget`) gate the `median < 16.67 ms` assertion behind `PERF_TESTS=1` (existing convention) — the render + measurement still run every pass (crash/NaN coverage) and print the median, but GPU-timestamp time inflates under concurrent-suite GPU contention, so the numeric budget is enforced only in a deliberate serial perf run. **(3) Soak `cancel()`** asserts a new deterministic `Report.cancelledEarly` flag instead of the wall-clock `actualDuration < 30 s` responsiveness bound. **Done-when: ✅** full parallel `swift test` 1636 tests/218 suites green (was 18 issues); app 388, lint 0, DocIntegrity 12/12; the previously-flaky suites still pass in isolation. No architectural decision (test-infra hygiene) → no D-record.

### Increment QG.4 — Per-paradigm multi-frame harness templates ✅ (2026-07-10, D-182)

Closed the gap that let three Aurora Veil increments ship green while smearing live: only `mv_warp` had a reference multi-frame harness, so `staged` / `ray_march` / `feedback` authors faced a from-scratch build and skipped the checklist's "write the harness FIRST." Now every paradigm has a named, env-gated (`HARNESS_TEMPLATES=1`) reference template driving the same dispatch path the live app uses, on a shared spine. **Shared core** (`HarnessTemplateCore.swift`): env gate, zeroed silence buffers, capture-texture alloc + clear + BGRA/rgba16Float readback, silence FeatureVector, metric hooks (non-degeneracy incl. half-float NaN, luma, dHash/Hamming byte-identical to `ArachneSpiderRenderTests`). **Templates** (each: 60 frames at silence, final-frame dHash golden + paradigm liveness metric, all A/B-validated): `AuroraVeilMVWarpAccumulationTest` re-based on the core (byte-identical assertions); `StagedPathHarnessTemplate` (Arachne, world→composite via production `encodeStage`, slots 6/7; per-stage non-degenerate); `RayMarchPathHarnessTemplate` (Lumen Mosaic, live `RayMarchPipeline.render` G-buffer→lighting→composite→post; luma floor guards BUG-016); `FeedbackPathHarnessTemplate` (Membrane surface-mode `runWarpPass`→composite→swap; accumulator held between the D-037 floor and saturation). **A/B evidence:** staged mis-bind slot 13 → constant-dark composite fails non-constant; ray-march unbind slot-8 follower → black cells (meanLuma 0.47→0.15) fails golden (Hamming 33); feedback skip compose → decays to 0.0 fails the D-037 floor — each reddened then restored. **Scope:** env-gated (skips <1 ms in default `swift test`), surfaced via `closeout_evidence.sh`, production presets only (FA #27). Runtimes ~0.2–0.5 s/template. **Done-when: ✅** shared core + 3 templates green on subjects; AuroraVeil green + unweakened; A/B validated per template; checklist Part 2 obligation 2 + registry §8 + D-182 recorded; DocIntegrityTests green. No preset defects surfaced. D-182 records the decision.

### Increment QG.2 — Render-comparison loop: mechanized side-by-side review ✅ (2026-07-09, D-181)

Mechanizes the mid-session side-by-side check (REVIEW.1 measured 35 % compliance on the prose rule) per the D-161 ratchet. New `Scripts/compare_render.sh <preset> [session-dir]` locates the newest `RENDER_VISUAL=1` frames for a preset, pairs them with every image in `docs/VISUAL_REFERENCES/<preset>/`, and writes ONE composite sheet (reference left, render frames right, filenames burned in); missing references/frames → non-zero exit. Compositor = `Scripts/compare_render_composite.swift` (repo-native `swift` — no ImageMagick on the dev box; mirrors `PresetVisualReviewTests.buildContactSheet`). **Enforcement (rebased onto DOC.9's now-tracked skills):** `docs/PRESET_SESSION_CHECKLIST.md` Part 1 step 5 mandates run→Read→verdict-table before every tuning commit; the **`preset-session` + `closeout` SKILL.md** point at it (`closeout` §3 names the sheet the canonical preset artifact); `Scripts/closeout_evidence.sh` Step 5 surfaces the sheet path. **No auto-scoring** (D-064 — reader is Claude's eyes). **Two fixes the loop surfaced:** (1) the Lumen harness capture rendered black cells — LM.4.7's palette table was never bound (`setPalette`) in `renderPresetVisualReview` → now binds a curated palette, colour restored; (2) the WCAG contrast cert gate scoped to certified presets (Matt's call — unblocks in-dev Ricercar, see BUG-067). **Done-when: ✅** sheet built against Lumen Mosaic (4 refs × 9 colour frames); missing-input paths error cleanly; DocIntegrityTests green incl. `skillIntegrity` (D-181 index row + header + citations resolve). D-181 records the decision.

### Increment QG.1 — Per-preset audio route-coverage gate ✅ (2026-07-09, D-180)

Mechanized per-route firing evidence into a standing test, killing the failure class where a preset route silently never fires while closeouts claim it works (`vocalsPitchConfidence` sat at 0 % for ~5 months). **Manifest:** every preset declares its routes in an `audio_routes` sidecar array (`{route, primitive, kind}`, SHADER_CRAFT §17.1), decoded onto `PresetDescriptor.audioRoutes`; backfilled **145 routes across 13 presets** (12 certified + MitosisGen2) by auditing each `.metal` preamble *and* CPU driver (four parallel consumption audits — real reads only; dead paths like the unwired FerrofluidMesh/Particles excluded). **Gate:** `RouteCoverageTests` replays the three tempo fixtures through the production chain (`FixtureSessionCaptureGenerator` extended to write the features half offline — BeatGridAnalyzer→FFTProcessor→MIRPipeline→MoodClassifier→`csvRow`; captures checked in under `Fixtures/route_coverage/`, FA #27) and asserts per-kind floors (continuous non-constant / accent ≥1 rising crossing / structural ≥1 boundary in the set); un-gated at **0.73 s / 145 routes**. **Certification:** `FidelityRubricTests.certifiedPresetsDeclareAudioRoutes` requires a non-empty manifest; `RouteCoverageTests` independently reddens on any dead route; `AudioRouteSchemaTests` rejects unrecordable primitives. **The one red surfaced** (Nacre `bass_onset_kick`) was investigated to ground — `bassAttRel` demonstrably alive (7 crossings >0.05 on there_there), a threshold-calibration artifact, not a defect — and the accent threshold calibrated to the corpus with the evidence documented (NOT tuned to hide a defect). **Infra (QG.1a–c):** SessionRecorder CSV headers → shared constants (fixed a stale private generator copy predating IFC.4's columns), `SessionColumnSeries` by-name column access, 10 consumed-but-unrecorded FeatureVector primitives appended to the features schema. **Coverage boundary (QG.1.1 follow-up):** the offline StemAnalyzer fixture can't populate VisualizerEngine-CPU-computed StemFeatures fields (Ferrofluid Ocean's spike/aurora routes, Dragon Bloom's tumble clock) → documented, not declared (declaring would false-red a healthy live route); sparse/bright + live-session fixtures are the follow-up. **Done-when: ✅** schema + 145-route backfill; RouteCoverageTests green (0 red after calibration); certification gate + schema gate green; D-180 + SHADER_CRAFT §17.1 + SESSION_REPLAY discipline updated; BUG-067 filed for the pre-existing Ricercar contrast red on main (superseded by the unmerged rework branch — out of QG.1 scope).

### Increment QG.1.2 — Ricercar route-coverage manifest backfill ✅ (2026-07-09)

The BUG-067 spun-off follow-up. Ricercar's FL.13 flow-field (`RicercarFlowGeometry` + `RicercarFlow.metal`) landed on main via the rework merge without an `audio_routes` manifest. Audited both the CPU driver (`advanceEnvelopes`/`makeConfig`) and the shader: reads **11** audio primitives — `bass/mid/trebDev` (energy → flow vigour/turbulence), the 8-way per-colour `max(band-stem dev, family-capture dev)` hybrid (colour brightness + drift), and a computed-but-unwired `beatEnv` (test hook only, **not** routed — the FL.10 beat scatter was removed by FL.13, so no beat route declared). **Declared 7** (152 routes / 14 presets, gate green): `flow_vigour` ← `bass/mid/trebDev`; `{strings,brass,woodwinds,percussion}_ribbon` ← the band-stem `{vocals,bass,other,drums}EnergyDev` halves. **Deferred 4** family-capture `*ActivityDev` reads as **not-yet-armed** (QG.1.1) — structural zero because the offline `FixtureSessionCaptureGenerator` runs `StemAnalyzer` only, never the Layer-5a preview-derived `InstrumentFamilyAnalyzer` PANN sweep (so `*Activity` is 0 regardless of clip); declaring would red the un-gated battery (each ribbon's behaviour is already covered via its band-stem half). **Arm trigger:** port `InstrumentFamilyAnalyzer.analyzeFamilyActivity` into the offline generator (+ an orchestral fixture for amplitude). **FL.14 sequencing:** the unmerged FL.14 branch adds 4 `*AttackRatio` articulation routes (armable on all genres) → declare them in the FL.14 integration commit. **Done-when: ✅** manifest declared; `RouteCoverageTests` + `AudioRouteSchemaTests` green; deferred/FL.14 reads documented in KNOWN_ISSUES §QG.1.1 + registry.

### Increment QG.1.3 — Arm Ricercar's 4 family-capture routes (minimal variant) ✅ (2026-07-09)

The QG.1.2 arm-trigger, minimal variant (offline PANN, no new orchestral fixture). Extended `FixtureSessionCaptureGenerator.generateStemRows` to run `InstrumentFamilyAnalyzer.analyzeFamilyActivity` over each clip alongside `StemAnalyzer`, sampling the Layer-5a series by playback position (mirroring the live `setInstrumentFamilyActivity` / `SessionPreparer+Analysis`) and merging the per-frame `*Activity`/`*ActivityDev` into the stems.csv rows — headless, real-samples-in (FA #27). Declared the 4 family-capture routes in `Ricercar.json` (`{strings,brass,woodwinds,percussion}_ribbon` ← `*ActivityDev`, matching the existing ribbon route names so each colour gets both hybrid halves). Regenerated the 3 route-coverage fixtures (52-col stems.csv). **Audit-before-declare:** confirmed all 4 `*ActivityDev` columns non-constant on all 3 clips before declaring (PANN jitter clears the 1e-5 `continuous` floor even on the non-orchestral clips — no orchestral fixture needed). **Done-when: ✅** generator extended + routes declared; fixtures regenerated; `RouteCoverageTests` **156 routes / 14 presets, 0 red** + `AudioRouteSchemaTests` green; KNOWN_ISSUES §QG.1.1 + registry flipped to resolved. **Process note:** the first regen ran against the wrong checkout (`claude/ricercar-rework`, a pre-QG.1 branch) and silently produced 44-col family-less CSVs — caught by the audit-before-declare column check, not by a green run. Out of scope: the FL.14 `*AttackRatio` articulation routes (separate branch).
...
### BUG-063 — Lumen Mosaic freeze — the slot-8 triple-buffer "fix" was a regression; reverted to known-good, then subsumed-RESOLVED by BUG-064 ✅ (2026-06-26)
...
**AGC3.5 — fast-attack peak floor ✅ RESOLVED 2026-07-09 (`261c65a`; BUG-029 closed).** Matt live M7 "Smooth" (Wake Up + FFO, session `19-33-09Z`) + objective onset worst band **0.875** (pre-fix 4.8). A first "fixed" session showed 23× because it was built without the fix (`ricercar-rework`, not `origin/main`) — caught by replaying its `raw_tap.wav` through the fixed code (1.05) before assuming a fix defect ([[feedback_worktree_changes_reach_build]]). **Root cause:** AGC3.3 seeded the running average from the first audible frame (the tiny leading edge of a percussive attack) and set `agcScale = 0.5/avg` at once, while the slow warmup EMA (0.95 = 5 %/frame) lagged the full transient landing ~0.3–0.5 s later → `f.bass` railed 16–20×, all bands together. **Fix:** a fast-attack peak floor in `BandEnergyProcessor`, confined to a 60-frame onset window (opened at a session-start seed or on exit from a sustained-silence hold) — a frame whose energy exceeds 3.5× the running average snaps the average up toward it so `agcScale` can't lag. **Window-gated** because a first threshold-only cut flattened mid-track snares (caught by `FerrofluidBeatSyncTests` in closeout); the window confines it to cold-start so mid-track transients keep the normal EMA. **Validated on real audio:** `AGC3RealAudioReplayTests` replays the Wake Up `raw_tap.wav` (env `AGC3_REAL_WAV`; copyrighted audio not committed) — cold-start worst band **2.57 → 0.71**, the AGC-scale blowup eliminated. Committed FA #27-aware synthetic reproducer `agc3_rampedOnset_doesNotBlowUp_liveBandProcessor` (a real attack RAMP, not the step-function that missed the bug — fails at 2.14 un-fixed, passes fixed); steady-state byte-identical lock green; 114 DSP/mood/MIR tests green. **Done-when:** ⏳ Matt watches FFO arrive smoothly on a fresh hard-onset session (the non-waivable manual check — [[feedback_dont_waive_manual_gate]]). Then close BUG-029 + release note.
...
### Increment V.12 — ~~Glass Brutalist v2~~ + ~~Kinetic Sculpture v2~~ — FULLY CANCELLED

**Both halves cancelled — the increment is closed with neither preset shipped.**

**Glass Brutalist half — CANCELLED (GBRETIRE.1 / D-186, 2026-07-19).** The concept-viability gate (2026-07-19 roster survey) failed Glass Brutalist on both Gate 1 (D-020 makes the concrete audio-static → no instrument-as-hero role) and Gate 2 (2006-tier); this V.12 rebuild was scoped then abandoned, and the preset was retired. Its curated `docs/VISUAL_REFERENCES/glass_brutalist/` set was deleted with the preset.

**Kinetic Sculpture half — CANCELLED (KSRETIRE.1 / D-188, 2026-07-20).** The KSRB.1 geometry rebuild + Phase RMENV material lift were built to deliver this half, but the chrome-in-a-gallery result read as a "tinker toy" and a psychedelic pivot drifted off-concept; Matt stopped Kinetic Sculpture and retired it. Its `docs/VISUAL_REFERENCES/kinetic_sculpture/` set was deleted with the preset. (Phase RMENV engine work is retained for a future ray-march consumer — see KSRETIRE.1 / D-188.)
...
- Migrate: `VolumetricLithograph.metal` (3 sites), possibly other consumers found at session start (GlassBrutalist retired GBRETIRE.1 / D-186 — no longer a consumer).
...
- Regen `PresetRegressionTests` golden hashes for Volumetric Lithograph + Kinetic Sculpture (Glass Brutalist retired GBRETIRE.1 / D-186).
...
- Manual eyeball: VolumetricLithograph chamber walls + light shafts, KineticSculpture frosted glass (GlassBrutalist retired GBRETIRE.1 / D-186).
...
### PG.4 — Truchet Loom ❌ RETIRED (TLRETIRE.1, 2026-07-21, D-194)
Built PG.4.1 (density-mapping subdivision), PG.4.2 (per-beat flips + hue teams + glow), PG.4.3 (organic hue boundaries + grain) — all green, `certified: false`. **Matt's first live M7 rejected it fundamentally**: the square Truchet lattice + discrete per-beat flips read as a visible grid that jitters on the beat ("looks like a bug"), it matched none of the curated flowing-scallop references (hero `01_macro_labyrinth_floor.jpg`), and it never delivered "psychedelic geometry" ("I don't understand what this is or why this is psychedelic geometry"). The design doc's *mechanic* (blocky Truchet tiling) and its curated *references* (flowing fine-line op-art) were two different aesthetics; the concept was scrapped, not tuned. All preset code/tests/refs/design-doc deleted; the `SpectralHistoryBuffer` `flux_smoothed`/`beat_index` reserved slots (built for it, no other consumer) removed. Preset count 26 → 25. **Lesson (D-194):** reference images are the source of truth for the LOOK; validate a "port algorithm X" instruction actually produces the reference look before building; stills lied about the living result. Second PG-phase preset to die at M7 on a fidelity/concept miss (after Kinetic Sculpture).
...
- **Glass Brutalist** — Gate 3 only; Gate 1 fail on the defining constraint (D-020: concrete is deliberately audio-static "permanent mass" → the hero subject cannot be an instrument), Gate 2 fail (2006-tier; V.12 rebuild scoped + abandoned). **→ RETIRED GBRETIRE.1 / D-186 (2026-07-19).**
