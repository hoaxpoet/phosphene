...
- **CENSUS.3 — pilot run (1,000 tracks) + distribution review ⏳ planned.** Run on the Mac mini (hours). Deliverable: `docs/diagnostics/CENSUS_PILOT_REPORT.md` — disagreement histogram vs the 10 % threshold, mood-feature means/stds vs `mood_scaler.json` (note: single-shot valence/arousal is EMA-attenuated ≈ ×`emaAlpha` from neutral — interpret scale accordingly, or the review may prefer classify-to-convergence), key-confidence distribution, per-genre BPM error census, dual-rate feature deltas. Any retune proposals surface to Matt as product-level options; each lands as its own D-numbered increment.
