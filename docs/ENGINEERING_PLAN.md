...
### NB.2 → NB.9 — planned (budget unblocked)
Per `NIMBUS_PLAN.md`: NB.2 meso/micro detail → NB.3 lighting/internal-glow → NB.4 Breath + silence floor → NB.5 Pulse → NB.6 Mood → NB.7 Page → NB.8 perf tranche → NB.9 certification. (NB.1 budget resolved at full res; the half-res+MetalFX lever is now a headroom reserve for the heavier later increments, validated at NB.8.)
