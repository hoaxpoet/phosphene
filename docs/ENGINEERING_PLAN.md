...
### BUG-063 — Lumen Mosaic ray-march collapses ~1 s after selection (visual freeze) 🔬 OPEN, diagnose phase (2026-06-26)

P1 render-state. Surfaced live after the BUG-062 fix made every preset appear. Lumen Mosaic renders ~0.5–1.4 s then `frame_gpu_ms` collapses 8.2→0.9 ms in one frame and pins low for the rest of the dwell while `renderframe_cpu_ms` is unchanged — the ray-march pass still encodes but per-ray GPU work collapses (rays hit nothing → frozen). The loop, audio, the camera dolly (Lumen speed=0), the frame-budget governor (≤0.75 step mult), the bounded audio modulation, and the recent CLEAN.4.3/4.6 commits are all **ruled out with evidence**; static analysis is exhausted (the per-frame tick fires every frame, engine math NaN-free, scene/camera valid-but-constant) → it's a **runtime** corruption. Headless repro is infeasible (`preset.pipelineState` renders only the static G-buffer; the lit freeze needs the full deferred pipeline). Structural suspect: the slot-8 `patternBuffer` is single-buffered under 3-deep triple-buffering (write-during-read race). **A live `session.log` instrument landed** (`lumenFreezeDiagnosticTick` — slot-8 lights + cam_t/aspect/FOV + finiteness, Lumen-gated, temporary); awaiting Matt's ~10 s Lumen dwell to pin degenerate-uniform vs degenerate-light vs clean-CPU-frozen-GPU, then a fix increment. KNOWN_ISSUES BUG-063.
