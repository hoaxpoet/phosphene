...
### Increment QG.1.3 — Arm Ricercar's 4 family-capture routes (minimal variant) ⏳ code-complete, pending primary-Mac regen (2026-07-09)

The QG.1.2 arm-trigger, minimal variant (offline PANN, no new orchestral fixture). Extended `FixtureSessionCaptureGenerator.generateStemRows` to run `InstrumentFamilyAnalyzer.analyzeFamilyActivity` over each clip alongside `StemAnalyzer`, sampling the Layer-5a series by playback position (mirroring the live `setInstrumentFamilyActivity` / `SessionPreparer+Analysis`) and merging the per-frame `*Activity`/`*ActivityDev` into the stems.csv rows — headless, real-samples-in (FA #27). Declared the 4 family-capture routes in `Ricercar.json` (`{strings,brass,woodwinds,percussion}_ribbon` ← `*ActivityDev`, matching the existing ribbon route names so each colour gets both hybrid halves). `AudioRouteSchemaTests` green; ML + full test target compile clean in the worktree. **Remaining gate (steps 2 + 4):** the 3 route-coverage fixtures must be regenerated on the **primary Mac** (needs the vendored tempo `.m4a` clips + Metal + PANN weights — all absent in a worktree) then `RouteCoverageTests` confirmed green across all 14 presets. **Done-when:** ⏳ generator extended + routes declared + schema green (worktree); ⬜ fixtures regenerated + `RouteCoverageTests` green (primary Mac) → then flip KNOWN_ISSUES §QG.1.1 to RESOLVED. **Guardrail:** if the family DEV columns come out constant on the non-orchestral clips (PANN jitter fails the 1e-5 floor), STOP — the minimal variant is insufficient, an orchestral fixture is required (do NOT tune the floor). Out of scope: the FL.14 `*AttackRatio` articulation routes (separate unmerged branch).
