...
### Phase IFC — Instrument-Family Capture (D-177): IFC.1 model+license ✅, IFC.2 MPSGraph port + numerical parity ✅ (2026-06-30)
...
- **Scope guard:** port + parity only. **No** FeatureVector/StemFeatures wiring (that is IFC.4); the visual engine is source-agnostic so Ricercar's R.3/R.4 visual work is not wasted. **Next = IFC.3** (AudioSet-class → family mapping + temporal smoothing + per-family deviation normalization, D-026 pattern) → IFC.4 (pipeline integration + attribution notice) → IFC.5 (validation) → IFC.6 (Ricercar drive-layer swap).
