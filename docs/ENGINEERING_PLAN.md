...
- **CLEAN.1.2 BUG-031 fix** ✅ (2026-06-13, strategy **A** — Matt-approved via AskUserQuestion). `StemSeparator.separate()` holds `lock` across the full input→predict→output critical section + returns stems by value (`StemSeparationResult.stemWaveforms`); both call sites read the result, not the shared `stemBuffers`. Regression: `StemSeparatorConcurrencyTests` (per-caller-correct stems under concurrency, threshold-free). 3 inline test doubles updated to populate `stemWaveforms`. **Fix-implemented, NOT Resolved — needs Matt's manual concurrency validation** (integrated to `main` + pushed to origin as `da26a3a`, 2026-06-14; KNOWN_ISSUES BUG-031).
- **CLEAN.1.3 BUG-032 fix** ✅ (2026-06-13). Per-instance `streamingSessionGen` guard (twin of `localFileSessionGen`) on the prep-completion closure; `endSession()` cancels the prep task (mirrors `cancel()`); `resumeFailedNetworkTracks` single-flight (awaits the in-flight loop); `startSession` source-mutation moved after the state guard (both variants). Regression: `SessionLifecycleGenerationTests` (end-then-restart guard, rejected-startSession source order) + `SessionRecoverySingleFlightTests` (single-flight). **Fix-implemented, NOT Resolved — needs Matt's manual lifecycle validation** (integrated to `main` + pushed to origin as `da26a3a`, 2026-06-14; KNOWN_ISSUES BUG-032).
...
- **CLEAN.1.7 `[GAP-8]` E2E session-lifecycle integration test** ✅ (2026-06-13). New `Integration/SessionLifecycleE2ETests` drives a real `SessionManager` + `SessionPreparer` (fast fakes) through the full cycle — connect → prepare → ready → play → track-change (cache serves the next track) → end → restart-with-a-different-playlist — plus a cancel-mid-prepare path. The restart leg structurally catches the BUG-032 orphan class (a stale session-A prep must not overwrite session B's plan) as part of the whole cycle, where only per-VM + beat-grid-wiring tests existed before. Closeout ALL GREEN. **This completes the committed June Phase-1 scope (CLEAN.1.1–1.7), now integrated to `main` + pushed to origin as `da26a3a` (2026-06-14, merged through CLEAN.7.9 + DOC.7).** Remaining before the P1 bugs are marked Resolved: Matt's manual validations only (concurrency session, Instruments, device swap).
