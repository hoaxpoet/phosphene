...
- **CLEAN.1.3 BUG-032 fix** ✅ (2026-06-13). Per-instance `streamingSessionGen` guard (twin of `localFileSessionGen`) on the prep-completion closure; `endSession()` cancels the prep task (mirrors `cancel()`); `resumeFailedNetworkTracks` single-flight (awaits the in-flight loop); `startSession` source-mutation moved after the state guard (both variants). Regression: `SessionLifecycleGenerationTests` (end-then-restart guard, rejected-startSession source order) + `SessionRecoverySingleFlightTests` (single-flight). **Fix-implemented, NOT Resolved — needs Matt's manual lifecycle validation + integration to main** (KNOWN_ISSUES BUG-032). Next: CLEAN.1.4 (BUG-033 app-layer) is independent; CLEAN.1.6 (TSan/stress) locks in 1.2/1.3.
