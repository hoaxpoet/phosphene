...
### BUG-063 — Lumen Mosaic freezes for ~10 s on a frozen stem-warmup snapshot ⚠️ CODE-COMPLETE, pending live confirm (2026-06-26)

P1 render-state / pipeline-wiring. Surfaced live after the BUG-062 fix made every preset appear. **Two earlier diagnoses were wrong** and are kept as a record: (1) a slot-8 single-buffer write-during-read race — a real latent bug, triple-buffered in `f5ad0e2`, but NOT the cause (still froze); (2) a "GPU ray-march collapse" reading of the constant ~0.88 ms `frame_gpu_ms` — wrong: 0.88 ms is *normal* for Lumen's audio-static geometry. **Real root cause (session `…T18-37-39Z` + Matt's GPU capture of a correct-but-static mosaic):** the render path feeds the engine the cached 5a StemFeatures snapshot during the ~10 s stem warmup; it is byte-identical every tick. Lumen's D-019 warmup gate picks the stem-direct vs FV mix from `totalStemEnergy ≥ 0.06`, and the loud-but-frozen snapshot clears that threshold → `stemMix → 1` → the four lights lock onto it. **Lumen-unique** because its SDF geometry ignores audio (`(void)f; (void)stems;`), so frozen lights = a fully frozen frame; every other preset masks the warmup with time-based geometry motion. Proof: the logged intensities `[0.25 0.43 0.51 0.36]` are byte-exactly the frozen stem Rel values, frozen for 613 stem-frames (~10 s). **Fix:** `setStemFeatures(_:live:)` tags the cached snapshot not-live; `LumenPatternEngine.tick(…, stemsLive:)` pins `stemMix = 0` (FV fallback, Layer 1 continuous energy, varies from frame 1) until live stems converge. **Done-when:** ✅ all 34 Lumen suites + `test_bug063_*` regressions + app build + lint 0 — **pending Matt's live Lumen dwell from track start with no freeze** ([[feedback-visual-fix-needs-live-m7]]); then remove the `LUMEN_DIAG` instrument. KNOWN_ISSUES BUG-063.
