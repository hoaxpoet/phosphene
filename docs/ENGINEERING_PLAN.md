...
### Increment FLY.13 — Fractal Fly-By steering rebuild: avoid-walls + rate-limit + density governor (BUG-071) 🔨 (2026-07-25)

Rebuilt the corridor steering per Matt's call ("rebuild the steering — avoid walls"). Three track-independent guarantees, all measured against BOTH live sessions before Matt sees them:
- **Avoid-near-walls law** (replaces FLY.12's chase-the-far-opening): measure near-geometry proximity per screen half, ease toward the clearer side — keeps the camera centred in the channel instead of scraping the near wall it was aimed past.
- **Hard rate-limit** (`corridorMaxStep`, offset moves ≤ that per frame): the anti-jerk guarantee. FLY.12's EMA let a big target jump through in one frame → calm on one track, lurching on another. A velocity cap can't lurch and doesn't depend on the track. Measured lateral-jerk median ~1.0 on both sessions (was track-dependent).
- **Density governor** (`presetSteer.z`): the scale-zoom travel periodically plunges into regions dense in every direction (no channel exists to centre in — the real "goes through walls" frames). Measure whole-frame proximity → density → shader eases zoom base toward its open end (`FFB_ZOOM_OPEN`), pulling back to a coarser scale where channels reappear. Eliminated all-wall "mush" frames (0% on both sessions, was periodic). Rate-limited so scale never pops.
- Slowed `FFB_TRAVEL_RATE` 0.45→0.30. Perf p95 4.59 ms (7 ms Tier-2 ceiling). Files: `RayMarchPipeline+Corridor.swift`, `FractalFlyBy.metal`. **Pending live M7 — character shifted toward "flying over ornate terrain" at dense moments; robust but not confirmed to land the vision.**
