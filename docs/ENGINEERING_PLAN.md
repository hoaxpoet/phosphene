...
## Phase MOOD-FLUX — MoodClassifier flux-feature repair (BUG-066) 🔨 (2026-07-08; surfaced by CENSUS.3)

CENSUS.3 found the MoodClassifier's spectral-flux input running **~32× the scale `mood_scaler.json` was fit on** (deploy mean 8.06 vs 0.25; z ≈ +38) → that channel is saturated on every track, and mood is 30 % of the preset scorer. The DEAM training extractor and the runtime mood path compute flux with different STFT params (hop 512 vs 1024, magnitude norm ×2/fftSize vs /√fftSize, 48 kHz vs native); flux is fed raw into the z-score so it is the only exposed feature (bands AGC-normalized, centroid a ratio). Full diagnosis: [`docs/diagnostics/BUG-066-diagnosis.md`](diagnostics/BUG-066-diagnosis.md).

- **MOOD-FLUX.1 — instrument→diagnose ✅ (2026-07-08; commit below).** No fix code. Root cause traced through both feature extractors; BUG-066 filed (P2, ml.mood/calibration) with expected/actual/repro/artifacts/failure-class/verification-criteria. Evidence is the CENSUS.3 report §3 + the code trace. **Done-when: ✅** diagnosis doc + KNOWN_ISSUES entry landed; no production behaviour changed.
- **MOOD-FLUX.2 — fix: regenerate the mood model against runtime features ⏳ scoped, gated on Matt's go (Option 1 from the diagnosis).** Align `tools/train_mood_classifier.py`'s feature extraction to the runtime STFT contract (hop 1024 non-overlapping, `/√fftSize` magnitude norm, native-rate handling); re-extract features and **retrain the MLP + re-fit `mood_scaler.json` on the in-domain corpus** (28k tracks, `CORPUS_ML_OPPORTUNITIES.md` §5 Tier-2) rather than the 1,802 DEAM excerpts — a retrain is required regardless (re-scaling the scaler alone would misalign weights ↔ inputs), so the corpus is near-zero marginal cost and strictly better. Ships as a P2 fix increment: instrument→fix→validate→release-notes. **Verification (from BUG-066):** (1) automated — flux z-score no longer saturated (|z| < ~4 typical), `MoodClassifier` golden regenerated, bands/centroid unchanged; (2) **manual (required)** — Matt reviews before/after **preset picks** on known tracks (mood is taste-bearing; a green parity test is necessary, not sufficient). Likely carries a D-number for the mood-model regeneration.
- **MOOD-FLUX.3 — dependent cleanups ⏳ (fold in with .2 or after).** Correct the DECISIONS sample-rate-delta note (CENSUS.3 measured cross-path centroid skew ~9 %, not the recorded ~22 %); confirm whether the live mood path (if any) shares the same flux input.
...
### Phase MITOSIS — reaction–diffusion cell colony (the beat-synced successor to Filigree) ⏳ (2026-06-29; MITOSIS.0 sketch → MITOSIS.1 graduated; pending live sync listen + M7)
