...
### NB.6 → NB.9 — planned
Per `NIMBUS_PLAN.md`: ~~NB.5 Pulse (CUT — §1.3)~~ → NB.6 Mood (valence → colour cool↔warm, arousal → flow agitation; smoothed ~4 s in state, FA #25) → ~~NB.7 Page (CUT — §1.3)~~ → NB.8 perf tranche (set `complexity_cost.tier2` from the §6.4 profile) → NB.9 certification. (The half-res+MetalFX lever remains a headroom reserve, validated at NB.8.)
