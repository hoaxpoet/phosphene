...
### Increment QG.3 — Audio-visual coupling metric (report-first) ✅ (2026-07-10, D-185)

Built the instrument that puts a number on "the motion tracks the music": `CouplingReportTests` (gated `PHOSPHENE_COUPLING=1`; diagnostic, no content assertions) reconstructs a per-frame FeatureVector + StemFeatures from the checked-in `route_coverage` fixtures, renders each frame headlessly (the `PresetRegressionTests` harness), computes a **visual delta** (mean |luma(i) − luma(i−1)| over the 64×64 reduced render → `coupling/<preset>_<fixture>_visual_delta.csv`), and **cross-correlates** it against the energy envelope (composite + bass/mid/treble, lags 0–500 ms) — reporting peak Pearson r, lag, and sliding-10 s-window stationarity, plus a **negative control** (fixture A energy × fixture B frames, FA #27). **★ Blocking finding (report-first vindicated):** the offline single-fragment/zeroed-state substrate renders **11 of 13 certified presets fully static** (`visual_delta = 0` — slot-6 accumulators / feedback history / mv_warp marks aren't reconstructable from CSV, same reason those presets share dHashes in `PresetRegressionTests`), so the metric is measurable for only **2** presets (Ferrofluid Ocean, Murmuration) and only Murmuration/`love_rehab` (peak r **+0.30**) clears the **+0.08** noise floor. **QG.3.1 (the gate) is DEFERRED** — a gate needs a population; prerequisite is a headless multi-pass/state-reconstructing render. Provisional floor when that lands: peak composite r ≥ 0.15 (≈2× the noise ceiling), a hypothesis to re-derive. **No preset tuned; no KNOWN_ISSUES filed** (every below-floor result is a render-substrate measurement gap, not a dead route). Baseline table + control + recommendation: `docs/diagnostics/QG3_COUPLING_BASELINE.md`. `Scripts/closeout_evidence.sh` Step 7 surfaces the baseline pointer for preset closeouts (attached, never asserted). **Done-when: ✅** visual-delta CSVs written; report prints the full table; baseline doc + D-185 + EP + closeout-script land; full battery + lint green.
