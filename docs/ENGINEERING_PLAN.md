...
- **LFPLAN.1/.2 — re-enable AI planning for local-file playback** ⏳ (2026-06-19; **code-complete, pending Matt's live-playlist validation**). Matt's BUG-042 re-test showed the detector works but presets stayed on one visual — traced to local-file sessions running in **reactive fallback** (`buildPlan()` disabled by a 2026-05-28 BUG-021 revert that cited preset-cycling "one every ~5 s" with a 2-preset catalog). **Root cause of that cycling = THIS area's own junk detector:** `estimatedSectionCount = StructuralAnalyzer boundary count` (SessionPreparer+Analysis:287), so the pre-BUG-042 note-scale detector (~180 boundaries on a 5-min track) made `makeSections` cut ~180 tiny planner segments → a preset every ~1.6 s. **LFPLAN.1:** `PlannerSectionCountScalingTests` runs the real planner and pins the link — sectionCount 9 → 9 segments, 180 → 180 (`44bb745`). All three of the revert's named re-enable preconditions are now met: BUG-042 fixed (180→9), certified catalog **2 → 7**, and the Next-button force-quit was the separately-resolved BUG-059 ABBA deadlock (2026-06-18). **LFPLAN.2:** re-enabled `buildPlan()` on the local-file ready path (`a07b0d1`, reverting the revert) — `buildPlan()` is source-agnostic (reads `currentPlan` + cached profiles, both present on the LF path). Closeout ALL GREEN (engine 1531 / app 388 / lint 0 / doc 11). Runtime behaviour (preset variety, no cycling/lockup) is Matt's live-playlist gate — `VisualizerEngine` isn't app-test-constructible. `RELEASE_NOTES_DEV.md [dev-2026-06-19-171552]`.
