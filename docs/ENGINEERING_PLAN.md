...
**Required verifications** carried forward from CA.6 / CA.7a observations: (1) DASH.7 producer-side traces from `VisualizerEngine.assemblePerfSnapshot` through `BeatCardBuilder` / `StemsCardBuilder` / `PerfCardBuilder` matching the line-anchored consumer chain CA.6 verified; (2) ParticleGeometry siblings-not-subclasses pattern (D-097) — Murmuration is the sole conformer post-Drift Motes retirement (D-102); (3) MeshGenerator vertex-vs-mesh fallback (Apple silicon family gate) + the slot-4 mesh-shader path reuse documented in this increment; (4) RayTracing BVHBuilder + RayIntersector — confirm whether any production preset exercises the path or it's research-stage only.

**Same methodology as CA.1-CA.7a** (audit-only; sub-scope decision unnecessary at 2.2k LoC; visibility grep verification; cited grep for production-orphan claims; per-file verdicts; doc-drift corrections in the same increment).
