...
## Phase TONAL — Continuous harmonic state (Tonal Interval Vector) ✅ (2026-07-08 → 2026-07-10; D-178 Accepted, Matt GO; first preset = Nacre; scoping doc `docs/TONAL_ANALYSIS_SCOPING.md`)
...
whole-file analysis, not the 30 s streaming preview) and wrote 25 per-preset observations. This
phase is the remediation programme. His scope calls, 2026-09-04:
...
Wife, A New Career In A New Town) and tracks 8–11 are the Eno ambient side (Warszawa, Art Decade,
Weeping Wall, Subterraneans) with little or no kit. **Four of eleven tracks are near-beatless**, so
an unknown share of the "sync is weak" observations may be the material rather than the preset —
...
mechanism out. **Hazard, from BUG-097:** this is precisely the shape that bug took, a per-frame
render `dt` used as a musical duration. The rate must be derived from the musical clock; analysis,
render and fixture rates all differ and a steady-rate fixture cannot see the error. **Done-when:**
...
- **BUG-065** (P3, open) — live beat phase drifts monotonically across a track: 0 → 119 ms measured
  on Matt's own Lumen Mosaic session `2026-07-30T15-39-21Z`, 50 % of frames outside the ~60 ms
  perceptual window, BPM rock-constant so it is purely phase. Matt's note at the time: *"feels a
  little laggy."*
- **BUG-028** — meter is assumed simple; Money in 7/4 logged `beatsPerBar = 2`. This is the
  Ferrofluid 4/4 observation, and **Matt confirmed 2026-09-04 that 3/4, 6/8 and 7/8 material is a
  real part of his listening, naming Radiohead as the odd-meter test corpus** — so the meter half
  is in scope rather than theoretical, and Radiohead joins *Low* as programme test material.

Phase TRK is parked (D-206) after two levers were measured and failed; the parking condition is a
changed *grid* premise, and this review does not supply one — it supplies a changed *priority*.
The next identified lever remains DBN. **The actionable consequence regardless of that decision:
these four presets are not tuned while the phase drifts.** Compensating a preset against a moving
target bakes in an offset that has to be unpicked when the grid is fixed. **Done-when:** BUG-065 and
BUG-028 carry the four-preset corroboration and the *Low*/Radiohead material, their priority is
re-assessed against it, and Matt has decided whether to spend on DBN now or leave the four presets
untuned.

**PR.4 — Cytokinesis defects** (independent; runs any time). Two new reports, neither covered by an
existing entry: **cells overlap while dividing**, and the preset **hangs for seconds before
restarting**. The hang goes through the `defect-handling` protocol — repro capture and instrumented
evidence before any fix, since a multi-second stall in a render loop has several possible causes and
none of them is guessable from the symptom. The overlap is a design fix in the division model.
**Done-when:** both are filed with BUG IDs, the hang is reproduced and root-caused before a fix is
written, and the division no longer overlaps in a capture Matt signs off.

**PR.5 — light and colour: the roster does not tone-map.** Matt's Dragon Bloom note is *"washed
out, extreme brightness… reds look gorgeous, would like the same saturated colour across the visible
spectrum"*, clarified 2026-09-04 as **loss of saturation from overexposure, not a request for
spectrum cycling** — the reds are already right, he wants that saturation to survive at the other
hues. Verified in the tree while scoping this phase:

- The drawable is `.bgra8Unorm_srgb` (`MetalContext.swift:55`; the EDR path is a deferred TODO),
  while intermediates are `.rgba16Float`. So every preset's HDR output is **hard-clipped per channel
  at 1.0** on the final write.
- **`Nimbus` is the only preset in the roster that calls a tone-mapping operator at all** — and it
  calls the *legacy* `toneMapACES` in `ShaderUtilities.metal`, not the canonical `tone_map_aces` that
  D-062 superseded it with. Dragon Bloom, Glaze, Fata Morgana and Aurora Veil call none.
- Dragon Bloom's own source comment (`DragonBloom.metal:129`) reads *"HDR (>1) colour is intentional
  — additive glow, tonemapped downstream."* **There is no downstream tone-map.** That is a documented
  assumption that is false in the tree.

Per-channel clipping is precisely the operation that turns a saturated additive glow white: the
strongest channel pins at 1.0, the others catch up, and hue collapses toward the diagonal. So the
leading candidate for Dragon Bloom is a **missing tone-map, not a brightness parameter** — and if it
holds, it is one shared fix (route the four through `Utilities/Color/ToneMapping.metal`, which is
already written, tested and unused by them) plus a per-preset exposure calibration, rather than the
four independent tunings this looked like. **It is a candidate, not a conclusion:** the absence of
the call is verified, that it dominates what Matt saw is not, and per `defect-handling` the A/B comes
before the change. Fata Morgana ("too dark") and Glaze ("quite bright, wants more mixing and
blending") sit on the same axis from opposite ends; Aurora Veil's fleeting purple is a palette
residency question that may be independent. **Done-when:** an A/B of Dragon Bloom with and without a
tone-map operator, on a full-track *Low* capture rather than a still, establishes whether clipping is
...
preset exhausts its visual vocabulary before the track does — and note that *Low* is the local-file
path, so these are full-length tracks rather than 30 s previews, which is the condition under which
a vocabulary runs out. It is last because it is the only cluster with **no shared mechanism**: each
one is its own design problem at roughly the cost of a preset uplift.

**PR.8 — opener selection** (separate track, orchestrator not presets). *"Why is Witchlight first in
the sequence vs. last?"* — that is `SessionPlanner` ordering. Witchlight's sidecar declares
...
*"fine enough."* **Spectral Cartograph** — leaves rotation via PR.0; the cleanup Matt mentioned is
...
### Increment LFSTEM.2 — retire live separation on the local path ✅ (2026-08-27, payoff measured and NOT where it was predicted)

**Done-when:** a track with a series logs zero `STEM_SEPARATION` lines, a track without one still logs them at the old cadence, and a 4K before/after shows the delta. `separationSupersededBySeries()` skips the 142 ms MPSGraph dispatch when a series is installed — gated on the SERIES, never on the source, so a cache miss or failed analysis keeps the live path intact. The three consumers were checked before removing anything, per the spec: the per-frame analyser was already standing down (LFSTEM.1c); `ChainAnalyzer` has zero stem references and `SignalHealthMonitor`'s six are all sample-rate comments over raw tap samples, so no health surface can read "no separations" as a fault (the BUG-070 shape, checked and not reachable); and **the `stems/` WAV dump is lost on tracks with a series** — no substitute means the same thing, so the session log says so explicitly rather than leaving it to be discovered. Suppressions are counted into `GPU_PRESSURE` as `stem_suppressed` so the saving is measured, not asserted.

**The before/after, session `2026-08-27T19-51-09Z` against `2026-08-27T18-17-50Z`** — same file, same preset (Skein), same 3840×2160, steady window after the first 20 s. Suppression works: **zero `STEM_SEPARATION` lines**, `stem_suppressed` 1 → 56 over 110 s (one per 2 s, exactly the retired period), `ml_last=none`, no `stems/` directory. ⚠ **The frame-time payoff this increment was justified by did not appear.** `frame_gpu_ms` p50 **12.95 → 13.96 ms** and `frame_cpu_ms` p50 **26.93 → 28.11 ms** — the wrong direction, and inside session-to-session variance (p90 15.04 → 15.10, p99 16.23 → 15.98). The spec's claim that the 142 ms job "lands directly on the 4K frame budget that BUG-100 and BUG-106 have both been circling" is **recorded as wrong for the median frame**: MPSGraph dispatches on its own queue, and after BUG-110 left the GPU at ~13 ms of a 16.7 ms budget it interleaved rather than displacing render work. Where the removal *does* show is the dimension BUG-100 actually named — **the GPU working set is flat at 474 MB (3.9 %) against a climbing 549 → 586 MB before** — and, suggestively only, hitches over 33 ms at 0.17/s → 0.08/s (15 against 8 frames; too few to call). The increment is kept for the working set, for the dead compute, and for removing BUG-086's latency class by mechanism — **not** for a frame-time saving it does not deliver here. One caveat against over-reading the negative: this is one pair on a preset with headroom, and a genuinely GPU-bound preset at 4K could still show the delta.

### Increment LFSTEM.1e — the series is sampled per render frame ✅ (2026-08-27)

**Done-when:** stem motion is limited by the series' grid, not by the analysis loop. `publishStemSeriesFrame` runs once per RENDER frame from a dedicated `RenderPipeline.perFrameStemPublish` hook — **12.8 Hz → the series' 43 Hz**. It fires BEFORE the frame snapshots `latestStemFeatures` (that snapshot serves the particles update, the preset tick and the draw, so publishing after it lands a frame late; the wiring test asserts the ordering, not just the call). Separate hook from `meshPresetTick`, which presets own — Skein sets it for its painter clock. The analysis frame now publishes only the playback clock the render frame samples with, so the smoother is single-threaded behind `stemSeriesLock`, and `applyStemSeriesFrame` is deleted rather than left beside its replacement. **Owed: Matt's live look** — every change in this chain moved what stem-driven presets see and no test settles that.
...
### Phase FBS — Ferrofluid Beat Sync ✅ (2026-06-09 → 06-11; Stages 0–S6 all built, S5c/S5d live-validated "Looks great"; kickoff `docs/prompts/FFO_BEAT_SYNC_KICKOFF.md`)
### Increment AGC2 — BUG-027: per-band EMA deviation pivot + cold-start warmup ✅ (2026-06-05 → 06, D-146)
### Increment AGC3 — BUG-029: AGC `f.bass` cold-start spike (continuous-energy presets pop-and-drop at track onset) ✅ RESOLVED 2026-07-09 (AGC3.5, `261c65a`; both halves of the non-waivable gate met, confirmed live)
...
<details>
<summary><b>Superseded 2026-05-06 ordering</b> (retained for the reasoning trail — not the active queue)</summary>

> The 2026-05-06 ordering below (QR → DSP → V → MD → SB) is **superseded**; QR.6 decomposition is folded into CLEAN Phase 8. The phase sections are retained as history.
...
- `RENDER_VISUAL=1 swift test --package-path UzumeEngine --filter PresetVisualReview` writes Arachne3D contact-sheet PNGs to `/tmp/uzume_visual/<ISO8601>/`.
...
**FTR.2 — Routing rebuild.** ✅ (2026-08-03) Five routes, five distinct primitives; rubric **2/4 → 3/4, L2 green**; `fract(t)` clock removed; `tip_shimmer` deleted as measurably dead. **One primitive changed from the D-212 table on measurement:** canopy reach uses `bass_dev` through a soft knee `bd/(bd+0.12)` rather than a linear gain, because `bass_dev` measures zero on 66–89 % of frames (it is an upward-transient signal against a running EMA, not the fixed-0.5-pivot clamp D-212 assumed) — so the canopy reads as a per-transient bloom at 1.91/s and the section-scale arc is carried by the new `arousal` route instead. Also built `FractalTreeMeshRenderTest`, the first visual-review harness that works for mesh presets at all. Measured swings, before → after, in the closeout and in `docs/VISUAL_REFERENCES/fractal_tree/README.md`. Original scope: canopy reach ← `bass_dev`; branch spread ← `spectral_flux`; leaf hue ← `tonal_phase_fifths` (removing the `fract(t * 0.006)` clock term); global brightness ← `arousal`. One primitive per layer (FA #67). Every coefficient sized against the primitive's **measured p05→p95 span**, not a notional 0→1. Rename the shader's `features` parameter to `f` so the M4/L2 substring heuristic can see the deviation fields.

**QG.5 `response` floors are NOT attached here — Fractal Tree cannot reach the gate (corrected FTR.2, 2026-08-03).** The gate exists and works: `AudioRoute.Response`, `AudioResponseMetrics`, and `ResponseBandTests` all landed at QG.5 (`251f1332`). But joining it requires a Swift-side runtime conforming to `AudioResponseMetrics` that the fixture replay can drive and read metrics from — Witchlight has `WitchlightPath`. **Fractal Tree has no Swift-side code at all**: branch count, trunk length, spread and hue are all computed in the object/mesh shader on the GPU, and the preset is only `FractalTree.metal` + `FractalTree.json`. Declaring a band with no way to measure it is the failure `ResponseBandTests.makeRuntime` explicitly rejects. Closing this needs either a CPU mirror of the shader's parameter math or GPU readback of the mesh output — **its own infrastructure increment**, and one that would serve every mesh preset, not just this one. Until then FTR.2 discharges the obligation the pre-QG.5 way: a measured swing table per route, against each primitive's measured p05→p95 span, in the closeout.
...
**DYN.1 — Pre-normalisation spectral density on `FeatureVector`.** ✅ (2026-08-04) `SpectralAnalyzer.Result.density`/`.smoothedDensity` → `FeatureVector.spectralDensity`/`…Slow` (MSL floats 49–50), recorded to `features.csv`, routable via `AudioRoutePrimitives`. Fractal Tree's growth reads it as a LIFT on the arousal envelope, which is what lets a section step register where arousal has saturated. **Two traps found and recorded:** `FeatureVector` is declared in **two** places — `Common.metal` for engine shaders AND a hand-maintained copy in `PresetLoader+Preamble.swift` for runtime-compiled presets; updating only the first drops every preset silently (caught by the FA #44 count gate). And the `route_coverage` fixtures predate the field, so the route is deliberately **not** declared in the sidecar yet — declaring it reds QG.1 for a fixture reason rather than a route defect. **Owed:** re-capture the three fixtures, then declare `growth_density_lift`.

**FTR.3f — the DYN.1 integration was rejected live and corrected (2026-08-04).** Matt: *"too active now and the trunk of the tree is moving up and down constantly. not an improvement."* Two causes, both mine:
1. The density lift was folded into `reach`, which trunk length and thickness read. Raw density turns **5.59/s** — the same order as the `bass_rel` (5.88/s) whose bounce this preset had already been fixed for once. Smoothing cannot rescue a ratio of two moving averages: even widened to τ6 s/τ45 s the ratio still turns ~1.1/s against arousal's 0.5. **The rule now stated in the shader header: continuous geometry reads only slow signals; fast signals go to the quantised branch count.**
2. The tips' "asymmetric decay" was not a decay. `max(rise, fall * 0.62)` cannot know a branch HAS fired in a stateless shader, so the second term was simply a permanently lower threshold that lit far more of the canopy — the "too active" half. Reverted to the single-threshold form; a real release needs per-branch state (MEL/FTR.4 territory).

**Also fixed: a CSV column-shift bug DYN.1 introduced.** `primitiveCols` carried the terminating newline and `densityCols` was appended after it, so every frame's density values were written at the head of the FOLLOWING row — shifting every column by two for a whole session, with a plausible field count per line and no error. Found only when analysis produced a negative energy ratio. Now gated by `SessionRecorderCSVAlignmentTests`.
...
**But the scaler is not stale, and that is the finding.** Read back the annotated training set (`~/uzume_features_annotated.csv`, n = 818): flux mean **0.2516**, std **0.2044** — the shipped constants *exactly*. Nothing drifted. What the numbers describe is **training-set coverage**: the annotated set's maximum flux is 1.0167, and **15.3 % of the corpus exceeds that maximum entirely** — the model has never seen material that dense on feature [7]. 22.2 % of the corpus is beyond |z| > 3.
...
| | before | after |
|---|---|---|
| trunk turns/s | 1.64 | **0.52** |
| trunk span (min→max) | 0.348 | **0.344** |
| hold engaged | — | 95 % of frames |

**Four fallbacks, one mechanism.** The hold only freezes while (a) a bar clock has been seen — `barPhase01` is identically 0 without a `BeatGrid`, which is what keeps the preset off `BeatPredictor`'s raw-onset phase in reactive/streaming mode; (b) the last 8 beat intervals have sd ≤ 20 % of the mean, which excludes **beat-irregular tracks (D-154) by measurement rather than by a descriptor exclusion**, so Fractal Tree stays eligible for those tracks and simply behaves as it did before; (c) a beat arrived within 2× that mean, so a stalled `beatPhase01` (WL.2 measured exactly that) reverts to tracking instead of **freezing the tree**. Otherwise it tracks the live vector and the shader expression is bit-identical to the continuous one — which is also the cold-start answer: 8 intervals ≈ 4 s, so nothing steps from frame 1. Wrong-phase cold start is not solved and is not solvable (Cold-Start Phase Contract); a hold has no attack, so a phase error lands the change early or late rather than firing a wrong-phase accent. One test per path in `BeatHoldTests`.

**Full-track regression unchanged**, as it must be — `reach`'s arithmetic did not move. `TrunkTrajectoryReportTests`: Cherub Rock ratio 0.00…2.00, trunk 0.00…1.00, motion **0.0292/s**, time-to-0.15 24.4 s; Hummer 0.0172/s, 23.4 s. Identical to FTR.9 to four decimals.

**Motion gate (pre-M7), 11.6 s at the analysis rate, hold engaged on 100 % of frames:** 0 spike frames / 139, mean 2.26 against the continuous build's 2.18 — no jitter, pop or strobe introduced. Read as a sequence, the skeleton sits at one size for a stretch and then changes size while the fine tips come and go and the hue swings; the graphic register is intact. **What the sheet cannot settle is whether a step at the beat timescale reads as rhythmic or as stuttering — 8 samples 1.4 s apart cannot show within-beat stillness. That is the M7 question.**
...
**★★★ THERE IS NO 4K SHADER-COST PROBLEM, AND THE INSTRUMENT WAS THE PROBLEM.**
`MultiPassRenderHarness` reads every rendered frame back to the CPU — ~8 MB at 1080p, **~33 MB at
3840×2160** — and production never does. That cost follows PIXELS, not what a preset draws, so it
lands on the 4K column hardest and a resolution sweep including it measures the harness as much as
the roster. Measured, the readback costs a median **11.4 ms/frame at 4K** against 3.0 ms at 1080p.
...
**BUG-099 is resolved by measurement** and needed no product decision — neither the star-layer cut
nor the half-res extension it proposed; both were sized against a number more than 3× too high.
**Confirmed live** by the first instrumented session: Witchlight **6.43 ms at 4K, 49 fps, flat over
60 s**, against 273.88 ms before BUG-098 — harness and live agree within **13 %**. Fourth time in one
day a performance metric did not mean what its name suggested; the specific error was reusing a
harness built to COMPARE presets to answer an ABSOLUTE question about one.

**★★★ AND HALF OF THIS INCREMENT WAS BUILT TWICE, IN PARALLEL, ON THE SAME DAY.** I implemented a
per-preset render cap for Volumetric Lithograph (`max_render_megapixels`, a new descriptor field,
scaling the G-buffer). While it was in review a parallel session shipped the same feature to `main` —
also numbered PERF.11 — as `rayMarchRenderScale`, and **theirs is better informed on the point that
matters: `PresetDescriptor.effectiveRenderScale` had existed since MFX.1 (2026-07-23)**, gated behind
`usesMetalFXTemporal`. They found it and separated it; I added a parallel field without discovering
it, having grepped the DIRECT path's scale and stopped there.

**Matt's call: strip mine, keep theirs.** Deleted from this increment: the `maxRenderMegapixels`
descriptor field and sidecar key, `RenderPipeline.presetRenderScale`, and my ray-march scaling. The
shipped mechanism is theirs (VL at `render_scale: 0.5`, G-buffer **and** lighting scaled,
post-process at full resolution). ★ The one thing worth carrying from mine: it also scaled only the
G-buffer and reached the same conclusion independently — the composite pass samples through a linear
sampler and upscales for free, so no MetalFX and no extra pass is needed. Two sessions converging on
that is reassurance about the mechanism, not about the process.

**RE-MEASURED, which is what my half of the work is actually for.** Their commit estimated the
shipped scale *"should land near the 16.7 ms budget"*. With the readback removed the harness says
better than that:

| VL at 1920×1080 | ms |
|---|---|
| before (full scale, readback off) | 31.9 |
...
- **`absoluteCeilingMs = 60 ms`**, always on: the "arriving already broken" net this file's header
  had described since PERF.4 while **the constant appeared exactly once, in that comment — never
  implemented.** VL sat at ~31 fps at 1080p reading green at 5.9× the median.
- **`FRAME_BUDGET_STRICT=1`**, the real 60 fps check, isolation-only, now passing **20/20 at 1080p
...
**BUG-100 remains the live question.** A 24-frame cost measurement cannot see a 70-second drift.
The instrumented session reports thermal **`nominal` with the degradation NOT reproducing** —
Witchlight flat across 60 s at 4K, in a window comparable to the one where Stave went 2.9 → 11.7 ms.
That supports neither hypothesis and refutes neither; one non-reproduction on a different preset mix
is not a falsification. The instrument reports cleanly, so the next *degrading* session carries it.

⚠ **FIFTH ID COLLISION TODAY AND THE FIRST DUPLICATED BUILD.** Both increments were numbered
PERF.11; this one is renumbered PERF.12. The cost stopped being cosmetic — two sessions each spent an
increment on one feature, and the duplicate was only caught because the merge conflicted. Numbering
...
across four runs and the minimum is the honest figure, per this suite's own min-of-passes reasoning.
...
### Increment AV.3.x — Diffuse-glow reference re-curation + cert ❌ SUPERSEDED (2026-07-19 by AV.7 / D-185; marked at RECON.2, 2026-08-03)

**Why superseded.** This increment's goal — get Aurora Veil certified — was reached by a different route. **AV.7 certified Aurora Veil on 2026-07-19 (D-185, Matt M7 sign-off)** by *porting nimitz's Auroras verbatim* rather than by re-curating references around the existing diffuse-glow preset and certifying against the new set. `AuroraVeil.json` carries `certified: true` today, so this row's terminal step ("on Matt's yes, flip `certified: true`") is already done by other means. The two-preset split that motivated it (diffuse-glow here, active-curtain in Phase AC) was overtaken: the ported preset is the shipped Aurora Veil. **Do not execute the scope below** — re-curating the reference set now would re-anchor a certified preset against references it was not built from. Phase AC's dependency on this row is void; if Aurora Curtain is ever scoped it stands on its own. *(This row read "⏳ Planned" for two weeks after AV.7 certified.)*

**Original scope, retained for the reasoning trail.** Matt curates 4–5 diffuse-glow / pulsating-patch aurora reference images replacing the current curtain-form set in `docs/VISUAL_REFERENCES/aurora_veil/`. Update `AURORA_VEIL_README.md` annotations + mandatory-traits checklist + 9-Q rubric variant (some Qs may not apply to diffuse-glow). Update `AURORA_VEIL_DESIGN.md §5` to reframe design intent as diffuse-glow aurora. Re-run `PresetSessionReplay` against the new reference set; calibration should produce `withinFamily` verdicts for the Qs that apply. M7 review against new set. On Matt's "yes," flip `AuroraVeil.json certified: true`.
...
### Phase AC — Aurora Curtain (planned; its "post AV.3.x" dependency is void — see AV.3.x above)
...
The Phase V uplift trajectory left several presets at the post-V.6 cert baseline without per-preset fidelity work tailored to their visual contracts. The shipped catalog has **26 production presets** (recounted at RECON.2, 2026-08-03; the line previously said 15, a post-D-102 figure); the Phase V plan called for 12 fidelity-uplifted presets. Several catalog members are *certifiable* but have *not* been through a per-preset uplift session against curated references — Gossamer is the named example, but **Membrane / Starburst (post-SB) / Nebula / Plasma / Waveform / Fractal Tree / Arachne / Ricercar / Witchlight / Meniscus** are worth review (some are lightweight rubric and need only validation, others are full rubric and may need work). *(The candidate list previously named Glass Brutalist and Kinetic Sculpture — both retired, D-186 / D-188 — and Volumetric Lithograph and Spectral Cartograph; VL certified at VL.CERT 2026-07-26 and Spectral Cartograph is a diagnostic, not a roster preset. Fractal Tree is live work under Phase FTR, so coordinate before scheduling it here.)*
...
- **DBN.3 — wire as `BeatGridResolver` alternative, benchmark A/B.** ⏸ **Block condition SATISFIED but the decision was never made — awaiting Matt (flagged RECON.2, 2026-08-03).** The row below still reads "blocked on FT.1"; **FT.1 landed 2026-07-31 and came back NEGATIVE** — lifting the `tMax` clamp recovered no odd meter and regressed bohemian_rhapsody. So the gate this row waits on has been answered, and answered unfavourably, which means DBN.3 cannot simply "open" as written: its own gate (Take Five 5/4, Money 7/4, Solsbury Hill 7/4 decoding correctly) is exactly what full-track evidence just failed to deliver. **The successor state is a decision, not an increment** — the live options are FT.2 (re-scoped; the case now rests on category 3 alone), FT.3 / FT.3.1 (specced, awaiting Matt), or accepting the decoder's decline-rather-than-guess behaviour as the shipped outcome and wiring DBN.3 with a beats-only gate. Do not open DBN.3 against the criteria below without re-deciding which of those it serves. *(Original block text retained:)* every measurement behind the decoder's odd-meter failure came from a 30 s window — money's meter was decided from 51 beats of a 380 s track — so FT.1 must lift the `tMax` clamp and re-test before DBN.3 opens or any model-family question is raised. §5.1's bias is fixed, so a wiring A/B would measure the decoder rather than the bias — wiring in a decoder with a known meter-selection bias would A/B the bias, not the decoder. Gate on Take Five 5/4, Money 7/4, Solsbury Hill 7/4 — all currently decoded as 4 — plus no-regression on suite 1 and the three passing suite-2 AMLt scores.
