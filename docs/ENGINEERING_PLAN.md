...
**Carry-forward note re: fogNear default.** `SceneUniforms()` initializer hard-codes `fogNear = 20.0` (sceneParamsB.x) and there is no JSON-level override. For Ferrofluid Ocean's camera (surface in 4–14 m depth range) this puts the visible surface entirely BEFORE the fog band, so the D-022 mood-tinted `fogColor = sky * lightColor.rgb` path contributes nothing in production. The mood-tint gate explicitly sets `sceneParamsB.x = 0` in the test harness to isolate the matID==3 branch's D-022 propagation. In production this preset will rely on IBL ambient (also matID==3-tinted) to carry mood for surface pixels < 20 m; a future engine-level fix should add `scene_fog_near` JSON support so the fog band can match each preset's camera framing without touching `SceneUniforms` defaults that other ray-march presets depend on.
