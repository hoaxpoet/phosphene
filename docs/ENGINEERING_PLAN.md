...
### Ricercar.3 — Section-mark engine on Skein (planned, post-D-176)
Rebuild `Ricercar.metal`/`.json` on Skein's marks-on-top mv_warp stack: a **light** canvas + a composing "painter" laying elegant, ragged-edged painterly strokes (Skein's proven mark vocabulary — colour + viscosity/texture + weight + wet/dry), but **graceful/composed** (not Pollock drip). One section first, hand-fed, to prove the elegant-painterly mark reads on a light Fantasia canvas. Drops the substrate spike's flow-field + agent-bridge entirely. **No engine touch** (Skein's per-prefix `ricercar_*` overrides + marks-on-top, all preset-side).
