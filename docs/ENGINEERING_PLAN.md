...
### NB.9 — certification — automated prep ✅ (2026-06-05); **Matt M7 + cert flip OPEN**
...
**OPEN gate (non-bypassable):** Matt's live M7 on ≥ 5 tracks + a local file. On sign-off, the cert flip is: `Nimbus.json` `certified` false→true; add `"Nimbus"` to `certifiedPresets` in `FidelityRubricTests` (Nimbus's heuristic `meetsAutomatedGate` is false-by-construction — volumetric, no `mat_*`/`fbm` calls — same as Lumen Mosaic / Ferrofluid / Fata Morgana, so the M7 reference review is the load-bearing gate per SHADER_CRAFT §12.1); mark NB.9 ✅ here + in `NIMBUS_DESIGN §7` + `NIMBUS_PLAN`; note cert in `RENDER_CAPABILITY_REGISTRY`. **No push without Matt's "yes, push."**
