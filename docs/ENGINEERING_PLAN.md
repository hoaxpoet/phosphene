...
## Phase GATE — 2026-08-05 code-audit remediation

- **GATE.1 — restore release-configuration engine testing ✅ (2026-08-05; BUG-079).**
  Done-when met: the deterministic Arachne activation fixture is available to release
  test builds without becoming production API; the release-disabled build-state fixture
  setup uses that same seam; the placeholder `#expect(true)` test is removed; all affected
  Arachne suites pass in debug and release; and the bounded full release suite compiles,
  executes, and terminates. Final evidence: 38 Arachne tests / 6 suites green in each
  configuration; full release 1,784 tests / 266 suites green in 92.594 s. No renderer
  capability changed, so the renderer capability registry is intentionally unchanged.
- **GATE.2 — suite non-termination remediation.** Not addressed in GATE.1. The known
  non-termination did not recur during GATE.1's bounded release run; that absence is not
  a diagnosis or closure.
...
5. **Open defects worth scheduling** — **BUG-051** (m3u traversal guard), **BUG-060** (reopened 2026-08-03 — needs a captured stack, not another monitoring window). **BUG-079 closed in GATE.1 on 2026-08-05.** Full list: [`docs/QUALITY/KNOWN_ISSUES.md`](QUALITY/KNOWN_ISSUES.md) §Open Index.
...
- **DBN.2 — `BeatActivationDecoder` implementation ✅ (2026-07-30).** Done-when met: 14-case unit suite green on synthetic activations; budget test in `DSPPerformanceTests`. Pure Swift, offline-path only, **not yet wired into `BeatGridResolver`** (that is DBN.3). D-207's decline path ships rather than deferring: the result is "a meter **or** no confident bar", `beatsPerBar` is `Optional`, and declining withholds downbeats while keeping beats.
  ...
  - *Performance:* 17,067 → **1,350 ms** for a 30 s window (debug) via precomputed observation classes, per-frame terms, a flattened transition table and unsafe buffers. **GATE.1 / BUG-079 restored release-suite execution on 2026-08-05, but this test still asserts only its original regression ceiling, not the plan's 50 ms release gate.** Re-pointing that DBN measurement is separate from GATE.1's test-isolation fix.
