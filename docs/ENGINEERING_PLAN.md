...
- **DBN.3 — wire as `BeatGridResolver` alternative, benchmark A/B.** Next. §5.1's bias is fixed, so a wiring A/B would now measure the decoder rather than the bias — wiring in a decoder with a known meter-selection bias would A/B the bias, not the decoder. Gate on Take Five 5/4, Money 7/4, Solsbury Hill 7/4 — all currently decoded as 4 — plus no-regression on suite 1 and the three passing suite-2 AMLt scores.
