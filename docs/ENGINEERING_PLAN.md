...
- **MOOD-FLUX.3 — mechanize the bug class: unify the offline + live FFT-magnitude path ⏳ planned.** Collapse the offline `analyzeMIR.computeFFTMagnitudes`/`FFTContext` reimplementation and the live `FFTProcessor` into one source of truth (recommended: a shared allocation-free magnitude kernel in `Audio`), plus a divergence-guard test that fails if they ever fork again — so BUG-066 cannot recur. Behaviour-preserving (MOOD-FLUX.2 already aligned the values). Also folds in the dependent cleanups: correct the DECISIONS sample-rate-delta note (CENSUS.3 measured cross-path centroid skew ~9 %, not the recorded ~22 %); the live mood path was confirmed unaffected (it already used the correct `FFTProcessor` scale). Execution spec: `prompts/MOOD-FLUX.3-prompt.md`.
