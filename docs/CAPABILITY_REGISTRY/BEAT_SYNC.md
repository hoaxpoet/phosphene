...
**Captures consulted:** `~/Documents/uzume_sessions/2026-05-22T16-57-36Z/` (CS.1 baseline, "cap1"); `2026-05-22T19-03-59Z/` (CS.1.y.2 onset-fix, "cap2"); `2026-05-23T02-39-54Z/` (CS.1.y.2-redo round 2, "cap3"); `2026-05-24T15-07-31Z/` (M7, "cap4"). All four captures play the same 10-track playlist with identical Spotify previews.
...
- The Cold-Start Phase Contract (addendum below; operative summary in the `preset-session` skill since DOC.4) documents the achievable contract (continuous-energy from frame 1; BPM-prior + confidence-gated accents; graceful degradation on hard tracks).
...
## Addendum — Cold-Start Phase Contract, full text (moved from CLAUDE.md at DOC.4, 2026-06-11)

> CLAUDE.md §Cold-Start Phase Contract was condensed at the DOC.4 pruning pass (the operative contract + authoring rules stay there; this is the full pre-trim text, preserved verbatim for the historical record — incl. the revert narrative, the per-bullet practice notes, and the failure-mode characterizations). Links inside were written relative to the repo root.
