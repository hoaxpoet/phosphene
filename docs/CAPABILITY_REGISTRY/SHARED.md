...
|---|---|---|
| production-active | 22 files; 27 top-level types; ~85 public/internal methods | The dominant verdict — every load-bearing type is consumed across multiple modules. |
| production-orphan | 0 files; 2 protocols + 3 accessors | Two protocols (SpectralHistoryPublishing, StemSampleBuffering) + three accessors (`.retryStatus`, `.isConditionBound`, `Smoother.step`). Cited grep included per finding. |
