...
| **CA.5-FU-2** | Decide product intent for `LiveAdaptationToastBridge` engine-event observation. The file's docstring (lines 1–14) claims two observation sources (user actions + engine events), but only user actions wire through `emitAck`. Two options: (a) **wire engine adaptation events through `emitAck`** at `VisualizerEngine+Orchestrator.swift:189-197` (`for event in adaptation.events { if event.kind != .noAdaptation { toastBridge?.emitAck(event.message) } }`) — gates on the existing `phosphene.settings.visuals.showLiveAdaptationToasts` UserDefaults so users can opt out; OR (b) **rewrite the docstring** to drop the "engine events" line and clarify that `emitAck` is for user-action acks only. Product call: do we want toast feedback for orchestrator-driven adaptations the user didn't trigger? | The docstring matches the code, either by wiring engine events (option a) or by rewriting the docstring (option b). Engine + app builds clean; SwiftLint zero violations. If option a, a new test asserts the gate behaviour (UserDefaults flag respected). | <1 | Ready now |
...
| **CA.5-FU-4 (== CA.6)** | Audit `PhospheneApp/Views/` (47 files) + `PhospheneApp/ViewModels/` (12 files). Same methodology as CA.5: visibility cross-check, production-orphan grep verification, doc-drift triangulation against `ARCHITECTURE.md §UI Layer` + `§Module Map ViewModels/` + the `Views/Dashboard/` sub-block. Particular attention: (a) `PlaybackChromeViewModel` consumes the BUG-015 `currentTrackIndex` publisher and the `livePlannedSession` publisher — verify the subscription chains are race-free; (b) `DashboardOverlayViewModel` consumes the DASH.7 snapshot publisher with `.throttle(for: .milliseconds(33))`; (c) the U.x view tree (Idle / Connecting / Preparation / Ready / Playback / Ended / Onboarding / Settings) routes from `ContentView.sessionStateBody`; (d) `MetalView.swift` is the NSViewRepresentable wrapper at `PhospheneApp/Views/MetalView.swift`. **Pre-existing flakes** under U.10 / U.11 timing margins land in App tests — verify the timing margins per CLAUDE.md U.11. | Audit document `docs/CAPABILITY_REGISTRY/APP_VIEWS.md` (or `APP.md` appendix — auditor's choice) published; every public capability has a verdict; every Explore-agent-claimed public symbol cross-checked against visibility grep; every non-`production-active` finding either ships a doc-fix in CA.6 or is registered as a `CA.6-FU-N` follow-up; ARCHITECTURE.md `§Module Map Views/` + `§Module Map ViewModels/` corrected. | 1–2 | Ready now |
