...
### Services/ (28 files, ~3,700 LoC)

#### SettingsStore.swift (158 lines) — `production-active`

The **single** app-wide `@MainActor final class SettingsStore: ObservableObject` per D-091 / Failed Approach #55. 9 `@Published` fields. Generic encode / decode helpers for JSON-codec Codable values. `Keys` enum carries all UserDefaults keys.
...
UX_SPEC §9.4 audio-signal silence-error routing to `ToastManager` with condition-ID semantics. `silenceToastThresholdSeconds: 15`. Replaces the older `SilenceToastBridge` which fired at 30 s without condition ID.

| Capability | Verdict | Consumers | Notes |
|---|---|---|---|
| `PlaybackErrorBridge` | `production-active` | `PlaybackView` | UX_SPEC §9.4 |
| Constant `silenceToastThresholdSeconds` | `production-active` | Internal | UX_SPEC §9.4 |
...
App-layer settings value-type enums.

| Capability | Verdict | Consumers | Notes |
|---|---|---|---|
| `DeviceTierOverride`, `ReducedMotionPreference`, `SessionRetentionPolicy` | `production-active` | `SettingsStore`, view models | — |
