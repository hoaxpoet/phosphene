...
**CA.1-FU-1 closeout note (2026-05-21).** The recommended sequencing above was overtaken by events. BUG-015's fix (commit `b3f1efd9`, 2026-05-21) shipped *before* CA.1-FU-1's option-(a) gate, and the BUG-015 author chose option (b) for the prediction source: the wire at `PhospheneApp/VisualizerEngine+Orchestrator.swift:287-311` reads `mir.latestStructuralPrediction` directly. Per the §Verdict bullet at line 463 above (*"option-(b) work folds into BUG-015's fix … No separate CA.1-FU-1 increment is required"*), CA.1-FU-1 closes here without any code change. The per-frame `StructuralAnalyzer` chain in `MIRPipeline.process` stays on by design — gating it now would break the BUG-015 wire. Backlog row at §Follow-up Backlog flipped accordingly.
...
| **CA.4-FU-2** ✅ | Source-comment cleanups (trivial, < 5 LoC total). (a) `PresetScorer.swift:86` — change `(D-030)` → `(D-032)`. (b) `PresetSignaling.swift:9-10` — replace `"Arachne does NOT emit yet — wiring is V.7.8."` with `"Arachne emits via _presetCompletionEvent.send() in ArachneState.advanceStablePhase (D-095, V.7.7C.2); orchestrator subscribes via VisualizerEngine+Presets.activePresetSignaling() (BUG-011 round 8)."`. | Both source files reflect current state. swift build clean. | <1 | **Resolved 2026-05-20 (bundled by CA.4 audit, commit `faee28a7`).** Both corrections were landed inline with the audit increment itself, per CA.4's release notes (*"in-source comments at PresetScorer.swift:86 (D-030 → D-032) and PresetSignaling.swift:9-10 (current-state rewrite per D-095 + BUG-011 round 8)"*). The backlog row's "Ready now" status was a documentation inconsistency in the audit deliverable. Post-fact grep verification 2026-05-21: `PresetScorer.swift:86` reads `## Weight rationale (D-032)`; `PresetSignaling.swift:9-16` describes the wired V.7.7C.2 emission + BUG-011 round 8 subscription path. |
...
| **CA.1-FU-1 (re-scoped per §Resolution above)** ✅ | Pre-BUG-015: gate the per-frame `StructuralAnalyzer` + `NoveltyDetector` + `SelfSimilarityMatrix` chain in `MIRPipeline.process` so it only runs at preparation time. Saves audio-callback CPU with zero behavioural change since no runtime consumer exists pre-BUG-015. Post-BUG-015: if BUG-015's fix routes `liveBoundary` from `MIRPipeline.latestStructuralPrediction`, this gate is replaced by re-enabling per-frame execution; if BUG-015 routes from `StructuralPrediction.none` or a sentinel, this gate stays on. The audit recommends shipping the gate independently of BUG-015 (it's cheap, safe, and saves CPU today). | The per-frame chain runs only at preparation time (or is invoked via a debug flag). Audio-callback CPU profile shows a measurable reduction. The prep-time `SessionPreparer+Analysis.swift:289` consumer (`sectionCount` derivation) is preserved. Engine + app builds clean. | 1 | **Resolved 2026-05-21 (folded by BUG-015, commit `b3f1efd9`).** BUG-015's fix took the **option-(b)** branch identified in §Resolution line 463 above (*"If BUG-015's fix sources liveBoundary from pipeline.latestStructuralPrediction, then CA.1 option (b) is the same change as fixing BUG-015 — the wire and the source go together"*). The wire at `PhospheneApp/VisualizerEngine+Orchestrator.swift:287-311` passes `mir.latestStructuralPrediction` as `boundary` to `applyLiveUpdate(...)`; the inline comment at `:272-277` explicitly notes the fold (*"folds CA.1-FU-1 into this fix: the per-frame StructuralAnalyzer chain in MIRPipeline.process now has a runtime consumer"*). Gating the per-frame chain to prep-time only would now BREAK the BUG-015 wire — the chain stays on by design. No separate CA.1-FU-1 increment is required. |
