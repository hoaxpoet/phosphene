...
**Follow-up backlog produced:** 3 entries (CA.7b-FU-1 ARCH stale-entry fixes, CA.7b-FU-2 ParticleGeometryRegistry doc-add, CA.7b-FU-3 RayTracing keep/retire decision for Matt).
...
| **CA.7b-FU-3** | **RayTracing keep-or-retire decision.** Same shape as CA.7-FU-3 (ICB keep). Two options: (A) keep-by-design, document the deferred-Arachne3D consumer in BVHBuilder.swift / RayIntersector.swift docstrings + CA.7b audit registry-pin, no code changes; (B) retire — delete the 3 Swift files + `Shaders/RayTracing.metal` + 2 test files + comment cross-reference at `Sources/Shared/AudioFeatures+SceneUniforms.swift:9`. ~900 LoC + 8 tests removed under Option B; preserved in git history per CLAUDE.md FA #58 pattern (Drift Motes). Audit-author recommendation: Option A. | Matt's product call + corresponding commit (single registry-pin commit under Option A; multi-file deletion under Option B). | 1 session (decision + commit) | Open — kickoff-recommended next surface for Matt's input |
