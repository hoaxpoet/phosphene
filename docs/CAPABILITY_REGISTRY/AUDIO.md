...
| `production-orphan` | 2 files | `LookaheadBuffer.swift` (0 production instantiations; 9 test sites; dual-read-head architecture never wired). `MusicKitBridge.swift` / `MusicKitFetcher` (0 production usage; not in `buildFetcherList()`; not in any test). |
...
| **CA-Audio-FU-2** | `LookaheadBuffer` + `AudioInputRouter.onAnalysisFrame` + `.onRenderFrame` are production-orphan. Matt's product call: (a) wire LookaheadBuffer through AudioInputRouter callbacks (Phase MV anticipatory architecture intent — the 2.5 s lookahead would source Orchestrator anticipatory decisions); (b) keep as infrastructure for a future preset / phase (CA.7b RayTracing precedent); or (c) retire entirely (CA.7-FU-4 `setRayMarchPresetComputeDispatch` precedent). The doc-drift fix on ARCH §Audio Capture annotates as planned-but-unwired pending this decision. | Matt's product call | 0 product-call + variable implementation | Open |
| **CA-Audio-FU-3** | `MusicKitFetcher` is production-orphan in both production code and tests. BPM extraction is a stub (MusicKit Swift SDK does not expose tempo). Recommend **delete** the file (clean retirement). Alternative: keep with a future-infrastructure marker, but the "if 'reusable infrastructure' appears in a defense" check (CA.7b precedent) suggests delete is the right call. Matt's product call. | Matt's product call | 1 session (delete + pbxproj update + closeout) | Open |
| **CA-Audio-FU-4** | Add unit tests for `AudioInputRouter+SignalState.swift` tap-reinstall logic. The 105-line extension implements the critical recovery path (3 s / 10 s / 30 s backoff; three attempts; cancel-on-active) but has **zero dedicated tests**. Recommended coverage: `scheduleNextReinstall_attemptCount`, `cancelPendingReinstall_resetsAttempts`, `attemptTapReinstall_skipsIfStateChanged`, `backoffExhausted_logsOnly`, `nextActiveToSilent_resetsAttempts`. | Test file lands; CI green | 1 session | Open |
| **CA-Audio-FU-5** | Add unit tests for `InputLevelMonitor`. No `InputLevelMonitorTests.swift` exists in `Tests/Audio/`. Recommended coverage: `submitSamples_peakDecaysAt0_9995`, `submitMagnitudes_bandEnergyEMA`, `recompute_warmupReturnsUnknown`, `recompute_belowCriticalReturnsRed`, `recompute_hysteresisRequires30Frames`, `reset_clearsAllEnvelopes`. | Test file lands; CI green | 1 session | Open |
