...
**Scope:** `UzumeEngine/Sources/Audio/` — 16 files / 3,294 LoC.
...
> **Post-audit removal (CLEAN.2.1 follow-up, 2026-06-14):** `SpotifyFetcher.swift` and its single env-gated call site in `VisualizerEngine+Audio.swift`'s `buildFetcherList()` have been **deleted**. It was an optional, env-gated (`SPOTIFY_CLIENT_ID` + `SPOTIFY_CLIENT_SECRET`) Client-Credentials-flow fetcher that returned only a track's duration via `/v1/search` — never active in normal runs and redundant with iTunes / MusicBrainz / Now Playing duration. It was the last consumer of `SPOTIFY_CLIENT_SECRET`, so its removal eliminated the Spotify client secret from the codebase entirely. The audit narrative below is preserved as the historical record of the component as it existed at audit time (2026-05-21); every `SpotifyFetcher` reference in this file describes a component that **no longer exists**. Still present and unchanged: the always-on `ITunesSearchFetcher` + `MusicBrainzFetcher`, the optional `SoundchartsFetcher`, and the Session-layer Spotify OAuth connector (`SpotifyWebAPIConnector` / `SpotifyOAuthTokenProvider`).

The Audio module exposed 16 files / 3,294 LoC at audit time, organised in five clusters: (1) capture pipeline (`Audio.swift`, `AudioBuffer`, `AudioInputRouter`(+`SignalState`), `SystemAudioCapture`, `FFTProcessor`, `LookaheadBuffer`); (2) signal-quality monitors (`SilenceDetector`, `InputLevelMonitor`); (3) metadata fetcher cluster (`MetadataPreFetcher`, `MusicBrainzFetcher`, `SpotifyFetcher` [removed — see banner above], `SoundchartsFetcher`, `MusicKitBridge`); (4) streaming polling (`StreamingMetadata`); (5) protocol surface (`Protocols`).
...
| CA.3 carry-forward correction | 1 | CA.3 [SESSION.md line 145](SESSION.md) says "`TrackMetadata` (constructed at :295) live in the **Audio** module." Actual location: `UzumeEngine/Sources/Shared/AudioFeatures+Metadata.swift:30`. `PreFetchedTrackProfile` + `MetadataSource` likewise live in Shared, not Audio. CA.3 correction filed below. |
| Kickoff staleness | 1 | Kickoff says BUG-005 is "Audio-module-internal; SpotifyFetcher producer-side." Actual producer is **Session-layer** `SpotifyWebAPIConnector.swift:241` (extracts `preview_url` from `/items` endpoint) consumed by `PreviewResolver.swift:73`. Audio's `SpotifyFetcher` (since **removed** in the CLEAN.2.1 follow-up, 2026-06-14) called `/v1/search` for duration only and had no `preview_url` field. CA-Audio-FU-1 below. |
...
7. **BUG-005 producer is not in this audit's scope.** The kickoff cites BUG-005 (Spotify `preview_url` returns null) as "Audio-module-internal; SpotifyFetcher producer-side." Verification (at audit time; `Audio/SpotifyFetcher.swift` has since been **removed** in the CLEAN.2.1 follow-up, 2026-06-14): `searchTrack` at `:122-147` called `https://api.spotify.com/v1/search` returning `SpotifyTrack` with only `id` + `durationMs` (line 172-180). No `preview_url` handling existed in that file. The actual BUG-005 producer is **`UzumeEngine/Sources/Session/Connectors/SpotifyWebAPIConnector.swift:241`** (the playlist-items connector that extracts `track["preview_url"] as? String`), consumed by **`UzumeEngine/Sources/Session/PreviewResolver.swift:73`** (Spotify-first / iTunes Search fallback per D-070 / Failed Approach #47). Both sites live in **Session**, not Audio. **Kickoff staleness**: BUG-005 should be re-categorised as a Session-domain bug, not Audio. (Note: CA.3 already audited the producer side at the Session module surface; the bug stays Open with the same content, just with the correct domain attribution.) Filed as **CA-Audio-FU-1**.

8. **BUG-013 producer is correctly in Audio scope.** `SoundchartsFetcher.swift` parses `time_signature` correctly at line 187 + 191 (CodingKey mapping `case timeSignature = "time_signature"`); the field is `Int?` so a null/missing field decodes silently to nil. `fetchSongMetadata(uuid:)` at line 141 passes the value through to `PartialTrackProfile.timeSignature`. The Uzume-side override mechanism (BUG-013 Round 26, Session-side at `SessionPreparer+Analysis.swift:299` per CA.3) consumes the value and overrides `BeatGrid.beatsPerBar` when present. The producer-side handling is correct — the bug is the Soundcharts API not returning the field, not a parser defect. No code change needed in Audio for BUG-013. Note: there's also no fallback fetcher in `buildFetcherList()` that exposes `time_signature` from another source — the BUG-013 "Path B per-track hardcoded overrides" + "different metadata source" fix paths are both out of scope until a product decision lands.
...
```bash
grep -rn "44100\|44_100" UzumeEngine/Sources/Audio/
```

Returns 2 hits:

```
UzumeEngine/Sources/Audio/Protocols.swift:111:    ///   - sampleRate: Sample rate in Hz (will be resampled to 44100 if different).
UzumeEngine/Sources/Audio/Protocols.swift:111:    ///   - sampleRate: Sample rate in Hz (will be resampled to 44100 if different).
```
...
UzumeEngine/Sources/Presets/Shaders/Gossamer.metal:189:    float brightness  = 0.12 + f.bass * 0.76 + bassRel * 0.12;
Exit: 0
```
...
**Gap: zero dedicated tests for the reinstall logic.** `grep -rn "reinstall\|Reinstall" UzumeEngine/Tests` returns only `UserFacingErrorTests.swift` (unrelated). `SilenceDetectorTests.swift` covers state-machine timings only. `AudioInputRouter+SignalState.swift`'s tap-reinstall logic (the 105-line extension) has **no unit tests**. The recovery path is critical (without it, scrub-induced silence permanently kills audio) and untested. Filed as **CA-Audio-FU-4**.
...
```bash
$ grep -rn "CGRequestScreenCaptureAccess\|CGPreflightScreenCaptureAccess" UzumeApp UzumeEngine/Sources
UzumeApp/VisualizerEngine+PublicAPI.swift:20:        var permitted = CGPreflightScreenCaptureAccess()
UzumeApp/VisualizerEngine+PublicAPI.swift:21:        if !permitted { permitted = CGRequestScreenCaptureAccess() }
UzumeApp/VisualizerEngine+PublicAPI.swift:41:                if CGPreflightScreenCaptureAccess() {
UzumeApp/Permissions/PermissionMonitor.swift:21:    /// `true` when `CGPreflightScreenCaptureAccess()` reports permission granted.
UzumeApp/Permissions/ScreenCapturePermissionProvider.swift:1:// ScreenCapturePermissionProvider — Abstracts CGPreflightScreenCaptureAccess for testability.
UzumeApp/Permissions/ScreenCapturePermissionProvider.swift:2:// Never calls CGRequestScreenCaptureAccess (system dialog doesn't compose with URL-scheme flow).
UzumeApp/Permissions/ScreenCapturePermissionProvider.swift:20:        CGPreflightScreenCaptureAccess()
UzumeApp/Views/Onboarding/PermissionOnboardingView.swift:8:// Never calls CGRequestScreenCaptureAccess.
UzumeEngine/Sources/Shared/UserFacingError.swift:31:    /// Screen-capture permission not granted (`CGPreflightScreenCaptureAccess() == false`).
```

`CGRequestScreenCaptureAccess()` is called exactly once, at **`UzumeApp/VisualizerEngine+PublicAPI.swift:21`** inside `startAudio()` — the entry point for audio capture. The code path: preflight read first (`CGPreflightScreenCaptureAccess()` at line 20), and only request if not already granted. `ScreenCapturePermissionProvider.swift:2` explicitly documents the single-request-point invariant: *"Never calls CGRequestScreenCaptureAccess (system dialog doesn't compose with URL-scheme flow)"* — meaning the permission provider abstraction (used by the permission monitor and onboarding view) is preflight-only; the request prompt is reserved for the audio-start path.
...
**Reality** (as of audit time; `SpotifyFetcher.swift` has since been **removed** in the CLEAN.2.1 follow-up, 2026-06-14). Audio module's `SpotifyFetcher.swift:122-147` (`searchTrack`) called `https://api.spotify.com/v1/search`:
...
The response struct decoded only `id` + `duration_ms`. **No `preview_url` field existed in the Audio `SpotifyFetcher` data path at all.** The fetcher existed to populate `PartialTrackProfile.duration` (only) and has since been **removed** (CLEAN.2.1 follow-up, 2026-06-14).

The actual BUG-005 producer is `UzumeEngine/Sources/Session/Connectors/SpotifyWebAPIConnector.swift:241`:
...
- `SpotifyFetcher` (**removed** in the CLEAN.2.1 follow-up, 2026-06-14; at audit time, line 72-74): built `PartialTrackProfile(duration: result.durationMs.map { Double($0) / 1000.0 })` — no timeSignature (Spotify deprecated `/audio-features` in Nov 2024; the search endpoint doesn't expose `time_signature`).
...
Per the kickoff and per DOC.3, FAs #45-47 were promoted from CLAUDE.md to `docs/RUNBOOK.md §Spotify connector setup` per the 2026-05-13 doc-refactor. These FAs describe the **UzumeEngine/Sources/Session/Connectors/SpotifyWebAPIConnector** behaviour (the playlist-items connector at `/items` endpoint), NOT the Audio-module `SpotifyFetcher` (the search-based metadata enrichment fetcher, since **removed** in the CLEAN.2.1 follow-up, 2026-06-14). RUNBOOK is the canonical doc surface; the (now-removed) Audio `SpotifyFetcher` was not affected by FAs #45-47.

> **Removed since audit:** the Audio-module `SpotifyFetcher` described in this subsection was **deleted** in the CLEAN.2.1 follow-up (2026-06-14). The detail below is preserved only as the historical record of how that (now-gone) fetcher differed from the still-present Session-layer connector.

Audio's `SpotifyFetcher` used (at audit time):
...
This was a separate Spotify code path from the Session-layer OAuth user-token connector. The two paths used different env vars (the Session-layer Spotify uses user OAuth; the Audio fetcher used client credentials, and was the **last consumer of `SPOTIFY_CLIENT_SECRET`** — its removal eliminated that secret from the codebase).

**Verdict:** FAs #45-47 did not apply to the Audio module's SpotifyFetcher. The Audio module's Spotify integration was the metadata-enrichment search path, not the preview-URL extraction path that FAs #45-47 cover — and it has since been removed.
...
```bash
$ grep -rn "LookaheadBuffer(" UzumeApp UzumeEngine/Sources --include='*.swift'
# (no hits — zero production instantiations)
```

```bash
$ grep -rn "LookaheadBuffer" UzumeApp UzumeEngine/Sources --include='*.swift'
UzumeEngine/Sources/Shared/AnalyzedFrame.swift:13:                          # doc-comment only
UzumeEngine/Sources/Audio/AudioInputRouter.swift:116:                       # doc-comment only
UzumeEngine/Sources/Orchestrator/TransitionPolicy.swift:134:               # doc-comment only — "Matches the LookaheadBuffer delay of 2.5 s."
```

```bash
$ grep -rn "onAnalysisFrame\|onRenderFrame" UzumeApp UzumeEngine/Sources --include='*.swift'
UzumeEngine/Sources/Audio/AudioInputRouter.swift:113:    public var onAnalysisFrame: ((_ frame: AnalyzedFrame) -> Void)?
UzumeEngine/Sources/Audio/AudioInputRouter.swift:117:    public var onRenderFrame: ((_ frame: AnalyzedFrame) -> Void)?
...
**User-Agent:** `"Uzume/1.0 (https://github.com/hoaxpoet/uzume)"` (line 26) — MusicBrainz API policy requires a descriptive User-Agent. Rate limit (1 req/sec per docs/header comment) is NOT explicitly enforced in code — relies on the per-track event frequency to stay polite. Worth noting; not a finding unless production traffic patterns surface a 429.

**No findings.**

#### SpotifyFetcher.swift (180 LoC) — `removed` (was `production-active` at audit time)

> **REMOVED in the CLEAN.2.1 follow-up (2026-06-14).** `SpotifyFetcher.swift` and its single env-gated call site in `VisualizerEngine+Audio.swift`'s `buildFetcherList()` have been deleted. The fetcher was optional and never active in normal runs (its `SPOTIFY_CLIENT_ID` / `SPOTIFY_CLIENT_SECRET` env vars are never set), and it returned only a track's duration — redundant with iTunes / MusicBrainz / Now Playing. It was the last consumer of `SPOTIFY_CLIENT_SECRET`, so its removal eliminated the Spotify client secret from the codebase. The audit-time description below is retained only as a historical record of the (now-deleted) component.

Was a search-only Spotify Web API client. `public final class SpotifyFetcher: MetadataFetching, @unchecked Sendable`. Env-gated. The "verdicts" in the table below were `production-active` **at audit time (2026-05-21)**; the entire file no longer exists.

| Public API (at audit time) | Verdict (at audit time) | Consumer | Note |
|---|---|---|---|
| `sourceName = "Spotify"` | removed (was `production-active`) | logging | |
| `init(clientID:clientSecret:)` | removed (was `production-active`) | `fromEnvironment()` static | |
| `fromEnvironment() -> SpotifyFetcher?` static | removed (was `production-active`) | was `VisualizerEngine+Audio.swift:64` — call site deleted | Env vars `SPOTIFY_CLIENT_ID` + `SPOTIFY_CLIENT_SECRET` (the latter's last consumer) |
| `fetch(title:artist:) async -> PartialTrackProfile?` | removed (was `production-active`) | MetadataPreFetcher | Returned `(duration)` only — audio features endpoint deprecated Nov 2024 |

**Internal (at audit time):** Cached `accessToken` + `tokenExpiry` (with 60 s safety buffer) under `lock: NSLock` at line 31. `getAccessToken()` returned cached token if not expired; else POST to `accounts.spotify.com/api/token` with Basic auth header. `searchTrack(title:artist:token:)` called `/v1/search` with `q=track:title artist:artist&type=track&limit=1`; decoded only `SpotifyTrack { id, duration_ms }`.

**Note:** This was **NOT** where BUG-005 fires (BUG-005 is in the Session-layer `SpotifyWebAPIConnector` /items endpoint; this fetcher used /v1/search and never touched `preview_url`). See §Verification of metadata-fetcher BUG surfaces above for the full producer trace.

**No findings** (component since removed — see banner).
...
```bash
$ grep -rn "MusicKitFetcher\b" UzumeApp UzumeEngine/Sources --include='*.swift'
UzumeEngine/Sources/Audio/MusicKitBridge.swift:23:// MARK: - MusicKitFetcher
UzumeEngine/Sources/Audio/MusicKitBridge.swift:30:public final class MusicKitFetcher: MetadataFetching, @unchecked Sendable {
# (only the declaration — zero callers)

$ grep -rn "MusicKitFetcher\b" UzumeEngine/Tests UzumeAppTests --include='*.swift'
...
None required, but worth noting: §Spotify connector setup carries the FAs #45 / #46 / #47 promoted from CLAUDE.md per DOC.3. These FAs cover the **Session-layer** `SpotifyWebAPIConnector` (the /items endpoint connector that fires for playlist preparation). The **Audio-layer** `SpotifyFetcher` (this audit's scope) used a different code path (Client Credentials flow + /v1/search endpoint for audio-feature enrichment, env vars `SPOTIFY_CLIENT_ID` + `SPOTIFY_CLIENT_SECRET`) — but has since been **removed** in the CLEAN.2.1 follow-up (2026-06-14), so only the Session-layer connector remains. The disambiguation that CA-Audio-FU-8 recommended is now moot for the Audio layer (the conflation source is gone); see CA-Audio-FU-8 below.
...
This is a CA.3 carry-forward error. `MetadataPreFetcher` does live in Audio, but `TrackMetadata`, `PreFetchedTrackProfile`, and `MetadataSource` all live in `UzumeEngine/Sources/Shared/AudioFeatures+Metadata.swift` (lines 30, 69, 10 respectively). CA-Audio updates `SESSION.md` to remove the false co-location claim.
...
|---|---|---|---|---|
| **CA-Audio-FU-1** | Document BUG-005 producer-domain correctly (it is **Session-layer** `SpotifyWebAPIConnector.swift:241` → `PreviewResolver.swift:73`, not the Audio-module `SpotifyFetcher` — the latter has since been **removed** in the CLEAN.2.1 follow-up, 2026-06-14). Kickoff staleness only — the BUG body in KNOWN_ISSUES.md is already correct; only the kickoff prompt asserted the wrong domain. **Resolution:** registry-only note (this audit closes by documenting the correct attribution above). No code change. | This doc | 0 (closed at write-time) | Resolved in CA-Audio |
...
- **Pass 0 BUG cross-check** caught one significant kickoff staleness (BUG-005 producer-domain attribution). The 5-minute KNOWN_ISSUES.md cross-check saved hours of investigation into a non-existent SpotifyFetcher preview-URL handling path. (The `SpotifyFetcher` component itself was subsequently **removed** in the CLEAN.2.1 follow-up, 2026-06-14.)
...
- **The ARCH Module Map drift is now a 5-in-a-row systemic finding** (CA.5 / CA.6 / CA.7a / CA.7b / CA-Audio all surfaced module-map drift at the file-listing granularity). CA.7b recommended *"a future bulk pass against `find` output rather than continuing one-or-two-items-per-increment"*. CA-Audio confirms the need: 2 files missing here, same per-increment overhead. **Recommend filing a standalone module-map sync increment** that runs `find UzumeEngine/Sources UzumeApp -name '*.swift' | sort` against the ARCH Module Map blocks and adds every missing file in one pass. Estimate: 0.5 session.
