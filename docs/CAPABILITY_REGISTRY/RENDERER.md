...
8. **One production-orphan extension API (deliberately reserved)** — `RenderPipeline.setRayMarchPresetComputeDispatch(_:)` (RenderPipeline+PresetSwitching.swift:164). Called only with `nil` in production; the original V.9 Session 4.5b Phase 2b consumer (Ferrofluid Ocean per-frame particle update) was deactivated at Phase 1 round 4 ("particles are pinned, one-shot bake at preset apply is sufficient" — VisualizerEngine+Presets.swift:265-267). API kept by design (CA.7-FU-4).
...
| `production-orphan` | 2 | ICB cluster (App ↔ Renderer boundary-noted) + `setRayMarchPresetComputeDispatch` (kept-by-design). |
...
**4 follow-ups registered** — CA.7-FU-1 (mv_warp test reachability tightening), CA.7-FU-2 (depth-debug pass removal), CA.7-FU-3 (ICB cluster status review), CA.7-FU-4 (`setRayMarchPresetComputeDispatch` retention decision).
...
| **CA.7-FU-3** | Decide ICB cluster status: keep (planned consumer pending) or retire (consumer never materialised in 2+ years). Currently zero production callers; 5 GPU tests gate the renderer-side wiring. If keeping: extend ARCH §Renderer with a "Planned: ICB-pass presets" forward-look paragraph. If retiring: delete `RenderPipeline+ICB.swift`, the `.icb` enum case, the App-side `case .icb:` no-op switch arm, the `IndirectCommandBufferState` shaders + tests, and the GPU Contract Details §ICB Architecture paragraph. | Matt's call. | 1 (if retiring) | Open. Matt-decides. |
| **CA.7-FU-4** | Decide `setRayMarchPresetComputeDispatch(_:)` status. Currently kept-by-design for a Ferrofluid Ocean Phase 2b revival that was deactivated at Phase 1 round 4. The dispatch hook (RenderPipeline+RayMarch.swift:170-172) fires only if a non-nil closure is set; no production code sets one. Either: (a) keep, and document the "kept-by-design for future revival" intent inline; (b) remove the closure storage + dispatch site and re-introduce when a real consumer materialises. | Matt's call. | 0.25 (either way) | Open. Matt-decides. |
