...
**M7-round-2 learnings (Matt 2026-06-09 — "strokes appear as overlapping circles that smooth into a line after a second" + "the wet doesn't fully read as glistening"). The wet sheen draws the eye to the live edge, so the live edge must read as wet dribbled paint, which exposed two issues:**

- **Once you have a real wetness channel, RETIRE the trailing-tail "fade-in" stand-in — the pour must LAND SOLID.** A per-frame trailing tail that ramps radius (`mix(0.18,1.0,ageFrac)`) and opacity (`mix(0.04,1.0,…)`) across N co-located samples at the slow / looping live edge draws as a **stack of nested thin faint discs (concentric rings)** — "overlapping circles" — that only smooth once the paint bakes past the tail window (~a second). That age-taper was the *stand-in* for a wet leading edge before the wetness channel existed; with the channel, the pour should land at **full opacity, constant radius** (speed→width only), reading as a continuous dribble immediately — and the wetness/sheen carries the "fresh = wet" read. Bonus: a full-coverage fresh stroke stamps **full wetness along its whole recent length** → the sheen glistens on the live dribble, not just a faint tip.

- **Glistening needs TWO specular terms: a BROAD gloss + a SPARKLE.** A single broad gloss (smooth normal) reads "glossy but not glistening" (no catch-lights); a single micro-sparkle (perturbed normal) reads "dark/grainy" because it *scatters the broad gloss away* (a random normal jitter lowers the average `N·H` since the smooth normal was near-optimal) leaving sparse glints on a dimmed base. Use **both**: (1) a **broad gloss** from the smooth luminance-gradient normal — a soft sheen across the wet body that keeps the wet mean bright (= reads "wet/glossy"); plus (2) a **sparkle** from a **fine noise-gradient micro-normal** (`perlin2d` forward-difference at a high frequency, ~200–250×) — bright pinpoint catch-lights (= the glisten). Gate **both** by `specWet × paint`. This is the difference between "ok wet look" and "glistening wet paint." (Skein.4 wet boost 25 → 76 mean with the broad term; max glint 162 → 192 with the sparkle; all 4 stems still read through.)

- **Do NOT use a saturation "deepen" to add wet depth — it DARKENS in sRGB.** Over-saturating around luma (`luma + (col−luma)·(1+k)`) is luma-preserving in *linear*, but the per-channel sRGB encode is asymmetric (low channels expand, high channels compress near 1) → the net sRGB-byte luma **drops**. So "wet = deeper" via saturation actually makes wet paint *darker*, fighting the "glossy" read. Let the **additive broad gloss** carry the wet brightening (it also preserves hue, so colours read through) and drop the deepen.
