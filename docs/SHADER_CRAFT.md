...
| `scene_orbit_speed` | `0` (camera-static) | Camera orbit speed (rad/s) around the world Y-axis through the origin, seeding `RayMarchPipeline.cameraOrbitSpeed`. Orthogonal to `scene_dolly_speed` — no current preset drives both. Sidecar-owned, same `applyPreset`/`SessionReplayHarness` dual-seed pattern as the dolly (WHIT.2b). Makes a ray-marched shape's roundness/depth legible via parallax when a static shot wouldn't sell it. **No current consumer** — Rosette shipped at 0.12 (WHIT.2b) and removed it (WHIT.2c, Matt: "eliminate the orbit"; a constant-speed orbit reads as disconnected from the music, and for a wholly z=0-planar scene periodically flattens the whole composition edge-on). Capability kept — cheap, generic, and the failure was Rosette's constant unmodulated rate against a flat scene, not the mechanism itself; a future preset with real depth variation and/or an audio-modulated rate is a different case. |
