...
## [dev-2026-05-27-a] CSP.1.1 — Instrumentation gap closed + Membrane consumer

**Increment:** CSP.1.1 (follow-up to CSP.1). **Status:** Implemented 2026-05-27. Tests green, SwiftLint clean, app build succeeds. Manual A/B validation on Membrane outstanding (Matt's gate).

### What this addresses

CSP.1's first A/B run on Lumen Mosaic this morning returned "no difference observed." Two findings drove the follow-up:

1. **Instrumentation gap.** CSP.1 added `softTempoPulse01` to the FeatureVector but did NOT log it in `features.csv`. The A/B could not be verified from session artifacts. Matt would have had to take CSP.1's word for it that the toggle actually flipped the field's value between sessions. Gap on me.
2. **Wrong test bed.** Matt's read: Lumen Mosaic stacks three visible beat-rate signals already — per-cell palette walk on each team beat, per-cell brightness jitter ([0.30, 1.60] range), and the 20 % bar pulse on each downbeat. A 15 % global brightness modulation has no headroom against that activity. LM doesn't have a "rhythmically inert cold-start" baseline to improve on; the preset always looks synced because the per-cell signal fires regardless of grid lock state.

### Changes

- **CSV instrumentation** (`SessionRecorder.swift`, `SessionRecorder+CSV.swift`): `soft_tempo_pulse01` appended as the trailing column of `features.csv`. Append-only invariant preserved (existing columns stay in place; the new column is the new trailing column). Per-frame write logs `fv.softTempoPulse01` so future A/B sessions are verifiable from the artifact — ON-arm sessions will show non-zero values during the 0–12 s cold-start window; OFF-arm will show identical zeros throughout.
- **Membrane consumer** (`Membrane.metal`): `membrane_D` adds `features.soft_tempo_pulse01 × 0.30` as a global displacement term. Distinct primitive at a distinct timescale from the existing terms (one-primitive-per-layer per Failed Approach #67):
  - `breath` is FBM at fixed time × 0.16 (always-on, untethered to tempo)
  - `wave` is sin at fixed time × 0.33 (untethered to tempo)
  - `ring` is the per-kick shockwave (event-rate, sparse)
  - The new soft-tempo term is breathing at the cached BPM, only active during the cold-start window
  Per-kick `ring` events go structurally quiet between kicks — the new term fills that gap without competing at the same timescale.
- **Test updates** (`SessionRecorderTests.swift`): column-position assertions updated to reflect the new trailing column; new `test_recordFrame_softTempoPulse01_writtenToCSV` regression-locks the CSV round-trip.

### Magnitude rationale

LM consumer uses `1 + 0.60 × pulse` on cell brightness → peak combined effect ~15 % brightness modulation. Lost in noise on LM's busy baseline.

Membrane consumer uses `+ 0.30 × pulse` on the displacement equation → peak displacement contribution 0.075, similar in scale to the always-on `breath × 0.40` term (average contribution ~0.20). On Membrane's quieter baseline this reads as ambient breathing at tempo rather than getting lost. The same 0.30 magnitude on LM would have been irrelevant; the same 0.60 on Membrane would have been over-loud.

### Verification

- **Engine:** 1278 / 1278 tests pass. New CSV-round-trip test green. Existing column-position tests updated.
- **SwiftLint `--strict`:** 0 violations.
- **App build:** succeeds.
- **Manual M7 (outstanding):** Matt's binary judgment on a Membrane A/B with a low-confidence track from the BSAudit.3 validate-3 set. Now verifiable from `features.csv` post-run (ON arm: `soft_tempo_pulse01` non-zero during 0–12 s; OFF arm: identically zero).

### Touched files

- `PhospheneEngine/Sources/Shared/SessionRecorder.swift`
- `PhospheneEngine/Sources/Shared/SessionRecorder+CSV.swift`
- `PhospheneEngine/Sources/Presets/Shaders/Membrane.metal`
- `PhospheneEngine/Tests/PhospheneEngineTests/Shared/SessionRecorderTests.swift`
- `docs/ENGINEERING_PLAN.md` — new CSP.1.1 sub-increment row.
- `docs/RELEASE_NOTES_DEV.md` — this entry.

### Outcome handling

After Matt's Membrane A/B:
- **Better:** keep both LM and Membrane consumers, extend to other beat-accent-consuming presets in a follow-up increment.
- **Worse or no different on Membrane too:** the time-based fade premise didn't pay off across two test beds, one of which (Membrane) was structurally favourable. Revert CSP.1 + CSP.1.1 (`git revert <CSP.1.1 hash> <CSP.1 hash>`), file as Failed Approach, close CSP phase. The hypothesis would be falsified, not just the implementation.
...
- CSP.1 (`[dev-2026-05-26-c]`) — the parent increment this follows up.
- BSAudit.3.revert.docs (`[dev-2026-05-26-b]`) — the doc-state alignment that preceded CSP.1.
- Failed Approach #67 (CLAUDE.md) — one-primitive-per-layer at one timescale. Membrane consumer respects this: soft-tempo breathing ≠ ambient-FBM breath ≠ time-rate sin wave ≠ per-kick ring.

---

## [dev-2026-05-26-c] CSP.1 — Cold-Start Perceptual Tempo Scaffold (soft tempo-pulse, A/B-ready)

**Increment:** CSP.1 (single increment, time-based fade design). **Status:** Implemented 2026-05-26. Engine + app tests pass; manual A/B validation outstanding (Matt's gate). **Outcome:** new `FeatureVector.softTempoPulse01` field, phase-humble tempo scaffold during the cold-start window, wired into Lumen Mosaic, gated by a UserDefaults A/B toggle.

### Hypothesis under test

During tracks where Phosphene can't claim beat phase confidently in the cold-start window, beat-accent-driven presets fall back to pure continuous-energy reactivity and the visual reads as "alive but not rhythmic." We have a reliable tempo from the Beat This! preview analysis from frame one. The hypothesis: surface a quiet, phase-humble tempo hint — a soft pulse at the song's BPM, with a 6–12 s time-based fade, never claiming alignment with audible beats — and beat-accent presets read as tempo-aware rather than purely energy-led.

### Design

- **New `FeatureVector` field:** `softTempoPulse01: Float` ∈ `[0, 0.25]`. Replaces the `_pad3` slot to preserve byte-identical layout of all existing fields. Mirrored on the MSL side as `soft_tempo_pulse01` in both `Common.metal` and `PresetLoader+Preamble.swift`.
- **BPM source:** cached `BeatGrid.bpm` via `liveDriftTracker.currentBPM`. When no grid is installed, the field is 0 (no claim).
- **Fade envelope:** full amplitude for `t ∈ [0, 6 s]`; smoothstep ramp from 6 s to 12 s; zero after 12 s. Anchored to `MIRPipeline.elapsedSeconds`, which resets via `MIRPipeline.reset()` on every track change. Matches the D-019 stem warmup window so by the time stems are providing reliable beat info, the scaffold has stepped aside.
...
- **Engine suite:** 1278 tests / 163 suites pass aside from 4 pre-existing environmental flakes (`PlaylistConnectorTests` requires Apple Music to be running). New `MIRPipelineSoftTempoPulseTests` suite (11 tests) passes:
  - `softTempoPulse_toggleOff_isAlwaysZero`
  - `softTempoPulse_noGrid_isAlwaysZero`
  - `softTempoPulse_degenerateZeroBPM_isAlwaysZero`
  - `softTempoPulse_atFirstFrame_isNearTrough`
  - `softTempoPulse_atHalfPeriod_isNearPeakAmplitude`
  - `softTempoPulse_atFullPeriod_isNearTrough`
  - `softTempoPulse_neverExceedsAmplitudeBudget`
  - `softTempoPulse_afterFadeEnd_isZero`
  - `softTempoPulse_resetReturnsToFullFade`
  - `softTempoPulse_periodMatchesCachedBPM`
  - `softTempoPulse_differentBPMs_produceDifferentPeriods`
- **App suite:** passes aside from pre-existing parallel-execution timing flakes (`FirstAudioDetectorTests` ×3, `PlaylistConnectorTests` ×1 — all pass in isolation, none touch CSP.1 code paths).
- **SwiftLint `--strict`:** 0 violations.
...
**If better:** CSP.1 cert. Plan an extension to other beat-accent-consuming presets.
**If worse or no different:** revert, log as a Failed Approach (the time-based premise didn't pay off), reconsider direction.

### Touched files

- `PhospheneEngine/Sources/Shared/AudioFeatures+Analyzed.swift` — new `softTempoPulse01` field (reclaimed from `_pad3`).
- `PhospheneEngine/Sources/Renderer/Shaders/Common.metal` — matching MSL field.
- `PhospheneEngine/Sources/Presets/PresetLoader+Preamble.swift` — MSL mirror (preamble preset compilation).
- `PhospheneEngine/Sources/DSP/MIRPipeline.swift` — new properties (`softTempoPulseEnabled`, `softTempoPulseAmplitude`, `softTempoPulseFadeStartS`, `softTempoPulseFadeEndS`) + `computeSoftTempoPulse(elapsedSeconds:)` helper + per-frame write in `buildFeatureVector`.
- `PhospheneEngine/Sources/Presets/Shaders/LumenMosaic.metal` — `lm_cell_intensity` takes `softTempoPulse01` and multiplies brightness by `1 + 0.60 × pulse`. New constant `kSoftTempoMagnitude = 0.60`.
- `PhospheneApp/VisualizerEngine.swift` — read UserDefaults toggle in init, apply to `mirPipeline.softTempoPulseEnabled`.
- `PhospheneEngine/Tests/PhospheneEngineTests/DSP/MIRPipelineSoftTempoPulseTests.swift` — 11-test regression suite.
- `docs/ENGINEERING_PLAN.md` — new Phase CSP + Increment CSP.1 row.
- `docs/RELEASE_NOTES_DEV.md` — this entry.

### Related

- BSAudit.3.revert.docs (`[dev-2026-05-26-b]`) — unblocked this work by clarifying that `accentConfidence` doesn't exist in production. CSP.1 switched its fade signal from confidence-based to time-based as a direct result.
- Failed Approach #69 (CLAUDE.md) — the "no more short-window automated cold-start beat-phase derivation" rule. CSP.1 stays within that rule: it doesn't attempt to derive phase, only to scaffold tempo perception.
- BUG-016 (`[dev-2026-05-26-a]`) — Lumen Mosaic palette fix landed prerequisite to CSP.1's M7 validation (no point A/B-testing a preset that renders black-and-white).
