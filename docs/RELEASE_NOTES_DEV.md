...
## [dev-2026-05-27-b] CSP.2 — Ferrofluid Ocean spike-height cold-start fix (cached perception + live overall bass)

**Increment:** CSP.2 (single increment, two layers in one shader function). **Status:** Implemented 2026-05-27. Engine + app tests pass; manual M7 validation outstanding (Matt's gate).

### What this fixes

Ferrofluid Ocean's "biggest cold-start problem" symptom: when a new track starts, spike heights sit at a song-appropriate but **fixed** height for the first ~5 seconds (set by the cached preview-snapshot stems, but not responding per-frame to live audio). After ~5 seconds, live per-frame stem analysis kicks in and the spikes start pulsing with the isolated bass — but for the first few seconds the visual reads as inert against the music.

### What the viewer sees now

- **Frame 1.** Spikes already at a song-appropriate height (Layer 1 — cached perception). Bass-heavy track → tall baseline; sparse vocal-led track → short baseline. ±25 % range across the [0, 0.5] cached bass-proportion range (Matt's "Visible" magnitude approval, 2026-05-27).
- **0–8 s.** Spikes pulse with the **live overall bass** in the mix (Layer 2 — `f.bass_dev`, AGC-normalised deviation primitive, available frame 1). Bass hits → spikes rise above baseline. Quiet → settle back.
- **~5–8 s.** Smooth crossfade as live per-frame stem analysis comes online; spikes start tracking isolated bass instead of overall mix bass.
- **8 s+.** Identical to today's behaviour — isolated bass drives spike pulsing.

### How it works

Two small new fields plumb the necessary information to the shader:

1. **`FeatureVector.trackElapsedS`** — track-relative wall-clock seconds, reset to 0 at every `MIRPipeline.reset()` (the track-change call). Populated in `MIRPipeline.buildFeatureVector` from the existing `elapsedSeconds: Double` accumulator. Float-cast for GPU upload (precision loss is sub-ms at hour timescales, irrelevant for the ~10 s cold-start window). Reclaimed from `_pad3` — same byte-identical-layout pattern CSP.1 used.

2. **`StemFeatures.cachedBassProportion`** — bass proportion (`bassEnergy / total`) from the cached preview-analysis snapshot. Installed at track-change in `VisualizerEngine+Stems.swift:481-490` from `CachedTrackData.stemFeatures`. **Preserved across live `setStemFeatures(_:)` calls** by `RenderPipeline+PresetSwitching.swift:setStemFeatures` (the per-frame analyzer's `cachedBassProportion` field is ignored; the field retains whatever `setCachedBassProportion(_:)` last set it to). Cleared to 0 on track-change with no cache hit (live reactive mode / ad-hoc playback). Reclaimed from `_sfPad2`.

In the FFO shader, `fo_spike_strength` (`FerrofluidOcean.metal:85`) now combines:

```
baseline = 1.0 + clamp((cached_bass_proportion - 0.25) × 1.0, -0.25, +0.25)
blend    = smoothstep(0.5, 8.0, track_elapsed_s)
source   = mix(f.bass_dev, stems.bass_energy_dev, blend)
spike    = baseline + 0.35 × source
```

Original 0.35 per-frame coefficient preserved verbatim.

### Verification

- **Engine:** 1275 / 1275 tests pass (8 new CSP.2 plumbing tests added across two suites — `trackElapsedS` resets + accumulates correctly; `cachedBassProportion` preserved across multiple live updates).
- **App build:** succeeds.
- **SwiftLint `--strict`:** 0 violations.

**Manual M7 (outstanding — Matt's gate):**

1. Set state: `defaults delete com.phosphene.app softTempoPulseEnabled` (the prior CSP.1 toggle is gone after the revert, but the command is harmless).
2. Run Phosphene. Cycle to Ferrofluid Ocean via `Shift+→`. Play a track from the start. Watch the first 12 seconds.
3. Expected: spike heights at a song-appropriate baseline from frame 1, pulsing with the live bass from frame 1 onwards, smoothly handing off to isolated-bass behaviour around 5–8 s.
4. Try a bass-heavy track (e.g. hip-hop) vs a sparse vocal-led track. Spike baselines should be visibly different at frame 1.

If the symptom is gone, CSP.2 cert. If still inert, paste the session.log and features.csv and I'll dig in (the new fields are in features.csv via the existing schema — `track_elapsed_s` doesn't have its own column yet, but `bass_dev`/`bass_energy_dev` are loggable for diagnosis).

### Touched files

- `PhospheneEngine/Sources/Shared/AudioFeatures+Analyzed.swift` — new `trackElapsedS` field (reclaimed from `_pad3`).
- `PhospheneEngine/Sources/Shared/StemFeatures.swift` — new `cachedBassProportion` field (reclaimed from `_sfPad2`).
- `PhospheneEngine/Sources/Renderer/Shaders/Common.metal` — matching MSL field additions.
- `PhospheneEngine/Sources/Presets/PresetLoader+Preamble.swift` — MSL preamble mirror.
- `PhospheneEngine/Sources/DSP/MIRPipeline.swift` — populates `trackElapsedS` from `elapsedSeconds`. File-length suppression re-added (the file now exceeds the 400-line warning by 6 lines).
- `PhospheneEngine/Sources/Renderer/RenderPipeline+PresetSwitching.swift` — `setStemFeatures(_:)` preserves `cachedBassProportion`; new `setCachedBassProportion(_:)` method.
- `PhospheneEngine/Sources/Presets/Shaders/FerrofluidOcean.metal` — `fo_spike_strength` rewritten to compose Layer 1 (cached baseline) + Layer 2 (cold-start crossfade) + warm path. New constants at top of file.
- `PhospheneApp/VisualizerEngine+Stems.swift` — computes cached bass proportion at track-change from `CachedTrackData.stemFeatures` and installs via `setCachedBassProportion`. Clears to 0 on cache miss.
- `PhospheneEngine/Tests/PhospheneEngineTests/DSP/CSP2DataPlumbingTests.swift` — new file, 8 contract tests across two suites.

### Outcome handling

After Matt's M7:
- **Better:** CSP.2 cert. Same pattern (proxy-during-cold-start, crossfade to warm) likely applies to Volumetric Lithograph's terrain pulse and camera dolly — file as CSP.3 follow-up if Matt wants.
- **No different / worse:** investigate diagnostically (the new fields are A/B-verifiable from `features.csv` — if `track_elapsed_s` doesn't reset to 0 on track change, or `cached_bass_proportion` is 0 even with a cached track, that's the bug). Don't immediately revert — first confirm the plumbing is firing as designed.

### Lessons applied from CSP.1

- "Hear" → "perceive": this uses both live audio (`f.bass_dev`) AND cached pre-playback analysis (`cached_bass_proportion`).
- One-primitive-per-layer (Failed Approach #67): spike heights are ONE layer; the source switches sequentially from `f.bass_dev` to `stems.bass_energy_dev` over the warmup window. No competition at the same timescale.
- Plain-English communication: the design decision (Visible ± 25 % magnitude) was framed in product terms before implementation, not as a numeric tuning question.
...
- `[dev-2026-05-27-a]` (above) — CSP.1 + CSP.1.1 reverted; cleared the field for this approach.
- `[dev-2026-05-26-b]` — BSAudit.3.revert.docs; established that `accentConfidence` doesn't exist in production, so CSP.2 doesn't rely on it.
- Failed Approach #67 (CLAUDE.md) — one-primitive-per-layer at one timescale. CSP.2 respects this: one layer (spike heights), one timescale (tempo / per-beat rate), two sources sequenced.
