...
## [dev-2026-06-19-201951] LFPLAN.4 — min-dwell + cold-start gate (stop the cold-start preset cycling)

Matt's first LFPLAN.3 live session (`2026-06-19T19-21-39Z`, Smells Like Teen Spirit + In Bloom) confirmed the plan now executes — but track 0 **machine-gunned 8 presets in the first ~24 s** (Kinetic Sculpture → Plasma → Membrane → Fractal Tree → Skein → Waveform → Murmuration → Nimbus, ~every 3 s) then locked for the rest of the track. It's a cold-start **transient**: while stems/mood/structure converge (~24 s) the LiveAdapter re-patches the plan on nearly every ~3 Hz orchestrator tick, and LFPLAN.3's `applyPlannedSegment` applied each churn faithfully. The historical 2026-05-28 cycling symptom returning through the new plan-execution door.

**LFPLAN.4:** the first planned segment per track still applies **promptly** (the track starts on its planned visual — this also closes the ~0.5 s default-preset flash), but every *subsequent* auto-apply is gated by two constraints with independent teeth: a **cold-start suppression window** (`planColdStartSuppressWindow` = 20 s from track start — just under the observed convergence so the track lands on the settled preset, not a mid-convergence transient) AND a **min-dwell** (`planApplyMinDwell` = 15 s from the previous apply — mirrors the reactive path's 60 s `lastReactiveSwitchTime` cooldown, shorter because planned sessions *want* section-scale switches). With 20 > 15 both gates bind in different timing regimes (cold-start governs the initial settle, dwell the steady-state spacing). New `lastPlannedApplyTrackTime` state, reset at both track-change sites (`+Capture` streaming, `+LocalFilePlayback`) alongside the LFPLAN.3 markers. Manual picks are untouched — they go through `applyPresetByID`/cycle, not `applyPlannedSegment`. On the live trace the fix collapses the track-0 burst **8 → 2** (Kinetic Sculpture @3.6 s prompt → settled preset @~24 s). Lint 0; runtime (burst gone, transitions feel deliberate / ≥15 s apart, no Next-button lockup) is Matt's live-validation gate — `VisualizerEngine` isn't constructible in the app-test sandbox. The 2026-05-28 cycling/lockup is the regression this guards against; a live test showing rapid cycling or a Next lockup means back out, don't push through.
...
But the playlist visibly stayed on one preset. Two downstream causes, both *separate* from the detector: (1) the reactive orchestrator only switches to a higher-scoring preset (`scoreGap > 0.05`, ReactiveOrchestrator:285) — it switched once, exactly on section 1→2, then sat because the same preset kept winning the scoring; (2) the session ran in **reactive fallback** because `buildPlan()` is disabled for local files (a 2026-05-28 BUG-021 revert).

**LFPLAN — re-enabling local-file planning.** The revert disabled `buildPlan()` because the orchestrator cycled presets "one every ~5 s" with a 2-preset catalog, leaving an explicit re-enable gate ("certified catalog grows AND plan walker's short-segment behaviour verified"). Found the cycling's root cause: `estimatedSectionCount = StructuralAnalyzer boundary count` (SessionPreparer+Analysis:287), so the pre-BUG-042 junk detector (~180 boundaries on a 5-min track) made `makeSections` cut ~180 tiny planner segments → a preset every ~1.6 s. **So BUG-042's fix is the load-bearing precondition.** **LFPLAN.1:** new `PlannerSectionCountScalingTests` runs the real planner and pins it — sectionCount 9 → 9 segments, 180 → 180 (`44bb745`). All three re-enable preconditions are now met: BUG-042 fixed (180→9), certified catalog 2 → 7, and the Next-button force-quit was the separately-resolved BUG-059 ABBA deadlock (2026-06-18). **LFPLAN.2:** re-enabled `buildPlan()` on the local-file ready path (`a07b0d1`, reverting the revert) — it's source-agnostic (reads `currentPlan` + cached profiles, both present on the LF path) and flips the orchestrator to session mode. Closeout ALL GREEN (engine 1531 / app 388 / lint 0 / doc 11). Runtime behaviour (preset variety, no cycling/lockup) is Matt's live-playlist validation gate — `VisualizerEngine` isn't constructible in the app-test sandbox.

## [dev-2026-06-19-153439] CLEAN.6.2 — minNoveltyFloor recalibrated 0.02→0.01 (BUG-042, diagnosed against a live SLTS capture)

Matt's live test of the section-scale fix (Smells Like Teen Spirit, session `2026-06-19T14-50-27Z`) caught what the 30 s tempo-fixture replay structurally couldn't: the fix was **over-conservative** — 1 boundary in 5 min at confidence 0, so the orchestrator never switched presets on structure. Diagnosed offline by sweeping `minNoveltyFloor`/`thresholdMultiplier` over the session's `raw_tap.wav` through the production FFT→MIRPipeline path (the `StructuralSectionScaleReplay` diagnostic gained a `PHOSPHENE_REPLAY_WAV` sweep mode; `MIRPipeline` gained a default-nil `structuralAnalyzer` injection; `StructuralAnalyzer`/`NoveltyDetector` gained `minNoveltyFloor`/`thresholdMultiplier` pass-through — all default-preserving). Result: floor **0.02 → 1 boundary, conf 0**; floor **0.01 → 9 sections at conf 0.64** at musically real times (26 s = the intro→verse drop, then ~every 20–50 s); 0.005 starts adding near-duplicates. Root cause: 0.02 was sized for the noisy *pre*-decimation stream; the 0.5 s decimation smooths it so real-section novelty peaks land at ~0.005–0.02, so 0.02 gated out every real section. **Recalibrated to 0.01** (Matt's call: land it + re-test). Re-verified: 19 structural unit tests green (incl. the evolving-no-boundary guard); 3 tempo fixtures stay junk-free (0/0/1 on 30 s). Correction to `[dev-2026-06-19-032321]`'s "no recalibration needed" — the 30 s clips couldn't show false-negatives; a full track did. BUG-042 stays Open pending Matt's live re-test (does SLTS now switch on its sections; does 0.01 hold on other material — calibrated on one track).

## [dev-2026-06-19-032321] CLEAN.6.2 — structural stream re-expressed at section scale (BUG-042 fix landed, pending validation)

BUG-042: the structural analyzer's geometry was note-scale. At the live ~94 Hz feed (48 kHz / 512-hop), the 600-frame ring held 6.4 s of memory, the 8-frame checkerboard was 85 ms, and `minPeakDistance` 120 = 1.28 s — so it emitted a "section" every ~1.5 s, and post-BUG-040 the even spacing pushed confidence to 0.85–1.00, flowing junk through the gates. Matt approved collapsing instrument+diagnose into the kickoff report: root cause is fully characterised from the code + the on-record real-capture measurement (no on-disk session still carries the section columns or the 16-dim structural input), so recalibration is necessarily a separate real-capture-gated step.

**Fix.** `StructuralAnalyzer.process` now mean-aggregates raw frames into one structural frame every 0.5 s (2 Hz, **time**-bucketed → feed-rate-independent, which kills the 60-vs-94 ambiguity for good) before the similarity matrix. The unchanged frame-denominated constants then mean section-scale things: 600-frame ring = 5 min, 8-frame kernel = 4 s; `minPeakDistance` retuned 120 → 16 (8 s minimum section); the decimated rate (exactly 1/bucketPeriod) timestamps boundaries, so the FPS estimate is gone. This is the bug entry's proposed direction and the *only* computationally viable one — the checkerboard is O(halfWidth²) per curve point, so a 4 s block at 94 Hz (halfWidth ≈ 188) is infeasible; decimation keeps halfWidth = 8. Skein's BUG-046 10 wall-s guard is untouched (harmless after the fix); the orchestrator's ungated `StructuralPrediction` consumer (ReactiveOrchestrator / LiveAdapter / TransitionPolicy, all gate at confidence ≥ 0.5 with no spacing guard) is the one this actually fixes.

**Tests re-expressed at the new rate** (note-scale fixtures no longer register a section): StructuralAnalyzer — BUG-035 ring-wrap (shrunk ring so a decimated stream still wraps), BUG-040 timestamps (15 s sections, tolerance widened for the 4 s kernel), prediction/confidence (15–20 s sections) — plus the MIRPipeline BUG-040 test (lengthened past the 8 s edge guard). The "evolving material registers nothing" fixture became a monotonic linear drift: the pre-fix incommensurate sinusoids had a ~25 s slowest period, which the section-scale detector *correctly* found as 3 sections (good evidence the re-expression works). The 60 KB note-scale `aaba_structural_fixture.json` was replaced by a 6-line inline section-scale AABA (boundaries at 40 s / 60 s). DSP suite 23/23 + MIRPipeline structural green; swiftlint --strict 0.

**Pending validation (real-capture-gated, FA #27 — no synthetic).** (1) Recalibrate `minNoveltyFloor` (interim 0.02, calibrated on the pre-decimation stream) against a real session feature stream; (2) Matt's live read that preset-switching lands on real musical sections. Code-complete on the worktree branch — needs to reach `main`/primary for the live read. BUG-042 stays Open until Matt validates.
