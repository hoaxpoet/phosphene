...
**NOT yet Resolved.** This is the live login path — **manual E2E (Matt: log in → playlist loads) is required** before flipping the `KNOWN_ISSUES` AUDIT-2026-06-09 OAuth items to Resolved. Unit tests prove the mechanism; only a real login proves end-to-end auth. The branch must reach an integrated build first (worktree). Push pending Matt's "yes, push." Remaining Phase 2: CLEAN.2.3 (wire-or-hide dead UI), 2.4 (entitlement review).
