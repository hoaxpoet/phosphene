...
- **Registered presets** — Aurora Veil (SR.1) and Murmuration (MM.3, route specs only in `MurmurationRoutes.swift`). Murmuration has no `<Preset>Rubric.swift` yet, so a `--preset murmuration` run reports route firing + motion-band analysis (both preset-agnostic) but the rubric section still uses the Aurora Veil question set; do not pass `--references-dir` for Murmuration until its rubric lands. Other presets need their own `<Preset>Routes.swift` (+ `<Preset>Rubric.swift` for visual grading).
