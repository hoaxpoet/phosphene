...
| GPU emergent boids flock (grid-binned, audio-coupled) | Supported | `Sources/Renderer/Geometry/MurmurationFlockGeometry.swift`; `Shaders/MurmurationFlock.metal` | Murmuration's flock engine (D-097 sibling): reset→bin(atomic)→boids over a 3×3×3 grid neighbourhood + guide-segment roost + banking, 3D→screen. **MM.3 audio coupling (`computeAudio`)**: L1 bass drift/elongation, L2 drum curl-wave (darkening band on `pad0`), L4 mid edge flutter, L5 vocals breathing — all D-026 deviation primitives, inert at silence. `FlockParams` 144 B (setBytes per frame, MSL mirror). `activeParticleFraction`-governed. Verified via `MurmurationFlockAudioTests` (real dispatch path). M7 cert pending (MM.5). |
