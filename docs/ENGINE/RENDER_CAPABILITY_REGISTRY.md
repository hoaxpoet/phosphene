...
| GPU orientation-flocking engine (Flock2 port, grid-binned, audio-coupled) | Supported | `Sources/Renderer/Geometry/MurmurationFlockGeometry.swift`; `Shaders/MurmurationFlock.metal` | Murmuration's flock engine (D-097 sibling). **MM.6 REBUILD** on Hoetzlein's **Flock2** orientation model (faithful MSL port of `advanceOrientationHoetzlein`/`findNeighborsTopological`/libmin quaternion): reset→bin(atomic)→`murmuration_boids`, topological-7/240°-FOV neighbours, 4 heading rules → reaction-limited quaternion control → faithful **metre-space** aero (lift/drag/thrust/gravity, source constants) → dynamic-stability realign; metres→clip at render; banking darkening = wing-area-to-camera (`|up.z|`). Travelling dark bands **emerge** (not injected). 64 B `MurmurationBird` (quaternion+target), 208 B `FlockParams` (setBytes per frame, MSL mirror). Density-invariant scaling (`cbrt(count)`). **MM.6 audio coupling (`computeAudio`) — global-envelope, not per-bird**: L1 bass drift + envelope elongation, bar-anchored heading-swing maneuver (downbeat-triggered, energy-gated, drum-modulated; banking wave emerges), L5 vocals vertical dilation — all D-026, inert at silence. `activeParticleFraction`-governed. Verified via `MurmurationFlockAudioTests` (real dispatch path; subtle routes via settled-equilibrium + multi-bar averaging). M7 live cert pending (MM.5). **Replaces the MM.2/MM.3 force-boids substrate (FA #73).** |
