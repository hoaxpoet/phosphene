...
| GPU procedural particle system (compute + render, UMA buffer) | Supported | `Sources/Renderer/Geometry/ProceduralGeometry.swift`; `Shaders/Particles.metal` | Murmuration flock path (preset files renamed `Starburst.*` → `Murmuration.*` in MM.0). `activeParticleFraction` scales dispatch under the budget governor. Conformer/engine-shader rename (`ProceduralGeometry` → flock-specific) deferred to MM.2's flock redesign. |
