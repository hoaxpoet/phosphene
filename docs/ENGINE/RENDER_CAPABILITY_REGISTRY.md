...
| Audio-modulated thin-film thickness (matID == 2 / matID == 3 metallic substrate, baseline ± arousal × range) | Supported | V.9 Session 4 Phase B: matID == 2 and matID == 3 branches in `Shaders/RayMarch.metal` now compute `kFerrofluidFilmThicknessNm = baseline + clamp(features.arousal, -1, 1) × range`; MSL constants 220 nm baseline / 40 nm range match the production `FerrofluidOcean.json` `ferrofluid` block defaults via `PresetDescriptor.FerrofluidParams`. Visual test gates: `testFerrofluidOceanMoodTintAtmosphereShifts`, `testFerrofluidOceanMoodTintIBLPropagation` (both unaffected — thickness modulation is upstream of `rm_finishLightingPass`). | Range [180, 260] nm stays inside the subtle blue-to-cyan band (SHADER_CRAFT §10.3.x); the rainbow oil-slick failure mode lives above ~300 nm. Reusable by any preset adopting the matID == 2 stage-rig path or matID == 3 single-light thin-film fallback. |
| Cassie-Baxter spike-tip droplet SDF (per-Voronoi-cell hemispherical beads composed via `op_smooth_union` with the height-field surface SDF) | Supported (preset-private) | V.9 Session 4 Phase A: `Presets/Shaders/FerrofluidOcean.metal` `fo_droplet_sdf(p, fieldStrength, dropletStrength, mesoStrength, swellHere)` with radius 0.04 × dropletStrength, apex-fraction 0.6 × radius; called from `sceneSDF` via `op_smooth_union(surfaceSdf, dropletSdf, 0.04)`. | Cell-center xz approximated as `p.xz` (cell width 0.25 m, camera distance 4–14 m — error bounded by `v.f1`). Apex Y = swell + fieldStrength × 0.15. Phase B strength `clamp(2.0 × max(0, f.bass_att_rel), 0, 2)` collapses to 0 at silence (silence-state semantics: no beads on calm body). Preset-private at V.9 Session 4; promote to `Utilities/Geometry/` if a second consumer ships. |
| Meso domain-warp turbulence on Voronoi spike lattice (2-octave fbm offset on sample position) | Supported (preset-private) | V.9 Session 4 Phase A: `Presets/Shaders/FerrofluidOcean.metal` `fo_meso_warp(xz, strength)` (2 × `fbm4` at scale 2.0, amplitude 0.15); applied inside `fo_ferrofluid_field` before Voronoi sampling. | SHADER_CRAFT §3.4 application. Phase B strength `clamp(0.5 + 1.5 × max(0, f.mid_att_rel), 0, 2)` — baseline 0.5 silence-state turbulence (never zero per §5.8). Preset-private; lift to `Utilities/Noise/` if a second consumer ships. |
| Tactile micro-normal perturbation in matID == 2 lighting branch (`noiseFBM` gradient, never audio-modulated) | Supported (Ferrofluid Ocean only) | V.9 Session 4 Phase A: `Shaders/RayMarch.metal` matID == 2 branch samples `noiseFBM` (texture(7)) at three offset world.xz positions and derives a tangent-space normal vector applied at amplitude 0.02 to the G-buffer normal before BRDF consumption. | NEVER audio-modulated — substrate's intrinsic tactile identity per §5.8 silence-state semantics. Scale 15 wraps the 256×256 noise texture so visual repetition is < 7 cm world-space. Failed Approach #44 guard: no Metal type-name shadowing in the local variable names. |
