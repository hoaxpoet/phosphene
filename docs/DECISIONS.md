...
| D-188 | Accepted | Truchet Loom breakup polish (PG.4.3, scoped) — the §A2 "breakup" layer, held to the parts that don't pre-empt Matt's live M7: (1) the per-path hue-team block edges are **domain-warped** by a cheap single-octave value noise (`tl_vnoise`) so team boundaries WANDER organically instead of snapping to a hard square grid (fixes the PG.4.2 cosmetic defect); (2) a **subtle static paper grain** (screen-anchored, ±0.014) on the §A2 breakup scale. **Perf lesson:** the first cut used `fbm4` (4-octave perlin3d ×2/pixel) for the warp and blew p99 to 8.87 ms (over the 8 ms budget) — replaced with a 4-tap value-noise (~8× cheaper), restoring p95 ≈ 2.2 ms. **Explicitly HELD for post-M7** (surfaced to Matt): deeper nesting toward cap 4 (Matt chose Restrained/cap 3 at PG.4.1 review; "Deep" was gated behind "if peaks feel underwhelming at M7") and the curl-warp organic-flow motion (a felt-motion change). Golden regenerated (procedural/deterministic). `certified:false`. D-188 is free on `main` + the rmenv branch (no collision, unlike D-186/D-187). |
| D-187 | Accepted | Truchet Loom rhythm + colour (PG.4.2) — three routes added to the PG.4.1 weave, each on its own primitive/layer (FA #67). **Per-beat tile flips**: a bounded (~22 %) hash-selected subset re-route their arc each beat, seeded by a new monotonic `beat_index` counter (SpectralHistory slot 3391, incremented CPU-side on each `beat_phase01` sawtooth wrap) so the re-routing EVOLVES beat to beat; crossfaded over `beat_phase01` (animates, no pop); gated by `pulse_amp01` → silent at cold-start/silence (cold-start phase contract). Orientation-swap keeps per-cell ink steady → measured beat luminance swing 0.0055 (D-157, not a strobe). **Per-path hue teams** ← `spectral_centroid`: coarse spatial blocks get a quantised hue TEAM (coherent along paths → coloured ribbons, not per-cell rainbow), the whole set phased by centroid. **Bounded path glow** ← `bass_dev`: subtle additive glow on the freshly-subdivided (high-depth) ribbons; drop-able. `beat_index` follows the D-186 reserved-slot pattern (no new binding). Golden regenerated (hue-team luma shift; geometry byte-identical, flips gated off in the fixtures). Automated gate now 3/4 (in-source `bass_dev` makes L2 pass). `certified:false` — PG.4.3 (deeper nesting/polish) remains. Preset count unchanged (28). |
| D-186 | Accepted | Truchet Loom added (PG.4.1) — a `direct`-pass multiscale curved-Truchet op-art weave (family `geometric`, PG psychedelic-geometry phase). **Density-mapping hero**: a smoothed `spectral_flux` sets a continuous global subdivision level, so busy passages shatter the weave into nested sub-tiles and sparse passages merge them into large sweeping arcs — musical busyness rendered as geometric complexity. Ported (not derived, FA #73) from IQ's canonical two-quarter-arc Truchet SDF + Carlson's ½-scale recursive rule (Shadertoy 4t3BW4, offline-unretrievable → authored from the published rule). Smoothing lands via a new single-float `flux_smoothed` EMA slot (idx 3390) in `SpectralHistoryBuffer` (reused reserved region; read directly, no new binding). Drift ← `arousal` speed on an `f.time` baseline (alive at silence, D-037). `certified:false` — reviewable v1; per-beat tile flips + per-path hue teams are PG.4.2. Preset count 27 → 28. |
...
## D-186: Truchet Loom — multiscale curved-Truchet density-mapping weave (PG.4.1, 2026-07-20)
...
## D-187: Truchet Loom rhythm + colour — per-beat flips, hue teams, glow (PG.4.2, 2026-07-20)

**Status.** Accepted. Builds on D-186 (PG.4.1). `certified: false`; PG.4.3 (deeper nesting, chromatic/grain/AA polish, possible curl-warp) remains.
...
1. **Per-beat tile flips (rhythm).** On the cached-grid beat, a bounded ~22 % hash-selected subset of tiles re-route their Truchet arc. To make the subset EVOLVE beat to beat (not oscillate the same tiles), a new **`beat_index`** monotonic counter is added to `SpectralHistoryBuffer` (slot 3391), incremented CPU-side on each `beat_phase01` sawtooth wrap — same reserved-slot pattern as the D-186 flux EMA (no new binding). The flip crossfades over `beat_phase01` so it animates rather than pops, and is gated by `pulse_amp01` so cold-start/silence (where grid phase may be wrong) show the static weave. An orientation swap preserves per-cell ink, so global luminance stays steady — **measured beat luminance swing 0.0055** (D-157: bounded footprint + steady luminance, not a strobe).
...
**References.** D-186 (PG.4.1 base + reserved-slot pattern), [D-026], [D-028] (`beat_phase01`), [D-037], [D-154]/[D-157] (beat-locked motion constraints), [D-180] (route manifest), FA #67 (one primitive per layer), `docs/presets/psychedelic_geometry/PG_4_TRUCHET_LOOM.md` §A3/§A4/§A9.

---

## D-188: Truchet Loom breakup polish — organic hue boundaries + grain (PG.4.3, scoped; 2026-07-21)
...
**References.** D-186/D-187 (PG.4.1/4.2), [D-037], `docs/SHADER_CRAFT.md` §3 (noise) / §9 (perf budgets), `docs/presets/psychedelic_geometry/PG_4_TRUCHET_LOOM.md` §A2 (breakup scale) / §A9.
