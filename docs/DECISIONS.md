...
## D-124 — V.9 Ferrofluid Ocean redirect: "ferrofluid replaces ocean" framing

**Status:** Accepted (2026-05-13)

### Context

The pre-redirect V.9 spec (`SHADER_CRAFT.md §10.3` as authored under D-064) framed Ferrofluid Ocean as "lab-scale Rosensweig instability rendered iconically" — a hex-tile spike lattice with bass-driven field strength, single-warm-key + strong-IBL lighting per §5.2, distant purple fog, and caustic underlighting from below the surface. The visual reference set was curated against that framing under D-065 with 11 images, including a Reynisfjara horizon shot (`01_macro_horizon_dark_coast.jpg`) and a Salar de Uyuni mirror-horizon shot (`01b_macro_mirror_horizon.jpg`) anchoring the macro scale to a landscape-vista framing, plus a Chicago Cloud Gate shot (`08_lighting_warm_key_dark_metal.jpg`) anchoring the lighting recipe to §5.2.

Design conversation with Matt (2026-05-13) revealed that the pre-redirect framing was a drift from the original intent of Ferrofluid Ocean. The intended concept is closer to "a fixed camera zoomed in to a portion of an ocean where the water has been replaced by ferrofluid material" — meso-to-macro scale, not horizon vista; the body of liquid moves with audio-driven Gerstner swell underneath the spike lattice (not just spike height modulation); the chromatic story lives in a moving multi-light stage-rig (not in fog tint and IBL alone); the silence state is a calm dark body of liquid (not a "10% baseline lattice"). This is structurally a D-071-class amendment — spec drift from references is the inverse failure mode, but the corrective is the same shape: rewrite the spec anchored to a refreshed reference set, ensuring the two are in sync before the next Claude Code session lands.

### Decisions

**(a) `SHADER_CRAFT.md §10.3` is rewritten to anchor on the new framing.** The macro scale moves from horizon-vista to ocean-portion. Macro Gerstner swell displacement is added as a new trait composed underneath the spike lattice. The lighting recipe moves from §5.2 (single-directional + strong IBL) to a new §5.8 stage-rig recipe (4–6 animated colored lights in slow orbital motion). Caustic underlighting from below the surface is removed entirely. Thin-film interference (via `thinfilm_rgb` from `Utilities/PBR/Thin.metal`) is **promoted from §12.3 catalog-wide strongly-preferred to per-preset mandatory in the Ferrofluid Ocean README** under the redirect — filling the third-material slot vacated by caustic underlighting's removal. The catalog-wide §12.1 mandatory list is unchanged; this is a per-preset hardening, not a §12 amendment. The performance ceiling is raised from 6.0 ms to 7.0 ms p95 at Tier 2 1080p to absorb the multi-light evaluations and Gerstner additions.

**(b) `docs/VISUAL_REFERENCES/ferrofluid_ocean/` is amended to match.** Four references are retired (`01_macro_horizon_dark_coast`, `01b_macro_mirror_horizon`, `08_lighting_warm_key_dark_metal`, `09_lighting_caustic_underglow_cyan`); four are added (`01_macro_ferrofluid_at_swell_scale`, `02b_meso_swell_motion_dark_water`, `02c_meso_excited_water_specular`, `10_silence_calm_body`); one new reference (`08_lighting_aurora_over_dark_water`) takes the slot vacated by Cloud Gate; one existing reference (`04_specular_razor_highlights`) is elevated to a dual-anchor role for both specular character and the new stage-rig lighting. The retained 7 references (`02`, `03`, `03b`, `04`, `05`, `06`, `07`) carry forward with annotations preserved or surgically amended. Final folder count: 12 images (within the D-065 §(a) composite-preset allowance).

**(c) The Anish Kapoor / Cloud Gate IP carve-out is sunset.** The retirement of `08_lighting_warm_key_dark_metal.jpg` removes the only depicted copyrighted artwork from the V.9 reference set. The IP note in the prior README is dropped.

**(d) Audio routing is extended with four new continuous routes.** Gerstner swell amplitude ← arousal (baseline) + `stems.drums_energy_dev` (accent); beam orbit speed ← arousal; beam color rotation ← `stems.vocals_pitch_hz` (normalized inline; confidence-gated at ≥ 0.6) with `stems.other_energy_dev` fallback for instrumental tracks; beam intensity ← `stems.drums_energy_dev` envelope. The pre-redirect caustic-underlighting route from `f.spectral_centroid` is removed. The swell-amplitude and spike-height routes are **independent in the routing domain** (different feature sources) — calm-body-with-spikes (low arousal/drums + high bass) and agitated-body-without-spikes (high arousal/drums + low bass) are both reachable states. Note that on most kick-and-bass-driven music `bass_energy_dev` and `drums_energy_dev` are partially correlated, so audio-domain independence is weaker than routing-domain independence; the contract holds in principle but the two extreme states may be rare on real music. This is intentional — a future fidelity uplift should not collapse the routing into a single shared envelope to "tighten" the visual.

**(e) Stage-rig motion is envelope-driven, never onset-driven.** Beam orbit speed, beam color, beam intensity, and swell amplitude are all routed through `*_energy_dev` deviation primitives and arousal smoothing — never `*_beat` rising edges. This is the same rule already in force for spike height (per the original §10.3) and is restated here to prevent beam motion from being beat-strobed under future authoring drift. "Stage lighting reads as a club rig" is added to the anti-reference list to make the failure mode named.

### Reason

The pre-redirect framing was internally coherent but produced a visual destination that doesn't match the preset's stated intent ("ferrofluid replaces ocean"). The composite-preset reference structure under D-065 made the drift legible — the references that needed retirement (horizon vistas, Cloud Gate IBL, underwater caustics) were teaching traits the redirect doesn't want, and the references that needed adding (swell motion, beam-as-stage-rig, silence-as-calm-body) had no slot in the prior framing. Rewriting §10.3 against the refreshed reference set first, then running V.9 sessions against the corrected pair, is the D-071 pattern applied prophylactically rather than after an M7 failure.

### What was rejected

- Keeping the pre-redirect spec and treating the redirect as a future v3. Would force V.9 to ship a visual destination Matt does not want, requiring a v3 cycle immediately. Cheaper to land the redirect now.
- Layering moving beams over the pre-redirect single-key + IBL recipe. Two lighting paradigms operating on the same surface compound rather than compose; the single warm key would compete with the orbital beam rig for highlight ownership on each spike. The §5.8 recipe replaces §5.2 entirely for this preset.
- Retaining caustic underlighting as a subtle depth cue alongside the new beam rig. Two chromatic light sources on the same surface (beams from above, faint cyan from below) compound rather than compose. The Gerstner swell motion is sufficient to imply depth without underglow; the beam rig owns the chromatic story end-to-end.
- Sub-letter-suffixing all new references onto existing slots (e.g. `01c`, `01d`) instead of retiring `01`/`01b`. Would compound rather than replace; reading sessions would have multiple conflicting macro-scale references where one suffices. Retire-and-replace is cleaner.
- Coupling Gerstner swell amplitude to `bass_energy_dev` (same feature that drives spike height). Physically wrong (swell and spikes are independent mechanisms in the metaphor — environmental ocean motion vs. magnetic field response) and would collapse the calm-body-with-spikes / agitated-body-without-spikes state space into a single correlated routing axis. Arousal-baseline + drums-accent preserves the routing-domain independence per §(d).
...
- **Sample rate:** LF 44100 Hz (file native), tap 48000 Hz (system default). Unavoidable structural delta; FFT bin width scales with the rate ratio, which shifts `spectralCentroid` by ~22 % in normalized units (LF 0.087, tap 0.068) and propagates downstream into `MoodClassifier` outputs (valence +34 %, arousal -38 %). Cross-path *absolute* mood comparison is NOT valid; cross-path *relative* mood comparison within a session IS valid.
...
## D-129 — LF.2 dispatch model: blocking pre-analysis, in-memory cache only (LF.2, 2026-05-27)

**Status:** Accepted (2026-05-27).

**Context.** LF.1 proved local-file playback works end-to-end via `AVAudioEngine` (D-128). LF.1.5 characterized the LF path's analysis output vs the process-tap path on the same audio — load-bearing musical metrics agree; structural deltas all explainable. LF.1's known follow-up: the live `BeatGrid` was still installed ~10 s into the track by the live Beat This! analyzer after AGC convergence, leaving beat-driven preset accents wrong-phase for the first ~10 s. This is the same warmup gap the streaming path closes via `SessionPreparer.analyzePreview` on the 30 s preview clip — but LF.1 had no equivalent.

**Decision.** A new `@MainActor async` entry point `VisualizerEngine.prepareAndStartLocalFilePlayback(url:)` runs `SessionPreparer.analyzePreview(...)` on the file PCM BEFORE the audio router starts. The cached `BeatGrid` + `StemFeatures` are stored in `StemCache` with a synthetic `TrackIdentity` (`spotifyID = "local:" + url.path`); `resetStemPipeline(for:caller:)`'s existing cache-hit branch installs both into the live pipeline via `MIRPipeline.setBeatGrid(...)` + `pipeline.setStemFeatures(...)`. The audio router is started only after pre-analysis completes — UI sees ~2 s blank between env-var hook firing and first rendered frame.

Three design choices baked in:

1. **Blocking pre-analysis dispatch.** Pre-analysis runs inside `Task.detached(priority: .userInitiated)`; the LF.2 method `await`s the result before starting the router. Rejected alternative: parallel pre-analysis (audio starts immediately; cache install happens mid-track ~2 s in). The blocking model gives the cleanest "BeatGrid installed at t≈0" closeout-gate evidence and matches LF.2's done-when bar. ~2 s UX latency is acceptable for the dev-only env-var hook scope.

2. **In-memory cache only.** The `CachedTrackData` for an LF track is stored in `StemCache` keyed by the synthetic identity for the lifetime of the process. No disk persistence. Persistent content-keyed caching (so a second launch of the same file skips re-analysis) is LF.3 territory.

3. **Re-use of `SessionPreparer.analyzePreview` (now `public`).** The LF.2 path drives `analyzePreview` directly, bypassing the higher-level `SessionPreparer.prepare(tracks:)` orchestration (LF.2 is single-file + ad-hoc, no playlist, no Network I/O — fields like `PreviewResolver` / `PreviewDownloader` do not apply). Visibility change: `analyzePreview` is now `public` rather than `internal`. The function is already designed as a self-contained pure function (nonisolated, static, no captures); exposing it surfaces it for both the streaming path (existing caller via `SessionPreparer.prepareTrack`) and LF.2 (new caller via `VisualizerEngine.prepareAndStartLocalFilePlayback`).

**The "full-track" framing is structurally aspirational at LF.2 scope.** The underlying analyzers have fixed window limits:

- `StemSeparator.separate(...)` silently truncates to `requiredMonoSamples = 440320` (~10 s at 44.1 kHz) via `padOrTruncate`. The Open-Unmix HQ MPSGraph has a fixed window; no tiling.
- `BeatThisModel.predictCore(...)` clamps to `tMax = 1500` frames (~30 s at 22050 Hz / hop 441). Fixed transformer input length; no sliding-window aggregation.

`SessionPreparer.analyzePreview` accepts full-file PCM and the analyzers silently process the first 10/30 s. The MIR pass (`analyzeMIR`) iterates vDSP FFT over the full input and IS sample-count-agnostic.

The LF.2 win is therefore not "analyze the full track" — it is:

- **Same PCM bytes pre-analyzed AND played.** The cached `BeatGrid`'s phase is correct on the live audio by construction. The streaming path's preview-clip indirection (iTunes Search returns a different recording for each track) is the source of Beat This!'s cross-capture instability per BSAudit.2; LF.2 eliminates that entirely for local files.
- **Pre-analysis happens before audio starts.** The BeatGrid + StemFeatures are installed at session start, not ~10 s in. `BEAT_GRID_INSTALL: source=preparedCache` appears in session.log line 5 (before `raw tap capture started`), vs the LF.1 baseline where `source=liveAnalysis` appears at line 8 (~5 s after `signal quality → green`).
- **`stems.csv` shows non-zero stem features from frame 0** (the cached `StemFeatures` snapshot is installed via `pipeline.setStemFeatures(cached.stemFeatures)` before any audio flows).

True full-track stem + beat analysis would require StemSeparator tiling (concatenate per-tile outputs into a full-track stem waveform) and Beat This! sliding-window aggregation. Both are explicitly out of LF.2 scope; they would be LF.3+ work if a need arises (e.g., per-section-aware orchestration on long local files).

**Format coverage.** `LocalFilePlaybackFormatCoverageTests` (engine swift-testing) exercises the decode + offline-analysis surface against three formats: M4A/AAC, MP3, FLAC. Gated by `LF_FORMAT_COVERAGE=1` (matches the `SOAK_TESTS=1` opt-in pattern). Each test decodes via `PreviewAudio.fromLocalFile(at:)` (the new public Session-module entry point), runs the result through `SessionPreparer.analyzePreview` with real ML deps (`StemSeparator`, `StemAnalyzer`, `MoodClassifier`, `DefaultBeatGridAnalyzer`), and asserts non-empty BeatGrid + finite StemFeatures. WAV is descoped — `AVAudioFile` decode for raw WAV is already covered by the `PreviewDownloader` magic-byte path. Fixtures created by transcoding `love_rehab.m4a` via `afconvert -f flac -d flac` (FLAC) and `ffmpeg -codec:a libmp3lame -b:a 192k` (MP3) — both .gitignore'd under `PhospheneEngine/Tests/Fixtures/tempo/` per the existing pattern.

**Rejected alternatives.**

- **Parallel pre-analysis dispatch** (audio starts immediately; cache install happens ~2 s in). Cleaner UX but the closeout gate "BeatGrid installed at t≈0" requires blocking. Adoptable later if UX feedback demands it.
- **Truncate the LF pre-analysis input to 30 s explicitly** rather than passing full-file PCM through analyzePreview. Cosmetically cleaner but no different in outcome — the analyzers truncate either way.
- **Expand StemSeparator + Beat This! windows to handle full tracks** (tiling + sliding-window aggregation). Significantly larger scope; would touch ML dispatch budgets (16 stem-sep tiles for a 3-minute song = ~2.3 s of additional GPU work per pre-analysis). Probably its own increment if a downstream need arises.
- **Surface the synthetic `TrackIdentity` as a first-class concept (e.g., new `TrackIdentitySource` enum case).** Premature. The synthetic identity's only consumer is `StemCache.store(_:for:)` + `loadForPlayback(track:)` — both are key-based and don't introspect the identity's provenance. A first-class concept becomes worth introducing when (a) the UI needs to render local-file tracks differently from streaming tracks, or (b) the orchestrator needs to behave differently for them. Neither is in LF.2 scope.
- **Add a `localFilePlayback` case to `ResetStemPipelineCaller`.** Considered. The enum already has a `.other` case documented as "any future call site" — exactly the LF.2 use case. A dedicated case would clarify the wiring-log breadcrumbs slightly but adds enum-surface area without changing behaviour. Refile if a future increment needs the granularity.
...
- ID3 / Vorbis tag extraction, album-art display (LF.4).
- `SessionManager` integration (LF.4). LF.2 stays in ad-hoc / env-var-driven flow.
- Cross-fixture LF-vs-tap A/B comparison across multiple tracks. The LF.1.5 comparison is single-fixture and stays that way; LF.2's A/B is LF-before-vs-LF-after only.
- Stem-cache eviction policy / memory budgets. LF.3 / LF.4 work.
- Concurrency hardening of `tapSampleRate` propagation. Separate ongoing task (D-128 also notes this).
- Mid-track resumption on `AVAudioEngineConfigurationChange`. Still best-effort from beginning, per LF.1 / LF.4 deferral.
- Production telemetry / startup latency dashboards.

**Verification.**

- LF.1 regression gate green: `swift test --package-path PhospheneEngine --filter AudioInputRouterSignalStateTests` (11/11).
- LF.1.5 / SoakTestHarness gate green: `SOAK_TESTS=1 swift test --package-path PhospheneEngine --filter SoakTestHarnessTests` (7/7).
- New LF.2 format-coverage tests green: `LF_FORMAT_COVERAGE=1 swift test --package-path PhospheneEngine --filter LocalFilePlaybackFormatCoverageTests` (3/3 — M4A, MP3, FLAC).
- Full engine suite green: 1281/1281 tests passing.
...
- ~~Multi-file playlist semantics (LF.5).~~ **Done (LF.5 / D-132).**
- Crossfade / gapless segue (LF.5+ if user demand surfaces).
- ~~ID3 / Vorbis tag extraction (LF.5).~~ **Done (LF.5 / D-132).** Album-art display still deferred — data captured, UI is LF.6.
- ~~"Recent Files" submenu (LF.5).~~ **Done (LF.5 / D-132).**
- Settings UI for the cache-size cap.
- Per-track cache invalidation UI.
- Streaming-path persistent cache.
- ~~File-association handling.~~ **Done (LF.5 / D-132 — `CFBundleDocumentTypes` + `.onOpenURL` extension).**
...
## D-132 — LF.5 multi-file source model + recents + file-association + schema v2 (LF.5, 2026-05-28)

**Status:** Accepted (2026-05-28).

**Context.** LF.4 (D-131) graduated local-file playback to a first-class user-facing feature on the macOS app surface — `File → Open Local File…`, ⌘O, drag-and-drop, SessionManager-driven lifecycle, LRU eviction, cache-clear menu. The whole arc shipped against a single load-bearing operation: **one file at a time**. LF.5 lifts the ceiling — folder ingest, M3U playlist ingest, multi-file drag-and-drop, file-association handling (double-click `.m4a` in Finder), and an "Open Recent" submenu for one-click resume. ID3 / Vorbis / MP4-atom metadata extraction surfaces real song titles instead of filenames. Mid-session track transitions advance the orchestrator's per-track preset selection in the same shape the streaming path uses.

**Decision.** Six coordinated changes:

1. **`SessionOrigin` extends with three multi-file cases.** `.localFiles([URL])` (flat drag-and-drop list), `.localFolder(URL, expanded: [URL])` (folder pick — `URL` is the originally-selected root, `expanded` is the recursively-walked alphabetical audio file list), `.localPlaylist(URL, expanded: [URL])` (M3U file — `URL` is the `.m3u` file, `expanded` is the parsed track URL list). All four LF origin shapes (`.localFile`, `.localFiles`, `.localFolder`, `.localPlaylist`) expose `allLocalFileURLs` (full queue) + `localFileURL` (first / current head) + `isLocalFile` (true). The shape difference matters at the UI layer (folder name vs M3U filename vs flat track count) and at the audio-router layer (multi-file → EOF advance, single-file → loop).

2. **`SessionManager.startLocalFiles(at:origin:)` is the canonical LF API.** Walks the URL list sequentially via the `LocalFilePreparing` delegate (typically `VisualizerEngine`), populates `preparingTracks` with one placeholder identity per URL, transitions `.idle/.ended → .preparing → .ready` on completion. LF.4's `startLocalFile(at:)` is now a thin wrapper: `await startLocalFiles(at: [url], origin: .localFile(url))`. The 14 LF.4 lifecycle tests pass unchanged through the wrapper; the 13 new LF.5 lifecycle tests cover multi-file queue length 3, per-origin source discrimination, same-origin no-op, different-URL replace, empty-list no-op, no-preparer fallthrough, and mid-queue cancellation. Same-origin re-entry detection uses `SessionOrigin == ` (the multi-file equality compares both root URL + expanded list).

3. **`SessionPreparer.prepareLocalFiles(urls:placeholders:via:)` worker.** New method mirrors the streaming-path `prepare(tracks:)` shape but takes a URL queue + a `LocalFilePreparing` delegate. Walks the URL list sequentially, calls `delegate.prepareLocalFile(url:)` per file, publishes `trackStatuses` transitions (`.analyzing(.stemSeparation) → .analyzing(.caching) → .ready` on success; `.partial(reason:)` on delegate nil result), advances `progress: (completed, total)`. SessionManager observes the same publishers `PreparationProgressView` already binds to for streaming-path sessions. Cancellation honoured at file boundaries via the existing `preparationTask` cancellation handler.

4. **`M3UParser` (engine module).** Defensive `.m3u` / `.m3u8` parser. Tolerates UTF-8 BOM, CRLF + LF line endings, comment lines (`#EXTM3U` / `#EXTINF` / plain `#`), absolute paths, `file://` URLs, and relative paths resolved against the M3U file's parent directory. Returns `ParseResult { urls, skippedLines }` so callers log `STEM_QUEUE_SKIP` per skip without the parser growing a `SessionRecorder` dependency. Three error conditions throw `ParseError`: `fileUnreadable`, `malformedUTF8`, `noEntriesResolved`.

5. **PersistentStemCache schema v2 + ID3 / Vorbis / MP4-atom metadata.** `PreviewAudio` gains async helpers `extractMetadata(at:)` / `extractArtwork(at:)` via `AVAsset.commonMetadata` (uniform across all three audio container formats). `LocalFileMetadata { title, artist, album }` is `Codable + Sendable`. Schema bumps to v2:

    - `metadata.json` gains optional `metadata: LocalFileMetadata?` field.
    - Optional sibling `artwork.bin` holds raw image bytes (PNG / JPEG — whatever the container embedded). `expectedFiles()` excludes it (artwork is non-mandatory).
    - `store()` signature: `metadata: LocalFileMetadata = .init()` + `artworkData: Data? = nil` defaults — streaming-path callers unaffected.
    - `load()` populates `PersistentStemCacheEntry { cached, decodedDuration, metadata, artworkData }`.
    - Schema-v1 entries throw `schemaMismatch` → caller re-prepares with v2. One-time ~2 s cost per cached track on next play; LF.4 user caches were small.

    On cache hit, `VisualizerEngine.runLocalFilePreparation` reconstructs the enriched `TrackIdentity` (title from metadata, filename fallback) so the cached source label surfaces real titles in the WIRING log.

6. **`File → Open Recent ▸` + file-association.** New `LocalFileRecentsStore` (app-layer, `@MainActor`, `@StateObject`) persists last 10 opens (`kind ∈ {file, folder, m3u}`) as JSON in `phosphene.lf.recents` UserDefaults. LRU-style move-to-front; defensive load truncates oversized persisted state. `RecentItem.isMissing` surfaces stale entries; the submenu renders these disabled with "(missing)" suffix; clicking removes them. `Info.plist` gains two `CFBundleDocumentTypes` entries (Audio File: m4a / mp3 / flac with UTTypes; Audio Playlist: m3u / m3u8). Both register `LSHandlerRank=Alternate` — Phosphene does NOT claim default opener; the user opts in via the Finder "Open With…" panel. `PhospheneApp.swift`'s `.onOpenURL` distinguishes `phosphene://` (Spotify OAuth, U.11) from `file://` (LF.5 file-association); the file-URL branch routes by extension into the same `openLocalFile / openLocalFolder / openLocalM3U` dispatch path the Recents submenu uses.

**Mid-session track transitions.** `LocalFilePlaybackProvider` gains an `@Sendable () -> Void onFileEnded` callback. When set, `scheduleFileLoop` invokes it INSTEAD of re-scheduling the file (LF.5 multi-file advance). When nil (LF.1 / LF.4 / single-file default), the file loops forever — preserves the dev-workflow env-var hook behaviour per Matt's audit answer. `AudioInputRouter.onLocalFilePlaybackEnded` relays the callback into the freshly-constructed provider at `start(mode: .localFilePlayback)` time. `VisualizerEngine.advanceLocalFileQueue()` pops the next URL + identity from `currentSource.allLocalFileURLs[currentTrackIndex + 1]`, stops the current router, installs the next BeatGrid via `resetStemPipeline(caller: .trackChange)`, restarts the router, bumps `currentTrackIndex`. Queue exhaustion → `sessionManager.endSession()` (matches streaming-path session-end UX → `EndedView`).

**Per-Matt audit (2026-05-27) decisions.**

- **Folder cap at 200 URLs.** Folder + multi-drop queues larger than 200 truncate with a localized NSAlert; the first 200 alphabetical files queue. Predictable resource use under the 500 MB cache cap (≈ 70 cached tracks). Larger folders means the user must pick smaller subsets for full coverage.
- **Single-file loops, multi-file advances.** Single-file queues (the env-var hook, `File → Open Local File…`, 1-track folders) continue the LF.1 loop-forever behaviour. Multi-file queues (folders with ≥ 2 files, M3U playlists, multi-file drag-and-drop) advance through tracks and transition to `.ended` on completion.
- **API consolidation.** `startLocalFile(at:)` becomes a thin wrapper around `startLocalFiles(at: [url], origin: .localFile(url))` — single entry point + single lifecycle. The 14 LF.4 tests pass unchanged through the wrapper.

**Rejected alternatives.**

- **`startLocalFiles(at: [URL])` without `origin:` parameter.** Loses the input shape (folder vs M3U vs flat drop) which UI labels + WIRING logs need.
- **Async metadata extraction on every cache hit.** Re-reads AVAsset.commonMetadata when loading from disk — defeats the purpose of caching. v2 schema persists metadata once; cache hit just reads the JSON envelope.
- **Multi-file env-var hook (`PHOSPHENE_LOCAL_FILES_PLAYBACK=...`).** Adds complexity to the env-var parser (colon-separated paths? newline-separated?) without a clear use case beyond CLI dev workflows. Deferred until someone asks for it.
- **`LSHandlerRank=Default` for audio types.** Hijacks the user's existing default opener (Music.app on macOS); the user must explicitly grant Phosphene-as-default via Finder if they want it.
- **Drag-to-reorder of the queue mid-session.** Tracks already play in fixed order; reordering would require pause / re-queue plumbing. LF.6+ territory if user demand surfaces.
- **Album-art display in `PlaybackView` chrome.** Data captured at LF.5 (sibling `artwork.bin`); UI surface is LF.6.
- **`.fpl` (Foobar2000) playlist files.** Binary format; would need a separate parser. Defer until someone asks.
- **In-app M3U editor / "Save current queue as M3U" export.** UX surface area for a niche use case.
- **Multi-file path inside the LF.4 `_completeLocalFileReady` helper.** Two parallel single-file / multi-file paths inside SessionManager would duplicate state-tracking. Consolidating into `startLocalFiles` with the LF.4 wrapper is the cleaner shape.

**Out of scope (deferred).**

- Crossfade / gapless segue between LF tracks (hard cuts at LF.5).
- Album-art display in PlaybackView (data captured; UI is LF.6).
- Per-track skip / next / prev controls in PlaybackView (UX-2 invariant — Phosphene doesn't control playback).
- Drag-to-reorder of the queue mid-session.
- Manual track removal from the queue (cancel + re-pick is the workaround).
...
- Multi-file env-var hook.
- Cross-machine library sync.
- Network-streamed files.
- Multi-process cache safety (Phosphene is single-instance).

**Verification.**

- All LF.1–LF.4 regression gates green (no regression past prior baselines).
- LF.5 new tests green: `SessionManagerLocalFileTests` (29/29 — incl. 13 LF.5 lifecycle + 2 LF.5 per-track-status observer), `M3UParserTests` (9/9), `PersistentStemCacheTests` (16/16 — incl. 5 LF.5 schema-v2), `LocalFileRecentsStoreTests` (12/12), `LocalFilePlaybackFormatCoverageTests` (4/4 — incl. LF.5 3-fixture queue).
- Full engine suite green: 1358 tests / 172 suites (LF.4 baseline 1328 + 30 net new LF.5 tests).
- Soak tests green: `SOAK_TESTS=1 swift test --filter SoakTestHarness` (7/7, 315 s).
- App suite green: full PhospheneApp test target passes.
- Sample-rate literal gate green: `Scripts/check_sample_rate_literals.sh` exit 0.
- Localized-strings gate green: `Scripts/check_user_strings.sh` exit 0.
...
## D-141 — Nimbus: reverse the "nothing on the beat" premise → stem-driven beat lobes (NB.5)

**Date:** 2026-06-05. **Status:** implemented (NB.5); pending Matt's live manual-validation sign-off. Supersedes the §1.3 "nothing on the beat" premise and the cut NB.5 (Pulse/embers) slot.

**Context.** NB.4 shipped Nimbus's energy-only model: one broadband-energy `bloom` follower swelling the body's size/brightness/flow, with *nothing on the beat* by design (the original §1.3, citing FA #4 / FA #33; per-beat embers were cut 2026-06-04 as "too much activity"). The first real-music test (the *Atlas* / Battles session `2026-06-05T14-35-14Z`, a relentless 136-BPM track) falsified the model.

**Evidence (the session).** (a) **Too subtle, and structurally floored.** `bloom` averaged three bands `(bass+mid+treble)`; on this bass-dominated capture mid energy meaned **0.04** and treble **0.004** (effectively dead), so the two silent bands vetoed the average — even a +2.25 bass-kick deviation pulled it only to ~0.08 → bloom ~0.55 (baseline). The body sat near floor-size the whole session; the video confirms a small, near-static blob. (b) **The beat is the unanswered signal.** `beatComposite` fired > 0.5 on **52.7 %** of frames; the grid locked (136 BPM, lock_state=2 on 69 %). (c) **The stems are the robust signal.** All four stem energy-deviations swing hard (drums 1.90, bass 2.09, vocals 2.84, other 2.40 peaks) — they carry the structure the 3-band FeatureVector lost.

**Decision (Matt's call: "wrong model — drive from the beat, per stem"; chose "one mass, heaves per-stem" over hard quadrants).**
1. **The band plays the body.** Four stem followers in `NimbusState` (`kickPunch`, `bassLobe`, `vocalsLobe`, `otherLobe`), each pushing a soft bulge of the **single** envelope (`rr / (1 + kick + Σ lobe·cos²)` — a star-convex deformation that **cannot fragment** into separate blobs, protecting the §1.4 one-mass identity). Drums = whole-body punch + brightness pop (the kick is the spine of the beat); bass heaves DOWN, lead/"vocals" flares UP, other swells to the SIDE. `NimbusStateGPU` 16→32 bytes.
2. **`bloom` re-sourced** from the broken 3-band average to the **mean of the four stem energies** (never floored by one dead band); stays the slow size/brightness/flow swell.
3. **FA #4 is honoured, not broken** — beat is an *accent* (the bloom is still the primary slow swell); Nimbus has no feedback loop to amplify jitter, the onset pulse is zero-delay, and a gas heave with a soft decay is forgiving of ±80 ms, so a prominent beat-punch is safe on this preset specifically.

**Perf lesson (durable).** The lobe falloff is evaluated ~7× per in-body march sample (primary + cone-shadow). The first cut used `pow(cos, 1.5)` and the budget **doubled** (2.66 → 5.15 ms); the GPU *predicates* (does not skip) the guard, so it was paid even at rest. `sqrt` → 4.58 ms; **cos² (pure mul-adds)** → 3.74 ms. **In a per-march-step function, never use `pow()`/transcendentals for a falloff — use a polynomial.** Full data: `NIMBUS_DESIGN.md §1.3 / §5.4 / §6.5`. Deferred to NB.6: per-track-distinct gas seeding + PresetSessionReplay registration.

## D-142 — Nimbus: energy warms the mood colour + the bright core keeps its hue (NB.10, M7 r1)

**Date:** 2026-06-05. **Status:** implemented (NB.10); pending Matt's live re-review (M7 round 2). Amends the NB.6 mood model (valence-only colour).

**Context.** Matt's first M7 (session `2026-06-05T18-26-37Z`, 7 tracks) would not certify. Two findings; this decision addresses the mood half (the beat half is D-143, a separate workstream). His notes: Billie Jean "devoid of mood colour (white/gray)"; Get Lucky "golden fading to neutral"; B.O.B. "purplish to begin — why? this is a very energetic track"; overall "mood colour is VERY subtle — room to make the colour more expressive."

**Evidence (the session).** The valence signal is *not* the problem — after the 4 s smoothing it travels well (Billie Jean colour-driver 0.26→0.73; Seven Nation Army 0.06→0.84). The colour wasn't reaching the screen: (a) the bright core **desaturated toward near-white** above a luminance threshold — exactly the brightest, most-visible pixels — so Billie Jean read "white/gray" even though its valence reached +0.47; (b) the poles were **muted** (greyed lavender / soft gold), so mid-valence tracks sat in a muddy middle; (c) colour tracked **valence only**, and the classifier hears B.O.B. (aggressive-but-dark) as **low-valence, high-arousal** (smoothed valence −0.58→+0.20) → cool/purple, contradicting the intuition that an energetic banger should look hot.

**Decision (Matt's call: "energy warms it too").**
1. **Warmth = valence + ENERGY.** A new `warm01` drives the colour: `valence01` lifted by `energy01 = 0.55·arousal01 + 0.55·bloomV` (arousal = the classifier's energetic↔calm; `bloomV` = the slow loudness swell), then expanded around mid (`kNimbusMoodContrast`) for a bigger swing. An energetic track reads hot regardless of valence; calm tracks stay on the pure valence axis. (This routes arousal + bloom into the colour layer in addition to agitation/size — coherent, *not* FA #67 "competing rhythms": all are the same slow mood/energy dimension expressed across channels, reading as one unified "energy" cue, not two clocks fighting.)
2. **The bright core keeps its MOOD HUE** (brightened: `moodTint · kNimbusCoreHueGain + small white`), it no longer washes to near-white — the old desaturation killed the colour on the most-visible pixels.
3. **Saturated poles** — vivid indigo-violet (cool) ↔ rich amber/gold (warm); the ambient fill + haze halo warm with `warm01` too (D-022 propagation).

**Verified.** `NimbusBloomFollowerTest.test_moodTravel` — valence cool↔warm R/B **0.85→3.11** (was 0.71→1.79), and a NEW energy-warmth gate (neutral valence, low↔high energy R/B **0.85→2.89**) locks the B.O.B. fix. Mood strip shows a vivid violet cool pole, a rich gold warm pole, and a gold high-energy/neutral-valence body. Golden hash unchanged (the dHash is a luma-gradient fingerprint, invariant to the hue change). All values are starting points — Matt's eye sets the finals. Full data: `NIMBUS_DESIGN.md §1.3 / §5.4`.

**AMENDED — r1.5 (2026-06-05, same day).** The v1 implementation above *regressed* live (Matt M7 r1.5, session `2026-06-05T19-03-04Z`): "you clobbered the mood effect… displays neutral." Reconstructing `warm01` from the session showed the bug — `valence01 + 0.6·(energy01−0.25)` added a **flat ~+0.25 warm bias** to every moderate-energy track (Sad Song warm01 → 0.75 gold; the cool↔warm range collapsed). Fix: warmth is now **primarily valence**; energy-warmth is **AROUSAL-gated past a high threshold** (`0.50·smoothstep(0.65, 0.95, arousal01)`), so only genuine bangers warm and moderate tracks stay on the valence axis (`bloom` dropped from the colour — it already drives size/brightness, and is unreliably low on quiet captures). Contrast 1.35→1.60 (make the subtle valence travel visible); `moodTau` 4.0→2.5 s (let colour TRAVEL within a track instead of crushing to the near-neutral mean — the r1 "fades to neutral"). Re-verified on the session: In Undertow now cool (0.33), range restored. **Two further findings the r1.5 review surfaced:** (1) the bodies were small/dim → mood colour hard to see. I first (wrongly) blamed a quiet capture / Spotify normalization; Matt confirmed normalization off + 100 % volume, and root-causing it showed the real cause is a **bloom mis-calibration**, fixed in **r1.6 below** (NOT an input problem). (2) The mood **classifier** reads "Sad Song" as **+0.11 valence** (audio-mood ≠ title-mood), so it renders warm-ish regardless of the shader — a classifier characteristic, not a Nimbus issue.

**AMENDED — r1.6 bloom recalibration (2026-06-05, same day; Matt: "input problem, solve permanently").** The small/dim bodies on normal music are NOT a quiet-capture/input issue — the AGC makes band/stem energy level-independent. Root cause (measured): `{stem}Energy` = the stem's 3 AGC bands (bass+mid+treble) **summed**, but the AGC normalises the *6-band TOTAL* to 0.5, so a 3-band sum centres at `0.5 × (3-band/6-band ratio) ≈ 0.30` (measured p50 across 3 real sessions: 0.24 / 0.27 / 0.41), **not** the 0.5 the bloom assumed. The old `bloom = meanStem·1.4 − 0.2` mapped that 0.30 centre to bloom ≈ 0.13 → a tiny dim body on *all* normal music; the Atlas session Matt approved only looked right because it's an unusually dense/loud master (p50 0.41 → 0.38). Fix: `NimbusState` `bloomGain` 1.4→**1.9**, `bloomOffset` −0.2→**−0.06** (lift the floor, keep the dynamic range). Verified: meanStem 0.27 → bloom **0.45** (was 0.18), 0.14 → 0.21, 0.55 → 0.98; silence floors at 0. Regression-locked by `test_bloomVisibleOnTypicalMusic`. **This is the same mis-calibration class as BUG-027** (every energy value centres ~0.3 not 0.5); the system-wide fix — make the AGC produce a true 0.5 centre so every preset calibrates correctly — is BUG-027's domain and a separate project (it re-tunes every preset). The Nimbus recalibration is the bounded fix; it makes Nimbus bodies bigger on all music (Atlas included — Matt re-judges at M7 r2).
