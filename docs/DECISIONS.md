...
**AMENDED — r1.5 (2026-06-05, same day).** The v1 implementation above *regressed* live (Matt M7 r1.5, session `2026-06-05T19-03-04Z`): "you clobbered the mood effect… displays neutral." Reconstructing `warm01` from the session showed the bug — `valence01 + 0.6·(energy01−0.25)` added a **flat ~+0.25 warm bias** to every moderate-energy track (Sad Song warm01 → 0.75 gold; the cool↔warm range collapsed). Fix: warmth is now **primarily valence**; energy-warmth is **AROUSAL-gated past a high threshold** (`0.50·smoothstep(0.65, 0.95, arousal01)`), so only genuine bangers warm and moderate tracks stay on the valence axis (`bloom` dropped from the colour — it already drives size/brightness, and is unreliably low on quiet captures). Contrast 1.35→1.60 (make the subtle valence travel visible); `moodTau` 4.0→2.5 s (let colour TRAVEL within a track instead of crushing to the near-neutral mean — the r1 "fades to neutral"). Re-verified on the session: In Undertow now cool (0.33), range restored. **Two confounds the r1.5 review surfaced (NOT shader bugs, do not tune for them):** (1) the session was captured **quiet** — raw tap −24 dBFS after a 6 s silent start, AGC bands 0.00–0.37 vs ~0.5 for normal volume → low bloom → small dim bodies → colour imperceptible. Almost certainly Spotify "Normalize Volume" on (Failed Approach #30 / RUNBOOK). (2) The mood **classifier** reads "Sad Song" as **+0.11 valence** (audio-mood ≠ title-mood), so it renders warm-ish regardless of the shader — a classifier characteristic, not a Nimbus issue.
