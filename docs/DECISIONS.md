...
- **`inspired_by` decoding + unknown-key rejection.** Still open, and now the single most load-bearing gap in this area: the same missing mechanism explains why the toggle could not be wired honestly (no per-preset origin check) and why the D-120 fields could be re-added to a shipped preset unnoticed (no unknown-key rejection). Neither built at MD.0. A future increment that adds `inspired_by` decoding should carry the unknown-key gate with it.
