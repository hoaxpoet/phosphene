...
**Consequence for the program: categories 2 and 4 need a changed premise, not another attempt at
these levers.** Phase TRK is already parked (D-206). Phase DBN's next increment, DBN.3, was to
wire the decoder in and A/B it — but the decoder gets odd meters wrong and its confidence signal
does not separate, so that A/B would measure the known deficiency rather than the decoder. **Do
not open DBN.3 as specified without first deciding what the changed premise is.** Candidates not
yet explored: a different model family, a different training target, or sourcing bar position
from something other than a downbeat activation stream.
...
1. **`pulse_amp01` is dead.** Pinned at 1.000 from p5 to max across all four captures, including 318 383 frames of live streaming. No dynamic range; it cannot drive anything. Any preset or design citing it as a driver is citing a constant.
