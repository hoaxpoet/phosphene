...
## D-144 — Nimbus: energy warms the mood colour + the bright core keeps its hue (NB.10, M7 r1)

> **Renumbered D-142 → D-144 at integration (2026-06-05):** the parallel Skein.ENGINE.1 session claimed D-142/D-143 on `main` at the same time; this Nimbus decision and the beat-grid project moved to D-144 / D-145.
...
1. **Warmth = valence + ENERGY.** A new `warm01` drives the colour: `valence01` lifted by arousal (the classifier's energetic↔calm), then expanded around mid (`kNimbusMoodContrast`) for a bigger swing. An energetic track reads hot regardless of valence; calm tracks stay on the pure valence axis. (See r1.5 below for the gating that makes this safe.)
...
**Verified.** `NimbusBloomFollowerTest.test_moodTravel` — valence cool↔warm R/B grew (0.71→1.79 → 0.54→3.11 after r1.5), plus a NEW energy-warmth gate (neutral valence, low↔high arousal) locks the B.O.B. fix. Mood strip shows a vivid violet cool pole, a rich gold warm pole, and a gold high-energy/neutral-valence body. Golden hash unchanged (the dHash is a luma-gradient fingerprint, invariant to the hue change). All values are starting points — Matt's eye sets the finals. Full data: `NIMBUS_DESIGN.md §1.3 / §5.4`.

**AMENDED — r1.5 (2026-06-05, same day).** The v1 implementation *regressed* in analysis (the warm-bias path `valence01 + 0.6·(energy01−0.25)` added a flat ~+0.25 warm bias to every moderate-energy track → Sad Song warm01 → 0.75 gold, the cool↔warm range collapsed). Fix: warmth is now **primarily valence**; energy-warmth is **AROUSAL-gated past a high threshold** (`0.50·smoothstep(0.65, 0.95, arousal01)`), so only genuine bangers warm and moderate tracks stay on the valence axis. Contrast 1.35→1.60 (make the subtle valence travel visible); `moodTau` 4.0→2.5 s (let colour TRAVEL within a track instead of crushing to the near-neutral mean). Re-verified: In Undertow renders cool (0.33), range restored. (Note: the M7 r1 / r1.5 reviews actually ran the older `main` Nimbus — these changes were stranded on a side branch until this integration; the fixes address the real NB.6 issues regardless.)

**AMENDED — r1.6 bloom recalibration (2026-06-05, same day; Matt: "input problem, solve permanently").** The small/dim bodies on normal music are NOT a quiet-capture/input issue — the AGC makes band/stem energy level-independent. Root cause (measured): `{stem}Energy` = the stem's 3 AGC bands (bass+mid+treble) **summed**, but the AGC normalises the *6-band TOTAL* to 0.5, so a 3-band sum centres at `0.5 × (3-band/6-band ratio) ≈ 0.30` (measured p50 across 3 real sessions: 0.24 / 0.27 / 0.41), **not** the 0.5 the bloom assumed. The old `bloom = meanStem·1.4 − 0.2` mapped that 0.30 centre to bloom ≈ 0.13 → a tiny dim body on *all* normal music; the Atlas session Matt approved only looked right because it's an unusually dense/loud master (p50 0.41 → 0.38). Fix: `NimbusState` `bloomGain` 1.4→**1.9**, `bloomOffset` −0.2→**−0.06**. Verified: meanStem 0.27 → bloom **0.45** (was 0.18), 0.14 → 0.21, 0.55 → 0.98; silence floors at 0. Regression-locked by `test_bloomVisibleOnTypicalMusic`. **Same mis-calibration class as BUG-027** (every energy value centres ~0.3 not 0.5); the system-wide fix is BUG-027's domain (a separate project — it re-tunes every preset).
...
## D-147 — BUG-029 fix approach: ease the AGC loudness meter in at each track start (seed-from-first-audible + hold-through-silence); AGC3.2

**Date:** 2026-06-05. **Status:** decided (Matt's AGC3.2 call); implementation = AGC3.3; validation + catalog M7 = AGC3.4. Sibling of D-146 (the AGC2 deviation fix) — both are cold-start fixes touching the `BandEnergyProcessor` family, at different layers (D-146 = the deviation EMA pivot; D-147 = the AGC band values themselves).
