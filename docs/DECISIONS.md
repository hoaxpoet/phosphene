...
| D-195 | Accepted | Reference/diagnostic images leave git — reverses D-113/PUB.2's "keep reference media in LFS" (LFS.1, 2026-07-21, Matt's override). The `docs/VISUAL_REFERENCES` + `docs/diagnostics` raster images (jpg/png/gif) are gitignored, local-only: their LFS `.gitattributes` rules dropped, 189 tracked images `git rm --cached` (kept on disk). Text records in those dirs (READMEs, diagnoses, rendering contracts, palette HTML, source_*.txt/json) stay tracked. Trigger: a $1.48 GitHub LFS receipt. Consequence: reference images no longer ship on clone — contributors don't get them. Weights (~167 MB, the LFS bulk) untouched; their lever stays the PUB.2 §1 Release-asset cutover. Historical image blobs reclaimed by the maintainer-run `Scripts/reclaim-lfs-visual-refs.sh` (image-only `filter-repo` purge) + GitHub GC. See D-195 section. |
...
## D-195: Reference/diagnostic images leave git — reference media is local-only, not LFS (LFS.1, 2026-07-21)

**Status.** Accepted. Reverses the reference-media half of the PUB.2 cutover plan (D-113 §publishing runbook, "weights ship as a Release asset, LFS keeps reference media only", Matt-confirmed 2026-07-12). Matt's informed override at LFS.1.

**Context.** A GitHub LFS payment receipt ($1.48, Jun 21–30 2026) prompted the question of eliminating LFS charges. Audit of what was in LFS: ML weights (~167 MB, 479 `.bin` files — a runtime-bundled resource via `Package.swift` `resources: [.copy("Weights")]`, NOT removable without breaking clone-and-build) and dev-only reference/diagnostic images (`docs/VISUAL_REFERENCES` ~114 MB + `docs/diagnostics` ~1 MB, referenced by no build target). CI was already optimized (CLEAN.5.8: no `lfs: true`, weights-only cached pull), so CI is not the bandwidth driver.

**Decision.** The `docs/VISUAL_REFERENCES/**` + `docs/diagnostics/**` raster images (jpg/png/gif) are gitignored and removed from version control — local-only dev material. Concretely (LFS.1): their LFS rules dropped from `.gitattributes`; image-only ignore globs added to `.gitignore`; 189 tracked images `git rm --cached` (kept on disk). **Scope is images only** — the text records living in those same dirs (READMEs, BUG diagnoses, rendering-architecture contracts, palette HTML, `source_*.txt/json`) stay tracked; an early whole-directory ignore that would have dropped ~96 text records was caught and narrowed.

**Consequences.**
- Reference images no longer ship on `git clone` — contributors don't get them. Accepted trade-off: they are Matt's private working source-of-truth for the LOOK, not contributor deliverables. Reinstate a single file with `git add -f` if one ever needs versioning.
- This does not by itself drop the bill: go-forward config only. GitHub does not GC LFS objects on push, so the historical image blobs keep occupying storage until the maintainer runs `Scripts/reclaim-lfs-visual-refs.sh` (fresh mirror clone → `git filter-repo --path-regex` image-only history purge → force-push) and requests a GitHub GC. That is a destructive, clone-invalidating step, deferred to Matt.
- **Weights are untouched** — the ~167 MB LFS bulk is a required runtime bundle. Its cost lever remains the PUB.2 §1 Release-asset cutover (`Scripts/fetch_weights.sh` + `SHA256SUMS` already committed), which is non-destructive and independent of this decision.

**Amends.** `docs/PUBLISHING.md` §0/§2/§3 (reference media is no longer LFS; the §2 rewrite scope now includes the jpg/png via the reclaim script, not just the GIFs). `docs/ENGINEERING_PLAN.md` LFS.1 entry.

**References.** D-113 / `docs/PUBLISHING.md` (the superseded "keep reference media in LFS" plan), D-064 (visual-references library structure), D-009 (weights are MPSGraph ML, runtime-required), CLAUDE.md §What NOT To Do (gitignore vs. untrack: ignore rules do not untrack already-tracked files).
