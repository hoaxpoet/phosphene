...
| D-241 | Accepted (M7 pending) | **The performance chrome is retokenized in place, and it goes quiet but never gone (DS.6, 2026-09-03; defaults from the DS.6 prompt's §DECISION-NEEDED, pending Matt's M7).** `PlaybackChromeView` and its children stay the composition they were and are drawn from the design system only: no colour outside `UzumeAppColor`, `DashboardTokens` confined to `Views/Dashboard/`, no second control tree. (1) The track card's "Planned"/"Reactive" pill is **removed** — it reported the session's structure, which the surprise model ([D-238]) keeps from the listener; `OrchestratorDisplayState` is deleted. (2) After 3 s of inactivity the chrome goes **quiet**, not hidden: `PlaybackControlsCluster` reduces to End session alone (`ChromeVisibility.quiet`); only the Space toggle hides it outright. The quiet control is `textPrimary` on the certified `PerformanceBackdrop` at full opacity — not the low-opacity glyph the default described — because a dimmed glyph over a bright preset cannot hold the 3:1 icon floor and the certified composite already guarantees 4.5:1; "quiet" is one small control, not a faint one. (3) **Track information is a preference**, `uzume.settings.visuals.showTrackInformation`, default shown, persisted; the cluster's "Show/Hide track info" control (the DS.4a words, [D-239]) and Settings move the same value; hidden means the card, its artwork and the track-change announcement are gone from the tree. (4) Mouse move, **any key press and a track change** restore the full chrome — UX_SPEC §7.2 had promised all three; only the mouse was wired. (5) The first quiet timer waits for the arrival ([D-240]) to fade before its 3 s. (6) State changes take the design system's 240 ms exponential ease-out (`UzumeAppMotion`, app-side because the vendored tokens carry no motion); reduced motion crossfades. (7) "Still preparing" is a status placement: `StatusTone.info` on its opaque field, not a colour of its own ([D-234]). (8) The transport bar takes `--shadow-raised` and loses the purple glow. Backdrop numbers unchanged; `PresetContrastCertificationTests` untouched. §Rationale below. |
...
## D-241: The performance chrome is retokenized in place, and it goes quiet but never gone (DS.6)

**Date:** 2026-09-03 · **Increment:** DS.6 · **Status:** Accepted (defaults from the DS.6 prompt's
§DECISION-NEEDED; M7 pending — Matt's verdict is recorded in `docs/ENGINEERING_PLAN.md` §DS.6)
...
`DashboardTokens` under a header that cited `.impeccable.md` and "coral is action". And the
behaviour disagreed with the design system in one place: after 3 s the chrome faded to nothing,
where `DESIGN.md` §Curator Control Surface says it "may reduce to quiet edge controls after
inactivity but cannot become undiscoverable". Two of UX_SPEC §7.2's three restore triggers
(key press, track change) were never wired; only the mouse was.
...
3. **Quiet, never gone.** `PlaybackChromeViewModel` owns three states (`ChromeVisibility`):
   `.full`; `.quiet` — after 3 s of inactivity the cluster reduces to End session alone, the one
   control a Curator may need without a mouse move, and every other surface fades; `.hidden` —
   the Space toggle only. Mouse move, any key (a local `keyDown` monitor the view model owns;
   Space excluded so the toggle sees the state it was pressed in) and a track change restore
   `.full`. The first timer waits `ArrivalTransitionView.totalDuration` so "visible for 3 s on
   session start" begins when the arrival has faded, not under the whiteout.
4. **The quiet control is not faint.** The prompt's default described a low-opacity End-session
   glyph. A glyph at reduced opacity over an arbitrary preset frame cannot be certified: over a
   bright frame it blends away, and the 3:1 icon floor fails exactly when the listener most needs
   the control. The quiet control is therefore `textPrimary` on the same `PerformanceBackdrop`
   composite the certification measures, at full opacity — small and alone is what makes it
   quiet. `PresetContrastCertificationTests` is unchanged and covers it (white ≥ 4.5:1 > 3:1).
   If Matt wants it dimmer, that is a choice to trade the guarantee on bright presets, made knowingly.
...
chrome is quiet is unseen until the mouse moves — pre-existing; whether a toast should wake the
chrome is a product call). The toast stretches to the full window height inside the chrome
(pre-existing, found by the harness; see BUG-113).
