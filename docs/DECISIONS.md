...
- **ASH.** `PlaybackErrorBridge` nudges on `.low` OR `.critical` (test `test_criticalBand_firesAudioLevelsLowNudge`). ★ Two ASH tuning notes left open (Matt's call, not folded in): the live nudge and the post-session verdict both flag a *quiet song intro* (−24 dBFS on "Hummer") as degraded — arguably a false alarm; a "degraded only after audio has been loud" gate or a startup-grace window would reduce it.
