...
- **Per-track / per-session web identity — DEFERRED. Two non-decided options documented for future product call.**
  ...
  - V.7.7C.5.1 ships **Option A** (per-segment variation — different web every Arachne instance) as the immediate fix. B and C are tracked under the `V.7.7C.5.2` stub in `docs/ENGINEERING_PLAN.md`. Decision pending product call after V.7.7C.5.1 manual-smoke.
...
- **V.7.10 cert review.** Matt M7 contact-sheet review + cert sign-off. Five remaining V.7.10 prerequisites after V.7.7C.5.1: per-chord drop accretion; anchor-blob discs at polygon vertices; background-web migration crossfade visual; spider movement (V.7.7C.6); V.7.7C.5.1 manual-smoke confirmation.
...
- **dHash drift table (V.7.7C.4 → V.7.7C.5 → V.7.7C.5.1).**

  | Hash | V.7.7C.4 | V.7.7C.5 | V.7.7C.5.1 |
  |---|---|---|---|
  | Arachne steady | `0x06129A65E458494D` | `0x06129A65E458494D` | `0x8000000000000000` |
  | Arachne beatHeavy | `0x0000000000000000` | `0xC6921125C4D85849` | `0x04101A6444186969` |
  | Arachne quiet | `0x06129A65E458494D` | `0x06129A65E458494D` | `0x8000000000000000` |
  | Spider forced | `0x06129A55C258494D` | `0x06D29A65E458494D` | `0x800080C004000000` |

  V.7.7C.5.1 hashes drop to near-zero on PresetRegression because the harness's frame-phase-0 + zeroed slot-6/7 buffer + thinner+dimmer silk pushes foreground contribution below dHash quantization. Real visual divergence is observed in `PresetVisualReviewTests`.
