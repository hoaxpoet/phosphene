...
- **V.7.10 cert review.** Matt M7 contact-sheet review + cert sign-off. Six total V.7.10 prerequisites: per-chord drop accretion; anchor-blob discs at polygon vertices; background-web migration crossfade visual; WORLD reframe (✅ V.7.7C.5); spider movement (V.7.7C.6); V.7.7C.5 manual-smoke confirmation.
...
- **dHash drift table (V.7.7C.4 → V.7.7C.5).** Arachne `steady` / `quiet` `0x06129A65E458494D` (UNCHANGED — PresetRegression doesn't bind slot 6/7 + worldTex; the §4 reframe + canvas-filling foreground don't surface in regression-mode); Arachne `beatHeavy` `0x0000000000000000` → `0xC6921125C4D85849` (small per-pixel composition drift from the smaller coefficient + dust mote phase difference). Spider forced `0x06129A55C258494D` → `0x06D29A65E458494D` (7-bit Hamming drift — `ArachneSpiderRenderTests` binds a `state.reset()`-seeded `ArachneState` so the off-frame anchors flow through `decodePolygonAnchors`).
