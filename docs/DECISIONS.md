...
| D-182 | Accepted | Audio-visual coupling metric — cross-correlation of per-frame visual delta vs. energy envelope (lags 0–500 ms, peak Pearson r + lag + sliding-window stationarity), `CouplingReportTests`, **report-first, no gate**. QG.3.1 (the gate) deferred: the offline single-fragment/zeroed-state render substrate renders 11/13 certified presets fully static, so the metric is measurable for only 2 presets — a gate needs a population. Prerequisite = a headless multi-pass/state-reconstructing render. Report always attached to preset closeouts, never asserted against; low coupling means "not measured as present," never "preset is bad" (M7 seat stays the coupling authority). §Rationale below. |
...
## D-182: Audio-visual coupling metric — cross-correlation of visual delta vs. energy, report-first (QG.3)
...
**References.** `docs/diagnostics/QG3_COUPLING_BASELINE.md` (baseline table + control + recommended floor), `docs/diagnostics/QG1_REPLAY_AUDIT.md` (the "no headless render" gap this inherits), [D-180] (route-coverage gate + the CPU-computed StemFeatures fixture boundary), `docs/ENGINE/SESSION_REPLAY.md` (SR.1 uncalibrated-proxy doctrine), FA #27 (no hand-authored envelopes). `CouplingReportTests.swift`.
