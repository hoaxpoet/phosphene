...
## D-178: Phase TONAL — continuous harmonic state via the Tonal Interval Vector (TIV) — scoped, awaiting go/no-go
...
**Decision.** Adopt TIV as the harmonic-state representation; **scope as 4 increments** (TONAL.0 audit → .1 `TonalAnalyzer` infra → .2 dumper + corpus calibration → .3 first-preset M7). Infra and preset increments never bundled. **DEFER the build pending Matt's go/no-go** and his first-preset pick (Nacre — recommended, its description already claims harmony-hue — or Mitosis).

**Status.** Proposed (finding + direction + evidence-backed decisions); build awaiting Matt's go/no-go.
