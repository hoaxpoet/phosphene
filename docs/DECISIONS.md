...
*On-disk output paths.* `~/Documents/phosphene_sessions/` holds every session capture Matt has
recorded, and roughly 250 documented diagnostic invocations across the repo resolve against it.
The same applies to `~/phosphene_features.csv`, `~/phosphene_diag.log`,
`~/phosphene_beatbench_fixtures/`, `~/Documents/phosphene_soak/` and `/tmp/phosphene_visual/`.
