...
**Decision.** Park phase TRK. BUG-065 stays open and bounded, `PHOSPHENE_BEAT_PLL` stays
...
**Status:** Accepted (2026-08-03). **Decided, not yet executed** — the deletion is its own increment.
...
**§13.2 — Layout. Never adopted.** `PhospheneEngine/Sources/Presets/Shaders/Milkdrop/` does not exist and will not; all 28 sidecars are flat in `Shaders/`. Presets are named for the Phosphene preset (`Witchlight.metal`), not the source. Moving 7 of 28 into a subdirectory changes `PresetLoader` enumeration and every golden path for zero user-visible gain — and the `<theme>_<source_name>` convention is in direct tension with D-113 besides: a Phosphene preset named after the `.milk` it was inspired by reads as a port of that `.milk`.
...
**§13.5 — Settings exposure. Pending Matt (MD.0 DECISION-NEEDED #1).** `phosphene.settings.visuals.milkdrop.inspired` was never adopted; the shipped surface is still the QR.4 / D-091 `#if DEBUG` stub keyed `phosphene.settings.visuals.includeMilkdropPresets`, reading "Coming in a future update," `.disabled(true)`. Seven presets have shipped past it. **The toggle is inert, not merely hidden:** `SettingsStore.includeMilkdropPresets` is persisted and read by `SettingsViewModel` and `VisualsSettingsSection` and by nothing else, while `PresetScoringContextProvider` builds `excludedFamilies` from `excludedPresetCategories`, a different key. Flipping it changes nothing about preset selection. Ship / delete / defer diffs are drafted and costed in the MD.0 closeout; **neither was implemented.**
...
D-123 superseded D-120 and its five commits (`a2e8a6aa..5f29aefe`) were reverted in `0981ca4f` on the same day they landed. The revert missed two sidecars: `Meniscus.json` and `CymaticResonance.json` still carried `concept_tags` and `motion_paradigm`. `docs/prompts/PHASE_CA_KICKOFF_CA4_ORCHESTRATOR_2026-05-20.md` §3 explicitly asked a session to verify the reversion was clean; it was not, and `docs/CAPABILITY_REGISTRY/ORCHESTRATOR.md` went on to assert twice that it was.

Both fields stripped at MD.0; `ORCHESTRATOR.md`'s "clean" claim corrected; the design docs that still prescribe the fields (`MENISCUS_PLAN.md`, `PG_0_OVERVIEW.md`, `PG_CR_CYMATIC_RESONANCE.md`) carry a D-123 supersession note. Neither field is reintroduced — D-123 reverted them on a reasoned basis (the planner is multi-preset-per-song and does not want repeat penalties).
...
Both readings hang on the same question as D-115, which is why MD.0 routes them to Matt together rather than resolving either. See DECISION-NEEDED #2 in the MD.0 closeout.
...
- **DECISION-NEEDED #1 (Matt):** what "Include Milkdrop presets" should do — ship / delete / defer. Both diffs drafted and costed in the MD.0 closeout; **neither implemented.** Default if unanswered: leave as-is, record in `KNOWN_ISSUES.md`, and do not ship a release with a dead "coming soon" row visible.
- **DECISION-NEEDED #2 (Matt):** D-115 release-bundle composition, open since 2026-05-12. MD.0's recommendation is **C' (10 + 10)**, treating D-119's ≥ 50 % as a steady-state target rather than a first-release gate — A' (7+13) prices first release at roughly twice C' on the strength of a per-preset estimate the work has already falsified. Default if unanswered: `ENGINEERING_PLAN.md` records C' as the working assumption with the divergence from A' flagged, and D-115 stays open.
- **`inspired_by` decoding.** The block is documentation-only. Whether `PresetDescriptor` should decode it — and whether a gate should assert every uplift carries a well-formed one — is unresolved. Deliberately not done at MD.0.
