...
| BUG-029 | P3 | dsp.beat | AGC `f.bass` cold-start spike pops presets at every track onset — **AGC3.5 fast-attack peak floor implemented + validated on real audio (Wake Up worst band 2.57→0.71); code-complete, pending Matt's live M7 (non-waivable)** |
...
- [ ] **Manual (non-waivable):** Matt confirms continuous-energy presets (Ferrofluid Ocean) arrive smoothly on a **hard-onset** track — no pop-and-drop. *(AGC3.5 pending; do NOT re-close without this.)*
