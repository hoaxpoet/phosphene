...
| BUG-063 | P1 | renderer / render-state | **✅ FIX LANDED 2026-06-26 (triple-buffered the slot-8 buffer) — pending Matt's live confirm.** Lumen Mosaic froze ~0.5–1.4 s after selection on a static, still-colourful frame, recovering only on switch-away (Matt live, sessions `…T21-07-18Z` + `…T22-28-51Z`). **Root cause:** the slot-8 `LumenPatternState` buffer was a SINGLE shared `MTLBuffer`, overwritten by the CPU every `tick()` while the GPU read it across up to 3 in-flight frames (`maxFramesInFlight`=3) — a write-during-read race; the lighting pass periodically locked onto stale bytes → frozen frame. **Diagnosis was decisive:** the `LUMEN_DIAG` live instrument proved the CPU state was clean the whole time (lights finite, `cam_t`/aspect/FOV rock-steady, intensities responding to audio), and Matt confirmed the freeze is static-*colourful* (not dark) — so the corruption was purely the GPU read of the stale buffer. Loop hang / audio / dolly (speed 0) / budget governor / audio-modulation / CLEAN.4.3-4.6 all ruled out with evidence. **Fix:** `LumenPatternEngine` triple-buffers the slot-8 buffer (3-deep ring; each `tick()` rotates + writes the next slot); `applyPreset` re-binds `currentBuffer` every frame so the GPU never reads a slot the next tick overwrites. Regression: `test_tickRotatesThroughThreeDistinctRingBuffers`; PresetRegression byte-identical; Lumen flash-harness SAFE; app 388; lint 0. The `LUMEN_DIAG` instrument is kept until the live confirm, then removed. |
