...
| `ToastManagerTests.autoDismiss_afterDuration` | A 0.05 s-duration toast is enqueued, then the test sleeps a fixed real-wall-clock window and asserts `visibleToasts.isEmpty`. Under `@MainActor` parallel-suite contention the auto-dismiss continuation can slip past the wait → toast still visible. The window was already ratcheted 400 ms → 1000 ms and **still flaked** (CLEAN.2.3.8 closeout, 2026-06-15; passed on isolated retry + on a re-run of the full app suite). | Re-run (it's a timing flake, not a regression). **Proper fix** (per the deterministic-over-budget-widening rule, CLEAN.7.9/7.10): make `ToastManager`'s auto-dismiss delay injectable and assert the dismissal deterministically (await the specific removal / drive a controllable scheduler) instead of sleeping a fixed window — do **not** widen the budget further. Candidate for a small CLEAN.7.x-style fix increment. |
