...
| BUG-063 | P1 | renderer / render-state | **⚠️ CODE-COMPLETE, pending live M7 (2026-06-27).** **Root cause FOUND** — NOT the slot-8 race and NOT a GPU collapse (both earlier diagnoses wrong). The engine is fed a **frozen StemFeatures snapshot** for the first ~10 s of stem warmup (the cached 5a profile, byte-identical every tick), and Lumen's D-019 warmup gate (`stemMix` from `totalStemEnergy ≥ 0.06`) mistakes the loud-but-frozen snapshot for live stems → `stemMix→1` → the four lights lock onto it. **Lumen-specific because its SDF geometry ignores audio** (`(void)features`), so frozen lights = a fully frozen frame; every other preset animates geometry over time and masks the warmup. **Proof:** the logged light intensities `[0.25 0.43 0.51 0.36]` are byte-exactly the frozen stem Rel values (`drumsEnergyRel=0.24872`, `bassEnergyRel=0.43455`, …), frozen for 613 stem-frames (~10 s) and unfreezing the instant the live analyzer converges (session `…T18-37-39Z` + Matt's GPU capture: a correct-but-static mosaic). The ~0.88 ms GPU is **normal** for Lumen's static geometry, not a collapse; the headless flash-harness never reproduced it because it feeds varying synthetic features, never the frozen warmup snapshot. **Fix:** `setStemFeatures(_:live:)` tags the cached snapshot `live: false`; `LumenPatternEngine.tick(…, stemsLive:)` gates `stemMix` on liveness and drives the FV fallback (Layer 1 continuous energy, varies from frame 1) until live stems arrive. Done-when: ✅ all Lumen suites (34) + app build + lint 0 + `test_bug063_*` regressions — **pending Matt's live Lumen dwell with no freeze** ([[feedback-visual-fix-needs-live-m7]]); then remove `LUMEN_DIAG`. Fix history: `f5ad0e2` (triple-buffer, ineffective — kept as race-hardening), instrument `b1af31e`/`39c059a`, this fix. |
...
### BUG-063 — Lumen Mosaic freezes for the first ~10 s after selection: the four stem-driven lights lock onto a frozen stem-warmup snapshot (2026-06-26)

**P1** · renderer / render-state · **⚠️ CODE-COMPLETE 2026-06-27, pending Matt's live confirm.** Surfaced live by Matt 2026-06-26 after the BUG-062 fix made every preset appear. **Two earlier diagnoses were wrong** (kept here as a record): (1) the slot-8 single-buffer write-during-read race — a real latent bug, fixed in `f5ad0e2`, but NOT the cause (still froze); (2) a "GPU ray-march collapse" reading of the constant ~0.88 ms `frame_gpu_ms` — also wrong: 0.88 ms is *normal* for Lumen's static geometry.

**Root cause:** for the first ~10 s of stem warmup the render path feeds the engine the **cached 5a StemFeatures snapshot**, which is byte-identical every tick. Lumen's D-019 warmup gate picks the stem-direct vs FV-fallback mix from `totalStemEnergy ≥ stemWarmupHigh (0.06)` — and the cached snapshot is *loud* (non-zero), so it clears the threshold → `stemMix → 1` → the four lights lock onto the frozen snapshot. **Lumen is uniquely affected** because its SDF geometry ignores audio (`sceneSDF` does `(void)f; (void)stems;`), so it has zero time-based motion: every other preset animates geometry over time and masks the warmup, but Lumen animates *only* through the stem-driven lights/cells, so frozen stems = a fully frozen frame.

**Fix (2026-06-27):** `RenderPipeline.setStemFeatures(_:live:)` now tags each push live (`true`, the per-frame audio path) or not-live (`false`, the cached snapshot / zero installed at track change in `resetStemPipeline`). `LumenPatternEngine.tick(…, stemsLive:)` reads the pipeline's `stemFeaturesAreLive()` flag and, while the stems are not live, pins `stemMix = 0` so the lights run on the FV fallback (Layer 1 continuous energy — `bassDev`/`beatBass`/`treble`, which vary from frame 1). When the live analyzer converges the flag flips and the energy-gated path resumes; it resets to not-live per track. The slot-8 triple-buffer change from the first (wrong) fix stays as correct race-hardening. Regressions: `test_bug063_notLiveStems_driveFVFallback_notFrozenSnapshot`, `test_bug063_liveStems_stillUseStemDirect`. The `LUMEN_DIAG` live instrument stays until Matt's live confirm, then is removed.
