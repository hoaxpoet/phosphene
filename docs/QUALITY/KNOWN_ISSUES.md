...
| BUG-053 | P2 | sample-rate / dsp | Live-tap MIR frozen at hardcoded 48 kHz; chroma/key + band boundaries wrong when tap ≠ 48 kHz |
...
**Severity:** P2 (masked on the common 48 kHz macOS output config — the tap's typical rate *and* `readTapFormat`'s fallback are both 48 kHz, so it is silent today; manifests only when the tap delivers ≠ 48 kHz, e.g. a 44.1 kHz-nominal output device or a device-swap to one — couples to **G1 / CLEAN.1.5**. Worst observable: live `estimatedKey` ~1.5 semitones sharp + ~8.8 % band-boundary shift; the key error is further masked where pre-fetched metadata supplies the key. No fps/crash impact). Filed by CLEAN.3.7a (GAP-2 trace). **This refutes the CLEAN.3.7 pre-kickoff trace's "streaming MIR is already rate-aware" assertion** — the streaming MIR is rate-*parameterized* but receives the construction-time default, not the live tap rate.
**Domain tag:** sample-rate / dsp (live MIR analysis: spectral + chroma/key + band energy)
**Suspected failure class:** `sample-rate` (primary) + `pipeline-wiring` (the captured `tapSampleRate` is threaded to the stem path but never to the live MIR).
**Status:** Open — diagnosed 2026-06-16 by end-to-end code trace (CLEAN.3.7a); **no fix yet.** The fix is architectural (the live MIR is built before the tap installs and `process()` carries no rate) → a separate increment, pending Matt's pick of approach (see below). 3.7b/c (doc reconcile + default-trap removal + regression gate) fold into that fix increment.
**Introduced:** `PhospheneApp/VisualizerEngine.swift:740` (`self.mirPipeline = MIRPipeline()` — built with the `sampleRate: Float = 48000` default) combined with `MIRPipeline.process(...)` carrying no `sampleRate` parameter (`PhospheneEngine/Sources/DSP/MIRPipeline.swift:143` init default + `:175` process signature). Long-standing.
**Resolved:** —
