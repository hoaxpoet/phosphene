...
**P1** · renderer / render-state · **OPEN — under investigation.** Split from BUG-063 after the triple-buffer revert fixed Lumen on Spotify but Matt observed it still frozen on local files (sessions `2026-06-28T15-32-06Z`, `2026-06-28T15-50-01Z` + a GPU frame capture).

#### Expected behavior
During local-file playback, Lumen Mosaic animates exactly as it does on Spotify — the Voronoi cells change colour on the beat and the four lights move with the music.

#### Actual behavior
The mosaic renders correctly but is **static** — cells do not recolour; only a faint per-beat pulse is visible. It does not recover for the duration of playback.

#### Reproduction steps
Play a local audio file, switch to Lumen Mosaic, watch for more than a few seconds. (Session `…T15-50-01Z`: local file "01 Cherub Rock.mp3"; `…T15-32-06Z`: a local→Spotify session where the local segment froze and the Spotify segment animated.)

#### Session artifacts
- `LUMEN_DIAG` (`…T15-32-06Z`): after a ~4 s warmup the engine's four light intensities **vary every frame** (`[0.55…]`, `[1.10 1.43…]`, `[2.05 1.94…]`) — the CPU engine is fully alive. `features.csv`: `beatPhase01` wraps every beat, grid locked at 171 BPM, `pulse_beat_index` climbs.
- Matt's Xcode GPU capture: the correct, full colourful Voronoi mosaic at 1800×1200 — a working render of **static content**, not black/garbage.
- `litTex=900×600` throughout the local-file window (vs the 1800×1200 seen on the Spotify path) — the frame-budget governor holds local-file playback at a lower quality because stem separation runs every ~5 s.

#### Suspected failure class
`render-state` — a stale GPU read of the slot-8 `LumenPatternState` buffer, occurring only on the live local-file render path (the headless harness and the Spotify path both animate correctly). Whether local-file Lumen ever animated is **unknown**; local-file playback postdates Lumen's certification, which is not evidence either way.

#### Verification criteria (to be met by the fix)
Manual: Matt confirms Lumen animates on a local file (cells recolour on the beat, lights move). Automated: a regression test for whatever code-level cause is pinned, if one is reproducible headlessly.

### BUG-063 — Lumen Mosaic freeze: the slot-8 triple-buffer "fix" was a regression; reverted to known-good (2026-06-26)

**P1** · renderer / render-state · **⚠️ REOPENED — regression reverted 2026-06-27, pending Matt's confirm.** Surfaced live by Matt 2026-06-26 after the BUG-062 fix made every preset appear. **Three diagnoses were attempted; the first fix was actively harmful:**
