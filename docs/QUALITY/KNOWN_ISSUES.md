...
**Still open, tracked separately.** The **third instance** — `docs/VISUAL_REFERENCES` and `docs/diagnostics` empty in the primary — is not fixed by `2b36c34d`; the script now only warns about it. It needs Matt's decision (re-download and promote to `required=yes`, or retire the image-linking half and treat the READMEs as the authority) and it directly blocks FTR.2's reference curation.


**THIRD INSTANCE, found by the fix's own `--verify` (2026-08-03).** `docs/VISUAL_REFERENCES` and `docs/diagnostics` report **0 gitignored files in the primary**. The reference images — the ones D-211 specifically extended this script to propagate, on the grounds that a worktree without them *"silently degrades preset work rather than failing"* — are absent from the primary too. So that degradation has been in effect everywhere, not just in worktrees, and `docs/VISUAL_REFERENCES/<preset>/` holds READMEs describing images nobody can see. Left `required=no` because promoting it would fail every run today, but it now warns loudly on every invocation. **Needs a decision:** re-download the reference sets and promote to `required=yes`, or accept that the READMEs are the authority and retire the image-linking half. Directly blocks the preset-session checklist's "look at the images" step — including FTR.2's reference curation.
