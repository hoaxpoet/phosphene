...
| BUG-053 | P2 | sample-rate / dsp | Live MIR was frozen at 48 kHz; chroma/key + bands wrong at 44.1 kHz (incl. LF playback). Fix landed `91a973e`, pending manual validation |
...
### BUG-053 — Live-tap MIR pipeline is frozen at a hardcoded 48 kHz and never sees the actual tap rate (chroma/key + band boundaries wrong when tap ≠ 48 kHz) (2026-06-16)

**Severity:** P2. The streaming-tap leg is masked on the common 48 kHz macOS output config (the tap's typical rate *and* `readTapFormat`'s fallback are both 48 kHz) and manifests only at a 44.1 kHz output device / device-swap (couples to **G1 / CLEAN.1.5**) — BUT the **local-file playback** path is *not* masked: `.localFilePlayback` opens 44.1 kHz AAC/m4a files at their native rate and feeds the live MIR, so every LF session of a 44.1 kHz file hit this (a normal feature, not rare hardware). Worst observable: live `estimatedKey` ~1.5 semitones sharp + ~8.8 % band-boundary shift; the key error is further masked where pre-fetched metadata supplies the key, and the *normalized* centroid/mood were ≈ correct by accident (bin over-count × hardcoded 24 kHz divisor cancelled). No fps/crash impact. Filed by CLEAN.3.7a (GAP-2 trace). **This refutes the CLEAN.3.7 pre-kickoff trace's "streaming MIR is already rate-aware" assertion** — the streaming MIR is rate-*parameterized* but received the construction-time default, not the live capture rate.
**Domain tag:** sample-rate / dsp (live MIR analysis: spectral + chroma/key + band energy)
**Suspected failure class:** `sample-rate` (primary) + `pipeline-wiring` (the captured `tapSampleRate` was threaded to the stem path but never to the live MIR).
**Status:** **Fix landed locally (CLEAN.3.7-fix, commit `91a973e`); automated gate green; pending Matt's manual validation.** Each rate-sensitive sub-analyzer gained an in-place `setSampleRate(_:)` (recomputes bin→Hz tables, preserves running state); `MIRPipeline.setSampleRate` forwards to the four + recomputes its Nyquist; `VisualizerEngine+Audio.processAnalysisFrame` calls it (analysis queue, off the RT thread) with the captured `tapSampleRate` — a no-op on 48 kHz, a recompute on a rate change. The hardcoded 24 kHz mood-centroid divisor → live Nyquist (paired so mood stays unchanged while the raw centroid becomes honest). Doc reconcile (Protocols.swift, ARCHITECTURE §Sample-rate contract) + the regression gate shipped with the fix. **Integrated to local `main` (merge `6b23286`); rate-observability follow-up `c68cc74`.** **Manual check for Matt** (the SPM-untestable live-wiring leg): open a 44.1 kHz file via local-file playback (or use a 44.1 kHz output device) and confirm `session.log` records **`MIR analysis rate → 44100 Hz`** (added in `c68cc74` — proves the live MIR adopted the real rate). **NB — key estimation is NOT a valid signal:** it has never tracked accurately (chronic Krumhansl inaccuracy, orthogonal to sample rate), and the rate bug shifts keys *sharp* anyway, so a key letter can neither confirm nor refute the fix. Session `2026-06-16T16-52-09Z` (Black Hole Sun, 44.1 kHz LF) showed `raw tap capture started sr=44100 Hz` and read key F (the chronic error, not the rate) — it predates `c68cc74` so the rate line wasn't emitted yet. Mark Resolved once the `MIR analysis rate → 44100 Hz` line appears.
**Introduced:** `PhospheneApp/VisualizerEngine.swift:740` (`self.mirPipeline = MIRPipeline()` — built with the `sampleRate: Float = 48000` default) combined with `MIRPipeline.process(...)` carrying no `sampleRate` parameter. Long-standing.
**Resolved:** — (pending manual validation; fix on local `main` at `91a973e`)

**Expected:** every rate-sensitive live-playback analysis stage interprets FFT bins using the **actual** tap sample rate — the one `SystemAudioCapture.readTapFormat` captures (`SystemAudioCapture.swift:278`) and the app already threads to the stem path as `tapSampleRate`. Bin→Hz math (centroid, rolloff, band cutoffs, chroma pitch-class mapping) must track the real rate, per the `SystemAudioCapture.sampleRate` contract (`:66-71`: *"if the rate changes, tear down and re-init dependent consumers"*).

**Actual:** the **live** `MIRPipeline` — the instance `processAnalysisFrame` runs every frame (`VisualizerEngine+Audio.swift:143`) — is constructed once at app init with the **48000 default** and is **never reconfigured**. `MIRPipeline.process()` takes no rate, so its four sub-analyzers (`SpectralAnalyzer`, `BandEnergyProcessor`, `ChromaExtractor`, `BeatDetector`) keep the bin→Hz tables they **precomputed at init** from 48000. The FFT's per-call `sampleRate: rate` (`VisualizerEngine+Audio.swift:110`) only sets `FFTResult.binResolution`/`dominantFrequency` *metadata* — **the magnitude array handed to the MIR sub-analyzers is rate-independent** (`FFTProcessor.swift:167`), so passing the live rate to the FFT does **not** make the MIR rate-aware. When the tap runs at 44.1 kHz but the MIR assumes 48 kHz:
- **Chroma / key:** every bin maps `12·log2(48000/44100) ≈ 1.52` semitones sharp → key estimate ~1.5 semitones off (`ChromaExtractor.swift:137-150`; its own doc comment even hardcodes *"the default 46.875 Hz bin resolution"* = 48000/1024). **The headline correctness casualty** — though often overridden where `preFetchedProfile.key` exists (`VisualizerEngine+Audio.swift:482`).
- **Band energies (the §Audio-Data-Hierarchy default primary driver):** cutoffs land ~8.8 % low in true-Hz terms (`BandEnergyProcessor.swift:186-199`); AGC absorbs the *level*, so the effect is a modest spectral mis-assignment, not a dead band.
- **Spectral centroid / rolloff (normalized):** the Hz over-count (×48000/44100) and the `nyquist = sampleRate/2` normalization both scale by 48000/44100 and **cancel** → normalized `spectralCentroid`/`spectralRolloff` (and the mood-classifier centroid, which divides by a hardcoded 24000 at `VisualizerEngine+Audio.swift:286`) are effectively correct.
- **Tempo / flux:** time-domain (inter-onset intervals) / frame-difference → rate-independent, unaffected.

The **offline** session-prep MIR is **correct** — `SessionPreparer+Analysis.swift:256` builds `MIRPipeline(... sampleRate: Float(sampleRate) ...)` with the file's real rate. The defect is the **live path only**. The two camps the CLEAN.3.7 kickoff flagged are real: `MIRPipeline`/sub-analyzers default `48000`; `StemAnalyzer`/`PitchTracker` default `44100`. The stem path is fine (it threads `tapSampleRate`; `StemSeparator` resamples to 44.1 kHz, Beat This! to 22.05 kHz). `Scripts/check_sample_rate_literals.sh` bans the `44100` literal but says nothing about `48000`, so it never caught this.

**Reproduction steps:** set the Mac's output device to a 44.1 kHz-nominal device (or any non-48 kHz interface), play a track with a clear tonal center; `session.log` shows `Tap format: 44100 Hz` (`SystemAudioCapture.swift:280`) but the published `estimatedKey` is ~1.5 semitones sharp of the true key (when not overridden by pre-fetched metadata). On a 48 kHz device the output is correct (the masking).

**Session artifacts:** static end-to-end trace (CLEAN.3.7a) — the divergent construction sites (`VisualizerEngine.swift:740` default vs `SessionPreparer+Analysis.swift:256` rate-aware), `MIRPipeline.process` signature (no rate), and the sub-analyzer init-time bin→Hz precompute. No committed streaming `session.log`/`features.csv` in the worktree records the production tap rate — **confirm the actual production rate via the `Tap format: X Hz` line in `~/phosphene_diag.log` / Console on a real session** (determines whether the defect is currently live or masked on Matt's hardware).

**Verification criteria (written before the fix):**
- [ ] Automated (GPU-free, CI-fast-gate-eligible): feed one magnitude buffer to a MIR/sub-analyzer constructed at 48 kHz vs 44.1 kHz and assert a band cutoff (e.g. the 250 Hz bass/mid boundary) maps to a **different** bin index — proving no stage hardcodes the rate; and assert the live construction path passes the actual tap rate, not the default. Extend `TapSampleRateRegressionTests` (today covers only the stem `StemSampleBuffer` path and explicitly documents this MIR gap).
- [ ] Manual: on a 44.1 kHz path (local-file playback or a 44.1 kHz output device), `session.log` records `MIR analysis rate → 44100 Hz` (commit `c68cc74`), proving the live MIR adopted the real rate. (Key estimation is NOT used — chronically inaccurate, and the rate bug shifts sharp not flat.)

**Fix approaches (engineering call — for the fix increment; product-adjacent because option 1 may briefly reset live key/beat state on a device swap):**
1. **Re-init / reconfigure the live MIR when the tap rate becomes known and on device-swap reinstall** — honors the `SystemAudioCapture.sampleRate:66-71` "re-init dependent consumers" contract and couples cleanly to G1. Cost: the MIR holds per-track state (beat grid, drift EMA, chroma accumulator, AGC running average); must reconfigure bin→Hz without clobbering lock state mid-session, or accept a brief reset.
2. **Thread `sampleRate` through `MIRPipeline.process()` + the sub-analyzers**, recomputing bin→Hz tables only when the rate changes. Cost: touches the RT-adjacent analysis path + 5 sub-analyzers' precomputed-table assumptions; must stay allocation-free on the audio thread (BUG-036 territory).
3. **Make the 48 kHz assumption explicit and loud** (the lazy option): assert/log if the tap ever reports ≠ 48 kHz and downgrade this to P3-latent. Cheapest; defensible given the masking + bounded impact; loses correctness on genuine 44.1 kHz devices.
