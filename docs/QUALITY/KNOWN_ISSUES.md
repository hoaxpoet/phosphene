...
| BUG-064 | P3 | audio.metadata / concurrency | **✅ RESOLVED 2026-06-27 (`7fd52f2`)** — `StreamingMetadata.onTrackChange` could fire *after* `stopObserving()` returned. `stopObserving()` cancels the polling task but can't await an in-flight `pollNowPlaying()`, which then ran its identity check after stop had reset `lastTrackIdentity`/`_currentTrack` to nil → a spurious `(previous: nil, current: <track>)` track-change on a stopped observer. Surfaced as the intermittent `StreamingMetadataTests.trackChange_secondTrack_hasPrevious` flake (extra event → `count → 3 == 2`) on the `fast-gate` runner. Fix: an `isObserving` flag set/cleared under the same lock as the identity reset; the poll's critical section bails (`guard isObserving`) once stopped. Regression: `inFlightPoll_afterStopObserving_doesNotFire` (blocks the reader to force the in-flight-across-stop race; verified failing with the guard neutered). Files to §Resolved at the next pruning pass. |
| BUG-063 | P1 | renderer / render-state | **OPEN — diagnosing (instrument next).** Lumen Mosaic freezes ~0.5–1.4 s after it's selected and stays frozen until you switch away (Matt live 2026-06-26, session `2026-06-26T21-07-18Z`). The render loop is healthy — `deltaTime` steady at 60 fps, the LumenPatternEngine `pulse_*` outputs + scene time keep advancing, no NaN in `features.csv` — so it's a **visual** freeze, not a loop/audio hang. Signature: `frame_gpu_ms` collapses **8.2 → 0.9 ms in one frame** (t≈81.9) and stays ~0.3–0.9 ms for the whole ~10 s dwell, while `renderframe_cpu_ms` is unchanged (~9–10 ms) — the ray-march pass still encodes but per-ray GPU work collapses → rays hit nothing → frozen/blank. **Ruled out (evidence):** loop hang (frames advance); audio freeze (features flow); camera dolly (Lumen `cameraDollySpeed`=0, camera static); frame-budget governor (caps ray-march step mult at 0.75 — can't drop 89 %); audio modulation (light/fog/fin all bounded-continuous, can't collapse the march). **Lead:** a degenerate/NaN value internal to Lumen's scene uniforms or the LumenPatternEngine slot-8 buffer feeding the SDF (sharp onset + sticks; not logged in `features.csv`). **Next:** static analysis exhausted (the per-frame tick fires every frame, engine math is NaN-free, scene/camera valid-but-constant — so it's a RUNTIME corruption); headless repro infeasible (`preset.pipelineState` renders only the static G-buffer — the lit-output freeze needs the full deferred pipeline). So a **live session.log instrument landed** (Matt's build): logs the slot-8 lights + `cam_t`/aspect/FOV + finiteness once a second, loud on the first degenerate. Awaiting a ~10 s Lumen dwell to pin: degenerate-scene-uniform vs degenerate-light vs clean-CPU-but-frozen-GPU (the single-buffer write-during-read race — slot-8 is one shared buffer under 3-deep triple-buffering). Recovers on switch-away (`applyPreset` resets ray-march + pattern state). |
...
### BUG-064 — StreamingMetadata fires a track-change event after stopObserving() (in-flight poll race) (2026-06-27)

**Severity:** P3 (no crash, no data loss; a narrow teardown-window race never observed in production — surfaced only as an intermittent CI test flake. A stray `onTrackChange` on a stopped observer could, in principle, re-trigger downstream track-change handling during session teardown / mode-switch.)
**Domain tag:** audio.metadata / concurrency (`StreamingMetadata.stopObserving` / `pollNowPlaying`)
**Status:** ✅ RESOLVED 2026-06-27 (`7fd52f2`).

**Expected:** After `stopObserving()` returns, no further `onTrackChange` callbacks fire.
**Actual:** `stopObserving()` cancels the polling `Task` but cannot await a `pollNowPlaying()` already in flight. That poll, having read its `NowPlayingInfo` before the stop, reaches its locked identity check *after* `stopObserving()` reset `lastTrackIdentity`/`_currentTrack` to nil; the non-nil identity then compares unequal to nil → `shouldFire == true` → a spurious `(previous: nil, current: <track>)` event fires on the stopped observer.
**Reproduction:** `inFlightPoll_afterStopObserving_doesNotFire` — block `nowPlayingReader` so a poll is guaranteed in flight when `stopObserving()` runs, release it, observe the stray callback (`callCount == 1` instead of 0 without the fix). Originally surfaced as the intermittent `trackChange_secondTrack_hasPrevious` flake on the GitHub `fast-gate` runner (`count → 3 == 2`).
**Failure class:** `concurrency`.
**Fix:** An `isObserving` flag, set in `startObserving()` and cleared in `stopObserving()` under the same lock that resets identity. `pollNowPlaying`'s critical section bails (`guard isObserving`) when observation has stopped, so a late poll neither fires nor re-pollutes the reset state. The lock serialises the poll's check against the stop's reset.
**Verification:** `inFlightPoll_afterStopObserving_doesNotFire` (deterministic regression — proven to fail with the guard neutered, pass with it restored); `trackChange_secondTrack_hasPrevious` reworked to assert the event *sequence* (Track A, then Track B carrying Track A as previous) rather than the flaky raw count; full `StreamingMetadata` suite 8/8 green ×5 consecutive runs; swiftlint --strict clean. No timeout widened.

**Resolved:** 2026-06-27 — `7fd52f2`. Files to §Resolved at the next pruning pass.

---

### BUG-063 — Lumen Mosaic ray-march collapses ~1 s after selection: GPU work drops to near-zero and the visual freezes until you switch away (2026-06-26)

**P1** · renderer / render-state · **OPEN — diagnose phase (instrument next).** Surfaced live by Matt 2026-06-26 after the BUG-062 fix made every preset appear.
...
#### Actual behavior
It renders correctly for ~0.5–1.4 s, then the image freezes and stays frozen for the rest of the dwell; switching to the next/previous preset recovers it. No crash, no UI hang — audio and the rest of the app keep running.

#### Reproduction steps
Select Lumen Mosaic and dwell on it for more than ~2 s. Session `2026-06-26T21-07-18Z`: the 2nd visit (`preset → Lumen Mosaic` at 21:08:39, ~10 s dwell) shows the collapse; the 1st visit (~1 s) was too short to reach it.

#### Session artifacts
`features.csv`: in the Lumen window, `frame_gpu_ms` collapses **8.2 → 0.9 ms in a single frame** (t≈81.9) and pins at ~0.3–0.9 ms for the rest of the ~10 s dwell, while `renderframe_cpu_ms` stays ~9–10 ms throughout (the ray-march pass is still encoded — per-ray GPU work collapses, i.e. rays escape/hit nothing). `deltaTime` steady ~0.017 s (60 fps, no loop stall); LumenPatternEngine outputs `pulse_phase01`/`pulse_beat_index` and `accumulatedAudioTime` keep advancing across the collapse; no `nan`/`inf` anywhere in the CSV.

#### Suspected failure class
`render-state` (most consistent with `precision` — a degenerate/NaN value entering a scene uniform or the slot-8 pattern buffer; sharp one-frame onset that then sticks).

#### Ruled out (with evidence)
- **Loop / pipeline hang** — frames keep advancing (`deltaTime` steady, frame count climbs to 5699).
- **Audio/analysis freeze** — `features.csv` flows the whole session.
- **Camera dolly fly-past** — Lumen's `cameraDollySpeed` is 0 (`VisualizerEngine+Presets.swift` switch defaults to 0); camera is static.
- **Frame-budget governor** — `RenderPipeline+BudgetGovernor` caps the ray-march step multiplier at 0.75 and only toggles SSGI/bloom; cannot produce an 89 % GPU drop.
- **Audio modulation** — light intensity, fog (`fogScale` ∈ [0.3, 2.0]) and fin position are all bounded and continuous; none can collapse the march, and the collapse is a sharp step, not a continuous drift.

#### Diagnosis progress (instrument → diagnose, no fix yet)
Static analysis is exhausted and all clean: the per-frame `meshPresetTick` (→ `LumenPatternEngine.tick`) fires unconditionally every frame (`RenderPipeline+Draw:126`); the engine math is NaN-free + bounded; the scene/camera are valid-but-constant per frame; dolly (speed 0), budget governor (≤0.75 step mult), audio modulation (bounded), and the recent CLEAN.4.3/4.6 commits are all ruled out (4.3 even *fixes* a NaN). Every readable path says Lumen should render fine → the corruption is **runtime**, not static-readable. A **headless repro is infeasible** with the single-fragment harness: `preset.pipelineState` for a deferred ray-march preset renders only the static G-buffer geometry; the animated lit output (where the freeze lives) needs the full deferred pipeline (G-buffer → lighting → SSGI → post). **Concrete structural suspect:** the slot-8 `patternBuffer` is a *single* shared `MTLBuffer` (`LumenPatternEngine.swift:466`), overwritten by the CPU every `tick()` while the GPU reads it across up to 3 in-flight frames — a genuine write-during-read race; not confirmed as the cause (shared with Skein/Arachne, which don't freeze). **Live instrument landed** (`VisualizerEngine+InitHelpers.lumenFreezeDiagnosticTick`, gated to Lumen-active): logs to `session.log` every ~30 frames + loud on the first degenerate — the slot-8 lights (finiteness + max |value|), `cam_t`/aspect/FOV, and the 4 light intensities. One ~10 s Lumen dwell pins it: degenerate-scene-uniform vs degenerate-light vs **clean-CPU-but-frozen-GPU** (→ the buffer race / shader). Then a separate fix increment.

#### Verification criteria (to be met by the fix)
Automated: a headless Lumen render of ≥150 frames asserts `frame_gpu_ms` (or the dumped scene uniforms) never collapse / never go NaN. Manual: Matt dwells on Lumen Mosaic ≥10 s live with no freeze.
