...
- **Next lever is not the tracker.** Two evidence sources and one controller topology have now been measured against the same frozen single-BPM grid. Awaiting Matt's call between the full decoder path (DBN, program phase DBN), shipping nothing and leaving drift bounded, or parking BUG-065.
