...
| BUG-057 | P1 | audio.capture | Silent-tap family — detector card + reinstall-fix (don't rebuild a working tap on a pause) RESOLVED 2026-06-17; residual = environmental wedged-`coreaudiod` (workaround `killall coreaudiod`) + open card-on-pause UX question (Matt's call). Sev may rerank |
