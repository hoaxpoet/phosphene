...
### QG.1.1 — Ricercar route-coverage: 4 family-capture reads (armed at QG.1.3; pending fixture regen)

**STATUS (QG.1.3, 2026-07-09) · code-complete, pending primary-Mac fixture regen.** The arm-trigger below is implemented in code: `FixtureSessionCaptureGenerator` now runs `InstrumentFamilyAnalyzer.analyzeFamilyActivity` over each clip and merges the per-frame `*Activity`/`*ActivityDev` into the stems rows (sampled by playback position, mirroring the live `setInstrumentFamilyActivity`), and the 4 `*ActivityDev` routes are declared in `Ricercar.json`. `AudioRouteSchemaTests` is green. **Remaining gate (steps 2 + 4 of QG.1.3):** regenerate the 3 route-coverage fixtures on the primary Mac (needs the vendored tempo `.m4a` clips + Metal + PANN weights — all absent in a worktree) via `PHOSPHENE_GEN_SESSION_DIR=… swift test --filter FixtureSessionCaptureGenerator`, copy the CSVs in, then confirm `RouteCoverageTests` is green across all 14 presets. **This entry flips to RESOLVED only after that green — and if the family DEV columns come out constant on the non-orchestral clips (PANN jitter fails to clear the 1e-5 floor), STOP: the minimal variant is insufficient and an orchestral fixture is required (do NOT tune the floor).**

**NOTE · coverage-gap (documented, not a defect).** The BUG-067 follow-up backfilled Ricercar's `audio_routes` manifest. Ricercar (FL.13 flow-field) reads **11** audio primitives; **7 are declared** and pass `RouteCoverageTests` on the canonical fixtures (`flow_vigour` ← `bass/mid/trebDev`; `{strings,brass,woodwinds,percussion}_ribbon` ← the band-stem `{vocals,bass,other,drums}EnergyDev` half of each per-colour hybrid). The other **4 reads** — the family-capture half of each colour's hybrid — were deferred as not-yet-armed (QG1_REPLAY_AUDIT §not-yet-armed convention) and are now armed at QG.1.3 (see STATUS above):
