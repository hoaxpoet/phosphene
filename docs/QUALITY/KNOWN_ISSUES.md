...
16. **Fix (CSP.3.5) ✅ 2026-05-28** — divisor `/10` → `/6`. Covers all typical playback (Money 1.36, LR ≤ 1.30, M7 session 1.52); rare `f.bass ≥ 1.0` peaks may produce brief gray-tip flicker on individual frames. See `[dev-2026-05-28-n]`.
17. **Validation (CSP.3.5 M7) — pending** — expected: white artifacts gone, CPU back under budget, spike magnitude preserved from CSP.3.3, PERF.3 brightness fix preserved.
