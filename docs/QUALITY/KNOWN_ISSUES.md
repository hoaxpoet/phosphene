...
**Domain tag:** dsp.structure
**Status:** **Fix landed (CLEAN.6.2, 2026-06-19) — code-complete, pending validation.** The analyzer now decimates its ~94 Hz input to one structural frame every 0.5 s (2 Hz) before the similarity matrix, so the fixed frame-denominated geometry is section-scale: 8-frame checkerboard = 4 s, `minPeakDistance` 16 = 8 s minimum section, 600-frame ring = 5 min. BUG-035/040 + AABA regression tests re-expressed and green at the new rate (DSP suite 23/23 + MIRPipeline structural green). **Validation so far (FA #27 — real audio only):** `StructuralSectionScaleReplay` (env-gated diagnostic) ran the 3 vendored tempo fixtures through the production FFT→MIRPipeline path → **0 boundaries on each ~30 s clip** (conf 0.00) vs the documented ~30 note-scale boundaries in ~50 s — so the machine-gun is gone and the interim `minNoveltyFloor` (0.02) does **not** over-trigger on real continuous music (**no recalibration needed**; it also still passes a real A→B boundary in the unit tests). **Remaining — needs a full 3–5 min real capture (Matt's live session; 30 s previews are too short to contain a real section change):** (1) confirm it FINDS real 15–60 s sections with confidence reflecting form; (2) Matt's live read that orchestrator preset-switching lands on them. **Resolved only after Matt's live validation.** 2026-06-11 (BUG-046): the Skein consumer's 10 wall-s boundary-spacing guard stays (harmless after the fix).
...
**Verification criteria:**
- [~] Automated (real audio): **negative direction DONE** — `StructuralSectionScaleReplay` (CLEAN.6.2; `Tests/.../Diagnostics/`) replays the 3 tempo fixtures through the production FFT→MIRPipeline path and finds **0 note-scale junk boundaries** on each ~30 s clip (the floor does not over-trigger). **Positive direction PENDING** — a 30 s preview is too short to contain a real section change, so confirming 1–6 sections on a 3–5 min track needs a full live capture. (`FixtureSessionCaptureGenerator` can't help — it writes stems.csv only, no structural stream.)
