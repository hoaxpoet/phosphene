...
#### Next step (instrument → diagnose, no fix yet)
Headless Lumen Mosaic repro over ~150 frames that dumps the ray-march scene uniforms + the LumenPatternEngine slot-8 buffer + a per-frame NaN/Inf check, to pin the exact value that goes degenerate ~1 s in. Then a separate fix increment (guard/clamp the offending term). Also check whether it's a regression (`git log` on the Lumen scene / pattern engine) vs longstanding-but-only-on-long-dwell.
