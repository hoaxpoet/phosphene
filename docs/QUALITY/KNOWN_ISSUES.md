...
|---|---|---|---|
| BUG-069 | P1 | app.engine / concurrency | `VisualizerEngine` cross-thread fields unguarded: `currentFamilySeries` (heap Array) written on MainActor track-change, read ~94 Hz on analysisQueue — memory-unsafe; 3 sibling scalars share the gap |
| BUG-068 | P1 | local-file / pipeline-wiring | LF multi-file plan order diverges from the URL queue after a mid-queue preparation failure — every later track plays one file's audio against another track's identity/beat grid/chrome |
...
**P1 · local-file / pipeline-wiring.** Found by the 2026-07-11 pre-publication ultra review; adversarially verified against the code. Diagnose+fix collapsed into one increment per Matt's Phase-1 go (the root cause is statically provable — no instrumentation step needed).
...
**P1 · app.engine / concurrency.** Found by the 2026-07-11 pre-publication ultra review; adversarially verified. Diagnose+fix collapsed into one increment per Matt's Phase-1 go (statically provable data race).
