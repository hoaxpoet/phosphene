...
| BUG-057 | P1 | audio.capture | Silent-tap family — **Phosphene-side RESOLVED 2026-06-17** (D-165): detector card + reinstall-fix (no rebuild of a working tap on a pause) + card pause-suppression, all validated. Residual = environmental wedged-`coreaudiod` only (workaround `killall coreaudiod`; not a code bug). Close / sev-rerank = Matt's call |
