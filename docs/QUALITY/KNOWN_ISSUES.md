...
|---|---|---|---|
| BUG-069 | P1 | app.engine / concurrency | **✅ RESOLVED 2026-07-11 (PUB.2, `3d89692`)** — the 4 VisualizerEngine crossing fields route through `analysisStateLock` accessors (tapSampleRate pattern); MIRPipeline track-metadata pair guarded by `trackMetadataLock`. TSan MIRPipeline suite + full battery green. Files to §Resolved at next pruning |
| BUG-068 | P1 | local-file / pipeline-wiring | **✅ RESOLVED 2026-07-11 (PUB.2, `22ded35`+`1ae6900`)** — LF plan now built from order-preserving `orderedTracks` (PrepOutcomes); regression `startLocalFiles_midQueueFailure_preservesURLQueueOrder`. Files to §Resolved at next pruning |
