...
3. **Fix** ✅ **LANDED 2026-06-17** — chose "don't reinstall on a pause" (Matt's pick): `.silent → reinstall` now only fires for a tap that **never delivered** audio (broken cold install); a tap that *was* delivering and went silent is a user pause → left alone, resumes on play (`SilenceDetector.hasEverDetectedSignal` latch + `resetSignalHistory()` per session; `scheduleNextReinstall` early-returns with `Tap reinstall SKIPPED`). Removes the pause-churn + dead-tap lottery; the detector card auto-clears on resume; preserves BUG-055/wedge recovery (never-delivered → still reinstalls). +3 tests. Pending step 4 (Matt's manual validation). Original direction options retained for reference:
