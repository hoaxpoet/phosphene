...
1. **Instrument** — the `.silent → reinstall` path has only `scheduled` / `starting` / `succeeded` / `failed` breadcrumbs; it is **missing the per-step depth** BUG-058 added to the *device-change* path. Add the same step breadcrumbs around the recreate inside `performTapReinstall` (and the `SystemAudioCapture` reinstall call it drives) so the **exact stalling call** is pinned: tap-created / format-read / aggregate-created / IO-proc-created / startDevice-returned, each with the new `gen`, device id, rate, and a first-seconds RMS probe on the *new* tap (does gen=2 deliver zero, or never start?). Reuse `onCaptureDiagnostic` → `session.log` (the BUG-057/058 sink). Commit & stop.
2. **Diagnose** — one instrumented pause→resume streaming session. Decide between: (a) **recreate hangs** (a `create*`/`startDevice` call blocks during the paused-output transition — matches the missing completion line; same as BUG-058); (b) **recreate completes but binds to a silent/idle stream** and never recovers when audio resumes; (c) the **IO proc starts before audio flows** and never delivers. Record root cause in `KNOWN_ISSUES.md` (BUG-057, and reconcile with BUG-058 — same root → say so).
