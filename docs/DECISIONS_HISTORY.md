...
## RB.2-2 archive batch (2026-06-11)

93 entries moved from `DECISIONS.md` per the RB.1 audit recommendations Matt adopted in-session: shipped one-time choices now embodied in code, reverted/superseded/abandoned entries, executed design-history records (Arachne, Lumen Mosaic, DASH, LF, Dragon Bloom spikes, Nimbus), and unexecuted Phase-MD planning artifacts. Standing constraints, live contracts for in-flight work (Skein/FBS/canvas configs), legal/brand posture, and gate rationales remain active in `DECISIONS.md`. Per-entry context: `docs/diagnostics/RB1_RULEBOOK_AUDIT.md` §2.4.
...
**Amendment 2026-08-07 (MD.0 — superseded in practice; see D-215 §13.1 and §13.2).** Neither the single `family` value nor the filesystem layout was ever adopted. **D-123** (2026-05-13, the day after this amendment) fixed `PresetCategory` at the 11 cream-of-crop theme cases and added no `.milkdropInspired`; the seven inspired-by presets that shipped between 2026-05-13 and 2026-08-07 file as `hypnotic` ×6 and `particles` ×1, and `grep` finds zero Swift references to `milkdrop_inspired` / `.milkdropInspired`. `PhospheneEngine/Sources/Presets/Shaders/Milkdrop/` does not exist; all 28 sidecars are flat in `Shaders/` and named for the Phosphene preset, not the source. The `inspired_by` sidecar block is the only marker of origin. See `MILKDROP_STRATEGY.md` §13.1–§13.2.
...
**Amendment 2026-08-07 (MD.0 — superseded in practice; see D-215 §13.5).** `phosphene.settings.visuals.milkdrop.inspired` was never created. The shipped surface is still the QR.4 / D-091 `#if DEBUG` stub keyed `phosphene.settings.visuals.includeMilkdropPresets`, reading "Coming in a future update" and `.disabled(true)`; seven inspired-by presets have shipped past it. The `excludedFamilies` wiring this amendment describes **does not exist**: `SettingsStore.includeMilkdropPresets` is persisted and read by `SettingsViewModel` and `VisualsSettingsSection` and by nothing else, while `PresetScoringContextProvider` builds `excludedFamilies` from `excludedPresetCategories`, a different key — so the toggle is **inert, not merely hidden**. Under D-123 there is also no `milkdropInspired` family to exclude, so honest wiring would need a per-preset `inspired_by` check, which needs `PresetDescriptor` decoding that does not exist. **Matt's call 2026-08-07: delete.** The stub — store property, persistence key, view-model flag, `#if DEBUG` view row, `settings.visuals.milkdrop.label` / `.coming_soon`, and three tests — was removed at MD.0 (app tests 407 → 404). **There is no Milkdrop Settings toggle and no plan for one.** D-119 commits Phosphene's identity to being Milkdrop-influenced, so a switch removing the majority of the catalog contradicts the brand call; the per-category exclusions already provide the granular control a listener would reach for. Shipping it was also not honestly wireable: `hypnotic` + `particles` hold seven Phosphene-native presets (Aurora Veil, Plasma, Filigree, Mitosis, Cytokinesis, Murmuration, Nebula — five certified), so family-based exclusion would have removed Phosphene's own work, and a per-preset check needs `inspired_by` decoding that does not exist. Persisted values under the removed key are orphaned and harmless; no migration. **D-106 is closed.** See `MILKDROP_STRATEGY.md` §13.5 and D-215.
...
**Amendment 2026-08-07 (MD.0 — superseded in practice; see D-215 §13.3 and §13.6).** The transpiler retirement holds, but the "source `.milk` files become reference material read by authors" premise did not. In practice **no author reads a `.milk` at all**: the runtime `.milk` converter renders directory presets poorly, so the render gallery is built from **butterchurn built-ins** via `tools/milkdrop-render/`, and six of the seven shipped uplifts read a butterchurn built-in JSON as their source artifact (only Dragon Bloom read a `.milk`, since removed at PUB.1). The MD.1 grammar audit this amendment "retained but reframed" is therefore **retired at MD.0** — its consumer does not exist, and seven uplifts certified without it. See `MILKDROP_STRATEGY.md` §13.3 / §13.6.
...
**Amendment 2026-08-07 (MD.0 — superseded in practice; see D-215 §13.4).** The list is **historical, not operative**. None of the nine named candidates was used for any of the seven inspired-by presets that shipped 2026-05-13 → 2026-08-07. The operative candidate list is [`docs/presets/MILKDROP_UPLIFT_PICKS.md`](presets/MILKDROP_UPLIFT_PICKS.md) (2026-06-01) — a shortlist Matt picked by eye from a rendered butterchurn gallery. The selection criteria here (simplicity / theme coverage / catalog gap) were downstream of transpiler-proofness, a criterion that died with the transpiler at the D-110 amendment; the operative criterion is Matt's eye on rendered motion, which is the only one that has produced certified presets. The Northern Lights pre-emption check is moot: Aurora Veil certified at AV.7 / D-185 (2026-07-19). See `MILKDROP_STRATEGY.md` §13.4.
...
**Amendment 2026-08-07 (MD.0 — still open after twelve weeks; measured state + revised recommendation; see D-215).** No pick has landed since 2026-05-12, so the distance to the D-114 20-preset first release is unknowable. **Measured at MD.0:** 26 production presets, **18 certified**, of which **7 are inspired-by** and 11 Phosphene-native. Against that state, A' (7+13) needs **six more uplifts** before first release; C' (10+10) needs **three**.

MD.0's recommendation is **C' (10 + 10)**, treating D-119's ≥ 50 % as a **steady-state target rather than a first-release gate**. The reason is empirical, not aesthetic: A' was priced on §12.1's "~2–3 days per preset," and the record since falsifies that estimate — **Witchlight took ten increments and Meniscus eleven**. A' therefore prices first release at roughly twice C' on the strength of an assumption the work has already disproved. D-119's brand commitment is unaffected; it is a statement about the catalog at steady state, and the first bundle is not the steady state.

**RESOLVED 2026-08-07 (Matt, MD.0 DECISION-NEEDED #2): C' — 10 Phosphene-native + 10 Milkdrop-inspired.** The operative form of D-115, closing a question open since 2026-05-12. A' (7+13) and B' (5+15) are superseded.

**What it means concretely.** At 18 certified — 11 native, 7 inspired-by — the first-release bundle needs **three more Milkdrop-inspired uplifts** and no further native cert work beyond the ten already available. Under A' it would have needed six.

**What it changes about D-119.** The ≥ 50 % inspired-by ratio is a **steady-state target, not a first-release gate**. D-119's brand commitment is unaffected — it is a statement about the catalog at rest, and the first bundle is not the resting state. This also settles the **D-122 trigger-4** assessment raised at MD.0: the measured 27 % roster / 39 % certified share is a transient of a catalog not yet composed into a release, outcome `proceed`, Phase MD not halted.

**Why C' over the A' default.** A' was priced on `MILKDROP_STRATEGY.md` §12.1's "~2–3 days per preset to certification." The record falsifies that estimate — **Witchlight took ten increments and Meniscus eleven** — so A' bought a stronger first-release brand signal at roughly twice the schedule cost, on the strength of an assumption the work had already disproved. See `MILKDROP_STRATEGY.md` §13.7 and §13.8.
