...
**This amendment does not resolve D-115** — the mix is a product-identity call and stays Matt's (MD.0 DECISION-NEEDED #2, routed alongside the D-122 trigger-4 assessment, which turns on the same question). Default if unanswered: `ENGINEERING_PLAN.md` records C' as the working assumption with the divergence from A' flagged, and D-115 stays open. **Do not silently keep A'.**
