...
- **`palette` slot — NOT CURATED.** Neither committed image delivers §6's "mind-altering hue
  flow." 01 is pale, 02 is two-tone. **No committed image currently grades VL's colour.**
  Until one exists, palette judgments rest on the design doc's prose and the link-only anchors.
- **`lighting` / `specular` slots — NOT CURATED.** How a folded surface should catch light
  without going muddy has no committed anchor.
