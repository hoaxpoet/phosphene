# Goldengrove — Visual References

**Purpose:** Reference images for the `Goldengrove` preset — the painterly, back-lit, golden-hour autumn tree that grows, blooms, and sheds across a song. Cited by filename in Claude Code session prompts per `SHADER_CRAFT.md §2.3`.
...
**Status:** v1.0 — all reference slots populated.

> **Provenance (FTR.1 / D-212, 2026-08-03).** This set was originally curated for the **Fractal Tree** V.10 painterly uplift. That increment was cancelled — Fractal Tree keeps its low-fidelity graphic look and its remaining work is audio reactivity, not fidelity — so the whole set transfers here intact, to the preset that was scoped for this register in the first place. Nothing about the images changed; only the preset they serve. `docs/VISUAL_REFERENCES/fractal_tree/README.md` is now a lightweight-rubric README with no image set. Rationale: D-212, `docs/presets/GOLDENGROVE_PLAN.md`, `docs/presets/GOLDENGROVE_CONCEPT.md`.
>
> The raster files themselves are untracked and local-only per D-211; `Scripts/link_fixtures.sh` symlinks them into a worktree. If your `docs/VISUAL_REFERENCES/fractal_tree/` still holds the images on disk, move them to `docs/VISUAL_REFERENCES/goldengrove/` — the filenames below are unchanged.
...
- `docs/presets/GOLDENGROVE_PLAN.md` — build plan and render-path contract
- `docs/presets/GOLDENGROVE_CONCEPT.md` — the concept and the grow / bloom / shed arc
- ~~`SHADER_CRAFT.md §10.4`~~ — superseded (was the Fractal Tree uplift plan; cancelled at D-212)
...
- `ENGINEERING_PLAN.md` Increment V.10 — **cancelled** (D-212); Goldengrove's gating criteria live in `GOLDENGROVE_PLAN.md`
