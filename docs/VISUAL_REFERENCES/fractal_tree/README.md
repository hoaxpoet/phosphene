...
**⚠ The guitar route is NOT a guitar route on all material, and this is the open item.** On *Seven Nation Army* `otherOnsetRate` correlates **+0.71 with `drumsOnsetRate`** (+0.14 on Cherub Rock, where FTR.8 justified it) and the tips span only **4 branches of the count's 46**. All four stems' onset rates share one distribution there. Do not widen the coefficient to make the guitar "register" — measured, that would make the drums louder in a layer labelled guitar. FTR.12 measures whether an independent guitar channel exists before anything else is routed to it.
