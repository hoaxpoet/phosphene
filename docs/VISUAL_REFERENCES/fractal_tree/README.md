...
**Changed at FTR.18 — the size correction is BOUNDED to the limiter inversion. Matt's M7: *"better than the previous, but ok overall. This is probably as good as we're gonna get right now."* The FTR.9→FTR.18 arc lands here; NOT certified (FTR.5 is his separate call).**

Level rank stays the size driver — six alternatives were measured and each lost span, height or pacing. Only its one defect is corrected: on a limited master the level DIPS as the band arrives (`r(trunk, density) = −0.64` on Carry The Zero, `musicRange` 3.6 dB), so `size = level + max(0, density − level) · (1 − smoothstep(0.15, 0.40, level))`. Both conditions must hold — level LOW *and* density HIGH — so a passage with healthy level cannot be lifted. Verified on the render: band entry +96 % ink, quiet passage byte-identical.

**★★★ Read this before trusting any metric in this file.** Four M7 rejections in this arc were each preceded by a GREEN measurement that modelled the wrong thing: turn rate (a freeze has a low one), step size (a freeze has small ones), per-frame float inequality (measures interpolation, not arrivals), per-frame pixel identity (slow smooth motion is sub-pixel), trunk length (the branch count carries the same term at **26×** the coefficient), glide-as-EMA (it chases a **beat-latched** target), and a subsampled render (`advanceBeatHold` left the glides on their frame-0 seed). **When a render and a metric disagree, neither is trustworthy until the disagreement is explained — bisect until one is proven wrong on a control.**
