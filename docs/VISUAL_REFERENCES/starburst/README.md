# Visual References — Starburst

**Family:** abstract
**Render pipeline:** feedback + particles + mv_warp
...
- [ ] **Hero noise function(s):** `<e.g. fbm8 + blue_noise_sample>` (from `Shaders/Utilities/Noise/`)
- [ ] **Material count and recipes:** `<e.g. mat_velvet + mat_marble + mat_sand_glints>` (from `Shaders/Utilities/Materials/` V.3)
- [ ] **Audio reactivity:** deviation primitives this preset must use (e.g. `f.bass_att_rel`, `f.mid_att_rel` per D-026; mv_warp bass zoom breath + mid rotation)
...
- mv_warp pass: `bass_att_rel` drives zoom breath, `mid_att_rel` drives slow rotation, decay=0.97 for long cloud smear.
