...
**Rubric:** full-rubric section scaffold (the formal `rubric_profile` field is set at **Skein.6 / V.6 certification**, per `SKEIN_PLAN.md`). lightweight is the *expected* landing — Skein's closest sibling Dragon Bloom certified lightweight, and the 3D-surface rubric items below are mostly N/A for a 2D painterly preset (the same Plasma / Nebula tension D-064 resolved by lightweight classification). The full-rubric shape is retained here for `CheckVisualReferences` conformance and to give the V.6 reviewer the complete structure to downgrade from.
**Last curated:** 2026-06-05 — six positives curated by Matt (Unsplash, see *Provenance*). The five anti-references, the wet/dry *specular* pair, and the restrained *palette* pole are still to source (see *Still to source*).
...
**Rubric-tension flag (mirrors Murmuration's README).** Skein is carried in the full-rubric scaffold for lint conformance, but §12.2 / §12.3 assume surface-shaded geometry that a 2D painterly feedback preset does not have — the same tension D-064 acknowledged for Nebula and resolved by lightweight classification. Recommendation deferred to **Skein.6 / V.6**: either reclassify Skein lightweight with a painterly stylization contract (the expected outcome, matching sibling Dragon Bloom), or read these items as N/A-by-paradigm. This README does not assume the resolution and sets no `rubric_profile`.
...
- **The five `05_anti_*` anti-references** above. Source as real photography / illustration where possible (lint-clean); AIGEN only under the D-065(b) carve-out + the lint caveat noted above.
...
The formal rubric tiers (mandatory / expected / strongly-preferred per `SHADER_CRAFT.md §12`) and the `rubric_profile` field in `Skein.json` are set at **Skein.6 / V.6** — not in Skein.0. Skein's closest sibling, Dragon Bloom (`direct + mv_warp` feedback), certified on the lightweight profile; Skein is expected to follow unless V.6 review decides otherwise. Regardless of profile, the binding bar is the **M7 manual gate** (`SKEIN_DESIGN.md §5.7`): Matt, live, on real music across ≥ 5 tracks plus a local file — the drip painting must read as **poured paint**, the painter must **perform**, the canvas must stay **legibly temporal** (wet-now / dry-past), and no frame may match an anti-reference. No automated metric substitutes.
