...
> **Amendment 2026-07-14 (AV.6 streak-core rebuild — real-time footage is now the MOTION reference).** Across AV.5 the aurora never contained real fine filaments (the rendering core was a bug'd nimitz port that produced a smooth wash; the soft-veil rework read as blobby fog). Matt supplied **real-time aurora footage** (`~/Desktop/Screen Recording 2026-07-14 at 1.41.13 PM.mov`, 286 s @ 60 fps) as the definitive look-and-motion reference and confirmed: **aurora IS a mass of fine translucent vertical streaks** — streaky is correct. Stills `10`/`11`/`12` below are curated frames from that footage (the streaky-curtain section, ~235–266 s); they are the AV.6 texture + framing target. **Motion is a footage judgment** — the `.mov` (and the per-second frame extraction to `/tmp/aurora_ref/`) is the real-time-motion reference stills cannot convey. AV.6 targets the **curtain of streaks only** (Matt's choice); the arc and overhead-corona forms in the footage are out of scope. Design: `docs/presets/AURORA_VEIL_DESIGN.md §5.11`.
...
| `10_macro_streak_curtain_realtime.jpg` | **AV.6 — the streak-curtain target (form + negative space).** Real-time footage frame: a curtain of MANY fine vertical streaks sweeping down, brightest green concentrations near the lower edge, translucent (stars punch through between streaks), fading up to blue/purple, against dominant dark sky. This is the overall form + framing target — a curtain occupying part of the sky, NOT a full-width fill. |
| `11_micro_fine_streaks_intense.jpg` | **AV.6 — fine streak texture (the load-bearing detail).** Close read of the streaks: many thin, FLAT, emissive vertical filaments at varying brightness, cyan-green intensifying to near-white where concentrated, stars visible through. FLAT emissive — no bright-core-to-dark-edge shading (that 3-D "volume and shadow" read is the AV.5 fog failure). This is the texture AV.6 must reproduce; if the render has no filaments this fine, it is wrong. |
| `12_meso_streak_curtain_diagonal.jpg` | **AV.6 — streak curtain, diagonal sweep + drape.** The streak curtain leaning/sweeping diagonally with fine rays hanging off a brighter lower edge; shows the meso-scale drape of the whole curtain plus the fine streak fringe. Bright green lower body, dark negative space to the sides. |
...
- `10_macro_streak_curtain_realtime.jpg`, `11_micro_fine_streaks_intense.jpg`, `12_meso_streak_curtain_diagonal.jpg` — frames curated from real-time aurora footage Matt supplied 2026-07-14 (`Screen Recording 2026-07-14 at 1.41.13 PM.mov`, streaky-curtain section ~235–266 s; a photographer's signature watermark is visible bottom-right). **Internal reference only — verify the footage's licensing/attribution before any public/shipped use** (the `.mov` is a screen recording of third-party video; source/creator to be confirmed with Matt). Screen-recording chrome (rounded border + cursor) cropped; scaled to 1280 px wide.
