...
| `02_meso_flowing_colour_masses.jpg` | **The flowing-field substrate.** Multiple **distinct colours** (cream / coral / pink / violet / blue / teal) bleeding **side-by-side, wet-into-wet**, with no single focal point — the drifting, merging colour ground the voices deposit into. Bonus: colour-as-identity (each colour its own mass) + the crispness *range* (fine wisp tendrils above, broad billows below). | The **upward-plume directionality** (the substrate drifts/advects, it does not rise) and the **pure-white ground** (ours is a warm light ground, tunable). | Matt-sourced photograph, 2026-06-29 (Unsplash-class abstract ink-in-water; no underlying artwork — verify exact attribution before any re-licensing audit). |
| `04_palette_register_legibility.jpg` | **Register colour = spatial band.** Flat, front-on **horizontal colour bands**, each a distinct hue read instantly as a separate "part" — the direct analogue of the LOW/MID/HIGH lane layout (colour = vertical position = register; the color-organ identity principle). | The **ruler-straight, parallel rows** — voices **weave** within and across their band, they never run straight (this *is* the rigid-parallel failure mode: read colour-legibility, **not** geometry). Perspective/horizon. The specific hues. | Matt-sourced photograph, 2026-06-29 (Unsplash-class tulip-field aerial; verify attribution before re-licensing audit). |

*(`03_micro` and the five `05_anti_*` remain on the text spec — deferred, non-blocking; see* Still to source *and* Anti-references*.)*
...
- ✅ **`02_meso_flowing_colour_masses.jpg`** — placed (Matt-sourced ink-in-water plume), annotated above.
- **`03_micro_*` — line crispness range.** A fine, crisp filament pole and a broad, smeared wash pole (the centroid axis). *(The `02` plume half-covers this — wisp tendrils vs broad billows; a dedicated micro is optional, not blocking.)*
- ✅ **`04_palette_register_legibility.jpg`** — placed (Matt-sourced tulip-field aerial), annotated above.
