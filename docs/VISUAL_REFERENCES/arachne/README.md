...
Files in this folder, ordered to walk the detail cascade (§1.2) from macro to micro to specular, then atmosphere/lighting, then palette anchor, then anti-references. Each name encodes the trait it demonstrates per `../_NAMING_CONVENTION.md`. References should be ≤ 500 KB each; crop and compress before committing.
...
| `04_specular_silk_fiber_highlight.jpg` | The narrow axial specular highlight running along the length of a silk fiber. Marschner-lite R lobe per §4.3 — a stripe-shaped highlight that follows the fiber's length, NOT a Phong-style point highlight. Real spider silk threads visible at extreme zoom in this image; study how each thread catches light along its axis as it passes through and between droplets. The Arachne silk threads must show this same axial-streak quality on every visible strand. |
...
| `06_atmosphere_dark_misty_forest.jpg` | Volumetric fog density and cool blue-grey palette anchor. Stacked tree layers fading into mist communicate the spatial depth that the §10.1.5 "web reads against air, not void" quality requires. Cool palette is intentional — it pairs against warm silk emission per §10.1.6's cool-blue back-rim. The dark base also signals: atmosphere stays *behind* and *darker than* the web at all times. A bright atmospheric reference would pull a session toward overcast-melancholy aesthetics, which is a different family. |
...
- [ ] **Detail cascade:**
  ...
  - specular = silk Marschner-lite R lobe per `04_specular_silk_fiber_highlight.jpg` (narrow axial highlight) + mirror specular on droplets
...
- [ ] **Material count and recipes (≥ 3):**
  - `mat_silk_thread` (Marschner-lite per §4.3) — primary silk surface
  - dielectric droplet material (clear refractive sphere with mirror specular) — adhesive beads
  - bioluminescent emission haze (rim-emission term per §5.3) — counts as third material/lighting recipe
  - `mat_bioluminescent_chitin` (§4.18) — for the rare spider easter egg only (D-040; ~1-in-10 songs)
...
- [ ] **Performance ceiling:** ≤ 5.5 ms p95 at 1080p Tier 2 (matches `complexity_cost.tier2 = 5.5` in JSON sidecar; silk Marschner-lite is the dominant cost).
...
- [ ] **Detail normals** — applicable to silk surface emission gradient (use `combine_normals_udn` on micro-displacement of strand cross-section). Recommended.
- [ ] **Volumetric fog or aerial perspective** — applicable. Soft 0.02 fog and a gentle dust-mote field behind the web make it read against air, not void. Mandatory per §10.1.5; references `06_atmosphere_dark_misty_forest.jpg` (volume + palette) and `07_atmosphere_dust_light_shaft.jpg` (mote density).
- [ ] **SSS / fiber BRDF / anisotropic specular** — applicable and **mandatory for this preset.** Marschner-lite silk per §4.3 is the single biggest fidelity lift Arachne needs. R lobe (axial highlight per `04_specular_silk_fiber_highlight.jpg`) + TT lobe (back-lit warm rim).

**Score target:** 3/4 expected traits (detail normals, volumetric fog, fiber BRDF).

## Strongly preferred traits (per §12.3 — at least 1 of 4)

- [ ] **Hero specular highlight in ≥60% of frames** — applicable. The Marschner-lite axial highlight on the dominant radials should be visible in nearly every frame; droplet mirror specular sparkles on capture-spiral threads on most frames. Reference `04_specular_silk_fiber_highlight.jpg`.
...
- [ ] **Chromatic aberration / thin-film interference** — applicable on the spider easter-egg carapace only (§4.18 bioluminescent chitin's iridescent thin-film). Skip on silk to preserve documentary realism (Gossamer V.8 owns chromatic aberration on wave peaks; not Arachne).
...
- **Spider as primary subject.** The spider is a 1-in-10-song easter egg with 5-min cooldown (D-040). If the spider dominates a frame, the trigger logic or render weight is wrong.
...
- **Continuous primary drivers** (deviation primitives, D-026): silk base emission ← `f.bass_att_rel`; dust-mote density ← `f.mid_att_rel`; per-strand micro-quiver phase ← `f.beat_phase01` (MV-3b, not `time`).
- **Beat accents** (deviation primitives, D-026): emission peaks ← `stems.drums_energy_dev` blended with `f.beat_bass_dev` fallback; hub anticipation pulse ← `f.beat_phase01` approach curve (`approachFrac * 0.004` style ramp, per `VolumetricLithograph` reference).
