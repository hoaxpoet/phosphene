...
- [ ] **Detail normals** — applicable to silk surface emission gradient (use `combine_normals_udn` on micro-displacement of strand cross-section). Recommended. Also applicable to near-frame bark layer per `18_bark_close_up.jpg` — bark normal/displacement gives the world depth that makes the web read as foreground.
...
- [ ] **Hero specular highlight in ≥60% of frames** — applicable. The Marschner-lite axial highlight on the dominant radials should be visible in nearly every frame; droplet mirror specular sparkles on capture-spiral threads on most frames. Reference `04_specular_silk_fiber_highlight.jpg`. Spider eye specular (when the easter-egg spider is on screen) per `19_spider_eye_specular.jpg`.
