...
3. **Visual harness output** — when the increment is preset-facing or visually observable, include the `RENDER_VISUAL=1` output paths or key frames; for preset increments the canonical artifacts are the `Scripts/compare_render.sh` comparison sheet (D-181, stills) **plus the `Scripts/motion_gate.sh` motion verdict (D-195, temporal)** — the still sheet and the motion signal are both required for anything with living behaviour; stills alone hid the Truchet jitter. Each carries its verdict table / motion verdict. State explicitly when a change is not visually verifiable.
