...
1. **Increment DSP.2 S4 — Weight loading + numerical match.** Load `small0` checkpoint weights from `ML/Weights/beat_this/` into the zero-init graph constants. Per-layer numerical match against PyTorch FP32 on the six reference fixtures within tolerance. Warm-predict < 300 ms on M1. Done when: `BeatThisModelTests` (S4 additions) pass; beat/downbeat outputs on love_rehab first 1500 frames match PyTorch reference within tolerance.
