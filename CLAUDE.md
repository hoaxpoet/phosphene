...
PhospheneEngine/
  ...
  Presets/
    ...
    Shaders/VolumetricLithograph.metal → Psychedelic linocut terrain (3.5.4 / v2). fbm3D heightfield swept by `s.sceneParamsA.x`; amplitude from attenuated `f.bass_att + 0.4 × f.mid_att` (slow-flowing). Three strata: palette-tinted near-black valleys, razor-thin emissive ridge-line seam, polished-metal peaks. Peaks use IQ cosine `palette()` driven by terrain noise + audio time + valence — saturated metallic colors that cycle. `pow(f.beat_bass, 1.5) × 0.7` flares peak/ridge palette into HDR bloom (no coverage shift — geometry stays stable). `sqrt(f.mid) × 1.6` polishes peak roughness as the "other" stem proxy. No `scene_fog` (linocut has no aerial perspective). SSGI omitted to preserve harsh shadows.
