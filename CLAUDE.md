...
PhospheneEngine/
  ...
  Presets/
    ...
    Shaders/Arachne.metal → Bioluminescent spider web mesh shader. Object shader dispatches 12 mesh threadgroups (one per web slot); mesh shader (64 threads) emits hub cap, anchor dots, radial spokes, and spiral segments. WebGPU struct (64 bytes) byte-matches Swift ArachneState.WebGPU. D-019 warmup + MV-3b beat anticipation in fragment. Additive blend. Organic family. Quiver wave is beat_phase01-locked (one propagation per beat). Glow uses exp2(-dist*3) falloff; sat=0.92. (Increment 3.5.5, D-038)
...
1. **Increment 5.3** — See `docs/ENGINEERING_PLAN.md` for the Phase 5 increment definitions.
