...
**Current test count: 379 tests** (288 swift-testing + 91 XCTest, across unit, integration, regression, performance). All must pass before any new code is merged.
...
Tests/ (371 tests: 280 swift-testing + 91 XCTest)
  ...
  Presets/                  → MurmurationStemRoutingTests (8: zero-stem fallback, drums beat response, turning wave non-uniformity, bass drift, other flutter, warmup blend smoothness, vocals density, buffer binding)
...
- **Increment 3.5.2 — Murmuration Stem Routing Revision** ✅ — Replaced the 6-band full-mix frequency workaround with true stem-driven routing via `StemFeatures` at GPU `buffer(3)`. `Particles.metal` compute kernel now accepts `constant StemFeatures& stems [[buffer(3)]]`. Routing: **drums** (`drums_beat` + `drums_energy`) → turning wave that sweeps across the flock over ~200ms (not instantaneous) using a smooth triangular bump centered on wave front `(1 - drums_beat)`, propagation direction alternating per beat epoch via `flock_hash(floor(t * 2.5))`; **bass** (`bass_energy`) → macro body drift velocity and shape elongation; **other** (`other_energy`) → surface flutter weighted by `distFromCenter` (periphery 1.0×, core 0.25×) and flock width; **vocals** (`vocals_energy`) → density compression `densityScale = 1 - vocals * 0.22` applied to `halfLength` and `halfWidth` (the "dark pulse"). Warmup fallback: `smoothstep(0.02, 0.06, totalStemEnergy)` crossfades from FeatureVector 6-band routing to stem routing — zero stems produces identical behavior to previous implementation. `ProceduralGeometry.update()` gains `stemFeatures: StemFeatures = .zero` parameter, bound at compute `buffer(3)`. `Starburst.metal` adds `StemFeatures` param; `vocals_energy` shifts sky gradient subtly warmer (≤10%). 8 `MurmurationStemRoutingTests` (zero-stem fallback, drums beat response, turning wave non-uniformity, bass drift, other flutter, warmup blend smoothness, vocals density, buffer binding). 288 swift-testing + 91 XCTest pass.
- **Increment 3.5.3 — Glass Brutalist Preset** ✅ — `GlassBrutalist.metal` + `GlassBrutalist.json` + `IBLManager` wired up in `VisualizerEngine`. Ray march preset: a stark brutalist corridor of massive concrete pillars and horizontal slabs framing near-mirror glass panels between each bay. Two shader functions (`sceneSDF` + `sceneMaterial`). Concrete SDF: floor/ceiling `sdPlane`s + abs-folded pillar columns (round-based Z repetition every 7 units) + horizontal cross-beams + side walls (`GB_CORRIDOR_X - abs(p.x)`). Glass SDF: thin vertical panels offset by half a cell in Z. Audio routing: `sub_bass + low_bass` → pillar Y-scale (primary); `mid` → glass Y-scale; `accumulated_audio_time` → slow corridor drift; `beat_bass` → transient 5% pillar squeeze (accent). Material: concrete → two-octave `perlin2D` albedo (roughness 0.82–0.92, metallic 0.0); glass → cool cyan, roughness 0.04, metallic 0.92. Light at (0.0, 4.5, 2.0) inside the corridor — illuminates floor, ceiling, side walls, and glass front face (NdotL > 0 for all primary surfaces). **Key lesson:** a light outside the corridor walls (x > GB_CORRIDOR_X) cannot illuminate interior surfaces, making them appear black without IBL. **IBLManager wired up:** `VisualizerEngine.init()` now creates IBLManager on a background queue (same pattern as TextureManager) and calls `pipe.setIBLManager(ibl)` — without this, IBL textures are unbound (zero), leaving mirror-glass and shaded concrete near-black. Passes: `["ray_march", "ssgi", "post_process"]`. 280 swift-testing + 91 XCTest pass.
...
2. **Phase 4 — Orchestrator.** Revised to include session planning mode alongside reactive mode.
