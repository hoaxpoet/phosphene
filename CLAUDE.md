...
PhospheneEngine/
  ...
  Presets/
    ...
    Shaders/VolumetricLithograph.metal → Psychedelic linocut terrain (3.5.4 / v3.3). fbm3D heightfield swept by `s.sceneParamsA.x` at slow rate 0.015; amplitude from attenuated `f.bass_att` + transient kick `smoothstep(0.22, 0.32, f.bass) × 0.40`. Three strata with narrow linocut coverage (~15% peaks): palette-tinted near-black valleys, razor-thin emissive ridge-line seam, polished-metal peaks. Peaks use IQ cosine `palette()` driven by terrain noise (× 0.9 spatial) + audio time (× 0.08 slow cycle) + valence — saturated metallic colors. `smoothstep(0.22, 0.32, f.bass)` drives palette brightness flare (× 2.5 peak, × 3.4 ridge) into HDR bloom + 0.03 coverage expansion on beats. v3.3 switched from `f.beat_bass` to continuous `f.bass` because the 400ms cooldown in the 6-band onset detector phase-locks beat_bass to a phantom 143 BPM on tracks with dense off-kick bass (like Love Rehab 125 BPM); `f.bass` local maxima track real kicks cleanly. `sqrt(f.mid) × 1.6` polishes peak roughness as "other" stem proxy. `scene_fog: 0` truly disables fog. Miss/sky pixels tinted by `scene.lightColor.rgb`. SSGI omitted.
