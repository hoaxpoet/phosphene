...
**Phase U (UX Architecture) in progress — U.1 through U.8 complete. Phase 4 (Orchestrator), Phase 3, and Phase 2.5 (session preparation) complete.** Recent landed work:
...
2. **Increment U.9 — Accessibility pass.** `NSWorkspace.accessibilityDisplayShouldReduceMotion` gates `mv_warp` and SSGI temporal feedback. Beat-pulse amplitude clamped to 0.5× when reduced motion active. Dynamic Type + VoiceOver labels. See `docs/ENGINEERING_PLAN.md §U.9`.
3. **Increment 6.1 — Progressive Session Readiness.** Gates the "Start now" CTA in `PreparationProgressView` (dormant in U.4 via `FeatureFlags.progressiveReadiness = false`). See `docs/ENGINEERING_PLAN.md §Phase 6`.
