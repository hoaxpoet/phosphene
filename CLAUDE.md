...
**Current test count: 314 tests** (239 swift-testing + 75 XCTest, across unit, integration, regression, performance). All must pass before any new code is merged.
...
1. **Increment 3.17 — SSGI Post-Process Pass** (infrastructure). Screen-space global illumination approximating short-range diffuse light bounces from existing G-buffer. Slots into post-process chain between lighting and composite.
2. **Increment 3.18 — DRM Silence Detection & Graceful Degradation** (audio resilience). `AudioInputRouter` sustained silence monitoring, user-visible indicator, ambient/generative fallback mode, automatic recovery on signal return.
3. **Phase 3.5 — Native Preset Library Expansion.** First entry: **3.5.1 Photorealistic Popcorn**, depends on 3.1b + 3.3 + 3.4 + 3.6 + 3.12 + 3.13 + 3.14 + 3.15 + 3.16 + 3.17 (all complete or to be completed before Popcorn).
