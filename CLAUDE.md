...
1. **Increment V.7.6.D — Diagnostic preset orchestrator semantics.** Follow-up to V.7.6.C: extend the `is_diagnostic` flag's effect beyond `maxDuration`. Scorer hard-excludes diagnostics from auto-selection; LiveAdapter never overrides into a diagnostic; manual switch is the only path to render Spectral Cartograph (or any future diagnostic). Small scope — adds an exclusion check to `DefaultPresetScorer` and a no-override branch to `LiveAdapter`. See D-073 for the V.7.6.C scope split.
2. **Increment V.7.7 — Arachne v8: background pass + background webs.** Per `ARACHNE_V8_DESIGN.md §6 step 4`. Atmospheric texture pass (mood-tinted gradient + defocused `worley_fbm` foliage + optional warm beam, half-res), pre-populated background dewy webs with Snell's-law refractive drops sampling the bg texture, fresnel rim, sharp specular pinpoint, dark edge ring. Foreground unchanged for now (still V.7.5 build code). Includes 5s low-pass smoothedValence/smoothedArousal state.
