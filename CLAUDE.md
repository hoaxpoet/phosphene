...
1. **Increment DSP.2 S3 — MPSGraph transformer encoder graph (build only).** Input projection, positional encoding (RoPE, precomputed once for head_dim=32), 9 encoder blocks (RMSNorm + multi-head attention + FFN), SumHead output. Random init — no weight loading yet. Per-layer shape tests against `docs/diagnostics/DSP.2-architecture.md`. Three workarounds required: RMSNorm manual, SDPA manual matmul+softmax (macOS 14 target), RoPE manual cos/sin. Done when: graph builds; one full forward pass on random input (T=1497 frames) completes; no MPSGraph compilation warnings.
