...
**Current test count: 307 tests** (232 swift-testing + 75 XCTest, across unit, integration, regression, performance). All must pass before any new code is merged.
...
1. **Increment 3.6 (deferred) — Render Graph Refactor.** Fires when capability flag count exceeds 4 (now at 6: mesh, postProcess, ICB, rayMarch, feedback, direct). The refactor is now overdue.
2. **Phase 3.5 — Native Preset Library Expansion.** First entry: **3.5.1 Photorealistic Popcorn**, depends on 3.1b + 3.3 + 3.4 + 3.12 + 3.13 + 3.14 + 3.15 (all complete).
