...
1. **Increment V.7.6.2 — Orchestrator: multi-segment + completion-signal + maxDuration framework.** Preset-system-wide infrastructure: new `PlannedPresetSegment`, `SessionPlanner` walks each track's section list, `PresetSignaling` protocol with `presetCompletionEvent`, segment-aware `LiveAdapter`, `PresetDescriptor.maxDuration(forSection:)` computed property, new `naturalCycleSeconds: Float?` JSON field. Arachne v8 needs this in place before V.7.7 can land. See `docs/ENGINEERING_PLAN.md §V.7.6.2` and `docs/ARACHNE_V8_DESIGN.md §3, §5, §6 step 2`.
2. **Increment V.7.6.C — Framework calibration pass.** Matt reviews the §5.3 computed table, flags presets where output disagrees with intuition; coefficients tuned per `ARACHNE_V8_DESIGN.md §5.4`.
