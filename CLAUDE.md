...
PhospheneEngine/
  ...
  Renderer/
    ...
    FrameBudgetManager      → Pure-state frame timing governor: QualityLevel ladder (full→noSSGI→noBloom→reducedRayMarch→reducedParticles→reducedMesh), asymmetric hysteresis (3 overruns down / 180 frames up), per-tier Configuration factories, reset() on preset change. D-057.
...
`FeatureVector struct` → `V.1 Noise utility tree (9 files)` → `V.1 PBR utility tree (9 files)` → `V.2 Geometry utility tree (6 files)` → `V.2 Volume utility tree (5 files)` → `V.2 Texture utility tree (5 files)` → `ShaderUtilities.metal functions` → `constexpr sampler declarations` → preset shader code.
...
**Phase U (UX Architecture) complete — U.1 through U.9. Increments 6.1 and 6.2 complete. Phase 4 (Orchestrator), Phase 3, and Phase 2.5 (session preparation) complete. Milestones A and B achieved.** Recent landed work:
...
2. **Increment V.3 — Shader utility library: Color + Materials cookbook.** Follows V.2 (Geometry + Volume + Texture complete). See `docs/ENGINEERING_PLAN.md §Phase V`.
