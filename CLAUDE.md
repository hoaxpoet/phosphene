...
### Remaining SwiftLint violations (24 — structural, deferred)

24 violations remain after the cleanup pass. All are structural and require dedicated refactoring (not formatting fixes):

| File | Violation | Lines |
|------|-----------|-------|
| `SessionRecorder.swift` | `type_body_length` (516 lines, limit 500) | Split into `SessionRecorder+CSV.swift` etc. |
| `SessionRecorder.swift` | `file_length` (793 lines) | Same split |
| `AudioFeatures+Analyzed.swift` | `file_length` (552 lines) | Extract substructure |
| `PresetLoader+Preamble.swift` | `file_length` (541 lines) | Split preamble strings |
| `VisualizerEngine+Audio.swift` | `file_length` (507 lines) | Split audio/MIR sections |
| Several files | `function_body_length` (72–103 lines) | Extract helpers |
| `StemAnalyzer.swift` | `large_tuple` | Named struct |
| `RayMarchPipeline+Passes.swift` | `line_length` (127 chars) | Break string |

These are tracked and will be addressed in a dedicated lint-cleanup increment before the next major phase.

The next ordered increment is:

1. **Increment 4.1 — PresetScorer** — `Orchestrator/PresetScorer.swift`: scored preset selection using the Increment 4.0 metadata schema.
