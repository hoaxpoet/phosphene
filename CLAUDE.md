...
**Phase U (UX Architecture) in progress — U.1 through U.7 complete (Milestone A gate met). Phase 4 (Orchestrator), Phase 3, and Phase 2.5 (session preparation) complete.** Recent landed work:
...
2. **Increment U.8 — Settings panel.** `SettingsView` sheet per `UX_SPEC.md §9`. Four groups: Audio, Visuals, Diagnostics, About. All fields persisted in `UserDefaults` via `SettingsViewModel`.
