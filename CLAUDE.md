...
**Autocorrelation (secondary, fallback).** `estimateTempo` runs every frame on the composite-flux onset history. Returns `(tempo, confidence)` for tracks where sub_bass IOI evidence is sparse or absent (a cappella, solo acoustic guitar). Same 80–160 clamp. Used by the live engine when `instantBPM`/`stableBPM` haven't converged; used unconditionally by the future DSP.2 BeatNet path as a confidence baseline.

**Reference-track results (post-DSP.1):** love_rehab 122–126 (true 125), so_what 135–138 (true 136), there_there 137–140 (true ~86 syncopated — kick is not on every beat; histogram correctly reads kick rate, not meter). DSP.2 (BeatNet via MPSGraph, planned, D-076 reserved) is the answer for the syncopated case. See `docs/diagnostics/DSP.1-baseline*.txt` for the diagnostic captures and `Scripts/dump_tempo_baselines.sh` to reproduce.
...
**Phase DSP open — DSP.1 ✅ 2026-05-03, DSP.2 (BeatNet via MPSGraph) scheduled. Phase U (UX Architecture) complete — U.1 through U.11 + U.6b. Phase 6 complete (6.1, 6.2, 6.3). Phase 7 complete (7.1, 7.2). Phase V: V.1–V.4 ✅, V.5 (reel ✅, references 6/11), V.6 ✅, V.7 ✅, V.7.5 ⚠ shipped, V.7.6 abandoned, V.7.6.1 ✅ 2026-05-02 (visual harness), V.7.6.2 ✅ 2026-05-02 (orchestrator multi-segment + maxDuration framework + signaling), V.7.6.C ✅ 2026-05-03 (linger factor calibration + diagnostic class), V.7.6.D ✅ 2026-05-03 (Orchestrator diagnostic exclusion), V.7.7/V.7.8/V.7.9 scheduled. Phase 4 (Orchestrator), Phase 3, and Phase 2.5 (session preparation) complete. Milestones A, B, and C achieved.** Recent landed work:
...
1. **Increment V.7.7 — Arachne v8: background pass + background webs.** Per `ARACHNE_V8_DESIGN.md §6 step 4`. Atmospheric texture pass (mood-tinted gradient + defocused `worley_fbm` foliage + optional warm beam, half-res), pre-populated background dewy webs with Snell's-law refractive drops sampling the bg texture, fresnel rim, sharp specular pinpoint, dark edge ring. Foreground unchanged for now (still V.7.5 build code). Includes 5s low-pass smoothedValence/smoothedArousal state.
