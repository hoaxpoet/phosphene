...
PhospheneEngine/
  ...
  Presets/
    ...
    Shaders/VolumetricLithograph.metal → Psychedelic linocut terrain (3.5.4 / v3). fbm3D heightfield swept by `s.sceneParamsA.x`; amplitude from attenuated `f.bass_att + 0.4 × f.mid_att` + transient kick `f.beat_bass × 0.35`. Three strata: palette-tinted near-black valleys, razor-thin emissive ridge-line seam, polished-metal peaks. Peaks use IQ cosine `palette()` driven by terrain noise + audio time + valence — saturated metallic colors that cycle. `pow(f.beat_bass, 1.2) × 1.5` drives a palette brightness flare (× 1.5 peak, × up to 3.4 ridge) into HDR bloom, plus a small 0.03 coverage expansion on beats. `sqrt(f.mid) × 1.6` polishes peak roughness as the "other" stem proxy. `scene_fog: 0` truly disables fog (v3 fixed the shared `makeSceneUniforms` fallback that was producing MAX fog). SSGI omitted to preserve harsh shadows.
