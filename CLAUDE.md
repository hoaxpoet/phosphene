...
PhospheneEngine/
  ...
  Presets/
    ...
    Shaders/VolumetricLithograph.metal → Psychedelic linocut terrain (3.5.4 / v3.2). fbm3D heightfield swept by `s.sceneParamsA.x` at slow rate 0.015; amplitude from attenuated `f.bass_att + 0.4 × f.mid_att` + transient kick `f.beat_bass × 0.35`. Three strata with narrow linocut coverage (~15% peaks): palette-tinted near-black valleys, razor-thin emissive ridge-line seam, polished-metal peaks. Peaks use IQ cosine `palette()` driven by terrain noise (× 0.9 spatial) + audio time (× 0.08 slow cycle) + valence — saturated metallic colors. `pow(f.beat_bass, 1.2) × 1.5` drives palette brightness flare (× 2.5 peak, × 3.4 ridge) into HDR bloom + 0.03 coverage expansion on beats. `sqrt(f.mid) × 1.6` polishes peak roughness as "other" stem proxy. `scene_fog: 0` truly disables fog (v3 fixed the shared `makeSceneUniforms` fallback that was producing MAX fog). Miss/sky pixels tinted by `scene.lightColor.rgb` (shared RayMarch.metal fix, all ray-march presets benefit). SSGI omitted to preserve harsh shadows.
