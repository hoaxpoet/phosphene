...
1. **Increment DSP.2 S5 — BeatGridResolver + post-processing.** Peak-pick per-frame beat/downbeat probabilities from `BeatThisModel` output. Meter inference from downbeat spacing. `BeatGrid` value type (Sendable, Hashable, Codable). Done when: `BeatGridResolverTests` pass; six fixtures resolve to beats/downbeats/BPM/meter within tolerance.
