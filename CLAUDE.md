...
PhospheneEngine/
  ...
  Presets/
    ...
    Shaders/VolumetricLithograph.metal → Psychedelic linocut terrain (3.5.4 / v3.4). fbm3D heightfield swept by `s.sceneParamsA.x` at slow rate 0.015; single audio driver `f.bass_att × 3.5` clamped to [0, 2.0]. Three strata with narrow linocut coverage (~15% peaks): palette-tinted near-black valleys, razor-thin emissive ridge-line seam, polished-metal peaks. Peaks use IQ cosine `palette()` driven by terrain noise (× 0.9 spatial) + audio time (× 0.08 slow cycle) + valence — saturated metallic colors. `smoothstep(0.06, 0.25, f.bass_att)` drives palette brightness flare (× 2.5 peak, × 3.4 ridge) into HDR bloom + 0.03 coverage expansion on beats. v3.4 switched from `smoothstep(f.bass)` to `smoothstep(f.bass_att)` because v3.3's threshold on raw bass missed half the kicks (many Love Rehab kicks peak 0.20-0.23 in f.bass, below 0.22 threshold → phantom 65 BPM). `f.bass_att` is pre-smoothed AND its maxima track real kicks at 127 BPM on 125 BPM track. `sqrt(f.mid) × 1.6` polishes peak roughness as "other" stem proxy. `scene_fog: 0` truly disables fog. Miss/sky pixels tinted by `scene.lightColor.rgb`. SSGI omitted.
