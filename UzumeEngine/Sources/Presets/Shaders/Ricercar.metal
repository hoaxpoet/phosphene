...
//
// Ricercar renders entirely through its `ParticleGeometry` (`RicercarEchoGeometry` tonemaps its HDR
...
// §FANTASIA REBUILD). The mark + display kernels live in `Renderer/Shaders/RicercarEcho.metal` (engine
...
fragment float4 ricercar_ground_fragment(VertexOut in [[stage_in]]) {
    // Deep indigo, gentle top-darker gradient — matches ricercar_echo_display_fragment's ground.
