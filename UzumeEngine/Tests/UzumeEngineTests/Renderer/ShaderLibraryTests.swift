...
// CLEAN.4.4 — the cache key is (name, pixelFormat), not name alone. A name-only
// key returns the first-compiled PSO for any later call sharing the name, handing
// back the wrong pixel format. This test is RED on the name-only key (same instance
// returned) and GREEN once the key carries the full compiled identity.
// (The companion supportICB case went with the ICB capability at RECON.18.)
