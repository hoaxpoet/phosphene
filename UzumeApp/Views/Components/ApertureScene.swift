...
struct ApertureScene {
    ...
    /// The tokens this scene may use, resolved once per frame.
    struct Palette {
        ...
        init(_ env: EnvironmentValues) {
            ...
            spectrum = loop.map { RGB($0, env) }
            ivory = RGB(UzumeAppColor.ivory, env)
            canvas = RGB(UzumeAppColor.canvas, env)
            rock = RGB(UzumeAppColor.surface, env)
        }

        /// The identity's spectrum around the whole circle. Continuous, never banded.
        func prism(_ unit: Double, vibrancy: Double) -> RGB {
